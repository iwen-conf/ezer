...
async fn dispatch_workspace_info_reports_server_version() {
    use ezer_workspace_types::rpc::workspace::{WorkspaceInfo, WorkspaceInfoReq};
    let handler = WorkspaceRpcHandler::new(make_handle());
    let value = handler
        .dispatch(
    ...
    assert_eq!(Some(ezer_version::VERSION.to_owned()), info.version);
}
#[tokio::test]
...
async fn list_background_tasks_rpc_stays_truthful_across_rebinds() {
    ...
    use ezer_workspace_types::rpc::workspace::ListBackgroundTasksResponse;
    let handle = make_handle();
    ...
    ) -> Vec<ezer_workspace_types::rpc::workspace::BackgroundTaskSummaryWire> {
        let value = handler
            .dispatch(
    ...
    let read_only = ToolServerConfig {
        tools: vec![tc(
            "Ezer:read_file",
            Some(ezer_tools::types::tool::ToolKind::Read),
        )],
...
async fn delete_scheduled_task_rpc_reports_honestly() {
    use ezer_workspace_types::rpc::workspace::DeleteScheduledTaskResponse;
    let handle = make_handle();
    let cfg = background_capable_cfg();
    let session = handle
        .create_session_with_config(
...
async fn kill_task_rpc_terminates_outstanding_background_task() {
    use ezer_workspace_types::rpc::workspace::{KillTaskOutcome, KillTaskResponse};
    let handle = make_handle();
    let cfg = background_capable_cfg();
    let session = handle
        .create_session_with_config(
...
async fn dispatch_store_session_image_preserves_binary_in_bound_folder() {
    use base64::Engine;
    use ezer_workspace_types::rpc::fs::{StoreSessionImageReq, StoreSessionImageRes};
...
async fn dispatch_store_session_image_enforces_size_limit_without_writing() {
    use base64::Engine;
    use ezer_workspace_types::rpc::fs::{
...
async fn dispatch_store_session_image_accepts_exact_size_limit() {
    use base64::Engine;
    use ezer_workspace_types::rpc::fs::{MAX_SESSION_IMAGE_BYTES, StoreSessionImageRes};
    let (handler, folder) = session_image_handler().await;
...
fn dispatch_client_fs_write_file_round_trips_through_envelope() {
    use base64::Engine;
    use ezer_workspace_types::rpc::fs::{ClientFsWriteFileReq, ClientFsWriteFileRes};
...
fn write_file_behind_client_fs_gate() {
    use ezer_workspace_types::rpc::fs::{ClientFsReadFileReq, ClientFsWriteFileReq};
    let rt = tokio::runtime::Runtime::new().unwrap();
    let handler = rt.block_on(async { WorkspaceRpcHandler::new(make_handle()) });
    let root = handler.workspace.root_cwd().unwrap();
    let params = serde_json::json!({
...
async fn dispatch_knows_every_typed_method() {
    ...
    use ezer_workspace_types::rpc::git::{GitBranchInfoReq, GitMetadataReq};
    use ezer_workspace_types::rpc::search::FuzzyStatusReq;
    use ezer_workspace_types::rpc::skills::DiscoverPluginsReq;
    use ezer_workspace_types::rpc::workspace::{
    ...
    use ezer_workspace_types::rpc::worktree::WorktreeCreateSyncReq;
    let handler = WorkspaceRpcHandler::new(make_handle());
    let methods = [
        <WorkspaceInfoReq as WorkspaceRpc>::METHOD,
        ...
        <ezer_workspace_types::rpc::presence::PresenceNoteReq as WorkspaceRpc>::METHOD,
...
async fn dispatch_stamps_client_rpc_activity_for_mutations_only() {
    ...
    use ezer_workspace_types::rpc::workspace::DropSessionReq;
...
async fn presence_note_stamps_only_visible_notes_for_live_sessions() {
    use ezer_workspace_types::rpc::presence::PresenceNoteReq;
    use xai_tool_protocol::IdleWithholdReason;
