...
//!
//! How the consume side caches this verdict is a `ezer-shell` concern, documented there.
...
use ezer_config_types::{BoolFlag, RemoteSettings};
...
fn feature_enabled_for_build(remote: Option<&RemoteSettings>, is_local_build: bool) -> bool {
    ...
    let user = ezer_config::load_from_disk().ok();
    let managed = ezer_config::load_managed_config().ok();
...
fn collect_repo_config_kinds(cwd: &Path, first_only: bool) -> Vec<&'static str> {
    ...
    let project_sources = ezer_agent::repo::StartupProjectSources::resolve(cwd);
    ...
    for path in crate::project_config::find_project_configs_in(&chain.dirs) {
        let Ok(root) = ezer_config::load_config_file(&path) else {
            continue;
        };
    ...
    if ezer_agent::prompt::agents_md::has_project_instruction_markers_in(
        project_sources.instruction_dirs(),
    ) {
        hit!("instructions");
    }
    if ezer_agent::prompt::skills::has_project_skill_dirs_in(project_sources.skill_dirs()) {
        hit!("skills");
    }
...
mod tests {
    ...
    fn simulate_release_build() -> EnvVarGuard {
        EnvVarGuard::set(ezer_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"))
    }
    ...
    fn is_local_build_honors_test_version_override() {
        ...
        {
            let _sim = EnvVarGuard::set(ezer_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"));
    ...
    fn grant_folder_trust_directory_dest_is_unreadable_not_process_local() {
        ...
        let deny_path = require_strict_fixture_child(
            &fixture,
            &fixture.join(ezer_config::TRUSTED_FOLDERS_FILENAME),
        );
