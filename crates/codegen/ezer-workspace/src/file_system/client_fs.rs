...
//! The ezer.com conversation-files UI and the chat backend call them, tunneled through the server.
//!
...
//!
//! Wire types live in `ezer_workspace_types::rpc::fs` (the `ClientFs*` types), shared with the backend caller.
...
mod tests {
    use base64::Engine;
    use ezer_workspace_types::rpc::fs::FsReadEncoding;
