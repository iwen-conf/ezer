...
// Not re-exported: the `ClientFs*` types live in `ezer_workspace_types::rpc::fs` and would collide with the shell-facing `ext_fs` names above
...
pub use ezer_workspace_types::rpc::fs::FsReadEncoding;
...
// These types are canonical in ezer-workspace-types; the re-export keeps existing import paths working
pub use ezer_workspace_types::rpc::search::{ClientId, ContentSearchRequest, TargetClientId};
