...
use ezer_tools::registry::types::{SessionContext, ToolRegistryBuilder, ToolServerConfig};
...
impl SessionTerminalBackend {
    ...
    ///
    /// [`LocalTerminalBackend`]: ezer_tools::computer::local::LocalTerminalBackend
    pub fn local(backend: ezer_tools::computer::local::LocalTerminalBackend) -> Self {
    ...
    pub fn backend(&self) -> &Arc<dyn ezer_tools::computer::types::TerminalBackend> {
        &self.backend
    }
...
pub struct WorkspaceBindConfig {
    ...
    pub tools: Option<Vec<ezer_tools_api::ToolConfigEntry>>,
...
pub enum ResolvedToolset {
    ...
    InvalidToolConfig(ezer_tools::registry::proto_convert::ToolConfigEntryError),
}
    ...
    pub fn resolve(
        ...
        if let Some(cfg) = &self.tool_config {
            for (idx, tool) in cfg.tools.iter().enumerate() {
                if let Err(err) = ezer_tools_api::config_validation::validate_name_override(
                    idx,
                    &tool.id,
        ...
        if let Some(tools) = &self.tools {
            ...
            for (idx, entry) in tools.iter().enumerate() {
                ...
                match ezer_tools::registry::proto_convert::tool_config_from_entry(
                    idx,
                    entry.clone(),
                ) {
            ...
            if !unserved_tool_ids.is_empty() {
                tracing::warn!(
                    ...
                    running_version = ezer_version::VERSION,
...
/// Maps 1:1 to [`ezer_hooks::discovery::HookSource`] but uses owned `PathBuf` so the config struct is `'static`.
#[derive(Debug, Clone)]
...
mod bind_mcp_config_tests {
    ...
    fn duplicate_server_names_dedupe_last_wins() {
        ...
        let names: Vec<&str> = config
            .servers()
            .iter()
            .map(ezer_mcp::servers::mcp_server_name)
            .collect();
        assert_eq!(
            names,
            ["other", "dup"],
    ...
    fn server_list_is_capped_deterministically() {
        ...
        let first = ezer_mcp::servers::mcp_server_name(first_server);
        let last = ezer_mcp::servers::mcp_server_name(last_server);
