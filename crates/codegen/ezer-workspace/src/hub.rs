...
impl HubHandle {
    ...
    pub(crate) async fn connect(
        ...
        let mut server_builder = ToolServerBuilder::default()
            ...
            .binary_version(ezer_version::VERSION)
            .image_capabilities(
...
struct CallCompletedGuard {
    ...
    outcome: ezer_session_events::ToolOutcome,
}
impl CallCompletedGuard {
    fn new(
        ...
        Self {
            ...
            call_id,
            session_id,
            outcome: ezer_session_events::ToolOutcome::Cancelled,
        }
    }
    fn set_outcome(&mut self, outcome: ezer_session_events::ToolOutcome) {
            ...
            while let Some(item) = inner.next().await {
                match item {
                    ...
                    ToolStreamItem::Terminal(Ok(run_result)) => {
                        _guard.set_outcome(ezer_session_events::ToolOutcome::Success);
                    ...
                    ToolStreamItem::Terminal(Err(e)) => {
                        ...
                        _guard.set_outcome(ezer_session_events::ToolOutcome::Error);
                        let e = match &virt {
...
mod tests {
    ...
    use ezer_tools::types::tool_metadata::ToolMetadata as XaiToolMetadata;
    #[derive(Debug)]
    ...
    use ezer_tools::notification::types::{ToolNotification, ToolNotificationHandle};
    ...
    fn bg_started_notif(task_id: &str) -> ToolNotification {
        use ezer_tools::notification::types::{
    ...
    fn task_completed_notif(task_id: &str) -> ToolNotification {
        use ezer_tools::computer::types::{TaskKind, TaskSnapshot};
        ...
        ) -> Result<ezer_tools::types::output::ToolOutput, xai_tool_runtime::ToolError>
        {
            ...
            Ok(ezer_tools::types::output::ToolOutput::Bash(
                ezer_tools::types::output::BashOutput {
                    ...
                    output_for_prompt:
                        ezer_tools::types::output::BashOutput::make_output_for_prompt(stdout),
                    exit_code: 0,
