...
fn permission_scope_root(cwd: &AbsPathBuf) -> std::path::PathBuf {
    match ezer_agent::repo::RepoDirChain::resolve(cwd.as_path()).git_root {
...
fn state_dir_for_cwd(cwd: &AbsPathBuf) -> std::path::PathBuf {
    ezer_config::sessions_cwd_dir(&permission_scope_root(cwd).to_string_lossy())
}
...
fn legacy_state_dir(cwd: &AbsPathBuf, dir: &std::path::Path) -> Option<std::path::PathBuf> {
    let legacy = ezer_config::sessions_cwd_dir(cwd.as_str());
...
async fn resolve_store_dirs(cwd: &AbsPathBuf, ensure: bool) -> StoreDirs {
    let cwd = cwd.clone();
    let fallback_dir = ezer_config::sessions_cwd_dir(cwd.as_str());
    tokio::task::spawn_blocking(move || {
        ...
        let dir = if ensure {
            ...
            ezer_config::ensure_sessions_cwd_dir(&root).unwrap_or_else(|e| {
                tracing::warn!(?e, "failed ensuring sessions cwd dir for permission state");
                ezer_config::sessions_cwd_dir(&root)
            })
        } else {
            ezer_config::sessions_cwd_dir(&root)
        };
...
"#,
    ...
    fn scope_root_outside_a_repo_stays_the_cwd() {
        ...
        if ezer_agent::repo::RepoDirChain::resolve(&dir)
            .git_root
            .is_none()
        {
