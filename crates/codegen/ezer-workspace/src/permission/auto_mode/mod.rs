...
//!
//! Port of common agent auto-permission classifier semantics adapted to ezer's `AccessKind` permission gate.
...
/// Tools / access kinds that never need a classifier call (safe allowlist mapped to ezer access kinds and known names).
...
pub fn mcp_access_detail(name: &str, input: &serde_json::Value) -> String {
    ...
    ezer_tools::util::truncate_line(&format!("{name} {compact}"), MCP_ACCESS_DETAIL_MAX_LEN)
        .into_owned()
}
...
pub fn permission_decision_args(access: &AccessKind, access_detail: Option<&str>) -> String {
    ...
    ezer_tools::util::truncate_str_with_marker(&raw, CLASSIFIER_TURN_MAX_LEN).into_owned()
}
...
fn classifier_reason(v: &serde_json::Value) -> Option<String> {
    v.get("reason")
        ...
        .filter(|r| !r.is_empty())
        .map(|r| ezer_tools::util::truncate_line(&r, CLASSIFIER_REASON_MAX_LEN).into_owned())
}
