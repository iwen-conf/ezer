...
fn load_managed_settings() -> ManagedSettings {
    let claude = ezer_config::claude_managed_settings_path()
        .and_then(|path| read_managed_settings_json(&path).map(|json| (json, path)));
    let toml_layers = managed_toml_policy_layers(
        ezer_config::managed_config_layers(),
        ezer_config::requirements_layers(),
    );
...
fn managed_toml_policy_layers(
    managed: Vec<ezer_config::ManagedConfigLayer>,
    requirements: Vec<ezer_config::RequirementsLayer>,
) -> Vec<PolicyLayer> {
    ...
    for layer in requirements {
        toml_layers.push(PolicyLayer {
            tier: match layer.source {
                ezer_config::RequirementsSource::Mdm => PolicyLayerTier::Mdm,
