...
use std::sync::Arc;
use ezer_tools::registry::types::ToolServerConfig;
use ezer_tools::types::tool::ToolKind;
use ezer_workspace_types::WorkspaceEvent;
...
impl ezer_tools::types::tool_metadata::ToolMetadata for BashCcoStub {
...
impl xai_tool_runtime::Tool for BashCcoStub {
    ...
    ) -> Result<ezer_tools::types::output::ToolOutput, xai_tool_runtime::ToolError> {
        let output = BASH_CCO_STUB_STDOUT.as_bytes();
        Ok(ezer_tools::types::output::ToolOutput::Bash(
            ezer_tools::types::output::BashOutput {
                output: output.to_vec(),
                output_for_prompt:
                    ezer_tools::types::output::BashOutput::make_output_for_prompt(
...
async fn rebind_explicit_to_explicit_with_in_flight_call_defers_then_corrects() {
    use ezer_session_events::ToolOutcome;
...
) -> Arc<dyn ezer_tools::computer::types::TerminalBackend> {
    let res = toolset.resources.lock().await;
    res.get::<ezer_tools::types::resources::Terminal>()
        .map(|t| t.0.clone())
...
///
/// [`TerminalRunRequest`]: ezer_tools::computer::types::TerminalRunRequest
pub(crate) fn terminal_run_request(
    command: &str,
    out_dir: &std::path::Path,
    tool_call_id: &str,
) -> ezer_tools::computer::types::TerminalRunRequest {
    ezer_tools::computer::types::TerminalRunRequest {
        ...
        notification_handle: ezer_tools::notification::ToolNotificationHandle::noop(),
...
impl crate::config::SessionContextFactory for PersistentShellFactory {
    ...
    fn build_terminal_backend(&self) -> crate::config::SessionTerminalBackend {
        crate::config::SessionTerminalBackend::local(
            ezer_tools::computer::local::LocalTerminalBackend::with_persistent_shell(),
        )
    }
    fn registry_builder(&self) -> ezer_tools::registry::types::ToolRegistryBuilder {
...
async fn restarted_workspace_recreates_session_and_reports_lost_task() {
    ...
    let ezer_tools::types::output::ToolOutput::TaskOutput(
...
async fn events_jsonl_captures_turn_tool_toggle_and_mcp_variants() {
    ...
    handle.shared().session_event_writer(sid).emit(
        ezer_session_events::Event::McpToolCallStarted {
...
fn turn_outcome_label_maps_every_variant() {
    use ezer_session_events::TurnOutcomeLabel;
...
async fn fork_session_cwd_override_used_when_set() {
    let handle = make_handle();
    let alt = std::env::temp_dir().join("ezer-workspace-test-cwd-override");
...
async fn hook_registry_snapshot_is_disconnected() {
    ...
    assert!(snap1.is_empty());
    {
        let spec = ezer_hooks::config::HookSpec {
            name: "injected".into(),
            event: ezer_hooks::event::HookEventName::SessionStart,
...
async fn bind_mcp_post(
    ...
    if let Some(session_id) = headers
        .get(ezer_mcp::servers::EZER_AGENT_ID_HEADER)
    ...
    let converge = {
        ...
        tokio::spawn(async move {
            ...
            crate::mcp::converge_session(
                ...
                &*hub,
                crate::mcp::McpReclaim::Always,
                ezer_session_events::EventWriter::noop(),
            )
            .await
        })
    };
...
async fn end_mcp_service_loop(handle: &WorkspaceHandle, session_id: &str, server: &str) {
    ...
    tokio::time::timeout(std::time::Duration::from_secs(5), async {
        while client.liveness_check().await != ezer_mcp::servers::LivenessCheck::TransportClosed
        {
            tokio::time::sleep(std::time::Duration::from_millis(10)).await;
        }
    })
...
struct ClientAtFirstUnregister {
    ...
    client: parking_lot::Mutex<Option<std::sync::Weak<ezer_mcp::servers::McpClient>>>,
...
async fn strict_bind_without_explicit_toolset_fails_closed_end_to_end() {
    ...
    assert!(
        reason.contains(ezer_version::VERSION),
...
async fn rebind_session_root_rewrites_existing_cwd() {
    ...
    let cwd_res = {
        let toolset = session.toolset();
        let res = toolset.resources.lock().await;
        res.get::<ezer_tools::types::resources::Cwd>()
            .expect("toolset Cwd")
            .clone()
    };
    assert_eq!(
...
async fn local_harness_virtualizes_inbound_and_outbound() {
    ...
    impl ezer_tools::types::tool_metadata::ToolMetadata for LocalPathEcho {
        fn kind(&self) -> ToolKind {
            ToolKind::Other
        }
        fn tool_namespace(&self) -> ezer_tools::types::tool::ToolNamespace {
