...
fn to_workspace_event_kind(kind: FsEventKind) -> ezer_workspace_types::FsEventKind {
    match kind {
        FsEventKind::Created => ezer_workspace_types::FsEventKind::Created,
        FsEventKind::Modified => ezer_workspace_types::FsEventKind::Modified,
        FsEventKind::Removed => ezer_workspace_types::FsEventKind::Removed,
        FsEventKind::Renamed => ezer_workspace_types::FsEventKind::Renamed,
        _ => ezer_workspace_types::FsEventKind::Modified,
    }
}
...
fn confine_to_root(
    paths: Vec<PathBuf>,
    kind: ezer_workspace_types::FsEventKind,
    root: &Path,
) -> (Vec<PathBuf>, ezer_workspace_types::FsEventKind) {
    ...
    let confined = match (kind, paths.as_slice()) {
        (ezer_workspace_types::FsEventKind::Renamed, [from, to]) => {
            match (relative(from), relative(to)) {
                (Some(from), Some(to)) => (vec![from, to], kind),
                (Some(from), None) => (vec![from], ezer_workspace_types::FsEventKind::Removed),
                (None, Some(to)) => (vec![to], ezer_workspace_types::FsEventKind::Created),
...
pub(crate) async fn refresh_codebase_graph_after_head_change(
    ...
    ezer_tools::util::detach_command(&mut diff_cmd);
    diff_cmd.envs(ezer_tools::util::pager_env());
...
pub(crate) fn codebase_graph_events_for_batch(
    ...
    let graph_kind = match kind {
        ezer_workspace_types::FsEventKind::Created => FileEventKind::Created,
        ezer_workspace_types::FsEventKind::Modified => FileEventKind::Modified,
        ezer_workspace_types::FsEventKind::Removed => FileEventKind::Removed,
        ezer_workspace_types::FsEventKind::Renamed => FileEventKind::Renamed,
    };
