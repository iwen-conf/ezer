...
use ezer_tools::types::output::ToolRunResult;
use ezer_tools::types::resources::SessionFolder;
use ezer_workspace_client::{WorkspaceClient, is_transport_fatal};
pub use ezer_workspace_types::rpc::agents_md::DiscoverAgentsMdReq;
pub use ezer_workspace_types::rpc::code_nav::{
...
pub use ezer_workspace_types::rpc::export_github::ExportGithubReq;
pub use ezer_workspace_types::rpc::fs::{
...
pub use ezer_workspace_types::rpc::git::{
...
pub use ezer_workspace_types::rpc::hooks::{
    HookEventNameWire, HookRegistryReq, HookRegistryWire, HookSpecWire,
};
pub use ezer_workspace_types::rpc::hunks::{
...
pub use ezer_workspace_types::rpc::repos::{
    ProvisionedRepo, RepoManifest, ReposListReq, ReposListResponse,
};
pub use ezer_workspace_types::rpc::search::{FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq};
pub use ezer_workspace_types::rpc::session::{BeginPromptReq, EndPromptReq, RewindToReq};
pub use ezer_workspace_types::rpc::skills::DiscoverSkillsReq;
pub use ezer_workspace_types::rpc::workspace::WorkspaceInfoReq;
pub use ezer_workspace_types::rpc::worktree::{
...
pub use ezer_workspace_types::rpc::{RpcActivityClass, WorkspaceRpc};
...
async fn load_repos_manifest(root: &std::path::Path) -> WorkspaceResult<RepoManifest> {
    let path = root.join(ezer_workspace_types::rpc::repos::REPOS_MANIFEST_RELATIVE_PATH);
    match tokio::fs::read(&path).await {
...
fn hook_registry_to_wire(
    registry: &ezer_hooks::discovery::HookRegistry,
...
fn wire_to_hook_registry(
...
) -> WorkspaceResult<ezer_hooks::discovery::HookRegistry> {
    ...
    let mut registry: ezer_hooks::discovery::HookRegistry =
        serde_json::from_value(value).map_err(|e| WorkspaceError::HubError(e.to_string()))?;
...
impl WorkspaceOp for StoreSessionImageReq {
    async fn execute(
        ...
        if self.content_base64.len()
            > ezer_workspace_types::rpc::fs::MAX_SESSION_IMAGE_BASE64_BYTES
        {
            return Err(WorkspaceError::HubError(format!(
                "session image exceeds {} byte limit",
                ezer_workspace_types::rpc::fs::MAX_SESSION_IMAGE_BYTES
            )));
        }
        ...
        tokio::task::spawn_blocking(move || {
            ...
            if content.len() > ezer_workspace_types::rpc::fs::MAX_SESSION_IMAGE_BYTES {
                return Err(WorkspaceError::HubError(format!(
                    "session image exceeds {} byte limit",
                    ezer_workspace_types::rpc::fs::MAX_SESSION_IMAGE_BYTES
                )));
...
impl WorkspaceOps {
    ...
    pub fn server_version(&self) -> Option<String> {
        match self {
            Self::Local { .. } => Some(ezer_version::VERSION.to_owned()),
    ...
    pub async fn hook_registry(&self) -> WorkspaceResult<ezer_hooks::discovery::HookRegistry> {
    ...
    /// Dispatch a tool call through the workspace. - **Local**: dispatches through the workspace session's [`FinalizedToolset`](ezer_tools::registry::types::FinalizedToolset) (in-process).
...
mod tests {
    ...
    async fn repos_list_reads_real_manifest_empty_one_and_many() {
        ...
        assert_eq!(
            empty.version,
            ezer_workspace_types::rpc::repos::REPOS_MANIFEST_VERSION
        );
        ...
        std::fs::write(
            tmp.path()
                .join(ezer_workspace_types::rpc::repos::REPOS_MANIFEST_RELATIVE_PATH),
    ...
    async fn repos_list_walks_up_from_rewritten_cwd() {
        ...
        std::fs::write(
            sandbox_ws.join(ezer_workspace_types::rpc::repos::REPOS_MANIFEST_RELATIVE_PATH),
            one.to_json_bytes().unwrap(),
        )
        .unwrap();
    ...
    async fn end_local_session_if_bound_releases_only_the_named_toolset() {
        ...
        let bind = |toolset: &Arc<ezer_tools::registry::types::FinalizedToolset>| {
            ops.bind_local_session(
                sid,
                handle.root_cwd().unwrap(),
                xai_hunk_tracker::HunkTrackerHandle::noop(),
        ...
        let first = Arc::new(ezer_tools::registry::types::FinalizedToolset::empty_for_test());
        let second = Arc::new(ezer_tools::registry::types::FinalizedToolset::empty_for_test());
        bind(&first);
        bind(&second);
        assert!(
    ...
    fn hook_registry_wire_round_trip_both_directions() {
        let spec = ezer_hooks::config::HookSpec {
            name: "global/safety".to_string(),
            event: ezer_hooks::event::HookEventName::PreToolUse,
            handler_type: ezer_hooks::config::HandlerType::Command,
            ...
            layer: ezer_hooks::config::HookProvenance::File,
        };
        let mut registry = ezer_hooks::discovery::HookRegistry::default();
    ...
    fn hook_event_name_wire_covers_all_upstream_variants() {
        use ezer_hooks::event::HookEventName as E;
    ...
    /// The lean types crate can't depend on `ezer-hooks`, and `hook_registry_to_wire` only couples the two via a serde round-trip.
    ...
    fn hook_spec_wire_covers_all_upstream_fields() {
        use ezer_hooks::config::HookSpec;
        ...
        let spec = HookSpec {
            ...
            source_dir: std::path::PathBuf::from("/home/u/.ezer/hooks"),
            extra_env: std::collections::HashMap::from([("FOO".to_string(), "bar".to_string())]),
            layer: ezer_hooks::config::HookProvenance::Managed,
        };
        assert_eq!(
