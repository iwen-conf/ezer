...
use ezer_mcp::servers::McpState;
use ezer_tools::notification::AcknowledgedToolNotification;
use ezer_tools::notification::types::ToolNotificationHandle;
use ezer_tools::registry::types::{FinalizedToolset, ToolConfig, ToolServerConfig};
...
impl WorkspaceSession {
    ...
    pub(crate) async fn set_cwd_for_virtualization(&self, cwd: PathBuf) -> Result<(), String> {
        ...
        res.insert(ezer_tools::types::resources::Cwd(cwd));
        Ok(())
    }
        ...
        match outcome {
            McpServerOutcome::Connected => {
                state.owned_clients.insert(
                    name.to_owned(),
                    Arc::new(ezer_mcp::servers::McpClient::stub(name)),
                );
            }
    ...
    pub(crate) async fn toolset_terminal_is_session_owned(&self) -> bool {
        ...
        match res.get::<ezer_tools::types::resources::Terminal>() {
...
pub struct WorkspaceShared {
    ...
    pub(crate) events: tokio::sync::broadcast::Sender<ezer_workspace_types::WorkspaceEvent>,
    pub(crate) respect_gitignore: bool,
    pub(crate) memory_config: Option<MemoryConfig>,
    pub(crate) hook_registry: Arc<parking_lot::RwLock<ezer_hooks::discovery::HookRegistry>>,
    pub(crate) hook_load_errors: Vec<ezer_hooks::error::HookError>,
    ...
    pub(crate) lsp: Option<std::sync::Arc<dyn ezer_tools::implementations::lsp::LspBackend>>,
    ...
    pub(crate) session_event_writers:
        Arc<dashmap::DashMap<String, ezer_session_events::EventWriter>>,
...
impl WorkspaceShared {
    ...
    /// When `events_enabled` is `false` this returns [`EventWriter::noop()`](ezer_session_events::EventWriter::noop).
    ...
    pub(crate) fn session_event_writer(
        &self,
        session_id: &str,
    ) -> ezer_session_events::EventWriter {
    ...
    pub(crate) fn session_event_writer_cached(
        &self,
        session_id: &str,
    ) -> Option<ezer_session_events::EventWriter> {
    ...
    pub fn subscribe_events(
        &self,
    ) -> tokio::sync::broadcast::Receiver<ezer_workspace_types::WorkspaceEvent> {
...
pub(crate) fn get_or_open_session_writer(
    enabled: bool,
    writers: &dashmap::DashMap<String, ezer_session_events::EventWriter>,
    workspace_home: &Path,
    session_id: &str,
) -> ezer_session_events::EventWriter {
    ...
    let create = ezer_config::create_dir_all_owner_only(&dir);
    ezer_config::set_dir_owner_only(&sessions_root);
    if let Err(e) = create {
        tracing::warn!(
...
mod tests {
    ...
    use dashmap::DashMap;
    use ezer_session_events::{Event, EventWriter};
