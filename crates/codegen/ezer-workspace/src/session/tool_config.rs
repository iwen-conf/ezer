...
pub(crate) fn resolve_session_toolset(
    ...
    notification_handle: Option<ezer_tools::notification::types::ToolNotificationHandle>,
) -> WorkspaceResult<(
    ToolServerConfig,
    ...
    let (effective, toolset) = resolve_session_toolset_rebuild(
        ...
        ezer_tools::types::context::TruncationConfig::default(),
    )?;
...
pub(crate) fn truncation_config_for_host(
    ...
    let mut truncation = ezer_tools::types::context::TruncationConfig::default();
...
pub(crate) fn resolve_session_toolset_rebuild(
    ...
    viewer_ctx: Option<xai_tool_runtime::WorkspaceViewerContext>,
    notification_handle: Option<ezer_tools::notification::types::ToolNotificationHandle>,
    terminal_backend: Arc<dyn ezer_tools::computer::types::TerminalBackend>,
    truncation: ezer_tools::types::context::TruncationConfig,
    ...
    if !truncation.per_tool_max_output_bytes.is_empty() {
        ...
        resources.insert(ezer_tools::types::resources::TruncationCfg(truncation));
    }
...
impl SessionContextFactory for WorkspaceSessionContextFactory {
    fn build_session_context(
        ...
        let notification_handle = ezer_tools::notification::ToolNotificationHandle::noop();
    ...
    fn build_terminal_backend(&self) -> crate::config::SessionTerminalBackend {
        crate::config::SessionTerminalBackend::local(
            ezer_tools::computer::local::LocalTerminalBackend::new(),
        )
    }
...
fn build_proxy_headers(base_url: &str) -> indexmap::IndexMap<String, String> {
    ...
    if base_url.contains("cli-chat-proxy") || base_url.contains("chat-proxy") {
        headers.insert("X-XAI-Token-Auth".to_string(), "ezer-cli".to_string());
        headers.insert(
...
fn build_web_fetch_config() -> ezer_tools::implementations::ezer_build::web_fetch::WebFetchConfig
{
    ...
    if ezer_config::env_bool("EZER_WEB_FETCH_ALLOW_LOCAL") == Some(true) {
...
fn default_web_search_model() -> String {
    std::env::var("EZER_WEB_SEARCH_MODEL")
        .unwrap_or_else(|_| ezer_config::DEFAULT_GATEWAY_MODEL_KEY.to_string())
}
...
mod tests {
    ...
    async fn tool_state_rehydrates_same_session_and_cold_starts_other() {
        use ezer_tools::types::resources::{State, WebCitationCounter};
