...
use ezer_tools::util::mcp_structured_content::render_structured_content;
...
pub(crate) async fn drive_server_starts(
    ...
    let mut remaining_names: HashSet<String> = configs
        .iter()
        .map(|config| ezer_mcp::servers::mcp_server_name(config).to_owned())
        .collect();
    ...
    let startup_timeout_sec =
        ezer_mcp::servers::McpClient::max_startup_within_deadline(deadline_secs);
    ...
    let mut pending: FuturesUnordered<_> = configs
        .into_iter()
        .map(|config| {
            let server_name = ezer_mcp::servers::mcp_server_name(&config).to_owned();
            ...
            async move {
                let client = ezer_mcp::servers::start_mcp_server(
                    config,
                    Some(overrides),
                    None,
                    None,
...
pub(crate) fn dedupe_servers_last_wins(servers: &mut Vec<agent_client_protocol::McpServer>) {
    ...
    for index in (0..servers.len()).rev() {
        ...
        let name = ezer_mcp::servers::mcp_server_name(server).to_owned();
        if !seen.insert(name) {
...
pub fn compose_built_in(
    ...
    let name = ezer_mcp::servers::mcp_server_name(&entry);
    let (same_name, mut composed): (Vec<_>, Vec<_>) = servers
        .into_iter()
        .partition(|server| ezer_mcp::servers::mcp_server_name(server) == name);
...
pub(crate) async fn converge_session(
    ...
    if !plan.start.is_empty() {
        ...
        let configs = desired
            .servers()
            .iter()
            .filter(|config| starting.contains(ezer_mcp::servers::mcp_server_name(config)))
            .cloned()
            .collect();
        let drive_scope = begin_mcp_drive(session).await?;
...
mod tests {
    ...
    fn cap_servers_keeps_the_first_entries_in_config_order() {
        ...
        assert_eq!(
            ...
            ezer_mcp::servers::mcp_server_name(last),
        );
    }
    ...
    fn dedupe_servers_last_wins_keeps_last_definition_in_place() {
        ...
        let named: Vec<(&str, &str)> = servers
            ...
            .map(|server| {
                ...
                (
                    ezer_mcp::servers::mcp_server_name(server),
                    http.url.as_str(),
                )
            })
            .collect();
