...
//!
//! Delegates to the existing discovery implementations in `ezer-agent` and `ezer-tools` rather than duplicating them.
//! The workspace stores configuration on [`WorkspaceShared`] and the [`MpscChannel`] methods call these helpers.
//!
//! Re-exports [`AgentsMdTracker`] from `ezer-tools` for external consumers that need per-session project-instruction tracking.
...
// Re-export AgentsMdTracker so consumers can reference it via the workspace crate without a direct ezer-tools dependency
pub use ezer_tools::types::agents_md_tracker::AgentsMdTracker;

// Re-export the config types that callers pass into WorkspaceConfig.
pub use ezer_agent::plugins::discovery::DiscoveryConfig as PluginDiscoveryConfig;
pub use ezer_agent::plugins::trust::TrustStore as PluginTrustStore;
pub use ezer_agent::prompt::skills::SkillsConfig;
...
pub async fn discover_skills(
    ...
    let span = ezer_telemetry::region::Region::from_span(tracing::info_span!(
...
pub async fn discover_agents_md(root_cwd: &Path, project_trusted: bool) -> Vec<Value> {
    ...
    let files = ezer_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd_str,
...
pub fn load_project_config(root_cwd: &Path) -> Value {
    ...
    match ezer_config::load_config_file(&config_path) {
...
mod tests {
    ...
    fn agent_config_file_wire_matches_workspace_types_mirror() {
        ...
        let src = ezer_agent::prompt::agents_md::AgentConfigFile {
            ...
            source: ezer_agent::prompt::agents_md::InstructionSource::Configured,
        };
        let json = serde_json::to_value(&src).unwrap();
        let mirror: ezer_workspace_types::rpc::agents_md::AgentConfigFile =
        ...
        assert_eq!(
            mirror.source,
            ezer_workspace_types::rpc::agents_md::InstructionSource::Configured
        );
        assert_eq!(
