...
use url::Url;
use ezer_diag_server::{self as diag_server, DiagHandle, ErrorClass};
use ezer_workspace::WorkspaceHostKind;
use ezer_workspace::config::{
    WorkspaceServerMetadata, merge_host_identity_metadata, merge_session_metadata,
};
use ezer_workspace::error::WorkspaceError;
use ezer_workspace_daemon::daemonize;
use ezer_workspace_daemon::preview_supervisor::{
...
/// Forwarded 1:1 to the `/usr/local/bin/ezer-preview-proxy` child (see `cli.rs` for the proxy's flag names).
...
/// `ezer-workspace-daemon` deliberately does not depend on `ezer-workspace`, so this binary owns the one adapter between them.
struct TrackerSink(std::sync::Arc<ezer_workspace::activity::ActivityTracker>);
...
async fn run(
    ...
    {
        use ezer_sandbox::{ProfileName, SandboxManager};
        let profile = match std::env::var("EZER_SANDBOX_PROFILE").ok() {
            ...
            None if ezer_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
        ...
        if profile == ProfileName::Off {
            ...
            sandbox.install();
            let active = ezer_sandbox::is_active();
            ...
            tracing::info!(
                ...
                active,
                restrict_network_at_known_linux_launches = ezer_sandbox::should_restrict_child_network(),
                "{status_msg}"
            );
        }
    }
    let mut status_config = ezer_workspace::StatusConfig::from_env();
    status_config.preview_control_port = args.preview.preview_control_port;
    let auth_provider = ezer_workspace::hub_auth::provider(
        &url,
    ...
    {
        let caps = ezer_workspace::image_capabilities::image_capabilities();
    ...
    ezer_workspace::init_metrics();
    let ws_handle = match ezer_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        ezer_workspace::LocalWorkspaceConnectOptions {
            metadata,
    ...
    let grace_budget = ezer_workspace::handle::termination_grace_from_env();
    ws_handle
        .two_phase_drain(
            grace_budget,
            ezer_workspace::handle::DrainReason::Sigterm,
        )
        .await;
...
mod tests {
    ...
    /// The scraper's own behavior is tested in `ezer-workspace-daemon`, and the tracker's in `ezer_workspace::activity`.
    /// This test covers only the adapter.
    #[test]
    fn tracker_sink_forwards_every_signal_to_the_activity_tracker() {
        use ezer_workspace::activity::ActivityTracker;
