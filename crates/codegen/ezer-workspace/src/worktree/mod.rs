...
use ezer_telemetry::events::{CloneCancellationDisposition, CloneOutcome, WorktreeLifecycle};
...
// Canonical in ezer-workspace-types; re-exported for existing paths.
...
pub async fn prepare_worktree_creation(req: &CreateWorktreeRequest) -> PrepareWorktreeResult {
    ...
    let worktree_path = resolve_worktree_path(&ezer_home(), req, &layout.git_root);
...
pub async fn create_worktree_streaming_in<N: WorktreeNotificationSender>(
    ...
    let (worktree_path_str, source_git_root) = if let Some(pinned) = &req.worktree_path {
        ...
        match create_root::resolve_create_source_layout(source_path, grove_enabled).await {
            Ok(layout) => (
                resolve_worktree_path(ezer_home, req, &layout.git_root),
...
/// Resolve the target worktree path for a fork operation. When the source path is already inside `~/.ezer/worktrees/<repo>/`, the repo name is derived from the directory structure rather than `git_root` (which would be the standalone worktree itself, causing nested paths).
...
pub fn worktree_auto_gc_layer_from_settings(
    s: &ezer_config_types::WorktreeAutoGcSettings,
) -> WorktreeAutoGcLayer {
    use std::collections::BTreeMap;
    use ezer_config_types::WorktreeKindMaxAge;
    ...
    let ezer_config_types::WorktreeAutoGcSettings {
        enabled,
        max_age_secs,
        min_interval_secs,
        dry_run,
...
fn load_local_worktree_auto_gc_settings(
    ...
    let root = ezer_config::load_config_file(&home.join(ezer_config::USER_CONFIG_FILENAME))
        .inspect_err(|error| {
...
fn worktree_auto_gc_settings_from_config(
    ...
    let auto_gc = root
        ...
        .as_table_mut()?
        .remove("auto_gc")?;
    ezer_config_types::WorktreeAutoGcSettings::deserialize(auto_gc)
        .inspect_err(|error| {
...
mod tests {
    ...
    fn local_auto_gc_policy_opts_in_only_when_enabled() {
        use ezer_config_types::WorktreeAutoGcSettings;
    ...
    fn worktree_auto_gc_settings_load_from_config_toml_document() {
        ...
        std::fs::write(
            home.path().join(ezer_config::USER_CONFIG_FILENAME),
            "[worktree.auto_gc]\nenabled = true\nmax_age_secs = 86400\n",
        )
        .unwrap();
        assert_eq!(
    ...
    async fn prepare_invalid_source_emits_worktree_ended_failed() {
        ...
        assert_eq!(
            ...
            Some(ezer_telemetry::events::CloneStrategy::Grove)
        );
    ...
    async fn sync_fork_response_includes_strategy_report() {
        ...
        let temp = tempfile::TempDir::new().unwrap();
        // tempfile `.../.tmp*/repo` slugs to shared ~/.ezer/worktrees/tmp-repo; shards
        ...
        assert_eq!(
            ...
            strategy
                .resolved_strategy
                .as_deref()
                .and_then(ezer_telemetry::events::CloneStrategy::from_strategy_str)
        );
        if ended.resolved_strategy == Some(ezer_telemetry::events::CloneStrategy::Copy) {
