...
use ezer_pager::agent_runtime::AgentRuntime;
use ezer_pager::app::{
...
use ezer_pager::app::{WorkspaceMgmtArgs, WorkspaceMgmtCommand, WorkspaceStartArgs};
use ezer_pager::client_identity::PAGER_CLIENT_VERSION;
use ezer_shell::agent::app::{run_headless, run_leader};
use ezer_shell::agent::config::Config as AgentConfig;
use ezer_shell::leader::{
...
use ezer_telemetry::process_info::{
...
fn process_identity(command: Option<&Command>, is_interactive: bool) -> Option<ProcessIdentity> {
    use ezer_telemetry::process_info::LeaderMode::Standalone;
...
fn apply_agent_endpoint_args(
    agent_args: &ezer_pager::app::AgentArgs,
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    ...
    use ezer_telemetry::debug_log::RMCP_SSE_NOISE_TARGET;
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(ezer_telemetry::sampling_log::layer())
        .with(ezer_telemetry::span_profile::layer(app_entrypoint))
        .with(ezer_telemetry::instrumentation::layer())
        .with(ezer_telemetry::hooks_log::layer())
        .with(ezer_telemetry::otel_layer::build_otel_layer(
            ezer_telemetry::otel_layer::OtelClientInfo {
                client_name: "ezer",
                client_version: ezer_version::VERSION,
                service_version: env!("VERSION_WITH_COMMIT"),
                app_entrypoint,
            },
            ezer_shell::agent::init::build_default_otel_layer_config(),
        ));
    ezer_telemetry::debug_log::install_firehose(registry, app_entrypoint);
    ezer_telemetry::external::init(
        ezer_shell::agent::config::resolve_external_otel_config(
            ezer_telemetry::external::config::ExternalClientInfo {
                service_version: env!("VERSION_WITH_COMMIT").to_owned(),
                client_version: ezer_version::VERSION.to_owned(),
                app_entrypoint: app_entrypoint.to_owned(),
            },
        ),
    );
}
/// `ezer setup`: rendering and exit codes only; fetch logic lives in `ezer_shell::managed_config`.
/// `json` prints the served configuration instead of installing it.
#[tracing::instrument(level = "debug", skip_all)]
async fn run_setup_command(json: bool) {
    use ezer_shell::managed_config::{self, SetupOutcome};
...
async fn run_leader_mgmt(args: LeaderMgmtArgs) -> Result<()> {
    match args.command {
        ...
        LeaderMgmtCommand::List { json } => {
            let leaders = ezer_shell::leader::discover_leaders().await;
            if json {
...
async fn kill_leaders() -> Result<()> {
    ...
    for d in &leaders {
        ...
        if let Err(e) = ezer_shell::util::kill_process_by_pid(pid) {
...
) -> Result<(LeaderDescriptor, ezer_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = ezer_shell::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = ezer_shell::leader::LeaderClient::connect(
...
fn leader_info_json(
    ...
    info: Option<&ezer_shell::leader::ControlPayload>,
...
fn workspace_command_gate(
    ...
    remote_settings: Option<&ezer_shell::util::config::RemoteSettings>,
) -> WorkspaceGate {
...
async fn run_workspace_mgmt(args: WorkspaceMgmtArgs) -> Result<()> {
    ...
    ) && let Some(profile) = ezer_sandbox::requested_confinement_profile()
    {
        anyhow::bail!(
...
async fn spawn_and_connect_leader(
    ...
    use ezer_login::ensure_authenticated;
    ezer_shell::util::config::set_remote_campaigns_from_settings(remote_settings);
    ...
    let (use_leader, _) = resolve_use_leader(
        ...
        remote_settings,
        true,
        ezer_sandbox::requested_confinement_profile(),
    );
    if !use_leader {
        anyhow::bail!(
...
fn shutdown_and_flush_telemetry(exit_code: i32) -> ! {
    {
        ...
        ezer_telemetry::sentry::flush_on_shutdown();
    }
...
async fn run_agent_command(
    agent_args: Box<ezer_pager::app::AgentArgs>,
    ...
    if matches!(
        ...
        ezer_shell::agent::app::suppress_otel();
    }
    init_tracing_simple("agent");
    let _otel_guard = ezer_telemetry::otel_layer::otel_guard();
    ezer_telemetry::instrumentation::install_panic_hook();
    if trust {
        use ezer_workspace::folder_trust::{grant_folder_trust, report_cli_trust_grant};
    ...
    if had_prefetch {
        ezer_shell::agent::remote_config::settings_get::warm_startup_settings(
    ...
    if !is_stdio && !is_leader {
        eprintln!(
            "ezer (pager) - v{}",
            ezer_version::display_version_with_commit(
                env!("VERSION_WITH_COMMIT"),
                ezer_update::channel_label(),
            )
        );
    ...
    ezer_shell::util::config::set_remote_campaigns_from_settings(remote_settings.as_ref());
    let raw_config = ezer_shell::config::load_effective_config()
        .map_err(|e| anyhow::anyhow!("Failed to load config: {}", e))?;
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(ezer_shell::sampling::types::parse_canonical_effort_token);
    let launch_yolo = ezer_shell::util::config::effective_yolo_for_launch(
        agent_args.yolo,
    ...
    agent_config.default_auto_mode = ezer_shell::util::config::effective_auto_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
        ezer_shell::util::config::PermissionMode::Ask,
    );
    ...
    agent_config.resolve_runtime_fields(&ezer_shell::agent::config::RuntimeResolutionContext {
    ...
    let requested_confinement = ezer_sandbox::requested_confinement_profile();
    let LeaderMode {
        use_leader,
    ...
    use ezer_telemetry::process_info::LeaderMode::{Attached, Standalone};
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = ezer_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            ezer_shell::agent::run_agent_server(server_config, agent_config).await
        }
        Some(AgentCmd::Leader(a)) => {
            ...
            run_leader(
                &agent_config,
                ezer_shell::agent::app::LeaderRunOptions {
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !ezer_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
...
fn schedule_startup_prewarm() {
    ezer_shell::agent::mvp_agent::warm_async_http_client();
}
fn configure_process_env(mut args: PagerArgs) -> Result<PagerArgs> {
    ...
    unsafe {
        ...
        if args.chat() {
            std::env::set_var(ezer_shell::agent::chat_modes::EZER_CHAT_MODE_ENV, "1");
        }
        if let Some(socket) = args.leader_socket.as_deref() {
            std::env::set_var(ezer_shell::leader::LEADER_SOCKET_ENV, socket);
        }
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<ezer_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(ezer_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<ezer_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(ezer_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    ezer_shell::heap_profile::install(ezer_shell::heap_profile::HeapProfileHooks {
...
fn dispatch_doctor_if_requested(args: &PagerArgs) -> bool {
    ...
    if let Err(error) = ezer_pager::doctor_cmd::run(doctor_args.clone()) {
        eprintln!("Error: {error:#}");
        std::process::exit(1);
    }
    true
...
fn main() {
    ezer_version::set_full_version(env!("VERSION_WITH_COMMIT"));
    ezer_telemetry::startup::mark_process_start();
    if let Some(code) = ezer_pager::app::mermaid_worker::maybe_run_render_subprocess() {
        std::process::exit(code);
    }
    if let Some(code) = ezer_pager::voice::maybe_run_capture_subprocess() {
        std::process::exit(code);
    }
    set_release_channel(ReleaseChannel::from_label(
        ezer_update::channel_name().unwrap_or_default(),
    ));
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    {
        ezer_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        ezer_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    install_heap_profile_hooks();
    unsafe {
        ezer_shell::agent::external_otel_pin::strip_conflicting_process_env();
    }
    let args = configure_process_env(args).unwrap_or_else(|err| {
        eprintln!("ezer: {err:#}");
        std::process::exit(1);
    });
    ezer_pager::memory_trace::start(ezer_pager::memory_trace::default_dir());
    raise_fd_limit();
    if let Err(e) = ezer_config::validate_requirements() {
    ...
    let _sentry_guard = ezer_telemetry::sentry::init(ezer_telemetry::sentry::Config {
        client: "ezer",
    ...
    let crashed = ezer_active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
    ...
    if let Err(e) = result {
        ...
        finalize_span_profile();
        let report = match e.downcast_ref::<ezer_pager::app::StartupFailure>() {
            Some(startup) => startup.user_report(),
            None => format!("Error: {e:#}"),
        };
        ezer_pager::best_effort_stderr::eprint_line(&report);
        drop(_sentry_guard);
        std::process::exit(1);
    }
    finalize_span_profile();
...
async fn async_main(mut args: PagerArgs) -> Result<()> {
    ...
    if let Some(Command::Completions { shell }) = &args.command {
        ezer_pager::completions_cmd::run(*shell);
        return Ok(());
    }
    if let Some(Command::Wrap(ref wrap_args)) = args.command {
        return ezer_pager::wrap_cmd::run(wrap_args);
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        ezer_pager::app::cli::SandboxStartup::Apply(profile) => profile,
        ezer_pager::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    if command_needs_pre_sandbox_policy_heal(args.command.as_ref()) {
        match ezer_shell::config::load_agent_config_disk_only() {
            Ok(agent_cfg) => {
                let auth_manager =
    ...
    ezer_shell::config::apply_sandbox(
        None,
    ...
    if let Some(command) = args.command.take() {
        match command {
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!({
                        ...
                        "channel": ezer_update::channel_name().unwrap_or("unknown"),
                    });
                    println!("{}", serde_json::to_string(&payload)?);
                } else {
                    write_version(
            ...
            Command::Inspect { json } => {
                ...
                ezer_shell::inspect::inspect(&cwd, json).await?;
                return Ok(());
            }
            Command::Setup { json } => {
            ...
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return ezer_pager::mcp_cmd::run(mcp_args).await;
            }
            Command::Plugin(plugin_args) => {
                init_tracing_simple("cli");
                let _otel_guard = ezer_telemetry::otel_layer::otel_guard();
                return ezer_pager::plugin_cmd::run(plugin_args).await;
            }
            Command::Models => {
                init_tracing_simple("cli");
                let _otel_guard = ezer_telemetry::otel_layer::otel_guard();
                let agent_config = ezer_shell::config::load_agent_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return ezer_pager::models::list_available_models(&agent_config).await;
            }
            ...
            Command::Worktree(worktree_args) => {
                ...
                let agent_config = ezer_shell::config::load_agent_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return ezer_pager::worktree_cmd::run(worktree_args, &agent_config).await;
            }
            Command::DiskUsage(disk_usage_args) => {
                init_tracing_simple("cli");
                let _otel_guard = ezer_telemetry::otel_layer::otel_guard();
                return ezer_pager::disk_usage_cmd::run(disk_usage_args);
            }
            ...
            Command::Sessions(sessions_args) => {
                ...
                let agent_config = ezer_shell::config::load_agent_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return ezer_pager::sessions_cmd::run(sessions_args, &agent_config).await;
            }
            Command::Usage(usage_args) => {
                init_tracing_simple("cli");
                let _otel_guard = ezer_telemetry::otel_layer::otel_guard();
                return ezer_pager::usage_cmd::run(usage_args);
            }
            Command::Share(ref share_args) => {
                ...
                let agent_config = ezer_shell::config::load_agent_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return ezer_pager::share_cmd::run(share_args, &agent_config).await;
            }
            Command::Export(export_args) => {
                init_tracing_simple("cli");
                return ezer_pager::export_cmd::run(export_args);
            }
            ...
            Command::Memory(memory_args) => {
                ...
                let config = ezer_shell::config::load_effective_config()?;
                let mode = ezer_shell::config::MemoryConfig::resolve(
                ...
                .mode;
                return ezer_pager::memory_cmd::run(memory_args, mode);
            }
            Command::Update {
                check,
                json,
            ...
            Command::Login {
                ...
                let authenticated = ezer_login::run_cli_login(
                    ...
                    devbox,
                    |auth_manager| {
                        ezer_shell::agent::init::update_telemetry_config(&config, auth_manager)
                    },
                )
                .await?;
                ezer_shell::agent::init::apply_post_login_config(authenticated).await?;
                println!();
    ...
    let headless_prompt = ezer_pager::headless::HeadlessPrompt::from_args(
    ...
    if headless_prompt.is_some() || args.memory_flush {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(ezer_pager::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some()
            && args.output_format == ezer_pager::headless::OutputFormat::Plain
        {
            args.output_format = ezer_pager::headless::OutputFormat::Json;
        }
        let memory_enabled_override = args.memory_enabled_override();
        let memory_flush = args.memory_flush;
        return ezer_pager::headless::run_single_turn(
            headless_prompt,
            args.verbatim,
            ezer_pager::headless::HeadlessOptions {
    ...
    let result = ezer_pager::app::run(args, bg_update_rx).await;
    ezer_sandbox::flush();
    match result {
        Ok(true) => {
    ...
    cryptify::flow_stmt!({
        {
            config.deployment_key =
                ezer_shell::agent::config::EndpointsConfig::default().deployment_key;
        }
    });
    config.npm_registry = std::env::var(obfstr::obfstr!("EZER_NPM_REGISTRY"))
        .ok()
        .or_else(ezer_shell::util::config::load_npm_registry_sync);
    if let Ok(root) = ezer_shell::config::load_effective_config_disk_only()
        && let Some(ch) = ezer_shell::util::config::channel_from_toml_opt(&root)
    {
        config.channel = ch;
    }
    config
...
async fn run_update_command(
    ...
    let telemetry_cfg = ezer_shell::config::load_agent_config_disk_only()
    ...
    ezer_telemetry::session_ctx::drain_pending(ezer_telemetry::session_ctx::CLI_DRAIN)
        .await;
    result?;
    Ok(())
}
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in ezer_shell::leader::discover_leaders().await {
        if d.classification != ezer_shell::leader::LeaderDiscoveryState::Reachable {
            continue;
        }
        ...
        let client = match ezer_shell::leader::LeaderClient::connect(
            socket_path,
            "ezer-update",
            ClientMode::Stdio,
            ClientCapabilities::default(),
        ...
        match client
            ...
            .await
        {
            Ok(Ok(ezer_shell::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
            ...
            Ok(Ok(ezer_shell::leader::ControlPayload::RelaunchDeclined { reason })) => {
...
mod tests {
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            ezer_shell::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: ezer_shell::heap_profile::JemallocStats) {
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(
            ezer_shell::heap_profile::stats().expect("shell stats after install"),
        );
        if !require_opt_prof() {
            assert!(!ezer_shell::heap_profile::prof_available());
            return;
        }
        assert!(ezer_shell::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(ezer_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(ezer_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
        let dump = TempHeapDump::new("shell");
        ezer_shell::heap_profile::dump_to_path(dump.path()).expect("shell dump");
        dump.assert_nonempty_dump();
    }
    #[cfg(unix)]
    #[test]
    ...
    fn workspace_command_gate_resolution() {
        use ezer_shell::util::config::RemoteSettings;
