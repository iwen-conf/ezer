...
/// The caller resolves the ezer home (nothing in this crate reads `EZER_HOME`); tests pass a temp dir.
...
pub fn default_db_path(ezer_home: &Path) -> PathBuf {
    ezer_home.join("dashboard").join("workspace.db")
}
