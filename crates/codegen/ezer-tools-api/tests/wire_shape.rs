...
//! field rename/retype in `ezer-tools.proto` cannot silently break those
//! wire contracts (the producer and consumer live in separate services).

use ezer_tools_api::ToolConfigEntry;

fn full_entry() -> ToolConfigEntry {
    ToolConfigEntry {
        id: "Ezer:grep".to_owned(),
...
fn finalize_request_minimal() -> ezer_tools_api::FinalizeToolServerConfigRequest {
    ezer_tools_api::FinalizeToolServerConfigRequest {
        tools: vec![],
        truncation: None,
...
fn minimal_entry_deserializes_from_id_only() {
    ...
    let back: ToolConfigEntry =
        serde_json::from_value(serde_json::json!({"id": "Ezer:read_file"}))
            .expect("deserialize minimal");
    assert_eq!(back.id, "Ezer:read_file");
...
fn finalize_request_callback_fields_default_when_absent() {
    let back: ezer_tools_api::FinalizeToolServerConfigRequest =
