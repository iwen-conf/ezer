//! `ezer update` is a recovery command: a config failure must not block it, and a WinGet install hands off to WinGet.
//!
...
fn output(mut command: Command) -> Output {
    ...
    let child = {
        ...
        command.spawn().expect("spawn ezer")
    };
    child.wait_with_output().expect("wait for ezer")
}

/// Run `ezer update` in a fresh isolated home against the local pointer base.
...
fn copy_into_winget_package(root: &Path) -> PathBuf {
    ...
    let exe = package.join(format!("ezer{}", std::env::consts::EXE_SUFFIX));
    let _exec = EXEC_LOCK.lock().unwrap_or_else(|e| e.into_inner());
...
fn winget_install_update_hands_off_without_update_writes() {
    ...
    for envs in runs {
        ...
        assert!(
            ...
            "ezer update on a WinGet install must exit 0 (env {envs:?})\nstderr:\n{stderr}"
        );
        assert!(
            stderr.contains("winget upgrade --id xAI.GrokBuild -e")
                && stderr.contains("WinGet ships only the stable channel"),
            "ezer update must hand off to WinGet and flag the ignored alpha channel (env {envs:?})\nstderr:\n{stderr}"
        );
    ...
    pinned
        ...
        .env("EZER_REQUIRED_MINIMUM_VERSION", "0.0.2");
    ...
    let capped_cases = [
        CappedCase {
            envs: &[
                ("EZER_MAXIMUM_VERSION", "5.0.0"),
                ("EZER_TEST_VERSION", "1.0.0"),
            ],
            succeeds: true,
            expected: install_5,
            prints_install: true,
        ...
        CappedCase {
            envs: &[
                ("EZER_REQUIRED_MAXIMUM_VERSION", "5.0.0"),
                ("EZER_TEST_VERSION", "6.0.0"),
            ],
            succeeds: true,
            expected: install_5,
            prints_install: true,
        ...
        CappedCase {
            envs: &[
                ("EZER_MAXIMUM_VERSION", "2000.0.0"),
                ("EZER_REQUIRED_MINIMUM_VERSION", "1000.0.0"),
            ],
            succeeds: false,
