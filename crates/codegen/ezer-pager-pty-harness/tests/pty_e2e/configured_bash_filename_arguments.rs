...
impl FilenamePager {
    async fn start() -> FilenamePager {
        ...
        std::fs::write(sandbox.ezer_home().join("config.toml"), CONFIG).expect("Ask config");
        ...
        .expect("child PATH");
        let grants = grant_files(sandbox.ezer_home());
    ...
    fn assert_unchanged(&self) {
        ...
        assert_eq!(
            CONFIG,
            std::fs::read_to_string(self.content.sandbox().ezer_home().join("config.toml"))
        ...
        assert_eq!(self.grants, grant_files(self.content.sandbox().ezer_home()));
    }
}
