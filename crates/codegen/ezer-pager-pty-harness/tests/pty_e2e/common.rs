...
pub(crate) use ezer_pager_pty_harness::{
...
/// `"logs"` mirrors `ezer_telemetry::unified_log::LOG_DIR`; the harness does not link telemetry.
...
// `seed_fake_oauth` / `oauth_credential_ops` live in `ezer_pager_pty_harness::flows` (re-exported above)
...
/// `ezer_workspace::trust::TRUST_FILE_NAME`; the harness does not link the workspace crate.
...
// `wait_for_labels_absent` lives in `ezer_pager_pty_harness::flows` (re-exported above)
...
/// (`ezer_pager::app::app_view::ESC_DOUBLE_PRESS_TEST_MS`; the pager clamps larger values to it).
...
pub(crate) use ezer_pager_pty_harness::host_clipboard::{
...
pub(crate) use ezer_pager_pty_harness::host_clipboard::clipboard_roundtrip_works;
