...
//!
//! All three accounts suppress the welcome privacy banner even with `EZER_PRIVACY_NOTICE_ROLLOUT=1`.
...
//! Row/input details are unit-tested in `ezer-pager` (`views/settings_modal/tests.rs`, `locked_coding_*`).
//! This suite covers the auth-to-render pipeline.
//!
//! ```bash
//! cargo test -p ezer-pager-pty-harness --test settings_locked_row_e2e \
//!   -- --ignored --nocapture
//! ```
