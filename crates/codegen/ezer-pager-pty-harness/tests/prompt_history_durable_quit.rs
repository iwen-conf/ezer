...
//!
//! ```bash
//! cargo test -p ezer-pager-pty-harness --test prompt_history_durable_quit \
//!   -- --ignored --nocapture
//! ```
...
use ezer_pager_pty_harness::{ContentController, PtyExitPoll, PtyHarness, keys, pager_binary};
...
/// The per-CWD history file lives at `<home>/.ezer/sessions/<enc-cwd>/prompt_history.jsonl`.
fn find_prompt_history(home: &Path) -> Result<PathBuf> {
    let root = home.join(".ezer").join("sessions");
