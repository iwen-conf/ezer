...
//!
//! Uses `ezer-pager-pty-harness` (`PtyHarness`) and Shift+Tab (CSI Z, compatible with `ptyctl` key injection) to cycle Normal to Plan to Auto.
...
//!
//! Run with:
//! `cargo test -p ezer-pager-pty-harness --test pty_auto_mode -- --ignored --nocapture`

use std::path::PathBuf;
use std::time::Duration;

use ezer_pager_pty_harness::{PtyHarness, pager_binary};
use ezer_test_support::TestSandbox;
...
fn pty_shift_tab_cycles_to_auto_mode_banner() {
    ...
    let mut harness =
        PtyHarness::new_in_sandbox(&binary, ROWS, COLS, &[], &sandbox, &env_refs, None)
            .expect("spawn pager in PTY (ezer-pager-pty-harness)");
