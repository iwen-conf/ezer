...
fn local_pager_binary_path() -> Result<PathBuf> {
    Ok(target_dir()?
        .join("debug")
        .join(format!("ezer-pager{}", std::env::consts::EXE_SUFFIX)))
}
...
fn ensure_local_pager_binary(binary: &std::path::Path) -> Result<()> {
    ...
    let output = cmd
        .output()
        .with_context(|| format!("failed to spawn {cargo} to build ezer-pager"))?;

    if !output.status.success() {
        bail!(
            "failed to build ezer-pager (exit {:?})\nstdout:\n{}\nstderr:\n{}",
            output.status.code(),
    ...
    if !binary.exists() {
        bail!(
            "ezer-pager build completed but binary missing at {}",
            binary.display()
        );
    }
    Ok(())
...
/// `PAGER_BINARY`, then `CARGO_BIN_EXE_ezer-pager`, else build `ezer-pager-bin` (the package that owns the binary).
pub fn pager_binary() -> Result<PathBuf> {
    ...
    if let Ok(path) = std::env::var("CARGO_BIN_EXE_ezer-pager") {
        let p = PathBuf::from(path);
        if p.exists() {
            return Ok(p);
        }
