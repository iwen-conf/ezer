...
use ezer_pager_pty_harness::scroll_matrix::{
...
#[command(
    name = "scroll-matrix",
    about = "Run the scroll validation matrix against ezer-pager",
    long_about = None,
)]
struct Cli {
    /// Cell tier to run.
    ...
    /// Pager binary.
    /// Defaults to PAGER_BINARY, CARGO_BIN_EXE_ezer-pager, or a locally-built debug binary.
