//! Scripted TUI scenario runner for ezer-pager.
//!
...
impl ScriptedScenarioRunner {
    ...
    pub async fn run(&self, scenario: &ScriptedScenario) -> Result<ScriptedRunReport> {
        ...
        if let Some(config_toml) = &scenario.environment.config_toml {
            ...
            fs::create_dir_all(&ezer_home)
                .with_context(|| format!("create scenario EZER_HOME {}", ezer_home.display()))?;
            fs::write(ezer_home.join("config.toml"), config_toml)
...
fn materialize_workspace(
    ...
    sandbox: &ezer_test_support::TestSandbox,
