...
/// Regression guard: ezer-build must DROP the working tail post-compaction.
...
async fn grok_build_compaction_drops_working_tail_regression_206460() {
    ...
    assert!(
        ...
        "ezer-build must drop recent_messages post-compaction",
    );
    assert!(
    ...
    assert!(
        !compacted
            ...
            .any(|i| matches!(i, ConversationItem::ToolResult(_))
                || i.text_content() == "Tool call omitted..."),
        "no tail (ToolResult or stub) may leak into the ezer-build compacted history",
    );
}
