...
//!
//! After the workspace-server self-daemonizes (see [`crate::daemonize`]) it spawns the unchanged `/usr/local/bin/ezer-preview-proxy` binary.
...
pub const PREVIEW_PROXY_BIN_PATH: &str = "/usr/local/bin/ezer-preview-proxy";
...
impl PreviewArgs {
    /// Map the forwarded fields to the proxy's exact CLI flag names (see `ezer-preview-proxy/src/cli.rs`).
...
/// The proxy's control path for the last-activity stamp; mirrors `ezer-preview-proxy`'s `/__control/activity` route.
const PREVIEW_ACTIVITY_PATH: &str = "/__control/activity";

/// Effective proxy `--control-port` when the supervisor didn't set one (mirrors the default in `ezer-preview-proxy/src/cli.rs`).
...
/// A trait, not the concrete tracker, so this crate stays free of `ezer-workspace` and does not re-enter every downstream build.
...
mod tests {
    ...
    /// Tracker accounting and the wiring live in `ezer_workspace::activity` and the server binary tests.
    struct TestSink {
        window_ms: u64,
    ...
    fn to_argv_maps_every_flag_to_the_proxy_cli_names() {
        // Flag names must match ezer-preview-proxy/src/cli.rs exactly.
        assert_eq!(
            sample_cfg().to_argv(),
            vec![
                "--preview-port",
