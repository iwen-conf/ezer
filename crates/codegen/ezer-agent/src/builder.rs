...
/// Backed by the shared vendor-to-ezer tool registry in `ezer-tools` (also used by the hook matcher).
fn claude_tool_kind(name: &str) -> Option<ToolKind> {
    ezer_tools::types::kind_for(name)
}
...
pub struct AgentBuilder {
    ...
    memory_v2_access: Option<ezer_tools::types::memory_v2::MemoryV2AccessResource>,
    is_non_interactive: bool,
    system_prompt_label: String,
    session_env: Option<Arc<HashMap<String, String>>>,
    state_path: Option<PathBuf>,
    memory_backend: Option<Arc<dyn ezer_tools::types::memory_backend::MemoryBackend>>,
    ...
    attribution_callback: Option<ezer_tools::SharedAttributionCallback>,
...
fn task_lifecycle_satisfier(
    tool_config: &ezer_tools::registry::types::ToolServerConfig,
) -> bool {
    use ezer_tools::types::tool::ToolNamespace;
    ...
    has(ToolNamespace::EzerBuild, "run_terminal_cmd", true)
        || has(ToolNamespace::EzerBuildConcise, "run_terminal_cmd", true)
        || has(ToolNamespace::OpenCode, "bash", false)
}
/// `Cursor:Shell` can background, but it does not satisfy ezer Build output tools.
...
fn strip_task_lifecycle(tool_config: &mut ezer_tools::registry::types::ToolServerConfig) {
    tool_config
        .tools
...
fn merge_tool_params(
    tool_config: &mut ezer_tools::registry::types::ToolServerConfig,
    ids: &[&str],
...
impl AgentBuilder {
    pub fn new(
        ...
        Self {
            ...
            terminal_backend,
            fs_backend: Arc::new(ezer_tools::computer::local::LocalFs),
            ...
            system_reminder_tag: ezer_tools::reminders::DEFAULT_REMINDER_TAG,
    ...
    pub fn with_preloaded_skills(
        mut self,
        skills: Vec<ezer_tools::implementations::skills::types::SkillInfo>,
    ) -> Self {
    ...
    pub fn with_memory_v2_access(
        mut self,
        access: Option<ezer_tools::types::memory_v2::MemoryV2AccessResource>,
        exposed: bool,
    ) -> Self {
    ...
    pub fn with_memory_backend(
        mut self,
        backend: Arc<dyn ezer_tools::types::memory_backend::MemoryBackend>,
    ) -> Self {
    ...
    pub fn with_web_search_config(
        mut self,
        config: ezer_tools::implementations::web_search::WebSearchConfig,
    ) -> Self {
    ...
    pub fn with_lsp(
        mut self,
        handle: std::sync::Arc<dyn ezer_tools::implementations::lsp::LspBackend>,
    ) -> Self {
    ...
    pub fn with_api_key_provider(
        mut self,
        provider: ezer_tools::types::SharedApiKeyProvider,
    ) -> Self {
    ...
    pub fn with_attribution_callback(
        mut self,
        callback: ezer_tools::SharedAttributionCallback,
    ) -> Self {
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        macro_rules! build_step_timer {
            ($step:literal) => {
                ezer_telemetry::startup_step_timer_grouped!("agent_build", $step)
            };
        }
        macro_rules! build_await_step {
            ($step:literal $(, $field:ident = $value:expr)* $(,)?) => {
                ezer_telemetry::startup_step_grouped!("agent_build", $step $(, $field = $value)*)
            };
        }
        ...
        if definition.inject_default_tools {
            ...
            if self.write_file_enabled && !has_write_tool && has_edit_tool {
                tool_config
                    .tools
                    .push((&ezer_tools::implementations::opencode::OpenCodeWriteTool).into());
            }
            if self.prompt_audience == PromptAudience::Primary {
        ...
        if self.prompt_audience == crate::prompt::context::PromptAudience::Subagent {
            ...
            tool_config.tools.retain(|tool| {
                !matches!(
                    tool.kind,
                    Some(
                        ezer_tools::types::tool::ToolKind::AskUser
                            | ezer_tools::types::tool::ToolKind::Feedback
                    )
                ) && tool.id != feedback_id
        ...
        if let Some(bytes) = self.mcp_max_output_bytes {
            tool_bridge.toolset().resources.lock().await.insert(
                ezer_tools::types::resources::TruncationCfg(
                    ezer_tools::types::context::TruncationConfig {
        ...
        if use_backend_search {
            if web_search_enabled && definition.hosted_tool_allowed("web_search") {
                hosted_tools.push(ezer_sampling_types::HostedTool::WebSearch { options: None });
            }
            if definition.hosted_tool_allowed("x_search") {
                hosted_tools.push(ezer_sampling_types::HostedTool::XSearch { options: None });
            }
            ezer_sampling_types::apply_tool_overrides(
                &mut hosted_tools,
...
fn resolve_shell_for_prompt() -> String {
    ...
    #[cfg(not(unix))]
    {
        ezer_config::shell::detect_windows_shell()
            .name()
            .to_string()
    }
}
...
mod tests {
    use super::*;
    use ezer_tools::types::definition::ToolDefinition;
    use ezer_tools::types::template_renderer::unresolved_template_markers;
    #[derive(Debug)]
    struct TestMemoryV2Access {
        roots: [PathBuf; 2],
    }
    impl ezer_tools::types::memory_v2::MemoryV2Access for TestMemoryV2Access {
        ...
        fn write_file(
            ...
            _path: &std::path::Path,
            _contents: &[u8],
        ) -> std::result::Result<ezer_tools::types::memory_v2::MemoryV2Write, String> {
            Ok(ezer_tools::types::memory_v2::MemoryV2Write::Outside)
        }
    ...
    async fn memory_v2_access_sets_prompt_roots_from_policy() {
        ...
        let access = ezer_tools::types::memory_v2::MemoryV2AccessResource(Arc::new(
            TestMemoryV2Access {
                roots: [
    ...
    async fn unexposed_memory_v2_access_installs_policy_without_prompt_block() {
        use ezer_tools::computer::local::LocalTerminalBackend;
        let access = || {
            ezer_tools::types::memory_v2::MemoryV2AccessResource(Arc::new(TestMemoryV2Access {
                roots: [
                    PathBuf::from("/memory/global"),
                    PathBuf::from("/memory/workspace"),
                ],
    ...
    fn task_model_guidance_resolves_model_param_override() {
        use ezer_tools::types::template_renderer::TemplateRenderer;
        use ezer_tools::types::tool::ToolKind;
    ...
    async fn custom_child_toolset_cannot_reintroduce_workflow() {
        ...
        definition.tool_config.tools = vec![
            (&ReadFileTool).into(),
            (&WorkflowTool).into(),
            ezer_tools::registry::types::ToolConfig::from_id("Ezer:workflow"),
        ];
        let names =
    ...
    async fn task_tool_tracks_whether_general_purpose_is_spawnable() {
        ...
        let disabled = AgentBuilder::new(
            std::env::temp_dir(),
            Arc::new(ezer_tools::computer::local::LocalTerminalBackend::new()),
        ...
        let bash = disabled
            .tool_bridge()
            .read_resource::<Params<ezer_tools::implementations::ezer_build::bash::BashParams>>(
            )
            .await
            .expect("bash params");
    ...
    async fn build_with_web_search(
        ...
        tool_overrides: Option<ezer_sampling_types::ToolOverrides>,
    ) -> crate::agent::Agent {
        use ezer_tools::computer::local::LocalTerminalBackend;
        use ezer_tools::implementations::web_search::WebSearchConfig;
        use ezer_tools::notification::ToolNotificationHandle;
    ...
    async fn disallowed_web_search_strips_function_and_hosted_tools() {
        ...
        assert!(
            !hosted
                .iter()
                .any(|t| matches!(t, ezer_sampling_types::HostedTool::WebSearch { .. })),
            "hosted WebSearch must be removed when web_search is disallowed, got: {hosted:?}"
        );
        assert!(
            hosted
                .iter()
                .any(|t| matches!(t, ezer_sampling_types::HostedTool::XSearch { .. })),
    ...
    async fn hosted_tools_bake_definition_tool_overrides_into_options() {
        let x_search = ezer_sampling_types::XSearchOptions {
            date_bound: Some(
                ezer_sampling_types::SearchDateBound::new(None, Some("2024-03-15".into()))
                    .unwrap(),
            ),
        };
        let agent = build_with_web_search(
            ...
            true,
            &[],
            Some(ezer_sampling_types::ToolOverrides {
        ...
        assert!(
            agent
                .hosted_tools()
                .contains(&ezer_sampling_types::HostedTool::XSearch {
