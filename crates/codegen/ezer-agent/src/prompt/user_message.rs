...
//! - `Default`: the legacy ezer prefix (built by the shell layer).
...
use ezer_tools::types::skill_discovery_tracker::{XmlRenderMode, format_announcement_xml};
...
pub enum UserMessageTemplate {
    /// Legacy ezer prefix (`<user_info>` and optional `<git_status>`), built directly by the shell layer.
...
/// One discovered rule file (AGENTS.md / Claude.md / .ezer/rules/*.md).
...
pub struct UserMessageContext {
    ...
    /// User-scoped rule files (~/.ezer/, ~/.claude/).
...
mod tests {
    ...
    fn format_rules_section_keeps_markdown_heading_off_the_open_tag() {
        let user = [RuleEntry {
            path: "/home/dev/.ezer/rules/personal.md".into(),
