...
//!
//! Rendering is done by `ToolBridge::render_prompt()` which delegates to `TemplateRenderer` in `ezer-tools`.
...
pub struct PromptContext {
    ...
    /// Identity in the primary ezer-build system prompt (`You are <label>…`).
    /// Not the UI picker name. Defaults to [`DEFAULT_SYSTEM_PROMPT_LABEL`].
    #[serde(default = "default_system_prompt_label")]
    pub system_prompt_label: String,
}
/// Default identity on trim-tool-descriptions (`You are ezer`).
pub const DEFAULT_SYSTEM_PROMPT_LABEL: &str = "ezer";
...
mod tests {
    ...
    fn test_agents_md_paths_use_display_cwd_in_rendered_section() {
        ...
        let overlay_path = "/root/.ezer/worktrees/my-project/ab-123-a-overlay";
        let ctx = PromptContext {
