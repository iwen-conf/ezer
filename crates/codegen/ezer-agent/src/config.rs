...
use ezer_tools::implementations::use_tool;
...
impl AgentDefinition {
    ...
    pub fn override_file_tools(
        &mut self,
        file_tools: Vec<ezer_tools::registry::types::ToolConfig>,
    ) {
...
mod tests {
    ...
    fn task_tool_rename_matches_task_tool_id_predicate() {
        ...
        assert!(ezer_tools::is_task_tool_id(&name));
    }
    ...
    fn bash_tool_rename_matches_writing_tool_kind() {
        ...
        assert_eq!(
            ezer_tools::tool_taxonomy::writing_tool_kind(&name),
            Some(ezer_tools::types::tool::ToolKind::Execute)
        );
    }
    /// Native presets only.
    #[test]
