...
async fn manual_auth_posts_to_events_endpoint_as_ezer_shell_dir_manual_auth() {
    ...
    ezer_telemetry::log_event(ManualAuth {
    ...
    for (key, expected) in [
        ...
        (
            "dev_build",
            serde_json::json!(ezer_version::IS_DEV_BUILD),
        ),
    ] {
        assert_eq!(
            meta.get(key),
    ...
    let conditional: &[&str] = &[
        ...
        ezer_telemetry::activity::COMPACTIONS_ACTIVE_KEY,
    ...
    for key in ezer_telemetry::client::RESERVED_EVENT_KEYS {
        assert!(
