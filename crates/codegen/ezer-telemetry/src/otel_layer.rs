...
pub use ezer_otel::config::{OtelClientInfo, OtelExporterConfig, OtelLayerConfig};
    ...
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    let mode = match crate::instrumentation::current_mode() {
        crate::instrumentation::InstrumentationMode::Server => {
            ezer_otel::provider::OtelProviderMode::Server
        }
        _ => ezer_otel::provider::OtelProviderMode::Local,
    };
    ezer_otel::provider::build_otel_layer(
        client,
        config,
        mode,
...
pub fn shutdown_otel() {
    let provider = std::thread::spawn(ezer_otel::provider::shutdown_provider);
