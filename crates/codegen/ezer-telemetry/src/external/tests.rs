...
fn tool_result_hook_rewrote_is_content_free() {
    for (hook_rewrote, want) in [(true, "true"), (false, "false")] {
        ...
        emit_event_into(&stream, &{
            let mut event = events::completed_for_test("run_terminal_cmd", "ezer");
...
fn tool_result_gates_off_collapses_and_reduces() {
    let stream = build(gates_off());
    emit_event_into(&stream, &{
        let mut event = events::completed_for_test("docs__post_message", "ezer");
...
/// the ezer-workspace sampler events.
#[test]
...
fn from_snapshot_copies_user_id_without_email() {
    let attrs = super::IdentityAttrs::from_snapshot(&ezer_auth::CredentialSnapshot {
...
fn identity_api_key_user_id_exports_without_email() {
    ...
    super::set_identity_on(
        &stream.ext,
        super::IdentityAttrs::from_snapshot(&ezer_auth::CredentialSnapshot {
            user_id: Some("api-key-principal".into()),
            ..Default::default()
        }),
    );
...
fn lock_content_gates_drops_prompt_and_response_not_email() {
    ...
    emit_event_into(&stream, &{
        let mut event = events::completed_for_test("run_terminal_cmd", "ezer");
        event.outcome = ezer_session_events::types::ToolOutcome::Error;
...
fn details_without_content_exports_preview_not_bodies() {
    let mut ev = events::completed_for_test("run_terminal_cmd", "ezer");
    ev.outcome = ezer_session_events::types::ToolOutcome::Error;
...
fn content_without_details_exports_bodies_not_preview() {
    let mut ev = events::completed_for_test("docs__post_message", "ezer");
    ev.outcome = ezer_session_events::types::ToolOutcome::Error;
