...
/// in `ezer-pager`) so the firehose directives below and the pager's own filter are built from the same constants. A
...
/// `ezer-pager/src/tracing.rs` for the consumer side.
...
mod tests {
    ...
    /// Bridge diagnostics moved to `ezer_gateway`; the firehose allowlist must keep it at debug.
    #[test]
    fn firehose_directives_allowlist_gateway_target() {
        assert!(firehose_directives().contains("ezer_gateway=debug"));
    }
    ...
    fn layer_routes_event_by_session_span() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            ...
            tracing::info_span!("session", session_id = %"sess-xyz").in_scope(|| {
                tracing::info!(target: "ezer_shell", "inside session");
            });
            tracing::info!(target: "ezer_shell", "outside session");
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_under_real_firehose_filter() {
        ...
        // Pins at the unit level that the `session` span (INFO, target `ezer_telemetry::session_ctx`) survives the real filter
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!(
                target: "ezer_telemetry::session_ctx",
                "session",
                session_id = %"sid-real"
            )
            .in_scope(|| {
                tracing::debug!(target: "ezer_shell", "filtered routing works");
            });
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_two_sessions_to_distinct_files() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!("session", session_id = %"sid-one").in_scope(|| {
                ...
                tracing::info!(target: "ezer_shell", "one first");
                tracing::info!(target: "ezer_shell", "one second");
            });
            tracing::info_span!("session", session_id = %"sid-two").in_scope(|| {
                tracing::info!(target: "ezer_shell", "two only");
            });
        });
        crate::appender::flush_file_log_guards();
