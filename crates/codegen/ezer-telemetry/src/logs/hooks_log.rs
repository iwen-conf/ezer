...
//! ## When to use
//!
//! Use regular `tracing::info!` / `tracing::debug!` / `tracing::warn!` with targets `ezer_hooks` or `ezer_agent::plugins`.
...
/// Filters to hooks (`ezer_hooks`) and plugins (`ezer_agent::plugins`) targets.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter = tracing_subscriber::filter::EnvFilter::new(
        "ezer_hooks=debug,ezer_agent::plugins=debug",
    );
