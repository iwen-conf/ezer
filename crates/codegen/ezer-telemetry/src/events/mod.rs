...
//!
//! Extracted from `ezer-shell` so binaries (TUI, sampler) can reuse them without the shell's HTTP product-analytics client.
...
mod tests {
    ...
    fn tool_call_completed_omits_tool_result_size_bytes_when_absent() {
        let mut with_size = completed_for_test("bash", "ezer");
