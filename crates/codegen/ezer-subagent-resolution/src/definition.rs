...
pub fn discover_agent_definition(
    ...
    ezer_agent::discovery::by_name_in_cwd_with_plugins(
        subagent_type,
        context.cwd,
        context.plugins,
    )
...
pub fn available_agent_names(context: &DefinitionResolutionContext<'_>) -> Vec<String> {
    let mut available: Vec<String> = ezer_agent::discovery::all_subagents_with_plugins(
        context.cwd,
        context.toggles,
        context.plugins,
    )
...
pub async fn render_subagent_initial_user_message(
    ...
    working_directory: &Path,
    compat: CompatConfig,
    paths: &ezer_agent::prompt::paths::PathsConfig,
    project_trusted: bool,
) -> Option<String> {
    if !definition.agents_md {
        return None;
    }
    let agents_md_files = ezer_agent::prompt::agents_md::read_agents_config_with_paths(
