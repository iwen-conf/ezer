...
fn register_worktree_writes_correct_fields() {
    // Isolate EZER_HOME so register_worktree's open_default write lands
    ...
    // (EZER_HOME is process-global) can't INSERT-OR-REPLACE our row.
...
fn gc_removes_an_expired_path_with_no_repo_and_counts_it_apart() {
    ...
    // `remove_worktree` reclaims either. EZER_HOME is the gc DB dir so
...
fn gc_with_delegate_removes_expired_and_unregisters() {
    ...
    // EZER_HOME == the gc DB dir so remove_worktree's open_default
