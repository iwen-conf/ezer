...
pub fn load_from_disk() -> std::io::Result<toml::Value> {
    // Live `$EZER_HOME`: `user_ezer_home()` / `ezer_home()` are OnceLock
...
/// A hook's origin (held by `ezer_hooks::HookSpec::layer`).
/// Defined here, not in `ezer-hooks`, since the dep direction is `ezer-hooks -> ezer-config`.
...
pub enum HookProvenance {
    ...
    Requirements,
    /// `$EZER_HOME/requirements.toml` while its bytes match the server-signed envelope (see [`crate::signed_policy::signed_requirements_attest`]).
    SignedRequirements,
    /// `$EZER_HOME/requirements.toml` without a signed attestation (user-writable).
    UserRequirements,
...
impl HookProvenance {
    ...
    /// The unsigned `$EZER_HOME` tiers never qualify, since the user owns that directory.
...
pub fn apply_version_overrides_with_registered(value: &mut toml::Value) -> std::io::Result<()> {
    match ezer_version::installed_semver() {
...
mod tests {
    ...
    /// The same `$EZER_HOME/requirements.toml` stamps the exempt `SignedRequirements` while its bytes verify against the server's signature, and drops back to `UserRequirements` the moment they differ.
    #[test]
