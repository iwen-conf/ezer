...
/// Prefer `ezer_shell::util::config::load_effective_config` when remote campaigns or `EZER_CAMPAIGNS_OVERRIDE` must be honored.
...
/// Path to `$EZER_HOME/campaigns_state.json` under `home`.
