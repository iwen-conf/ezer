#!/bin/bash
#
# ezer CLI installer (enterprise channel) — https://x.ai/cli/enterprise-install.sh
#
...
#
# Auth: EZER_DEPLOYMENT_KEY (takes precedence) or ~/.ezer/auth.json from `ezer login`.
...
#   EZER_DEPLOYMENT_KEY=<key> bash <(curl -fsSL https://x.ai/cli/enterprise-install.sh)
#
...
DOWNLOAD_DIR="$HOME/.ezer/downloads"
BIN_DIR="${EZER_BIN_DIR:-$HOME/.ezer/bin}"
...
mkdir -p "$HOME/.ezer/completions/bash" "$HOME/.ezer/completions/zsh"
"$BIN_DIR/ezer" completions bash > "$HOME/.ezer/completions/bash/ezer.bash" 2>/dev/null || true
"$BIN_DIR/ezer" completions zsh  > "$HOME/.ezer/completions/zsh/_grok"     2>/dev/null || true
...
CONFIG_FILE="$HOME/.ezer/config.toml"
...
# Also update shell config so ~/.ezer/bin is on PATH for future sessions
...
fish_add_path $HOME/.ezer/bin
# <<< ezer installer <<<'
    elif [ "$user_shell" = "zsh" ]; then
        new_block='# >>> ezer installer >>>
export PATH="$HOME/.ezer/bin:$PATH"
fpath=(~/.ezer/completions/zsh $fpath)
