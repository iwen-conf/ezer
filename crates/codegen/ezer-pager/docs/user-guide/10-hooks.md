...
Hooks let you run a script or send an HTTP request at key moments in a ezer session. Use them to automate tasks, enforce safety checks, log activity, send notifications, and integrate your own tools.

---

## What Are Hooks?

A hook is a shell command or HTTP endpoint that ezer calls when a specific lifecycle event occurs. Hooks can:
...
| Config | `requirements.toml` (signed cache, and `/etc/ezer`) | Always | Organization-enforced hooks; see [Enforced hooks](#enforced-hooks) |
...
ezer accepts Cursor's camelCase hook event names, so `~/.cursor/hooks.json` loads unchanged:
...
- **Event name** (top-level key): any event listed in [Hook Events](#hook-events). ezer skips unrecognized event names so a shared Claude or Cursor settings file still loads.
...
In a `matcher`, ezer maps Claude-style tool names to its own so hooks migrated from Claude fire correctly. Common aliases include:
...
When an event fires, ezer resolves it in four steps:
...
Hooks can also live directly in your ezer config, so a team can distribute them with the rest of their configuration instead of shipping separate JSON files. The same `hooks` object is read from three TOML files:

| File | Tier | Who sets it |
|------|------|-------------|
| `~/.ezer/config.toml` | User | You |
| `managed_config.toml` (`$EZER_HOME`, `/etc/ezer`) | Managed / system | Your organization |
| `requirements.toml` (`$EZER_HOME` signed cache, `/etc/ezer`) | Requirements | Your organization |
...
Hooks from the config layers your organization controls are enforced: they carry a `[policy]` badge in `/hooks`, `Space` refuses to disable them, an entry in `~/.ezer/disabled-hooks` does not skip them, and their source cannot be removed. Two layers qualify:

- The root-owned system files `/etc/ezer/requirements.toml` and `/etc/ezer/managed_config.toml` (`requirements/system:` and `system_managed:` names).
- The `~/.ezer/requirements.toml` that the deployment sync writes, while its bytes match the signed policy the server sent (`requirements/signed:` names). If the file is edited or its signature file is missing or unreadable, its hooks load as your own (`requirements/user:` names) and can be disabled again; an unreadable `requirements.toml` contributes no hooks. Organizations that need the file to stay intact set `fail_closed = true` in the same requirements, which refuses to start on an edited copy or a missing signature (an unreadable file is a read error and still starts). See [Configuration](26-config-reference.md#requirementstoml).

Hooks in `~/.ezer/managed_config.toml` and `~/.ezer/config.toml` are distribution, not enforcement: you can disable them.
...
- **What runs.** Only [enforced hooks](#enforced-hooks): the ones from the root-owned `/etc/ezer/requirements.toml` and `/etc/ezer/managed_config.toml`, and the ones in the synced `~/.ezer/requirements.toml` while it matches its signature. An edited synced file loads as your own hooks, which the pin skips.
- **What is skipped.** Every other hook, at dispatch: `~/.ezer/hooks`, every `~/.ezer/*.toml` file, project hooks, plugin hooks, agent frontmatter hooks, and the Claude and Cursor compatibility files. In `/hooks` they show `[disabled]`, and enabling them is refused ("Hooks outside managed policy are disabled by your organization."); `grok inspect` names the file that set the pin.
...
- **Windows.** There is no root-owned layer (no `/etc/ezer`), so only the signed synced `requirements.toml` hooks and ACP client hooks still run.

The key is a tighten-only policy pin: any native policy layer (`requirements.toml` or `managed_config.toml` in `$EZER_HOME` or `/etc/ezer`, or macOS MDM) or Claude's `managed-settings.json` can set it, the camelCase `allowManagedHooksOnly` is accepted everywhere, no layer can release it, and a non-boolean value engages it. Like the other policy pins it is read once at startup, so a pin added mid-session applies at the next start, not on `/hooks` Reload. `grok inspect` lists it under **Enforced by policy** as "Hooks outside managed policy disabled" with the file that set it.
...
Only hooks configured in a settings file (command and HTTP hooks) can ask, defer, or send `additionalContext`: a `PreToolUse` hook registered through the ezer-agent-sdk can allow or deny, and the rest is dropped — an `ask` or a `defer` there leaves the call to the normal permission flow and is logged as an unrecognized decision, and `additionalContext` never reaches the model.
...
- **Output replacement is settings-file only.** Command and HTTP hooks can do all of this. A `PostToolUse` hook registered through the ezer-agent-sdk can contribute a `block` reason and `additionalContext`, but cannot replace the tool output.
...
A script that tracks busy and idle should key on `promptId`, following the rules below. ezer mints one per turn, but a client that supplies its own in `_meta` owns its uniqueness, so treat the id as opaque and scope it to the session.
...
- **Settle unconditionally when there is no `promptId`.** That is ezer reporting on the session
...
- **Keep it to a local write.** Teardown gives the whole queue of turn-end reports half a second,
  ...
  `EZER_SESSION_END_HOOKS_TIMEOUT_MS`, in milliseconds, to change that default, capped at 60s).
...
- **camelCase input**: ezer's stdin envelope uses camelCase keys throughout where Claude uses snake_case. A script reading `.stop_hook_active` or `.background_tasks[].agent_type` must switch to `.stopHookActive` and `.backgroundTasks[].agentType` (the `hook_event_name` snake_case key carries Claude's PascalCase value, e.g. `"Stop"`; the `hookEventName` camelCase key carries ezer's snake_case value, e.g. `"stop"`). Hooks registered through the ezer-agent-sdk convert both the top-level keys and the `backgroundTasks`/`sessionCrons` entry keys to snake_case, so the wire's `.backgroundTasks[].agentType` reads as `.background_tasks[].agent_type` in the SDK.
- **`toolResult` field**: the `PostToolUse` tool output is `toolResult` (SDK: `tool_result`); ezer also emits a `tool_response` snake alias that copies `toolResult`, so a hook reading Claude's `.tool_response` works unchanged.
- **`updatedToolOutput` carries ezer's own output shape on built-in tools**: a `PostToolUse` replacement for a built-in tool is validated against the tool's output as ezer serializes it — the tagged object in that event's `toolResult` — so one written against another runtime's field names parses as the wrong shape and is ignored. On an MCP tool there is no shape to enforce, so `updatedToolOutput` passes through like its `updatedMCPToolOutput` alias. See [PostToolUse Output](#posttooluse-output).
...
- **StopFailure classes**: ezer emits six (`rate_limit`, `authentication_failed`, `invalid_request`, `server_error`, `max_output_tokens`, `unknown`). Capacity errors (503/529) classify as `rate_limit`. A matcher on an error class ezer does not emit never fires.
- **Default timeout**: ezer defaults observe hooks to 5 seconds, which is shorter than most. Set `timeout` explicitly on an imported hook that does real work.
...
- **`StopCancelled` is ezer-specific**: a config that uses it is not portable to a runtime with no interrupt hook.
- **`idle_prompt` fires on any turn end**: ezer fires it after an interrupted or errored turn too, not only a completed one, because it reports a state rather than an outcome. Its `message` is display text and can change between releases, so match on `notificationType` instead.
- **Subagent identity is `subagentType`, not `agent_type`**: ezer puts it in the payload of the events that can fire inside a subagent, matching its own `SubagentStart`/`SubagentStop`, rather than in the common fields.
- **permission_mode values**: ezer emits `default`, `auto`, `plan`, or `bypassPermissions`. Claude's `acceptEdits`/`dontAsk` have no ezer equivalent (ezer's `auto` is the nearest), so a check like `permission_mode === "acceptEdits"` never matches.
...
- **`/goal`**: ezer's goal loop is a separate feature that runs before the stop gate; it is not a prompt-type Stop hook.
...
ezer sets several environment variables on every hook process. These are useful when writing context-aware or plugin-aware hook scripts.
...
|-----------------------|-------------|
| `EZER_HOOK_EVENT`     | The name of the event that triggered the hook (e.g. `pre_tool_use`, `session_start`, `post_tool_use`, `session_end`, `stop`, `notification`). |
| `EZER_HOOK_NAME`      | The configured name of this specific hook (includes the plugin prefix for plugin-provided hooks). |
| `EZER_SESSION_ID`     | The unique identifier of the current ezer session. |
| `EZER_WORKSPACE_ROOT` | Absolute path to the root of the current workspace. |
| `CLAUDE_PROJECT_DIR`  | Absolute path to the workspace root. A Claude Code-compatible alias for `EZER_WORKSPACE_ROOT`, set for every hook. |
...
When a hook originates from a plugin, ezer additionally injects the following variables:

| Variable             | Description |
|----------------------|-------------|
| `EZER_PLUGIN_ROOT`   | Absolute path to the plugin's installed directory. |
| `EZER_PLUGIN_DATA`   | Absolute path to the plugin's writable data directory (for storing plugin state, caches, etc.). |

These values are provided by the plugin system. For the four plugin-related keys (`EZER_PLUGIN_ROOT`, `EZER_PLUGIN_DATA`, and their Claude aliases), the plugin adapter ensures the official plugin values always win over any user-declared values in the hook's `env` map.
...
Press `r` in the Hooks tab to reload all hooks from disk. ezer re-reads every hook source, so this picks up changes you made to hook files during the session.
...
- **See errors?** Capture logs by launching with `RUST_LOG=debug EZER_LOG_FILE=/tmp/ezer.log ezer`, then check `/tmp/ezer.log`.
