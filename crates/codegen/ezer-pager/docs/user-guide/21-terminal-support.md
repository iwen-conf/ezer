...
ezer runs as a full-screen TUI. It relies on terminal support for color,
...
Run `/doctor` in ezer to check the current session and see available fixes. If
ezer cannot start, run `ezer doctor` in your shell. Use `ezer doctor --json`
...
to guess if that directory is unavailable or unsafe. ezer preserves the file's
...
ezer deliberately does **not** run `tmux source-file` or change the live tmux
...
ezer detects these terminal emulators from environment variables:
...
- Inside tmux, variables that identify the outer terminal may not reach ezer.
...
Inside tmux there are two separate questions: what color ezer emits, and what
...
ezer writes through up to three routes, shown in `/doctor` under **Clipboard**:

- **native** — the local operating-system clipboard.
- **tmux** — the tmux paste buffer when ezer runs inside tmux.
...
terminal focused. Older compositors may require ezer to remain focused until
the copy message appears. ezer shows a startup warning when this applies; run
`/doctor` for the detected status and steps.

`EZER_CLIPBOARD_NO_DATA_CONTROL=1` is an advanced fallback that disables the
data-control route. Copies then use command-line clipboard tools.

#### OSC 52 kill switch

ezer emits OSC 52 on Linux and across tmux, SSH, or displayless containers when
that route is enabled. A terminal that does not implement OSC 52 may display the
encoded payload as text. Set `EZER_CLIPBOARD_NO_OSC52=1` before starting ezer to
...
A remote ezer process normally cannot read the local terminal's selection. Use
...
When ezer cannot identify the outer terminal over SSH, it predicts that OSC 52
...
default; override with `EZER_COPY_FILE`); the toast names that path when delivery
...
When an SSH session is not using `ezer wrap`, ezer shows the one-time tip
...
Zellij and tmux control mode can limit the alternate screen. ezer normally uses
...
### Zellij keybindings interfere with ezer

Zellij can intercept Ctrl/Alt keys before they reach ezer. On Zellij 0.41 or
...
mode, if `Ctrl+G` still does not reach ezer, open the command palette and select
...
WezTerm ships with the Kitty keyboard protocol disabled. Run `/doctor` in ezer.
...
keys. ezer therefore does not negotiate the protocol there, and Shift+Enter can
...
`Cmd+Enter` is not an advertised send or newline chord. ezer advertises
...
If ezer stops receiving mouse input, re-enable mouse reporting in the terminal:
...
After about 10 seconds without a transcript, ezer stops capture and shows
...
to the terminal hosting ezer. Open **System Settings → Privacy & Security →
...
shows the microphone ezer would use. If no input device is available, Doctor
...
problem, set `EZER_VOICE_CAPTURE=inprocess` to use the in-process fallback for
comparison.
...
terminals, Terminal.app, Konsole, mlterm, and others). ezer therefore
