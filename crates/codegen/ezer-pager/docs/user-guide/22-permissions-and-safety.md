...
Control what ezer can access and do: permission modes, allow/ask/deny rules, hooks, and the optional OS-level sandbox.

- **Modes** set how often ezer asks for approval (always-approve, auto, ask, and related).
...
When ezer edits a file, runs a command, or calls an external tool, it may pause for approval. Permission modes control how often that happens.
...
```bash
ezer -p "Run the tests" --always-approve
ezer agent --always-approve stdio
...
```bash
ezer --always-approve -p "Run the test suite"
ezer --permission-mode auto
ezer agent --always-approve serve --bind 127.0.0.1:2419 --secret <token>
```
...
```bash
ezer -p "Deploy the service" --always-approve --deny 'Bash(rm -rf *)'
```
...
Organizations can prevent always-approve from being enabled via CLI, TUI, or `/always-approve`. Set this in `requirements.toml` (user-level under `~/.ezer/`, or system-wide under `/etc/ezer/` for enforcement users cannot remove):
...
ezer can still load Claude-style permission **rules** from managed settings; always-approve is locked with `requirements.toml` as shown above.
...
ezer reads permission rules from three compatible sources. Rules from all sources are merged into one set; a rule's effect depends on its action (`deny` > `ask` > `allow`), not on which file it came from.
...
|-------|------|-----------------------|
| Global (all projects) | `~/.ezer/config.toml` | No |
| Project (committed) | `<project>/.ezer/config.toml` | Yes (commit it) |
| Project (personal) | `<project>/.claude/settings.local.json` | No (gitignore it) |
| Interactive grants | Stored internally by ezer, per project | No |

Notes on scoping:

- ezer discovers a `.ezer/config.toml` at every directory level from the repository root down to your working directory, so a subdirectory can add rules on top of the repo root's.
...
- ezer has no native `config.local.toml`. For personal, uncommitted rules in a project, use `.claude/settings.local.json`; ezer reads it directly (see [Claude Code Compatibility](#3-claude-code-compatibility-claudesettingsjson)).
...
To stop prompts for a specific command in one project, add a narrow allow rule to that project's `.ezer/config.toml` (or `.claude/settings.json`):
...
### 2. Native Configuration (`~/.ezer/config.toml` and `.ezer/config.toml`)
...
Rules from the global `~/.ezer/config.toml` and every project `.ezer/config.toml` (from the repo root down to your working directory) are merged into one rule set, alongside any `.claude/settings.json` rules.

Managed configuration deployed by your organization also contributes `[permission]` rules: the system `/etc/ezer/managed_config.toml`, and a user-level copy that ezer maintains automatically at `~/.ezer/managed_config.toml`. Managed rules merge like rules from any other source, with two properties specific to managed `allow` rules: your own `deny` and `ask` rules win over a managed `allow` (severity ordering), and a catch-all managed `allow` is ignored when always-approve is locked off. For rules that users cannot edit away, use the root-owned system `/etc/ezer/requirements.toml`.
...
ezer reads `~/.claude/settings.json` and `~/.claude/settings.local.json`, plus the project-level `<project>/.claude/settings.json` and `settings.local.json` (walking up to the repo root). The native `.ezer` source for permission rules is `config.toml`, described in the section above.
...
Supported `defaultMode` values include `default`, `auto`, `acceptEdits`, `bypassPermissions`, `dontAsk`, and `plan`. ezer reads `defaultMode` from its canonical location under `permissions`; a top-level `defaultMode` is also accepted when the nested key is absent.
...
**Chained commands.** ezer parses each command like a shell and splits it on `&&`, `||`, `;`, `|`, and newlines. The rule actions treat segments differently:
...
`MCPTool(...)` patterns match the full ezer tool name in `server__tool` form, with glob support: `MCPTool(linear__*)` matches every tool from the `linear` server. ezer tool names carry no `mcp__` prefix.
...
Interactive grants are stored in ezer's own state directory under your home directory, scoped to the git repository you launched ezer in (its repository root), so a grant accepted at the repo root also applies in sessions started from a subdirectory of the same repository. Outside a git repository, grants are scoped to the launch directory, and each git worktree keeps its own grants. A grant made in one project never applies in another, grants are not written into the repository, and they are not meant to be hand-edited.

To inspect or reset a project's grants, open the `sessions` subdirectory of your ezer home (the `.ezer` directory under your home directory, or `$EZER_HOME`): each project directory there (URL-encoded scope root) holds a `permission.toml` (plus per-client `permission_<client>.toml` variants) listing the remembered command prefixes, globs, MCP tools/servers, web-fetch domains, and "never allow" entries. Deleting the file resets that project's grants; the next matching tool call prompts again. Treat it as read-only state — to *add* rules, use the declarative `[permission]` configuration instead.

Interactive grants are personal, per-machine state. For an allowlist you can review in code review and share with teammates, use declarative rules in the project's `.ezer/config.toml` instead.
...
```bash
chmod +x ~/.ezer/hooks/git-gh-only.sh
```
...
```bash
ezer -p "Implement the feature using only git and GitHub CLI" \
  --allow 'Read' \
  --allow 'Grep' \
  --allow 'Bash(git *)' \
  --allow 'Bash(gh *)'
...
3. **Review project configuration from unfamiliar sources.** Folder trust gates project permission rules in `.ezer/config.toml` and `.claude/settings.json`, plus startup loading of project instructions and skills. Headless startup with these sources requires `--trust` or a prior grant. Review them and any project hooks before trusting an unfamiliar checkout (see [10-hooks.md](10-hooks.md)).
