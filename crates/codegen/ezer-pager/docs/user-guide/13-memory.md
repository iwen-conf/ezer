...
Memory lets ezer recall facts, decisions, and patterns from earlier sessions. ezer indexes the information you save and searches it automatically, so a new session can reuse relevant context.

---

## What Is Memory?

Without memory, each ezer session starts fresh: the model knows nothing about previous sessions. When you enable memory, ezer can:
...
curated notes, one file per subject, and is what ezer reads at the start of a
...
Notes you recorded with earlier versions of ezer are carried over
...
The toggle cannot override the process-wide force-disable (`--no-memory` or `EZER_MEMORY=0`); those hide `/memory` for the whole session.

### Priority Order

1. A process-wide force-disable (`--no-memory` compatibility flag or
   `EZER_MEMORY=0`) turns memory off.
...
3. Otherwise memory is enabled by `EZER_MEMORY=1`, `[memory] enabled = true`,
...
|----------|-------|-------------|
| `~/.ezer/memory/MEMORY.md` | Global | Facts that apply across all your projects |
| `~/.ezer/memory/<project-slug>-<hash8>/MEMORY.md` | Workspace | Project-specific conventions and context |
| `~/.ezer/memory/<project-slug>-<hash8>/sessions/` | Sessions | Per-session summaries and logs |

ezer suffixes each workspace directory with a short hash of the repository's identity. The identity is the `origin` remote in `org/repo` form when the directory is a Git repository with an `origin` remote, or the directory path otherwise. Because clones and worktrees of the same repository share an `origin` remote, they also share one memory directory.
...
When a session ends, ezer saves a structured metadata summary to that session's daily log. The summary contains:
...
ezer builds the summary from conversation metadata without an LLM call, without added latency. ezer skips the save for trivial sessions -- those with fewer than three substantive prompts, or fewer than 50 bytes of user text.
...
Ask ezer to remember something, and it appends the note to a `MEMORY.md` file -- the workspace file for project-specific items, or the global `~/.ezer/memory/MEMORY.md` for cross-project preferences:

```
> remember to always open PR links after pushing
```

ezer records entries as durable statements under organized headings, such as `## Preferences`, `## Project Context`, or `## Debugging`. The file watcher reindexes the change on the next memory search, so the new entry is searchable within the current session.
...
Run `/remember` with no text to enter remember mode, where the next line you type becomes the note. Either way, ezer opens a review panel showing the note (with an optional rewritten version you can toggle with `Tab`); the note is written only after you confirm. On save, ezer shows `Memory saved to ~/.ezer/memory/MEMORY.md`.

### Forget

Ask ezer to forget something, and it finds and removes the matching entry:

```
> forget the snake_case convention
```

Forget is best-effort: the model searches memory and removes entries that match. For guaranteed removal, edit the files under `~/.ezer/memory/` directly and delete the entry yourself. To locate a file, open the `/memory` browser and press `y` to copy its path.

### Recall

Ask what ezer remembers:

```
> what do you remember?
```

ezer searches across all memory files and summarizes what it knows, grouped by source: global preferences, project-specific knowledge, and session history. Use `/memory` to browse the raw files.

### Direct Editing

You can edit memory files directly under `~/.ezer/memory/`. The file watcher reindexes your changes on the next memory search. Use `/flush` to save the current session now, and `/dream` to consolidate session logs into organized topics.
...
When you save a note with `/remember`, ezer confirms in the scrollback:

```
Memory saved to ~/.ezer/memory/MEMORY.md
```

Background saves — automatic flush, automatic Dream, and session-end — run silently and do not post a scrollback message. `/flush` and `/dream` report their outcome in scrollback when you run them yourself. Use `/memory` at any time to browse what ezer has stored.
...
Dream also runs automatically. By default, ezer checks the consolidation gates at launch and periodically during a session, and runs Dream once enough time has passed and enough sessions have accumulated:
...
On the first turn of each session, ezer automatically searches memory for content relevant to the current project and injects it as context. This means ezer starts with knowledge from previous sessions without a reminder.
...
ezer searches memory automatically, but you can also trigger searches manually in the chat:
...
The `ezer memory` command manages memory from the shell. It has one subcommand, `clear`:
...
ezer memory clear --workspace

# Clear the global MEMORY.md
ezer memory clear --global

# Clear both workspace and global memory
ezer memory clear --all

# Skip the confirmation prompt (-y is the short form)
ezer memory clear --yes
```

To edit memory from the shell, open the files in your editor directly -- for example, `$EDITOR ~/.ezer/memory/MEMORY.md`.
...
| `watcher.enabled` | `true` | Watch `~/.ezer/memory/` for external edits and reindex |
...
| `flush_model` | unset | Model for the flush turn. When unset or `""`, ezer uses the session's primary model. |
...
When a session memory is old, ezer attaches a staleness note to it in search results. Older results get a stronger reminder to verify the current state before you rely on them. These notes help you spot stored facts that might no longer be accurate. Global and workspace memories never receive staleness notes, because they hold curated long-term knowledge.

---

## File Watcher

By default, ezer watches `~/.ezer/memory/` for external file changes. If you edit memory files directly (e.g., in your editor), the changes are picked up automatically on the next memory search:
...
1. Verify memory is enabled: check `ezer inspect` output.
2. Check `EZER_MEMORY` or `[memory] enabled` in effective TOML.
3. Check for `EZER_MEMORY=0` or a deprecated compatibility flag overriding config.
...
```bash
ls ~/.ezer/memory/
cat ~/.ezer/memory/MEMORY.md
$EDITOR ~/.ezer/memory/MEMORY.md
```
