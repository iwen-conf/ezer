...
Not `/config-agents` (alias `/agents`), which manages agent *definitions* and personas. Hidden in minimal mode; disable with `EZER_AGENT_DASHBOARD=0` or `[dashboard].enabled = false`.

### `/compact [context]`

Compress conversation history to reclaim context-window space. Pass a note to tell ezer what to keep:
...
ezer also auto-compacts once the context window hits 85% (tune it with `[session] auto_compact_threshold_percent`).
...
Every copy is also written to a backup file — `~/.ezer/last-copy.txt` by default, or `EZER_COPY_FILE` if set. Confirmed copies toast briefly (e.g. `Copied!`). Unverified OSC 52 deliveries and clipboard-unreachable fallbacks name the backup path so you can recover the text.
...
`/flush` and `/dream` require memory enabled through `EZER_MEMORY=1`, `[memory] enabled = true`, or managed remote settings. `/memory` is available whenever a memory store is configured for the session, including when `[memory] enabled = false` turned memory off, so you can browse saved notes and turn memory on for the session from inside the modal. It is hidden only when no memory store is configured, or when `--no-memory` / `EZER_MEMORY=0` turned memory off for the whole process. `/remember` is always available.
...
Run a prompt on a recurring interval. Give the interval as `30m`, `1 hour`, or `every 2 days`; leave it out and ezer will ask.
...
Set, manage, or check an autonomous goal. ezer works across rounds and only marks the goal complete after an independent evidence review confirms the claim; if that review can't reproduce the result or has no usable evidence, the goal stays active or pauses with concrete gaps.
...
Project workflows live in `.ezer/workflows/*.rhai`; user workflows live in `~/.ezer/workflows/*.rhai`. A same-process pause/resume continues the original immutable script, args, and `agent_budget` cap from committed host-call results — to iterate, edit the returned script copy and launch it as a new run.
...
Open the extensions modal on the **Workflows** tab — a browse-only catalog of the saved workflows ezer discovered (built-ins, project `.ezer/workflows/`, and user `~/.ezer/workflows/`), with each entry's source, description, and path. The same catalog is listed for the model under the skill listing in the session preamble. Launch one with `/workflow <name>` (or its own slash command), then watch it in `/workflow runs`.
...
- `/docs web` opens  in your browser.
...
For persisted per-turn token and cost totals of any local session, use `ezer usage <session-id> [turn]` from the shell. See [Session Management](17-sessions.md#the-ezer-usage-subcommand).
...
Any enabled skill with `user-invocable: true` in its SKILL.md frontmatter shows up as a slash command. (Turn a skill off via `/skills` and it stops being advertised.) So a skill at `~/.ezer/skills/commit/SKILL.md` runs as:
