...
Disable subagents with a CLI flag, an environment variable, or the config file (highest priority first). The same rules apply to the interactive `ezer` TUI, `ezer agent stdio`, and headless runs.

```bash
ezer --no-subagents                  # This session only
...
ezer manages worktrees through the `ezer/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
[subagents.models]
explore = "workbuddy"                 # route explore to a specific model
```
...
- `Subagent running: "do the thing" (Implementer · workbuddy) · Thinking`
