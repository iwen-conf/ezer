...
ezer reads settings from config files, environment variables, and CLI flags. This page covers the common options. The field list for `config.toml`, `managed_config.toml`, and `requirements.toml` is [26-config-reference.md](26-config-reference.md) (extracted to `~/.ezer/docs/user-guide/` on launch).
...
2. **Environment variables** (e.g. `XAI_API_KEY`, `EZER_MEMORY`)
3. **`requirements.toml` / MDM** (org-enforced; clamps every config layer below, including the overlay)
4. **`EZER_CONFIG` / `EZER_CONFIG_PATH` overlay** (above `config.toml` and managed, below `requirements.toml` / MDM)
5. **config.toml** (`~/.ezer/config.toml`)
6. **`managed_config.toml`** (org-deployed defaults; below `config.toml`)
7. **Built-in defaults**

Within the config-file tier, the layers merge lowest-to-highest: `managed_config.toml` → `config.toml` → `EZER_CONFIG` overlay → `requirements.toml` / MDM. So `requirements.toml` and MDM clamp **both** your `config.toml` and the overlay.

`EZER_CONFIG` / `EZER_CONFIG_PATH` (tier 4) are config **overlays**: a merged config layer, not direct-setting environment variables like `XAI_API_KEY` (tier 2). They set config keys (subject to the allowlist below), so read them as part of the config-file tier rather than the env-var tier.
...
# Env vars EZER_MAX_PARALLEL_IMAGE_GEN_CALLS / EZER_MAX_PARALLEL_VIDEO_GEN_CALLS
...
You can also flip it from the settings pane (`/settings` → **Disable vim input mode**); ezer writes your choice to `[ui] simple_mode`. `simple_mode` and `vim_mode` are independent — one governs the prompt editor, the other governs scrollback navigation. See [Keyboard Shortcuts](03-keyboard-shortcuts.md) for the full binding reference.
...
You can also override this with `EZER_DEFAULT_SELECTED_PERMISSION`, which is handy for headless or agent test runs that shouldn't mutate `config.toml`. Precedence: env var → `config.toml` → `always_allow_all_sessions`.
...
Toggle it at runtime with `/vim-mode`, or from `/settings` → **Vim scrollback navigation**. ezer writes the change to `[ui] vim_mode` immediately and applies it to every future pager session, including new agents and subagents in the same process. There's no per-session override — `config.toml` is the source of truth on next launch. `vim_mode` is independent of `simple_mode`.

#### Screen mode

`[ui] screen_mode` is the **default render mode** for plain `ezer` launches. Set it from `/settings` → **Default screen mode** (restart required) or edit `config.toml` by hand — both write the file. CLI flags (`--minimal` / `--fullscreen`) and slash commands (`/minimal` / `/fullscreen`) are session-scoped and do **not** write this key; after a slash switch, the reverse command returns you for that session only.
...
Each setting also has an environment-variable override, applied on first load only (again, handy for headless / test runs): `EZER_SCROLL_SPEED`, `EZER_SCROLL_MODE`, `EZER_INVERT_SCROLL` (`1`/`true`/`0`/`false`), and `EZER_SCROLL_LINES`. Precedence: env var → `config.toml` → default. Unrecognized values fall back to the default, and out-of-range numbers clamp.
...
`allow_local` is off by default (SSRF fail-closed). Turn it on (or set `EZER_WEB_FETCH_ALLOW_LOCAL=1`) and `web_fetch` may reach **explicit** loopback hosts only — private, link-local, and cloud-metadata ranges stay blocked. Resolution: TOML > env > default off.
...
`[toolset.ask_user_question]` is honored across **requirements.toml**, **managed config**, and your user **`config.toml`**. Precedence: requirements → env (`EZER_ASK_USER_QUESTION_TIMEOUT_ENABLED` / `EZER_ASK_USER_QUESTION_TIMEOUT_SECS`) → user config → managed → defaults. Set `timeout_enabled = false` in your user config to disable the automatic questionnaire timeout for yourself; `timeout_secs` must be a positive integer. You can also toggle `timeout_enabled` from `/settings` → **Ask-Question timeout** (under Agent & Approval); changes apply to newly started sessions.
...
Remote (HTTP/SSE) servers receive a default `User-Agent: ezer-cli/<version>` header; a
valid `User-Agent` entry in `headers` overrides it (Figma servers receive bare
`ezer-cli`). See [MCP servers](07-mcp-servers.md) for details.

MCP servers can also be set per-project in `.ezer/config.toml`. Project-scoped config contributes `[mcp_servers]`, `[plugins]`, and `[permission]` rules; every other section loads only from `~/.ezer/config.toml`.

Priority for `[mcp_servers]` and `[plugins]`: `.ezer/config.toml` (current dir) > `<repo-root>/.ezer/config.toml` > `~/.ezer/config.toml`. `[permission]` rules aren't overridden by priority — they merge across all files with `deny` > `ask` > `allow` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).

### Memory

Persist knowledge across sessions. Enable it with `[memory] enabled = true` or
`EZER_MEMORY=1`; an explicit `[memory] enabled = false` turns it off even when a
...
Background workflows — the `workflow` tool, named `.ezer/workflows/*.rhai` scripts, `/deep-research`, and `/workflow` launches — are **on by default**. Disable with config, env, or remote settings.

```toml
[workflows]
enabled = false                       # disable background workflows (or EZER_WORKFLOWS=0)
```

Project workflows are discovered from `<repo-root>/.ezer/workflows/`; user workflows from `~/.ezer/workflows/`. Discovery and invocation key off the script's `meta.name`, so keep each filename aligned with its `meta.name`. Built-ins win over project names, and project names win over user names, so keep names unique across scopes.
...
`ezer inspect` reports cells that still need session-start resolution as `?` until a value is available; cells with an explicit env or TOML value use that value. Affected discovery entries report `compatibilityStatus: "unresolved"` in JSON and `[compat unresolved]` in human output.
...
`[hints]` holds small persisted UI preferences: remembered answers and modal layout. ezer writes these for you as you use the TUI, but you can edit or delete them by hand; removing a key restores the default.

`[hints]` is read from the **effective config merge**, with the usual precedence: system managed → user `managed_config.toml` → user `config.toml` → user `requirements.toml` → system `requirements.toml`, higher layers winning. The TUI only ever **writes** these to your user `~/.ezer/config.toml`.
...
| `title.items` | array | (see above) | Items shown in the title bar. Options: `action-required`, `spinner`, `activity`, `session-name`, `cwd`, `model`, `turn-timer`, `ezer`. |
...
| ezer Desktop | None (native) | N/A | N/A |
| Unknown | BEL | No | No |

With `method = "auto"`, ezer detects the terminal brand and picks the best protocol. Set `method` explicitly to override that.

#### Notification hooks

Run your own commands when events fire. Hooks receive `$EZER_EVENT`, `$EZER_MESSAGE`, and `$EZER_SESSION_ID` in the environment.
...
# Push to ntfy server
[[ui.notifications.hooks]]
command = "curl -s -d '$EZER_MESSAGE' ntfy.sh/my-ezer-alerts"
...
- **`[features] telemetry`** / `EZER_TELEMETRY_ENABLED` — the product-analytics master switch. `/privacy` doesn't change it.
- **Coding data, retention, and training** — the Settings row `/privacy` opens; coding-data sharing, separate from telemetry.
- **`[telemetry] trace_upload`** / `EZER_TELEMETRY_TRACE_UPLOAD` — session traces; follows telemetry when unset.
- **`[telemetry] otel_*`** / `EZER_EXTERNAL_OTEL` — external OTEL to your own collector (below).
...
Set these only to point telemetry at your own infrastructure or to switch parts off. The built-in endpoint and credentials are managed by ezer — leave them unset to use the defaults.
...
```toml
[telemetry]
otel_enabled = true                                       # external OTEL master switch (= EZER_EXTERNAL_OTEL)
...
- `minimum_version` (`EZER_MINIMUM_VERSION`) is a soft anti-downgrade floor. The
  updater skips a target below it and keeps the current version. It never blocks
  startup.
- `maximum_version` (`EZER_MAXIMUM_VERSION`) is a soft ceiling. The updater caps
  its target at it and never installs above it.
- `required_minimum_version` (`EZER_REQUIRED_MINIMUM_VERSION`) and
  `required_maximum_version` (`EZER_REQUIRED_MAXIMUM_VERSION`) are hard bounds. If
  the running version is outside the range, the CLI exits at startup and instructs
  the user to install an approved version. `ezer update` and `ezer --version` keep
...
- An explicit `ezer update --version X` is allowed above the ceiling, to recover
...
Location: `~/.ezer/pager.toml`. This controls the TUI's look and feel. Changes apply on restart.
...
| `EZER_AUTH_EARLY_INVALIDATION_SECS` | Seconds before expiry to refresh (default: 300) |
| `EZER_OIDC_ISSUER` | OIDC issuer URL |
| `EZER_OIDC_CLIENT_ID` | OIDC client ID |

### Endpoints

| Variable | Description |
|----------|-------------|
| `EZER_CLI_CHAT_PROXY_BASE_URL` | Override API proxy base URL |
...
| `EZER_AGENT` | Custom agent definition path or name |
| `EZER_SANDBOX` | Sandbox profile (off, workspace, devbox, read-only, strict; or a custom profile name) |
| `EZER_EXIT_TIMEOUT_SECS` | Seconds after a quit is requested before the process is force-exited if teardown hangs (default: 20, `0` disables; a hard exit follows 5s later) |

### Logging

| Variable | Description |
|----------|-------------|
| `EZER_LOG_FILE` | Write logs to this file path (used verbatim as the path) |
| `RUST_LOG` | Log level filter (e.g. `debug`); controls the `EZER_LOG_FILE` log and headless stderr output |
...
| Variable | Description |
|----------|-------------|
| `EZER_TELEMETRY_ENABLED` | Enable/disable telemetry |
| `EZER_TELEMETRY_TRACE_UPLOAD` | Enable/disable session trace upload |
| `EZER_TELEMETRY_MIXPANEL_ENABLED` | Enable/disable Mixpanel specifically |
| `EZER_EXTERNAL_OTEL` | External OTEL to your collector (see [24-monitoring-usage.md](24-monitoring-usage.md)) |
| `EZER_FEEDBACK_ENABLED` | Enable/disable feedback system |
| `EZER_DEPLOYMENT_KEY` | Management API key for enterprise |
...
| Path | Description |
|------|-------------|
| `~/.ezer/config.toml` | Main configuration file |
| `~/.ezer/pager.toml` | TUI appearance configuration |
...
| `~/.ezer/lsp.json` | LSP server configuration (user-scoped) |
| `~/.ezer/logs/` | Internal log files (e.g. `unified.jsonl`, MCP server logs) |
| `.ezer/config.toml` | Project-scoped MCP servers, plugins, and permission rules |
...
### How ezer saves `config.toml`

Writes to **`~/.ezer/config.toml`** (`/settings`, `/vim-mode`, and other user-config saves) follow a leaf symlink. The atomic rename writes the referent (a file in your dotfiles repo). The link stays a link. If the link is dangling, the write creates the referent as a regular file.

Writes to a **project** `.ezer/config.toml` (MCP / plugin / permission edits) **replace** a leaf symlink with a regular file. That keeps a later save from following the link out of the repository.
...
Some settings can be set per-project by placing files in `.ezer/` inside your repository:

| File | What it configures |
|------|--------------------|
| `.ezer/config.toml` | MCP servers, plugins, permission rules, and the `[mcp] max_output_bytes` tool-result cap (other sections load only from `~/.ezer/config.toml`) |
| `.ezer/skills/` | Project-specific skills |
| `.ezer/hooks/` | Project-specific lifecycle hooks |
| `.ezer/agents/` | Project-specific agent definitions |
| `.ezer/lsp.json` | LSP server configuration |
| `.ezer/sandbox.toml` | Custom sandbox profiles |
...
|--------|----------|-------|
| User | `~/.ezer/lsp.json` | All projects |
| Project | `.ezer/lsp.json` | Current repository |
...
1. **Project** — `.ezer/lsp.json`
2. **User** — `~/.ezer/lsp.json`
