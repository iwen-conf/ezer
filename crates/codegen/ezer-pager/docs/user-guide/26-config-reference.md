...
This file ships with the CLI and is extracted to `~/.ezer/docs/user-guide/26-config-reference.md` on launch. It is the complete field list for `config.toml`, `managed_config.toml`, and `requirements.toml`. For conceptual guidance see [05-configuration.md](05-configuration.md).

## How to configure

Three files configure ezer, and they are written by different people.
...
| `agent.name` | `string` | `yes` | `user` | Built-in or discovered agent definition name. Also EZER_AGENT and `--agent-profile`. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `announcements` | `array of tables` | `—` | `user` | Local/managed announcement tables only. Payloads fetched from xAI / ezer.com / x.com are stripped and never shown. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `cli.auto_update` | `boolean` | `pin` | `user` | Check GitHub (`iwen-conf/ezer` or `$EZER_UPSTREAM_REPO`) on launch and show a one-time notice. Never hits first-party update channels and never auto-installs. Once-flag: `~/.ezer/update-notice.json`. Also EZER_DISABLE_AUTOUPDATER to suppress. |
...
| `cli.maximum_version` | `string` | `pin` | `user` | Highest CLI version that still runs without a hard block. Also EZER_MAXIMUM_VERSION. |
| `cli.minimum_version` | `string` | `pin` | `user` | Lowest CLI version that still runs without a hard block. Also EZER_MINIMUM_VERSION. |
...
| `cli.required_maximum_version` | `string` | `pin` | `user` | Hard maximum CLI version. Also EZER_REQUIRED_MAXIMUM_VERSION. |
| `cli.required_minimum_version` | `string` | `pin` | `user` | Hard minimum CLI version. Also EZER_REQUIRED_MINIMUM_VERSION. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `compat.claude.agents` | `boolean` | `yes` | `user` | Scan CLAUDE.md. Also EZER_CLAUDE_AGENTS_ENABLED. |
| `compat.claude.hooks` | `boolean` | `yes` | `user` | Scan Claude hooks. Also EZER_CLAUDE_HOOKS_ENABLED. |
| `compat.claude.mcps` | `boolean` | `yes` | `user` | Scan Claude MCP config. Also EZER_CLAUDE_MCPS_ENABLED. |
| `compat.claude.rules` | `boolean` | `yes` | `user` | Scan Claude rules. Also EZER_CLAUDE_RULES_ENABLED. |
| `compat.claude.skills` | `boolean` | `yes` | `user` | Scan Claude skills. Also EZER_CLAUDE_SKILLS_ENABLED. |
...
| `compat.cursor.agents` | `boolean` | `yes` | `user` | Scan agent definitions from Cursor compat sources. Also EZER_CURSOR_AGENTS_ENABLED. |
| `compat.cursor.hooks` | `boolean` | `yes` | `user` | Scan Cursor hooks. Also EZER_CURSOR_HOOKS_ENABLED. |
| `compat.cursor.mcps` | `boolean` | `yes` | `user` | Scan Cursor mcp.json. Also EZER_CURSOR_MCPS_ENABLED. |
| `compat.cursor.rules` | `boolean` | `yes` | `user` | Scan `.cursor/rules/`. Also EZER_CURSOR_RULES_ENABLED. |
| `compat.cursor.skills` | `boolean` | `yes` | `user` | Scan Cursor skills directories. Also EZER_CURSOR_SKILLS_ENABLED. |
...
| `endpoints.deployment_key` | `string` | `pin` | `user` | Management key for enterprise deployments. Also EZER_DEPLOYMENT_KEY. |
| `endpoints.feedback_base_url` | `string` | `yes` | `user` | Where feedback submissions go. Also EZER_FEEDBACK_BASE_URL. |
| `endpoints.managed_config_url` | `string` | `yes` | `user` | Override managed config endpoint. Also EZER_MANAGED_CONFIG_URL. |
| `endpoints.models_base_url` | `string` | `pin` | `user` | Custom inference base URL. Also EZER_MODELS_BASE_URL. |
| `endpoints.models_list_url` | `string` | `pin` | `user` | Override model-list URL. Also EZER_MODELS_LIST_URL. Alias `models_endpoint`. |
| `endpoints.trace_upload_bucket` | `string` | `yes` | `user` | Direct gs:// or s3:// bucket for traces; bypasses the proxy. Also EZER_TRACE_UPLOAD_BUCKET. |
...
| `endpoints.trace_upload_credentials_file` | `string (path)` | `yes` | `user` | Path to a GCS service-account JSON or AWS credentials file for that bucket. Also EZER_TRACE_UPLOAD_CREDENTIALS_FILE. |
| `endpoints.trace_upload_endpoint_url` | `string` | `yes` | `user` | Custom S3-compatible endpoint for s3:// bucket uploads. Also EZER_TRACE_UPLOAD_ENDPOINT_URL. |
| `endpoints.trace_upload_region` | `string` | `yes` | `user` | AWS region for s3:// bucket uploads; default us-east-1. Also EZER_TRACE_UPLOAD_REGION. |
| `endpoints.trace_upload_url` | `string` | `pin` | `user` | Proxy destination for traces when no direct bucket is set. Also EZER_TRACE_UPLOAD_URL. |
| `endpoints.xai_api_base_url` | `string` | `pin` | `user` | Public xAI API base. Also EZER_XAI_API_BASE_URL. |

### `features`

| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `features.active_agent_messages` | `boolean` | `pin` | `user` | Enable or disable `active_agent_messages`. Default false. Also `EZER_ACTIVE_AGENT_MESSAGES`. |
| `features.ask_user_question` | `boolean` | `pin` | `user` | Enable or disable `ask_user_question`. Default true. Also `EZER_ASK_USER_QUESTION`. |
| `features.auto_wake` | `boolean` | `pin` | `user` | Enable or disable `auto_wake`. Default true. Also `EZER_AUTO_WAKE`. |
| `features.backend_tools` | `boolean` | `pin` | `user` | Enable or disable `backend_tools`. Default true. Also `EZER_BACKEND_SEARCH`. |
| `features.campaigns` | `boolean` | `yes` | `user` | Enable remote campaign patches. `EZER_CAMPAIGNS=0` still disables even when requirements set this true. |
| `features.cancel_rewind` | `boolean` | `pin` | `user` | Enable or disable `cancel_rewind`. Default true. Also `EZER_CANCEL_REWIND`. |
...
| `features.compaction_detail` | `none / minimal / balanced / verbose` | `yes` | `user` | Verbatim detail level for `segments` compaction. Also EZER_COMPACTION_DETAIL. |
| `features.compaction_mode` | `summary / transcript / segments` | `yes` | `user` | Compaction strategy. Also EZER_COMPACTION_MODE. |
| `features.compaction_tool_choice` | `string` | `yes` | `user` | Tool-choice hint used during compaction. |
| `features.compaction_verbatim_input` | `boolean` | `pin` | `user` | Enable or disable `compaction_verbatim_input`. Default true. Also `EZER_COMPACTION_VERBATIM_INPUT`. |
| `features.dock` | `boolean` | `pin` | `user` | Enable or disable `dock`. Default false. Also `EZER_DOCK`. |
| `features.feedback` | `boolean` | `pin` | `user` | Enable or disable `feedback`. Default true. Also `EZER_FEEDBACK_ENABLED`. |
| `features.feedback_trace_card` | `boolean` | `pin` | `user` | Show a trace-upload consent question after `/feedback`. Default false. Also `EZER_FEEDBACK_TRACE_CARD`. |
...
| `features.lsp_tools` | `boolean` | `pin` | `user` | Enable or disable `lsp_tools`. Default false. Also `EZER_LSP_TOOLS`. |
| `features.managed_config` | `boolean` | `yes` | `user` | Fetch managed_config.toml and requirements.toml from the deployment. |
| `features.mcp_auto_restart` | `boolean` | `yes` | `user` | Auto-restart stdio MCP servers after transport failure. Also EZER_MCP_AUTO_RESTART. |
...
| `features.mcp_push_server_status` | `boolean` | `yes` | `user` | Pager subscribes to MCP server_status push. Process env EZER_MCP_PUSH_SERVER_STATUS wins at launch. |
| `features.mcp_recursive_config_watch` | `boolean` | `yes` | `user` | Watch `<cwd>/` and `<cwd>/.ezer/` for project MCP config edits. Name is a misnomer; watches are non-recursive. |
...
| `features.terminal_theme` | `boolean` | `pin` | `user` | Reveal the terminal-native `terminal` color theme during its rollout. Default false. Also `EZER_TERMINAL_THEME`. |
| `features.title_refresh` | `boolean` | `pin` | `user` | Early-session auto-title refresh. Pin this in requirements to beat EZER_TITLE_REFRESH. |
| `features.turn_summary` | `boolean` | `pin` | `user` | Enable or disable `turn_summary`. Default true. Also `EZER_TURN_SUMMARY`. |
| `features.two_pass_compaction` | `boolean` | `pin` | `user` | Enable or disable `two_pass_compaction`. Default true. Also `EZER_TWO_PASS_COMPACTION`. |
| `features.video_gen` | `boolean` | `pin` | `user` | Enable video tools / `/imagine-video`. |
| `features.voice_mode` | `boolean` | `pin` | `user` | Enable or disable `voice_mode`. Default true. Also `EZER_VOICE_MODE`. |
| `features.web_fetch` | `boolean` | `pin` | `user` | Enable or disable `web_fetch`. Default false. Also `EZER_WEB_FETCH`. |
| `features.write_file` | `boolean` | `pin` | `user` | Enable or disable `write_file`. Default true. Also `EZER_WRITE_FILE`. |
| `features.zdr_access_enabled` | `boolean` | `pin` | `user` | Advertise ZDR-incompatible tools when the team is on Zero Data Retention. Also `EZER_ZDR_ACCESS_ENABLED`. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `managed_mcps.enabled` | `boolean` | `pin` | `user` | Fetch managed MCP configs at startup. Also EZER_MANAGED_MCPS_ENABLED. |
| `managed_mcps.gateway_tools_enabled` | `boolean` | `yes` | `user` | Expose managed MCP gateway tools. Also EZER_MANAGED_MCP_GATEWAY_TOOLS_ENABLED. |
...
| `marketplace.require_sha` | `boolean` | `yes` | `user` | Tighten-only: remote plugin installs and updates must pin a full commit sha. Also `EZER_MARKETPLACE_REQUIRE_SHA`. Neither this key nor the env var can turn the gate back off. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `memory.enabled` | `boolean` | `pin` | `user` | Legacy memory switch. Also `EZER_MEMORY`; superseded when the v2 gate is enabled. |
...
| `model.<id>.supports_backend_search` | `boolean` | `yes` | `user` | Whether the endpoint supports ezer-hosted server-side search tools. |
...
| `models.default` | `string` | `pin` | `user` | Model used for new sessions. Also `EZER_DEFAULT_MODEL`, `--model`, `-m`. |
...
| `models.web_search` | `string` | `pin` | `user` | Model used by the client `web_search` tool. Also `EZER_WEB_SEARCH_MODEL`. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `sandbox.auto_allow_bash` | `boolean` | `pin` | `user` | Skip bash permission prompts when a sandbox profile is active. Also EZER_SANDBOX_AUTO_ALLOW_BASH. |
| `sandbox.profile` | `off / workspace / read-only / strict / string` | `pin` | `user` | Filesystem sandbox profile. Also `--sandbox` and EZER_SANDBOX. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `telemetry.otel_enabled` | `boolean` | `pin` | `user` | External OTEL master switch. Also EZER_EXTERNAL_OTEL. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `tools.disable_zdr_incompatible_tools` | `boolean` | `yes` | `user` | Restrict tools that need xAI-hosted output under ZDR. Also EZER_DISABLE_ZDR_INCOMPATIBLE_TOOLS. |
| `tools.media_gen.max_parallel_image_gen_calls` | `integer` | `yes` | `user` | Cap parallel image_gen/image_edit calls in one model step. Also EZER_MAX_PARALLEL_IMAGE_GEN_CALLS. |
| `tools.media_gen.max_parallel_video_gen_calls` | `integer` | `yes` | `user` | Cap parallel video_gen calls in one model step. Also EZER_MAX_PARALLEL_VIDEO_GEN_CALLS. |
| `tools.respect_gitignore` | `boolean` | `pin` | `user` | When true, search and read tools skip gitignored files. Also EZER_RESPECT_GITIGNORE. |
...
| `toolset.web_fetch.proxy_endpoint` | `string` | `yes` | `user` | Egress proxy URL for web_fetch. Also EZER_WEB_FETCH_PROXY. |
...
| `ui.collapsed_edit_blocks` | `boolean` | `yes` | `user` | Show edits as one-line +N/-M summaries. Also EZER_COLLAPSED_EDIT_BLOCKS. |
...
| `ui.contextual_hints.ssh_wrap` | `boolean` | `yes` | `user` | Recommend `ezer wrap` when SSH lacks a clipboard sink. |
...
| `ui.default_selected_permission` | `string` | `yes` | `user` | Preselected approval row on the first prompt of a session. Also EZER_DEFAULT_SELECTED_PERMISSION. |
| `ui.display_refresh.auto_cadence_enabled` | `boolean` | `yes` | `user` | Match stream/scroll cadence to display refresh rate. Also EZER_DISPLAY_REFRESH_AUTO_CADENCE. |
...
| `ui.group_tool_verbs` | `boolean` | `yes` | `user` | Fold consecutive read/search/list tool rows. Also EZER_GROUP_TOOL_VERBS. |
| `ui.hunk_tracker_mode` | `agent_only / all_dirty / off` | `yes` | `user` | File-change hunk tracking. Also EZER_HUNK_TRACKER and `--hunk-tracker-mode`. |
| `ui.invert_scroll` | `boolean` | `yes` | `user` | Reverse vertical scroll direction. Also EZER_INVERT_SCROLL. |
...
| `ui.mouse_reporting_toggle` | `boolean` | `yes` | `user` | Ctrl+R in scrollback toggles terminal mouse capture. Also EZER_MOUSE_REPORTING_TOGGLE. |
...
| `ui.prompt_suggestions` | `boolean` | `yes` | `user` | Next-prompt ghost text after each turn. Also EZER_PROMPT_SUGGESTIONS; a remote kill-switch can disable it fleet-wide. |
...
| `ui.remember_tool_approvals` | `boolean` | `yes` | `user` | Show per-tool Always allow options. Also EZER_REMEMBER_TOOL_APPROVALS. |
| `ui.render_mermaid` | `auto / on / off` | `yes` | `user` | How mermaid fences render: clickable open row or raw source. |
| `ui.screen_mode` | `fullscreen / minimal` | `yes` | `user` | Default render mode for plain `ezer`. Restart required. |
| `ui.scroll_lines` | `integer` | `yes` | `user` | Lines per scroll tick (1–10). Also EZER_SCROLL_LINES. |
| `ui.scroll_mode` | `auto / wheel / trackpad` | `yes` | `user` | Scroll input classification. Also EZER_SCROLL_MODE. |
| `ui.scroll_speed` | `integer` | `yes` | `user` | Mouse/trackpad scroll speed multiplier (1–100). Also EZER_SCROLL_SPEED. |
| `ui.show_thinking_blocks` | `boolean` | `yes` | `user` | Show thinking/reasoning blocks while streaming. Also EZER_SHOW_THINKING_BLOCKS. |
...
| `ui.theme` | `string` | `yes` | `user` | Color theme name, or `auto`/`system` to follow the OS. Also `/theme` and EZER_THEME. |
...
| Situation | What ezer does |
| --- | --- |
| A developer sets a key you pinned | The pinned value applies. `ezer inspect` lists the requirements file that contributed. |
...
| `requirements.toml` is missing or its signature does not verify | The pins do not apply, and ezer starts without them. Set `fail_closed = true` to refuse to start instead. |
...
Run `ezer inspect` on the developer's machine. It lists every config file that contributed, including requirements and managed layers, so a policy that is not applying is visible in one command.
