...
`EZER_CLONE` / `GROVE_CLONE` in this process, then ezer enable-all
(`EZER_GROVE` or `[cli] grove` in `~/.ezer/config.toml`), then `[clone] enabled`
authorize Clone IPC.

This does **not** enable Grove for session / `-w` worktrees. Those use a
separate gate (`EZER_WORKTREE_TYPE` and `[cli] grove_worktree` in
`~/.ezer/config.toml`; see [Configuration reference](26-config-reference.md)).
`EZER_WORKTREE_TYPE` and `[cli] grove_worktree` do **not** enable `ezer clone`.
`EZER_CLONE` / `GROVE_CLONE` / `[clone] enabled` do **not** enable session /
`-w` Grove.
...
Specific knobs still win: `EZER_WORKTREE_TYPE=copy` keeps session worktrees on
copy while clone can stay on; `EZER_CLONE=0` keeps `ezer clone` off while
...
|-------|--------|----------|-------|
| ezer | the model and API | `ezer login`, `ezer logout` | `~/.ezer/auth.json` |

`ezer clone` never reads `~/.ezer/auth.json` for Git. Signing into ezer does not
