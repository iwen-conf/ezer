...
//! `PAGER_BINARY=target/debug/ezer-pager cargo test -p ezer-pager --test mermaid_render_subprocess -- --ignored`
//! (Bazel wires `PAGER_BINARY` from `//crates/codegen/ezer-pager-bin:ezer-pager`).
//! Unlike the spawn-only suites in `ezer-pager-pty-harness`, this test stays here because it exercises pager library code.

use std::path::PathBuf;
use std::time::{Duration, Instant};

use ezer_pager::app::mermaid_worker::render_via_subprocess;
use ezer_pager::scrollback::blocks::mermaid_content::MermaidRenderQuality;

/// `PAGER_BINARY` (absolutized: Bazel sets a runfiles-relative path) or `CARGO_BIN_EXE_ezer-pager`.
fn pager_binary() -> Result<PathBuf, String> {
    for key in ["PAGER_BINARY", "CARGO_BIN_EXE_ezer-pager"] {
    ...
    Err("PAGER_BINARY/CARGO_BIN_EXE_ezer-pager not set; build ezer-pager-bin and export PAGER_BINARY".to_owned())
}
