//! `ezer mcp`: manage MCP server configurations from the command line.
...
Examples:
  ...
  ezer mcp add xcode -- xcrun mcpbridge
  ...
  # Add to the project config (./.ezer/config.toml) instead of ~/.ezer/config.toml
  ezer mcp add --scope project github -- npx -y @modelcontextprotocol/server-github";
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
pub enum McpScope {
    /// `~/.ezer/config.toml`, available in all your projects
    User,
    /// `./.ezer/config.toml`, shared with everyone working in this directory
    Project,
}
...
pub struct AddArgs {
    ...
    /// Place them after `--` so flags such as `-y` are passed to the server instead of ezer.
    #[arg(value_name = "ARGS")]
    args: Vec<String>,
    ...
    /// Config to write to: user (~/.ezer/config.toml) or project (./.ezer/config.toml)
...
fn run_list(json: bool) -> Result<()> {
    ...
    let servers = ezer_shell::util::config::load_mcp_server_configs_with_project(&cwd);
    let disabled = ezer_shell::util::config::disabled_mcp_server_names(&cwd);
    ...
    let blocked = ezer_shell::mcp_doctor::policy_blocked_servers(&cwd);
    ...
    if json {
        ...
        println!("No MCP servers configured. Run `ezer mcp add --help` to get started.");
    } else {
...
async fn run_add(args: AddArgs) -> Result<()> {
    ...
    let write_scope = match args.scope {
        McpScope::User => ezer_shell::mcp_doctor::McpWriteScope::User,
        McpScope::Project => ezer_shell::mcp_doctor::McpWriteScope::Project,
    };
    if let Some(refusal) =
        ezer_shell::mcp_doctor::policy_add_refusal(&cwd, name, &config, write_scope)
    {
        eprintln!("{refusal}");
        std::process::exit(1);
    }
    ...
    ezer_shell::util::config::save_mcp_server_config_at(&path, name, &config).await?;
    ...
    match transport {
        McpTransport::Stdio => {
            let Some(command) = source else {
                bail!(
                    "A command is required for stdio servers. Usage: ezer mcp add <name> -- <command> [args...]"
                );
            };
        ...
        McpTransport::Http | McpTransport::Sse => {
            ...
            let Some(url) = source else {
                bail!(
                    "A URL is required for {label} servers. Usage: ezer mcp add --transport {label} <name> <url>"
                );
            };
...
fn scope_target(scope: McpScope) -> PathBuf {
    match scope {
        McpScope::User => ezer_shell::util::config::user_config_path(),
        McpScope::Project => {
            ezer_shell::util::config::project_config_path(&current_dir_or_exit())
        }
    }
}
...
fn select_remove_site(
    ...
    use ezer_shell::util::config::user_config_path;
    ...
    project_site
        ...
        .or_else(|| {
            user_defined.then(|| {
                (
                    McpScope::User,
                    ezer_shell::util::config::user_config_path(),
                )
            })
        })
}
...
fn mcp_server_is_known(name: &str, cwd: &Path) -> bool {
    ezer_shell::util::config::cli_known_mcp_server_names(cwd).contains(name)
}
...
fn available_mcp_server_names(cwd: &Path) -> Vec<String> {
    let mut names: Vec<String> = ezer_shell::util::config::cli_known_mcp_server_names(cwd)
        .into_iter()
        .collect();
    names.sort();
    names
...
async fn run_set_enabled(name: &str, enabled: bool) -> Result<()> {
    ...
    if !mcp_server_is_known(name, &cwd) {
        ...
        if !available.is_empty() {
            ...
            eprintln!("No MCP servers configured. Run `ezer mcp add --help` to get started.");
        }
        std::process::exit(1);
    }
    ...
    if enabled && let Some(refusal) = ezer_shell::mcp_doctor::policy_enable_refusal(&cwd, name)
    {
        eprintln!("{refusal}");
        std::process::exit(1);
    }
    ...
    let already = if enabled {
        !ezer_shell::util::config::disabled_mcp_server_names(&cwd).contains(name)
    } else {
        ezer_shell::config::load_from_disk()
    ...
    let modified =
        ezer_shell::util::config::save_mcp_server_enabled_in(name, enabled, &cwd).await?;

    let now_disabled = ezer_shell::util::config::disabled_mcp_server_names(&cwd).contains(name);
    ...
    let user_config = ezer_shell::util::config::user_config_path();
...
async fn run_remove(name: &str, requested_scope: Option<McpScope>) -> Result<()> {
    ...
    let find_project_site = || {
        ezer_shell::config::find_project_configs(&cwd)
            .into_iter()
            .rev()
...
async fn run_doctor(json: bool, name: Option<String>) -> Result<()> {
    let cwd = current_dir_or_exit();
    let report = ezer_shell::mcp_doctor::run_doctor(&cwd, name.as_deref()).await;
    ...
    if json {
        ...
        ezer_shell::mcp_doctor::print_report(&report);
    }
...
mod tests {
    ...
    fn add_hyphen_flag_without_double_dash_is_rejected() {
        let err = PagerArgs::try_parse_from(["ezer", "mcp", "add", "fs", "npx", "-y"])
    ...
    fn add_infers_http_for_bare_positional_url() {
        for url in ["https://mcp.example.com/mcp", "http://mcp.example.com/mcp"] {
            let add = parse_add(&["ezer", "mcp", "add", "api", url]);
    ...
    fn add_default_transport_warns_on_url_looking_command() {
        ...
        let add = parse_add(&["ezer", "mcp", "add", "local", "localhost:3000"]);
    ...
    fn add_scope_project_parses_and_invalid_scope_is_rejected() {
        let add = parse_add(&[
            "ezer", "mcp", "add", "-s", "project", "fs", "--", "npx", "pkg",
        ]);
        assert_eq!(add.scope, McpScope::Project);

        let err = PagerArgs::try_parse_from([
            "ezer", "mcp", "add", "-s", "local", "fs", "--", "npx", "pkg",
        ])
        .expect_err("local is not a ezer scope");
    ...
    fn add_legacy_multi_value_env_is_rejected() {
        ...
        let add = parse_add(&[
            "ezer", "mcp", "add", "pg", "--env", "A=1", "B=2", "--", "npx", "-y", "server",
        ]);
    ...
    fn add_validates_name_env_and_headers() {
        let add = parse_add(&["ezer", "mcp", "add", "fs", "-e", "NOT_A_PAIR", "--", "npx"]);
        ...
        let add = parse_add(&["ezer", "mcp", "add", "bad name!", "--", "npx"]);
    ...
    fn add_rejects_mismatched_options_per_transport() {
        let add = parse_add(&["ezer", "mcp", "add", "fs", "-H", "X: y", "--", "npx"]);
    ...
    fn add_requires_a_source_for_each_transport() {
        let add = parse_add(&["ezer", "mcp", "add", "fs"]);
        ...
        let add = parse_add(&["ezer", "mcp", "add", "--transport", "http", "api"]);
    ...
    fn remove_accepts_optional_scope() {
        let args = PagerArgs::try_parse_from(["ezer", "mcp", "remove", "fs", "-s", "project"])
    ...
    fn grok_com_known_only_with_toml_definition() {
        ...
        std::fs::create_dir_all(defined.path().join(".ezer")).unwrap();
        std::fs::write(
            defined
                .path()
                .join(".ezer")
                .join(ezer_config::USER_CONFIG_FILENAME),
            format!(
                r#"
[mcp_servers.{name}]
...
"#
    ...
    fn enable_and_disable_parse_name() {
        let args = PagerArgs::try_parse_from(["ezer", "mcp", "enable", "user-grafana"])
        ...
        let args = PagerArgs::try_parse_from(["ezer", "mcp", "disable", "user-slack"])
    ...
    fn enable_disable_require_name() {
        let err = PagerArgs::try_parse_from(["ezer", "mcp", "enable"])
            .expect_err("enable without name must fail");
        assert_eq!(err.kind(), clap::error::ErrorKind::MissingRequiredArgument);

        let err = PagerArgs::try_parse_from(["ezer", "mcp", "disable"])
    ...
    fn select_remove_site_covers_scope_presence_matrix() {
        let user = ezer_shell::util::config::user_config_path();
        let project = PathBuf::from("/repo/.ezer/config.toml");
