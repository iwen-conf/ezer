//! Streaming output filter for `ezer wrap`: OSC 52 clipboard interception, host-image request handling, and DEC-mode observation.
//!
...
/// Delegates to [`ezer_shell::util::clipboard::set_text`], which uses `pbcopy` on macOS and `arboard` elsewhere.
...
fn set_local_clipboard(data: &[u8]) {
    ...
    if let Err(e) = ezer_shell::util::clipboard::set_text(text) {
...
pub(crate) fn host_clipboard_image_frame() -> Vec<u8> {
    let image = ezer_pager_render::clipboard::system_clipboard_get_image();
