...
fn clean_flags_require_yes_and_conflict() {
    ...
    for args in [
        ["ezer", "du", "--clean", ""],
        ["ezer", "du", "--clean-orphaned", ""],
        ["ezer", "du", "--yes", ""],
        ["ezer", "du", "--clean", "--clean-orphaned"],
    ] {
        let args = args.into_iter().filter(|arg| !arg.is_empty());
        assert!(crate::app::cli::PagerArgs::try_parse_from(args).is_err());
    }
    assert!(crate::app::cli::PagerArgs::try_parse_from(["ezer", "du", "--clean", "--yes"]).is_ok());
    assert!(
        crate::app::cli::PagerArgs::try_parse_from(["ezer", "du", "--clean-orphaned", "--yes"])
            .is_ok()
    );
}
...
fn missing_home_prints_unreadable_directory_skip() {
    ...
    let skipped = DiskUsageReport {
        ezer_home: "/nonexistent/.ezer".into(),
        skips: SkipCounts {
            unreadable_dirs: 1,
            ..SkipCounts::default()
        },
