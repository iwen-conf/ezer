...
it cannot find elsewhere. Inspect Grove artifacts with `ezer worktree redirect list <mount>` \
and purge them with `ezer worktree clean-artifacts <mount> --yes`."
)]
...
fn run_inner(args: DiskUsageArgs) -> Result<()> {
    ...
    if !present {
        let report = empty_report(&ezer_home);
        if args.json {
...
fn collect_report(ezer_home: &Path) -> Result<DiskUsageReport> {
    collect_report_in(ezer_home, &[])
}
fn collect_report_in(ezer_home: &Path, data_dirs: &[PathBuf]) -> Result<DiskUsageReport> {
    let _ = data_dirs;
