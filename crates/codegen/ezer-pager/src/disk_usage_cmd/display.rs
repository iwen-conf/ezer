...
//! `ezer_config::ezer_home()`, whose first call creates the home, must stay out of this module.
...
pub fn print_report(
    ...
    let home_label = home_prefix_label(&report.ezer_home);
    ...
    match report.registry {
        ...
        RegistryState::Unopenable => {
            writeln!(
                ...
                abbreviate(&report.registry_path, &report.ezer_home, &home_label)
            )?;
        }
        RegistryState::Corrupt => {
            writeln!(
                out,
                "  Worktree registry is damaged; rows show as untracked. Remove {} and run `ezer worktree db rebuild` to recreate it.",
                abbreviate(&report.registry_path, &report.ezer_home, &home_label)
            )?;
        }
    }
    if report.worktrees.is_empty() {
        ...
        for (wt, kind) in report.worktrees.iter().zip(&kind_cells) {
            ...
            writeln!(
                ...
                abbreviate(&wt.path, &report.ezer_home, &home_label),
            )?;
        }
    }
...
pub fn print_missing_home(ezer_home: &str, out: &mut impl Write) -> std::io::Result<()> {
    writeln!(
        out,
        "Nothing on disk yet at {}.",
        home_prefix_label(ezer_home)
    )
}
...
pub(crate) fn print_missing_home_report(
    ...
    print_missing_home(&report.ezer_home, out)?;
    print_redirect_rows(report, out)?;
    print_skip_notes(report, out)
}
...
fn home_prefix_label(ezer_home: &str) -> String {
    crate::util::display_ezer_home_prefix_for(Path::new(ezer_home))
}
