...
fn canonical_and_short_ids_resolve_to_canonical_id() {
    ...
    assert_eq!(command, "ezer doctor fix ssh-wrap");
    assert_eq!(
        resolve_fix_id(command.strip_prefix("ezer doctor fix ").unwrap()).unwrap(),
        SSH_WRAP_ID
    );
...
fn tmux_managed_items_coexist_and_each_apply_is_one_transaction() {
    ...
    assert_eq!(content.matches("# >>> ezer doctor >>>").count(), 1);
    for id in [
        TMUX_CLIPBOARD_ID,
        DCS_PASSTHROUGH_ID,
        TMUX_EXTENDED_KEYS_ID,
...
fn tmux_stale_plan_and_idempotence_reuse_managed_writer_safety() {
    ...
    assert!(matches!(
        ...
        Err(FixError::TmuxManaged(
            ezer_config::managed_text::ManagedConfigError::StalePlan(_)
        ))
    ));
...
fn bash_zsh_and_fish_plans_use_exact_paths_and_aliases() {
    ...
    for (shell, relative, alias) in [
        ("/bin/bash", ".bashrc", "alias ssh='ezer wrap ssh'"),
        ("/bin/zsh", ".zshrc", "alias ssh='ezer wrap ssh'"),
        (
            "/usr/local/bin/fish",
...
fn non_utf8_source_fails_closed_before_conflict_policy() {
    ...
    assert!(matches!(
        ...
        Err(FixError::Managed(
            ezer_config::managed_text::ManagedConfigError::UnsafePath { .. }
        ))
    ));
}
...
fn shell_aliases_expand_to_exact_argv_and_bypass_is_explicit() {
    ...
    if let Some(bash) = find_on_path("bash") {
        ...
        std::fs::write(&rc, "alias ssh='ezer wrap ssh'\n").unwrap();
        let command = format!(
    ...
    let mut shell = std::process::Command::new(bash);
    shell
        .args(["-ic", "alias ssh='ezer wrap ssh'; command ssh host"])
        .env("CAPTURE", &capture)
        .env(
            "PATH",
            format!(
...
/// tmux applies ezer's managed block last and the features merge.
