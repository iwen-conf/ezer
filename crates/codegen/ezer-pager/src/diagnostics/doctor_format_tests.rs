...
fn tmux_config_and_reload_notes_output_is_stable() {
    ...
    assert_eq!(
        ...
        concat!(
            ...
            "      Automatic setup: `ezer doctor fix dcs-passthrough`\n",
            ...
            "      Automatic setup: `ezer doctor fix tmux-extended-keys`\n",
...
fn limited_color_output_is_stable() {
    ...
    assert_eq!(
        ...
        concat!(
            ...
            "      Note: Add this export to your shell startup file, such as `~/.zshrc` or `~/.bashrc`, then restart ezer.\n",
        )
    );
}
...
fn wezterm_xtversion_runtime_evidence_output_is_stable() {
    ...
    assert_eq!(
        ...
        concat!(
            ...
            "      Note: For this session, type `\\` and then press Enter. ezer can't negotiate the Kitty keyboard protocol over SSH yet. `enable_kitty_keyboard = true` applies only to local WezTerm sessions.\n",
        )
    );
}
...
fn runtime_findings_merge_before_single_formatter_orders_issues_before_recommendations() {
    ...
    let issue = output.find("ezer is using the terminal bell").unwrap();
