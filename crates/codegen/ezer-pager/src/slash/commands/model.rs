...
use ezer_shell::sampling::types::{ReasoningEffortOption, supports_reasoning_effort_meta};
...
mod tests {
    ...
    fn split_model_effort_keeps_a_multi_word_label() {
        ...
        let (id, info) = model_with_reasoning("grok-4.7", "ezer 4.7");
        state.available.insert(id.clone(), info);
        assert_eq!(
            split_model_effort(&state, "ezer 4.7 Extra High")
                .map(|(model, token)| { (model.0.to_string(), token.to_string()) }),
            Some(("grok-4.7".to_string(), "Extra High".to_string()))
        );
        assert_eq!(
            split_model_effort(&state, "ezer 4.7 high").map(|(_, token)| token),
            Some("high")
        );
        assert!(split_model_effort(&state, "ezer 4.7").is_none());
        assert_eq!(
