...
pub use ezer_telemetry::debug_log::ACP_UPDATE_PAYLOAD_TARGET;
/// Target for the always-on compact ACP update summary line (kind, ids, status, payload sizes).
/// Cheap to format at streaming rate.
/// Defined in `ezer-telemetry` so the firehose directives and the pager filter share one constant (re-exported here for callsites).
pub use ezer_telemetry::debug_log::ACP_UPDATE_TARGET;
...
pub fn init_tracing() -> TracingHandle {
    ...
    let otel_layer = ezer_telemetry::otel_layer::build_otel_layer(
        ezer_telemetry::otel_layer::OtelClientInfo {
            client_name: "ezer",
            client_version: ezer_version::VERSION,
            service_version: ezer_version::full_version(),
            app_entrypoint: "tui",
        },
        ezer_shell::agent::init::build_default_otel_layer_config(),
    );
    let instrumentation_layer = ezer_telemetry::instrumentation::layer();
    let sampling_log_layer = ezer_telemetry::sampling_log::layer();
    let hooks_log_layer = ezer_telemetry::hooks_log::layer();
    let registry = tracing_subscriber::registry()
        .with(fmt_layer.with_filter(env_filter))
        .with(instrumentation_layer)
        .with(sampling_log_layer)
        .with(ezer_telemetry::span_profile::layer("tui"))
        .with(hooks_log_layer)
        .with(otel_layer);
    ezer_telemetry::debug_log::install_firehose(registry, "tui");
...
/// `ezer_gateway` carries the bridge diagnostics that moved out of `ezer_shell`.
...
fn default_directives() -> String {
    ...
    format!(
        "ezer_shell=info,ezer_gateway=info,ezer_login=info,ezer_pager=trace,ezer_tools=info,ezer_session_search=info,xai_acp_lib=info,{RMCP_SSE_NOISE_TARGET}=error,sampling_log=off,{ACP_UPDATE_TARGET}=debug,{ACP_UPDATE_PAYLOAD_TARGET}={payload_level}"
    )
}
#[cfg(test)]
mod tests {
    ...
    /// Bridge diagnostics moved to `ezer_gateway`; the curated TUI filter must allowlist it.
    #[test]
    fn default_directives_allowlist_gateway_target() {
        assert!(default_directives().contains("ezer_gateway=info"));
    }
