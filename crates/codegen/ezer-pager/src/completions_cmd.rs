//! `ezer completions <shell>`: generate shell completion scripts.
//!
...
pub fn run(shell: Shell) {
    ...
    let mut buf = Vec::new();
    generate(shell, &mut cmd, "ezer", &mut buf);
...
/// subcommand slot. Upstream bug: <https://github.com/clap-rs/clap/issues/6282>. `ezer worktree <TAB>` then
...
fn fix_zsh_root_prompt_positional(script: &str) -> String {
    ...
    for (from, to) in [
        ...
        (
            r#"curcontext="${curcontext%:*:*}:ezer-command-$line[2]:""#,
            r#"curcontext="${curcontext%:*:*}:ezer-command-$line[1]:""#,
        ),
...
mod tests {
    ...
    fn zsh_script() -> String {
        let mut cmd = PagerArgs::command().name("ezer");
        let mut buf = Vec::new();
        generate(Shell::Zsh, &mut cmd, "ezer", &mut buf);
    ...
    // Dispatch happens on `$line[2]`, so `ezer worktree <TAB>` re-offered every top-level command (upstream clap-rs/clap#6282)
    #[test]
