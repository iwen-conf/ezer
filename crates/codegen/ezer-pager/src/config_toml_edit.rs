...
/// Set `[hints].<key>` to `value` in `~/.ezer/config.toml`, preserving every other key and table. No-ops when the
...
mod tests {
    ...
    fn merge_round_trip_preserves_sibling_tables() {
        ...
        let path = dir.path().join(ezer_config::USER_CONFIG_FILENAME);
        fs::write(
            &path,
            "[ui]\ncompact_mode = false\n\n[mcpServers]\nx = \"y\"\n",
        )
