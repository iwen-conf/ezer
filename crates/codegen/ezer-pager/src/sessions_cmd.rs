...
pub async fn run(args: SessionsArgs, agent_config: &AgentConfig) -> Result<()> {
    ...
    let client = ezer_shell::agent::session_registry_client::SessionRegistryClient::new(
        agent_config.endpoints.proxy_url(),
        String::new(),
    )
    ...
    match args.command {
        SessionsCommand::List { limit } => {
            let sessions = ezer_shell::session::merge::fetch_merged(
                Some(&client),
                cwd.to_str(),
                ezer_shell::session::merge::CwdScope::WithSiblings,
                None,
                limit,
                // The CLI listing is an inventory, not the resume picker.
                ezer_shell::session::visibility::HeadlessPolicy::Include,
            )
            .await;
        ...
        SessionsCommand::Search { query, limit } => {
            ...
            use ezer_shell::session::merge::REMOTE_TIMEOUT;
            ...
            let search = ezer_shell::session::storage::search::start_if_enabled(agent_config);
        ...
        SessionsCommand::Delete { id } => {
            ...
            let deletion = ezer_shell::session::persistence::delete_session_history(
                &id,
                None,
                needs_remote,
                auth_manager.clone(),
