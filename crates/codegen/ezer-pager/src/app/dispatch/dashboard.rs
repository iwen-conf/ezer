...
use ezer_telemetry::events::CancellationScope;
        ...
        crate::views::dashboard::DashboardRowId::Workspace { session_id } => {
            workspace.is_some_and(|workspace| {
                workspace.members.iter().any(|member| {
                    matches!(member.kind, ezer_dashboard_store::MemberKind::Build)
...
fn dispatch_dashboard_load_local_build(
    ...
    let resolved = cwd_hint
        .and_then(|cwd| {
            ezer_shell::session::resolve_local_session(&session_id, &cwd.to_string_lossy())
                .map(|resolved_id| (resolved_id, cwd))
        })
        .or_else(|| {
            ezer_shell::session::resolve_local_session_any_cwd(&session_id)
...
pub(super) fn dispatch_dashboard_dispatch_slash(app: &mut AppView, text: String) -> Vec<Effect> {
    ...
    let result = {
        ...
        {
            use ezer_telemetry::events::{PagerCommandSource, PagerSlashCommand};
        ...
        if reg.is_restricted(invocation.token) {
            ...
            if let Some(d) = app.dashboard.as_mut() {
                d.dispatch.set_text("");
                d.set_error_toast(&format!(
                    "/{token} requires a paid plan: upgrade at {}",
...
fn stage_dashboard_model(
    ...
    effort: Option<ezer_shell::sampling::types::ReasoningEffort>,
) {
    let display = app
        .models
        .available
...
fn workspace_layout_target(
    ...
    let session_id = match row {
        DashboardRowId::TopLevel(agent_id) => app
            ...
            .and_then(|session_id| {
                ezer_dashboard_store::SessionId::new(session_id.0.to_string()).ok()
            }),
        DashboardRowId::Workspace { session_id } => {
            ezer_dashboard_store::SessionId::new(session_id.clone()).ok()
        }
    ...
    let target = session_id
        .map(|session_id| ezer_dashboard_store::MemberKey {
            session_id,
