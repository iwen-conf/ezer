...
pub(super) fn plugin_cta_candidates(
    ...
    let winner = match cta_marketplace {
        ...
        None => sources
            .iter()
            .position(|s| {
                ezer_plugin_marketplace::is_official_source_url(&s.source_url_or_path)
            })
            .or_else(|| {
                sources.iter().position(|s| {
                    s.source_name == ezer_plugin_marketplace::OFFICIAL_SOURCE_NAME
                })
            }),
    };
...
pub(super) fn plugin_cta_phase_for(
    ...
    use ezer_plugin_marketplace::matcher::{KeywordCandidate, match_plugin_keyword};
...
pub(super) fn handle_cta_plugin_install_done(
    ...
    log_event(ezer_telemetry::events::PluginCtaInstalled {
    ...
    match result {
        Ok(servers) => {
            ...
            if needs_auth {
                ...
                log_event(ezer_telemetry::events::ExtensionsModalOpened {
                    trigger: ezer_telemetry::events::ExtensionsModalTrigger::AuthHandoff,
    ...
    match result {
        Ok(mut response) => {
            ...
            if let Some(agent) = app.agents.get_mut(&agent_id) {
                ...
                if enabled {
                    agent.plugin_cta.dismissed = ezer_shell::config::dismissed_plugin_ctas();
                }
                ...
                if matches!(
                    ...
                    {
                        log_event(ezer_telemetry::events::PluginCtaImpression {
