...
fn dashboard_resume_owns_picker_lifecycle_and_leaves_welcome_untouched() {
    ...
    let _ = dispatch(
        Action::TaskComplete(TaskResult::SessionListLoaded {
            ...
            sessions: vec![],
            partial: None,
            scope: ezer_shell::session::unified_list::ListScope::Cwd,
            seq,
            query: None,
        }),
        &mut app,
...
fn dashboard_picker_filters_workspace_members_and_strict_loads_fuzzy_pick() {
    ...
    app.workspace_membership
        .set_snapshot_for_test(ezer_dashboard_store::WorkspaceSnapshot {
            grouping: ezer_dashboard_store::Grouping::State,
            members: vec![
                ezer_dashboard_store::Member {
                    session_id: ezer_dashboard_store::SessionId::new(visible_id.clone())
                        .unwrap(),
                    kind: ezer_dashboard_store::MemberKind::Build,
                    origin: ezer_dashboard_store::MemberOrigin::Local,
                ...
                ezer_dashboard_store::Member {
                    session_id: ezer_dashboard_store::SessionId::new(recoverable_id.clone())
                        .unwrap(),
                    kind: ezer_dashboard_store::MemberKind::Build,
                    origin: ezer_dashboard_store::MemberOrigin::Local,
                    cwd: Some(cwd.to_string_lossy().into_owned()),
                ...
                ezer_dashboard_store::Member {
                    session_id: ezer_dashboard_store::SessionId::new(pinned_id.clone())
                        .unwrap(),
                    kind: ezer_dashboard_store::MemberKind::Build,
                    origin: ezer_dashboard_store::MemberOrigin::Local,
                    cwd: Some(cwd.to_string_lossy().into_owned()),
                    ...
                    is_worktree: false,
                    last_change_unix_ms: 1,
                    pin_rank: Some(ezer_dashboard_store::RANK_GAP),
                    order_rank: None,
                },
                ezer_dashboard_store::Member {
                    session_id: ezer_dashboard_store::SessionId::new(
    ...
    app.workspace_membership
        .request_pin(
            ezer_dashboard_store::MemberKey {
                session_id: ezer_dashboard_store::SessionId::new(optimistic_pinned_id.clone())
                    .unwrap(),
                kind: ezer_dashboard_store::MemberKind::Build,
            },
            true,
    ...
    app.workspace_membership
        .request_pin(
            ezer_dashboard_store::MemberKey {
                session_id: ezer_dashboard_store::SessionId::new(pinned_id.clone()).unwrap(),
                kind: ezer_dashboard_store::MemberKind::Build,
            },
            false,
        )
...
fn v1_dashboard_picker_does_not_consult_workspace_view() {
    ...
    app.workspace_membership
        .set_snapshot_for_test(ezer_dashboard_store::WorkspaceSnapshot {
            grouping: ezer_dashboard_store::Grouping::State,
            members: vec![ezer_dashboard_store::Member {
                session_id: ezer_dashboard_store::SessionId::new(session_id.clone()).unwrap(),
                kind: ezer_dashboard_store::MemberKind::Build,
                origin: ezer_dashboard_store::MemberOrigin::Local,
                cwd: Some("/tmp".into()),
...
fn ready_workspace_app(
    mut app: AppView,
    store: ezer_dashboard_store::WorkspaceStore,
) -> AppView {
...
fn local_workspace_row_uses_the_same_strict_load_path() {
    ...
    store
        .insert_member(ezer_dashboard_store::NewMember {
            key: ezer_dashboard_store::MemberKey {
                session_id: ezer_dashboard_store::SessionId::new(session_id.clone()).unwrap(),
                kind: ezer_dashboard_store::MemberKind::Build,
            },
            origin: ezer_dashboard_store::MemberOrigin::Remote,
            metadata: ezer_dashboard_store::MemberMetadata {
                cwd: Some(cwd.to_string_lossy().into_owned()),
                title: None,
                model: None,
                last_turn_summary: None,
    ...
    assert!(matches!(
        member.origin,
        ezer_dashboard_store::MemberOrigin::Local
    ));
    store
    ...
    assert!(matches!(
        ...
        ezer_dashboard_store::MemberOrigin::Remote
    ));
...
fn voice_final_appends_to_dashboard_dispatch() {
    ...
    crate::voice::handle_voice_event(
        &mut app,
        ezer_voice::VoiceEvent::UtteranceFinal {
            text: "the build".into(),
        },
    );
    assert_eq!(
...
fn voice_dashboard_dispatch_submit_tears_down_voice() {
    ...
    assert!(matches!(
        rx.try_recv(),
        Ok(ezer_voice::VoiceCommand::PttRelease)
    ));
    crate::voice::handle_voice_event(
        &mut app,
        ezer_voice::VoiceEvent::UtteranceFinal {
...
/// `ezer dashboard` before login: the startup hook consumes the `EZER_OPEN_DASHBOARD_AT_STARTUP` env var and stashes
...
fn auth_complete_opens_deferred_dashboard() {
    ...
    assert!(
        ...
        "deferred `ezer dashboard` must open the dashboard after login",
    );
    assert!(
...
fn dashboard_confirm_worktree_applies_pending_model_and_plan() {
    let mut app = test_app();
    seed_model(&mut app, "test-model-4.5", "ezer 4.5");
    open_dashboard(&mut app);
    app.cwd_has_git_ancestor = true;
    ...
    if let Some(d) = app.dashboard.as_mut() {
        d.pending_model = Some(crate::views::dashboard::PendingDispatchModel {
            id: model_id.clone(),
            effort: Some(ezer_shell::sampling::types::ReasoningEffort::High),
            display: "ezer 4.5".to_string(),
        });
    ...
    assert_eq!(
        ...
        Some(ezer_tools::types::SessionMode::Plan),
    );
...
#[test]
fn dashboard_open_detects_standalone_ezer_worktree() {
...
/// `Action::OpenDashboard`, so this covers `/dashboard`, `Ctrl+\`, `ezer dashboard`, and the startup hook.
#[serial_test::serial(EZER_AGENT_DASHBOARD)]
#[test]
...
fn workspace_grouping_toggle_emits_sqlite_layout_write_not_config_write() {
    ...
    assert!(matches!(
        ...
        [Effect::WriteWorkspace {
            ...
            ..
        }] if patch.grouping == Some(ezer_dashboard_store::LayoutGrouping::Directory)
    ));
    assert!(
        effects
            .iter()
...
fn closing_loaded_workspace_agent_rebinds_selection_to_unloaded_row() {
    ...
    store
        ...
        .unwrap();
    assert!(matches!(
        store.apply_layout_patch(&ezer_dashboard_store::LayoutPatch {
            pin_assignments: vec![ezer_dashboard_store::PinAssignment {
                key: ezer_dashboard_store::MemberKey {
                    session_id: ezer_dashboard_store::SessionId::new("test-session").unwrap(),
                    kind: ezer_dashboard_store::MemberKind::Build,
                },
                pinned: true,
            }],
            manual_order: Some(vec![ezer_dashboard_store::MemberKey {
                session_id: ezer_dashboard_store::SessionId::new("test-session").unwrap(),
                kind: ezer_dashboard_store::MemberKind::Build,
            }]),
            grouping: None,
        }),
        ezer_dashboard_store::LayoutApplyOutcome::Committed(_)
    ));
    let snapshot = store.snapshot().unwrap();
    let mut app = test_app_with_agent();
...
fn workspace_layout_on_provisional_row_toasts() {
    ...
    let store =
        ezer_dashboard_store::WorkspaceStore::open(&temp.path().join("workspace.db")).unwrap();
    app.workspace_membership
...
fn workspace_row_ctrl_x_archives_membership_without_deleting_history() {
    ...
    app.workspace_membership
        .set_snapshot_for_test(ezer_dashboard_store::WorkspaceSnapshot {
            grouping: ezer_dashboard_store::Grouping::State,
            members: vec![ezer_dashboard_store::Member {
                session_id: ezer_dashboard_store::SessionId::new("saved").unwrap(),
                kind: ezer_dashboard_store::MemberKind::Build,
                origin: ezer_dashboard_store::MemberOrigin::Local,
                cwd: Some("/tmp".into()),
    ...
    assert!(
        app.workspace_membership
            .removal_pending_for_test(&ezer_dashboard_store::SessionId::new("saved").unwrap())
    );
    assert!(
...
fn workspace_overlay_cycle_reaches_provisional_but_not_hidden_live_agents() {
    ...
    app.workspace_membership
        .suppress_for_test(ezer_dashboard_store::SessionId::new("second").unwrap());
    app.active_view = ActiveView::Agent(AgentId(0));
    assert!(dispatch_dashboard_overlay_cycle(&mut app, 1).is_empty());
...
fn dashboard_slash_model_stages_pending_model() {
    ...
    seed_model(&mut app, "test-model-4.5", "ezer 4.5");
    open_dashboard(&mut app);
    let effects = dispatch_dashboard_dispatch_slash(&mut app, "/model test-model-4.5".into());
    assert!(
        effects.is_empty(),
    ...
    assert_eq!(pending.id.0.as_ref(), "test-model-4.5");
    assert_eq!(pending.display, "ezer 4.5");
...
fn dashboard_open_reseeds_pending_model_and_mode() {
    ...
    if let Some(d) = app.dashboard.as_mut() {
        d.pending_model = Some(crate::views::dashboard::PendingDispatchModel {
            id: acp::ModelId::new(std::sync::Arc::from("test-model-4.5")),
            effort: None,
            display: "ezer 4.5".to_string(),
        });
...
fn dashboard_attach_subagent_lazily_replays_deferred_transcript() {
    ...
    crate::app::subagent::set_replay_ezer_home_for_tests(Some(home.path().to_path_buf()));
    let mut app = test_app_with_agent();
    open_dashboard(&mut app);
    let parent = AgentId(0);
    app.agents
...
fn dashboard_overlay_cycle_non_overlay_noop_when_dashboard_disabled() {
    let _guard =
        crate::test_util::EnvVarGuard::set("EZER_AGENT_DASHBOARD", std::path::Path::new("0"));
    let mut app = test_app_with_agent();
    let id1 = AgentId(0);
    mark_agent_nonempty(&mut app, id1);
    let id2 = AgentId(1);
        ...
        effects.iter().any(|e| matches!(
            ...
            Effect::RenameSession { agent_id, title, kind, .. }
                if *agent_id == id
                    ...
                    && *kind == ezer_shell::session::unified_list::SessionKind::Build
        )),
    ...
    assert!(
        effects.iter().any(|e| matches!(
            ...
            Effect::RenameSession { agent_id, title, kind, .. }
                if *agent_id == id
                    && title == "Chat title"
                    && *kind == ezer_shell::session::unified_list::SessionKind::Chat
        )),
...
fn workspace_dashboard_stop_archives_loaded_row_and_repairs_selection() {
    ...
    app.workspace_membership
        .set_snapshot_for_test(ezer_dashboard_store::WorkspaceSnapshot {
            ...
            members: ids
                .iter()
                .enumerate()
                .map(|(index, _)| ezer_dashboard_store::Member {
                    session_id: ezer_dashboard_store::SessionId::new(format!(
                        "archive-{index}"
                    ))
                    .unwrap(),
                    kind: ezer_dashboard_store::MemberKind::Build,
    ...
    assert!(
        app.workspace_membership.removal_pending_for_test(
            &ezer_dashboard_store::SessionId::new(archived_session_id).unwrap()
        ),
...
fn workspace_archive_keeps_conversation_twin_open_and_registered() {
    ...
    app.workspace_membership
        .set_snapshot_for_test(ezer_dashboard_store::WorkspaceSnapshot {
            grouping: ezer_dashboard_store::Grouping::State,
            members: vec![ezer_dashboard_store::Member {
                session_id: ezer_dashboard_store::SessionId::new("shared-session").unwrap(),
                kind: ezer_dashboard_store::MemberKind::Build,
                origin: ezer_dashboard_store::MemberOrigin::Local,
                cwd: Some("/tmp".into()),
    ...
    assert!(app.workspace_membership.removal_pending_for_test(
        &ezer_dashboard_store::SessionId::new("shared-session").unwrap()
    ));
}
#[test]
fn confirmed_workspace_archive_reports_when_session_became_busy() {
    ...
    app.workspace_membership
        .set_snapshot_for_test(ezer_dashboard_store::WorkspaceSnapshot {
            grouping: ezer_dashboard_store::Grouping::State,
            members: vec![ezer_dashboard_store::Member {
                session_id: ezer_dashboard_store::SessionId::new("archive-race").unwrap(),
                kind: ezer_dashboard_store::MemberKind::Build,
                origin: ezer_dashboard_store::MemberOrigin::Local,
                cwd: Some("/tmp".into()),
    ...
    assert!(app.agents.contains_key(&id));
    assert!(!app.workspace_membership.removal_pending_for_test(
        &ezer_dashboard_store::SessionId::new("archive-race").unwrap()
    ));
    let dashboard = app.dashboard.as_ref().unwrap();
...
fn workspace_overlay_ctrl_x_archives_settled_only_agent() {
    ...
    app.workspace_membership
        .set_snapshot_for_test(ezer_dashboard_store::WorkspaceSnapshot {
            grouping: ezer_dashboard_store::Grouping::State,
            members: vec![ezer_dashboard_store::Member {
                session_id: ezer_dashboard_store::SessionId::new("only-archive").unwrap(),
                kind: ezer_dashboard_store::MemberKind::Build,
                origin: ezer_dashboard_store::MemberOrigin::Local,
                cwd: Some("/tmp".into()),
...
fn dashboard_stop_running_bg_task_emits_teardown_kill() {
    use ezer_shell::extensions::task::TaskKillSource;
    let mut app = test_app();
    let _ = dispatch_new_session_inner(&mut app, None);
    let _ = dispatch_new_session_inner(&mut app, None);
    let target = *app.agents.keys().next().unwrap();
