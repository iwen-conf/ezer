...
fn enable_always_approve_sends_response_and_flips_yolo_and_persists() {
    ...
    let effects = dispatch(
        Action::PermissionSelect(acp::PermissionOptionId::new(Arc::from(
            ezer_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID,
        ))),
        &mut app,
    );
...
/// The `set_yolo_mode_inner` early-return at the `app.active_view` guard MUST precede the `ezer_telemetry::log_event` call.
...
fn cycle_with_nudge_jumps_to_plan_from_a_published_ask() {
    ...
    publish_agent_ask_plan(&mut app);
    {
        let agent = app
            ...
            .get_mut(&AgentId(0))
            .expect("the test agent exists");
        agent.session_mode = ezer_tools::types::SessionMode::Ask;
