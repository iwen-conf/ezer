...
fn resume_foreign_session_consumes_hint_and_uses_each_tools_prompt() {
    use ezer_foreign_sessions::ForeignSessionTool;
    for (tool, prompt) in [
...
fn critical_announcement(id: &str) -> ezer_announcements::RemoteAnnouncement {
...
fn promo_announcement(id: &str) -> ezer_announcements::RemoteAnnouncement {
    ezer_announcements::RemoteAnnouncement {
        id: Some(id.into()),
        message: Some(format!("{id} message")),
        severity: Some("promo".into()),
        cta: Some(ezer_announcements::AnnouncementCta {
            label: Some("Go".into()),
    ...
    assert!(
        matches!(
            ...
            [Effect::RenameSession { kind, .. }]
                if *kind == ezer_shell::session::unified_list::SessionKind::Chat
        ),
...
fn load_sticky_chat_history_bypass_rename_kind_is_build() {
    ...
    assert_eq!(
        agent.rename_kind(),
        ezer_shell::session::unified_list::SessionKind::Build
    );
    let rename = dispatch(
        Action::RenameSession {
    ...
    assert!(
        matches!(
            ...
            [Effect::RenameSession { kind, title, .. }]
                if *kind == ezer_shell::session::unified_list::SessionKind::Build
                    && title == "local title"
        ),
    ...
    app.workspace_membership.set_read_only_for_test(
        ...
        ezer_dashboard_store::WorkspaceSnapshot {
            ...
            members: vec![ezer_dashboard_store::Member {
                session_id: ezer_dashboard_store::SessionId::new("read-only-delete").unwrap(),
