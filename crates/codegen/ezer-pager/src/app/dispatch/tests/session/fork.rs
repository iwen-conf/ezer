...
fn worktree_forked_sets_session_id_eagerly_and_emits_load() {
    ...
    let worktree_path = PathBuf::from("/tmp/ezer-worktrees/pager-fork");
    let session_cwd = PathBuf::from("/tmp/ezer-worktrees/pager-fork/sub");
...
fn worktree_forked_clears_sticky_branch_from_main_repo() {
    ...
    let worktree_path = PathBuf::from("/tmp/ezer-worktrees/pager-fork-sticky");
...
fn worktree_forked_with_strategy_shows_summary_in_scrollback() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::WorktreeForked {
            ...
            worktree_path: PathBuf::from("/tmp/ezer-worktrees/pager-fork"),
            session_cwd: PathBuf::from("/tmp/ezer-worktrees/pager-fork"),
            code_restored: false,
            restore_summary: None,
            restore_degree: None,
            resume_session_id: Some("orig-sess".into()),
...
fn worktree_forked_with_restore_failure_shows_warning_banner() {
    ...
    let worktree_path = PathBuf::from("/tmp/ezer-worktrees/pager-fail");
    let session_cwd = PathBuf::from("/tmp/ezer-worktrees/pager-fail/sub");
    dispatch(
        Action::TaskComplete(TaskResult::WorktreeForked {
            agent_id: id,
...
fn worktree_forked_retargets_suppress_to_child() {
    ...
    let effects = dispatch(
        Action::TaskComplete(TaskResult::WorktreeForked {
            ...
            session_id: acp::SessionId::new("forked-sess-1"),
            worktree_path: PathBuf::from("/tmp/ezer-worktrees/pager-fork"),
            session_cwd: PathBuf::from("/tmp/ezer-worktrees/pager-fork/sub"),
            code_restored: false,
            restore_summary: None,
            restore_degree: None,
            resume_session_id: Some("orig-sess".into()),
