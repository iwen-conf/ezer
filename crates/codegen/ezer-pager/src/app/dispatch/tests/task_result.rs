...
use ezer_shell::session::helpers::session_compact::COMPACT_CANCELLED_MSG;
use ezer_shell::session::unified_list::ListScope;
...
) -> ezer_foreign_sessions::RecentForeignSession {
    ezer_foreign_sessions::RecentForeignSession {
        tool,
...
fn foreign_resume_results_require_launch_token_and_canonical_cwd() {
    ...
    launch.foreign_session_compat = ezer_foreign_sessions::EnabledForeignSessionSources {
        cursor: true,
        ..Default::default()
    };
    ...
    dispatch(
        Action::TaskComplete(TaskResult::ForeignResumeHintDetected {
            ...
            hint: Some(foreign_resume_hint(
                ezer_foreign_sessions::ForeignSessionTool::Cursor,
            )),
        }),
        &mut launch,
    );
    assert_eq!(
        ...
        Some(ezer_foreign_sessions::ForeignSessionTool::Cursor)
    );
    ...
    dispatch(
        Action::TaskComplete(TaskResult::ForeignResumeHintDetected {
            ...
            hint: Some(foreign_resume_hint(
                ezer_foreign_sessions::ForeignSessionTool::Codex,
            )),
        }),
        &mut stale,
    );
...
fn switch_model_incompatible_agent_shows_question_modal() {
    ...
    let err = ezer_shell::agent::config::ModelSwitchIncompatibleAgentError {
...
fn session_success_arms_finish_startup_obligation() {
    ...
    for result in results {
        let mut app = test_app_with_agent();
        app.agents.get_mut(&id).unwrap().session.session_id = None;
        app.pending_startup = Some(ezer_telemetry::startup::PendingStartup::new());
...
fn delete_both_session_clears_modal_and_welcome_content_hits() {
    ...
    let hit = ezer_shell::extensions::session_search::SearchSessionHit {
...
fn delete_remote_session_clears_modal_and_welcome_content_hits() {
    ...
    app.workspace_membership
        .set_snapshot_for_test(ezer_dashboard_store::WorkspaceSnapshot {
            ...
            members: vec![ezer_dashboard_store::Member {
                session_id: ezer_dashboard_store::SessionId::new("remote-only").unwrap(),
    ...
    assert!(app.workspace_membership.removal_pending_for_test(
        &ezer_dashboard_store::SessionId::new("remote-only").unwrap()
    ));
    assert!(
...
fn delete_session_failed_keeps_all_entries() {
    ...
    app.workspace_membership
        .set_snapshot_for_test(ezer_dashboard_store::WorkspaceSnapshot {
            grouping: ezer_dashboard_store::Grouping::State,
            members: vec![ezer_dashboard_store::Member {
                session_id: ezer_dashboard_store::SessionId::new("s1").unwrap(),
                kind: ezer_dashboard_store::MemberKind::Build,
...
fn picker_delete_of_open_build_suppresses_immediate_workspace_re_adoption() {
    ...
    let id = ezer_dashboard_store::SessionId::new(session_id).unwrap();
    assert!(
...
fn delete_completion_reports_membership_failure_if_store_became_read_only() {
    ...
    app.workspace_membership.set_read_only_for_test(
        store,
        ezer_dashboard_store::WorkspaceSnapshot {
            grouping: ezer_dashboard_store::Grouping::State,
            members: vec![],
            data_version: 1,
        },
...
fn gate_refreshed_emits_check_subscription_on_gate_lift() {
    ...
    let settings = ezer_shell::util::config::RemoteSettings::default();
...
fn gate_refreshed_no_effect_when_still_gated() {
    ...
    let settings = ezer_shell::util::config::RemoteSettings {
...
fn test_gate() -> ezer_login::GateInfo {
    ezer_login::GateInfo {
        message: "Subscribe".into(),
        url: None,
        label: None,
...
fn verify_check_with_meta_resolves_pending_gate() {
    ...
    let meta = serde_json::to_value(ezer_login::AuthMeta::default()).unwrap();
    dispatch_task_result(
...
fn verify_check_with_gated_meta_shows_gate() {
    ...
    let meta = serde_json::to_value(ezer_login::AuthMeta {
        gate: Some(test_gate()),
        ..Default::default()
    })
    .unwrap();
...
fn compact_complete_error_surfaces_real_error_in_scrollback() {
    use ezer_shell::session::helpers::session_compact::{CompactErrorKind, compact_error_data};
