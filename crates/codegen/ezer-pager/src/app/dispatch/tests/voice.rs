...
fn voice_final_replaces_whitespace_only_draft() {
    ...
    crate::voice::handle_voice_event(
        &mut app,
        ezer_voice::VoiceEvent::UtteranceFinal { text: "hi".into() },
    );
    let p = &app.agents.get(&id).unwrap().prompt;
    assert_eq!(p.text(), "hi");
...
/// A free-tier user hitting the voice keybinding gets the MaxTier upsell instead of a doomed voice session.
...
fn voice_keybinding_on_restricted_tier_opens_upsell() {
    ...
    assert!(
        ...
        "restricted-tier voice keybinding must open the MaxTier upsell"
    );
    assert!(
        !app.voice_listening(),
...
fn voice_interim_sets_then_error_clears_state() {
    ...
    crate::voice::handle_voice_event(
        &mut app,
        ezer_voice::VoiceEvent::InterimTranscript {
            text: "partial".into(),
        },
    );
...
fn voice_interim_preview_replaces_active_selection() {
    ...
    crate::voice::handle_voice_event(
        &mut app,
        ezer_voice::VoiceEvent::InterimTranscript { text: "hi".into() },
    );
...
fn voice_stt_language_change_recycles_pipeline() {
    ...
    assert!(matches!(
        rx.try_recv(),
        Ok(ezer_voice::VoiceCommand::Shutdown)
    ));
    assert!(matches!(
        effects.as_slice(),
