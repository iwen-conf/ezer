...
use super::*;
use ezer_shell::sampling::error::is_free_usage_exhausted_error;
...
fn complete_session_usage(
    app: &mut AppView,
    session_id: &str,
    usage: ezer_shell::extensions::notification::PromptUsage,
) -> Vec<Effect> {
    dispatch(
...
fn team_auth_disables_agent_billing_surface() {
    ...
    app.apply_auth_meta(&ezer_login::AuthMeta {
...
fn session_usage_complete_pushes_block_and_chains_billing() {
    ...
    app.screen_mode = crate::app::ScreenMode::Minimal;
    let before = agent_scrollback_len(&app);
    let usage = ezer_shell::extensions::notification::PromptUsage {
        totals: ezer_shell::extensions::notification::PromptUsageModel {
            input_tokens: 1_000,
            output_tokens: 100,
            total_tokens: 1_100,
            model_calls: 3,
...
fn session_usage_complete_drops_stale_session() {
    ...
    let effects = complete_session_usage(
        &mut app,
        "old-session",
        ezer_shell::extensions::notification::PromptUsage {
            totals: ezer_shell::extensions::notification::PromptUsageModel {
                model_calls: 99,
...
fn free_usage_failure_opens_paywall_modal() {
    ...
    use ezer_shell::extensions::notification::{RetryState, SessionUpdate};
...
fn credit_limit_upsell_submit_shows_url_when_browser_unavailable() {
    ...
    let kind = LocalQuestionKind::CreditLimitUpsell {
        choices: vec![
            ezer_telemetry::events::CreditLimitChoice::UpgradeTier,
            ezer_telemetry::events::CreditLimitChoice::PurchaseCredits,
            ezer_telemetry::events::CreditLimitChoice::RetryLastPrompt,
        ],
    };
