...
fn log_coding_data_consent_selected(
    source: ezer_telemetry::events::CodingDataConsentSource,
    opted_in: bool,
    previous_opted_in: bool,
) {
    use ezer_telemetry::events::{CodingDataConsentChoice, CodingDataConsentSelected};
    ezer_telemetry::session_ctx::log_event(CodingDataConsentSelected {
        source,
...
pub(crate) fn commit_minimal_update_notice(app: &mut AppView, latest_version: &str) {
        ...
        && let Some(agent) = app.agents.get_mut(&id)
    {
        agent.scrollback.push_block(RenderBlock::system(format!(
            "Update available: v{latest_version}. Run `ezer update` to install."
        )));
    }
}
...
pub(in crate::app::dispatch) fn dispatch_privacy_banner_opt_in(app: &mut AppView) -> Vec<Effect> {
    ...
    set_coding_data_sharing(
        app,
        true,
        ezer_telemetry::events::CodingDataConsentSource::PrivacyBanner,
    )
}
