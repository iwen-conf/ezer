...
fn displaced_draft_feedback_notice(
    ...
    match outcome {
        ezer_shell::session::FeedbackOutcome::Submitted => super::notes::FEEDBACK_THANKS_NOTICE,
        ezer_shell::session::FeedbackOutcome::SubmittedCleanupFailed => {
            "Feedback was sent, but the stored draft could not be deleted. Delete it manually; do not resend."
        }
        ezer_shell::session::FeedbackOutcome::LocalOnly => {
            "Feedback was saved locally but was not sent. The draft was kept."
        }
        ezer_shell::session::FeedbackOutcome::OutcomeUnknown => {
...
struct WorkspaceCompletionContext {
    ...
    live_ids: std::collections::HashSet<ezer_dashboard_store::SessionId>,
}
    ...
    match result {
        ...
        TaskResult::SessionMetaFromDisk {
            ...
            if let Some(agent) = app.agents.get_mut(&agent_id) {
                if let Some((raw, is_manual)) = title
                    && let Some(t) =
                        ezer_shell::session::persistence::sanitize_and_cap_title(&raw)
                {
        ...
        TaskResult::ChangelogFetched { markdown, entries } => {
            ...
            app.changelog_bullets =
                ezer_shell::util::changelog::bullets_from_entries(&entries, 3);
            vec![]
        }
                ...
                if is_draft {
                    ...
                    if has_matching_modal {
                        match outcome {
                            ...
                            ezer_shell::session::FeedbackOutcome::OutcomeUnknown
                            | ezer_shell::session::FeedbackOutcome::Other => {
                                if let Some(modal) = agent.feedback_modal.as_mut() {
