...
struct CreditLimitCopy {
    ...
    second_choice: ezer_telemetry::events::CreditLimitChoice,
    payg_telemetry: bool,
}
...
pub(super) fn open_credit_limit_upsell(
    ...
    let copy = match mode {
        CreditLimitUpsellMode::UnifiedCredits => CreditLimitCopy {
            ...
            secondary_desc: "Purchase credits to keep using ezer",
            second_choice: ezer_telemetry::events::CreditLimitChoice::PurchaseCredits,
        ...
        CreditLimitUpsellMode::LegacyPayg { enabled: true } => CreditLimitCopy {
            ...
            second_choice: ezer_telemetry::events::CreditLimitChoice::PayAsYouGo,
            payg_telemetry: true,
        },
    ...
    log_event(ezer_telemetry::events::CreditLimitUpsellShown {
    ...
    if !max_tier {
        ...
        choices.push(ezer_telemetry::events::CreditLimitChoice::UpgradeTier);
    }
    options.push(QuestionOption {
    ...
    choices.push(ezer_telemetry::events::CreditLimitChoice::RetryLastPrompt);
...
fn apply_recheck_meta(app: &mut AppView, mut meta: ezer_login::AuthMeta) {
...
pub(super) fn handle_check_subscription_complete(
    ...
    let applied = match meta {
        Some(meta_val) => {
            match serde_json::from_value::<ezer_login::AuthMeta>(meta_val) {
                Ok(auth_meta) => {
...
pub(super) fn handle_credit_limit_recheck_complete(
    ...
    if let Some(meta_val) = meta
        && let Ok(auth_meta) = serde_json::from_value::<ezer_login::AuthMeta>(meta_val)
    {
        apply_recheck_meta(app, auth_meta);
    }
