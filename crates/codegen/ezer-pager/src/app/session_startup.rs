...
pub(crate) fn stamp_phase_traceparent(meta: &mut Option<agent_client_protocol::Meta>) {
    let Some(span) = ezer_telemetry::startup::current_phase_span() else {
        return;
    };
...
pub(crate) fn stamp_span_traceparent(
    ...
    if let Some(tp) = ezer_otel::traceparent_of_span(span) {
...
pub enum DeferredSessionStartup {
    ...
    ForeignResume {
        tool: ezer_foreign_sessions::ForeignSessionTool,
        native_id: String,
    },
}
...
pub fn fork_session_params(
    ...
    let source_cwd = ezer_shell::session::resolve_local_session_any_cwd(parent_session_id)
...
pub fn local_build_session_on_disk(session_id: &str, cwd: &Path) -> bool {
    let cwd_str = cwd.to_string_lossy();
    ezer_shell::session::resolve_local_session(session_id, &cwd_str).is_some()
}
...
pub use ezer_shell::session::persistence::RecentSessionSelection;
/// Resolve most-recent session id for cwd, or error.
async fn most_recent_session_id(
    cwd: &str,
    selection: RecentSessionSelection,
) -> anyhow::Result<(String, Option<String>)> {
    let summaries = ezer_shell::session::persistence::list_summaries(Some(cwd)).await?;
    let first = summaries
        .iter()
...
pub fn ensure_session_id_available(session_id: &str, cwd: &str) -> anyhow::Result<()> {
    ...
    if ezer_shell::session::persistence::session_exists_for_cwd(session_id, cwd) {
...
async fn resolve_existing_session(
    ...
    if let Some(local_id) = ezer_shell::session::resolve_local_session(session_id, cwd) {
    ...
    if let Some(original_cwd) = ezer_shell::session::resolve_local_session_any_cwd(session_id) {
        tracing::info!(
...
async fn restore_session_from_remote(
    ...
    if let Some((false, source)) =
        ezer_shell::util::config::session_registry_local_override_sourced(Some(&raw_config))
    {
        anyhow::bail!(
    ...
    let agent_config = ezer_shell::agent::config::Config::new_from_toml_cfg(&raw_config)
    ...
    let storage_client = ezer_shell::credential_factory::build_storage_client_for_proxy(
        &agent_config.endpoints.proxy_url(),
        deployment_key,
    ...
    let progress: ezer_shell::session::restore::ProgressCallback = Box::new(move |event| {
    ...
    let recovered_local_id = ezer_shell::session::find_local_child_for_remote(session_id, cwd);
...
mod tests {
    ...
    fn traceparent_of_span_captures_own_span_id_not_parent() {
        let _guard = ezer_otel::set_local_trace_subscriber();
        ...
        let child_own = ezer_otel::traceparent_of_span(&child).expect("child traceparent");
        let parent_own = ezer_otel::traceparent_of_span(&parent).expect("parent traceparent");
    ...
    #[cfg(feature = "local-workspace")]
    fn advertised_tools_env() -> ezer_test_support::EnvGuard {
        ezer_test_support::EnvGuard::set(
    ...
    fn resolve_local_workspace_empty_cli_attach_falls_back_to_env() {
        let _env = advertised_tools_env();
        let _sid = ezer_test_support::EnvGuard::set(
    ...
    fn resolve_local_workspace_cwd_only_is_not_a_request() {
        let tmp = tempfile::tempdir().unwrap();
        let _cwd = ezer_test_support::EnvGuard::set(
            EZER_CHAT_LOCAL_WORKSPACE_CWD_ENV,
            tmp.path().to_str().unwrap(),
        );
        let _enable = ezer_test_support::EnvGuard::unset(EZER_CHAT_LOCAL_WORKSPACE_ENV);
        let _mode = ezer_test_support::EnvGuard::unset(EZER_CHAT_LOCAL_WORKSPACE_MODE_ENV);
        let _sid = ezer_test_support::EnvGuard::unset(EZER_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV);
    ...
    fn resolve_local_workspace_own_env_defaults() {
        let _env = advertised_tools_env();
        let _enable = ezer_test_support::EnvGuard::set(EZER_CHAT_LOCAL_WORKSPACE_ENV, "1");
        let _mode = ezer_test_support::EnvGuard::unset(EZER_CHAT_LOCAL_WORKSPACE_MODE_ENV);
        let _sid = ezer_test_support::EnvGuard::unset(EZER_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV);
    ...
    fn resolve_local_workspace_defaults_cwd_and_denies_home() {
        ...
        let _allow =
            ezer_test_support::EnvGuard::unset(EZER_CHAT_LOCAL_WORKSPACE_ALLOW_HOME_ENV);
        let home = tempfile::tempdir().unwrap();
        let home_str = home.path().to_str().unwrap();
        let _home = ezer_test_support::EnvGuard::set("HOME", home_str);
        let _userprofile = ezer_test_support::EnvGuard::set("USERPROFILE", home_str);
        let err =
    ...
    fn resolve_local_workspace_refuses_uncheckable_toolset() {
        let _tools =
            ezer_test_support::EnvGuard::unset(EZER_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS_ENV);
        let tmp = tempfile::tempdir().unwrap();
        let err =
