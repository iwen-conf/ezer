...
#[command(
    name = "ezer",
    version = ezer_version::full_version(),
...
pub struct PagerArgs {
    ...
    #[arg(
        ...
        value_parser = clap::builder::PossibleValuesParser::new(
            ezer_shell::agent::config::PermissionMode::VALID_VALUES
        )
    )]
...
impl PagerArgs {
    ...
    pub(crate) fn local_resume_selection(
        &self,
    ) -> ezer_shell::session::persistence::RecentSessionSelection {
        ...
        ezer_shell::session::persistence::RecentSessionSelection::from_headless_policy(policy)
    }
    ...
    pub fn saved_resume_profile_for_cwd(&self, cwd: Option<&str>) -> Option<String> {
        ...
        match self.resume_target() {
            ResumeTarget::SessionId(id) => {
                ezer_shell::session::persistence::resumed_session_sandbox_profile(
                    Some(&id),
                    cwd,
                )
            }
            ResumeTarget::MostRecentForCwd => {
                ezer_shell::session::persistence::resolve_recent_session_sandbox_profile(
                    cwd,
    ...
    fn resolve_startup_sandbox(explicit: Option<&str>, saved: Option<String>) -> SandboxStartup {
        match (explicit, saved) {
            (Some(x), Some(s))
                if x.parse::<ezer_sandbox::ProfileName>().ok()
                    != s.parse::<ezer_sandbox::ProfileName>().ok() =>
            {
