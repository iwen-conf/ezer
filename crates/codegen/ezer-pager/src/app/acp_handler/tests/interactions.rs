#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn exec_vehicle_permission_enqueues_a_persisting_default_scope() {
        ...
        use std::sync::Arc;
        use ezer_workspace::permission::bash_command_splitting::BashCommandHighlights;
        ...
        assert!(
            ezer_workspace::permission::always_allow_scope_persists(
    ...
    fn mcp_elicit_does_not_replace_url_waiting() {
        ...
        );
        {
            ...
            assert!(ev.send_response(
                ezer_tools::mcp_elicitation::McpElicitExtResponse::Accept { content: None },
            ));
    ...
    fn a_status_snapshot_repaints_a_row_that_had_already_settled() {
        ...
        app.current_ui.status_line =
            ezer_status_line::test_support::StatusLineConfigFixture::from_kind(
                ezer_status_line::StatusLineType::Builtin,
            )
            .with_items(vec![ezer_status_line::StatusLineItem::Cwd])
            .into_config();
