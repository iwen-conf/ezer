...
use ezer_shell::extensions::notification::SessionUpdate as XaiSessionUpdate;
...
pub(super) fn compressed_entry(
    index: usize,
) -> ezer_shell::extensions::notification::ImageCompressedEntry {
    ezer_shell::extensions::notification::ImageCompressedEntry {
        index,
...
pub(super) fn make_deleted_ext_notif(
    ...
    make_deleted_ext_notif_with_reason(
        session_id,
        task_id,
        ezer_tools::notification::ScheduledTaskRemovedReason::Unknown,
        false,
    )
}
pub(super) fn make_deleted_ext_notif_with_reason(
    session_id: &str,
    task_id: &str,
    reason: ezer_tools::notification::ScheduledTaskRemovedReason,
    is_replay: bool,
) -> acp::ExtNotification {
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
...
pub(super) fn xai_hook_execution_notif_for_prompt(
    ...
    use ezer_shell::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
...
pub(super) fn xai_hook_execution_notif_with_runs(
    ...
    prompt_id: Option<&str>,
    is_replay: bool,
    runs: Vec<ezer_shell::extensions::notification::HookRunEntryDto>,
) -> acp::ExtNotification {
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
...
pub(super) fn make_git_head_changed_notif(
    ...
    let payload = ezer_workspace::session::git::GitHeadChanged {
...
pub(super) fn make_server_status_notif(
    session_id: &str,
    name: &str,
    status: ezer_shell::extensions::mcp::McpServerStatus,
    tools: Option<serde_json::Value>,
) -> acp::ExtNotification {
    use ezer_shell::extensions::mcp::{
