#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn retry_exhausted_rate_limited_empty_reason_uses_oauth_fallback() {
        use ezer_shell::sampling::error::RATE_LIMITED_USER_MESSAGE_OAUTH;
    ...
    fn retry_exhausted_api_key_rewrites_consumer_subscription_upsell() {
        use ezer_shell::sampling::error::RATE_LIMITED_USER_MESSAGE_API_KEY;
    ...
    fn apply_retry_state_disk_full_pushes_session_event() {
        use ezer_shell::extensions::notification::{
    ...
    fn apply_retry_state_context_length_shows_context_too_large() {
        use ezer_shell::extensions::notification::CONTEXT_LENGTH_ERROR_TYPE;
        let mut session = make_session(Some("s1"));
        let mut scrollback = ScrollbackState::new();
        apply_retry_state(
            &RetryState::Failed {
    ...
    fn compaction_lifecycle_refreshes_context_bar_from_trigger_counts() {
        ...
        let cancelled = XaiSessionUpdate::AutoCompactCancelled {
            reason: ezer_shell::extensions::notification::AutoCompactCancelReason::UserCancelled,
        };
    ...
    fn child_compact_started_refreshes_context_used() {
        ...
        child_view.context_state = Some(ezer_shell::session::ContextInfo::from_notification(
            90_000, 131_072,
        ));
        agent
            .insert_test_child(child_sid.into(), Box::new(child_view));
    ...
    fn summary_generated_ext(
        ...
        let meta = if title_is_manual {
            Some(ezer_shell::extensions::notification::title_is_manual_meta())
        } else {
            None
        };
    ...
    fn auto_title_notification_strips_controls_and_caps() {
        use ezer_shell::session::persistence::MAX_TITLE_SCALARS;
        let mut app = make_app_with_agent("sess-1");
        let dirty = format!(
