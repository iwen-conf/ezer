...
pub(super) fn handle_settings_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    if xai_ui && let Some(campaigns) = update.campaigns.clone() {
        ...
        ezer_shell::util::config::set_remote_campaigns_from_settings(Some(&rs));
    ...
    if let Some(v) = update.auto_permission_mode_enabled {
        ...
        ezer_shell::util::config::cache_remote_auto_permission_mode_enabled(Some(v));
        app.auto_mode_gate = ezer_shell::util::config::auto_permission_mode_enabled_from_disk();
    ...
    if let Some(v) = update.prompt_suggestions_enabled {
        ezer_shell::util::config::cache_remote_prompt_suggestions_enabled(Some(v));
    }
        ...
        && app.permission_mode_from_soft_default
    {
        ...
        let root = ezer_shell::config::load_effective_config()
    ...
    if let Some(v) = update.subscription_tier_display {
        ...
        if was_api_key
            ...
            && app
                .subscription_tier
                .as_deref()
                .is_some_and(ezer_shell::tier::is_restricted_tier_name)
        {
            app.voice_reset();
        ...
        && !msg.is_empty()
    {
        ...
        let effs = app.impose_gate(ezer_login::GateInfo {
            message: msg.clone(),
    ...
    let (requirements, user_config, managed_config) = (
        ezer_shell::config::load_merged_requirements(),
        ezer_shell::config::load_from_disk().ok(),
        ezer_shell::config::load_managed_config().ok(),
    );
    ...
    let remote = ezer_shell::util::config::RemoteSettings {
        group_tool_verbs: update.group_tool_verbs,
        ..Default::default()
    };
    let resolved = ezer_shell::util::config::resolve_group_tool_verbs(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    let resolved = ezer_shell::util::config::resolve_collapsed_edit_blocks(
        requirements.as_ref(),
        user_config.as_ref(),
        managed_config.as_ref(),
        Some(&remote),
    ...
    if let Some(remote_tags) = update.slash_command_tags.as_ref() {
        use ezer_shell::util::config::resolve_slash_command_tags;
        let effective_config = ezer_shell::config::load_effective_config().ok();
...
pub(super) fn apply_soft_default_permission_mode(
    ...
    let mode = ezer_shell::util::config::selected_permission_mode(effective_ui, remote)
        .unwrap_or_else(ezer_shell::util::config::default_interactive_permission_mode);
    ...
    app.current_ui.permission_mode = Some(if auto {
        ...
        ezer_shell::util::config::resolved_display_permission_mode(effective_ui, remote)
            .to_string()
    });
}
...
pub(super) fn handle_announcements_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) =
        serde_json::from_str::<ezer_announcements::AnnouncementsRefreshed>(notif.params.get())
    else {
        return false;
    };
...
pub(super) fn apply_announcements_update(
    app: &mut AppView,
    next_gen: u64,
    remote: &[ezer_announcements::RemoteAnnouncement],
    ...
    if ezer_announcements::prune_hidden_announcement_ids(
...
) -> Option<ezer_announcements::RemoteAnnouncement> {
...
pub(super) struct PagerSettingsUpdate {
    ...
    #[serde(
        default,
        deserialize_with = "ezer_shell::util::config::deserialize_tolerant"
    )]
    consent_gate: Option<ezer_shell::util::config::ConsentGate>,
}
