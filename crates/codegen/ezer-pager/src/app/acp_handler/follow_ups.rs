...
/// Deserialize shape of the `ezer/follow_ups` params emitted by the shell translator: `{ response_id, suggestions: [{ label, .. }] }`.
...
pub(super) struct FollowUpsParams {
    ...
    /// Carries the reserved `"ezer/replayed"` marker, which the shell never sets in v1.
...
/// A single `ezer/follow_ups` suggestion.
...
/// Handle `ezer/follow_ups`: render follow-up suggestion chips for the latest assistant response.
/// The reserved `_meta["ezer/replayed"] == true` marker suppresses rendering (it is absent today and treated as optional).
...
pub(super) fn handle_follow_ups(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    if params
        .meta
        .as_ref()
        .and_then(|m| m.get("ezer/replayed"))
