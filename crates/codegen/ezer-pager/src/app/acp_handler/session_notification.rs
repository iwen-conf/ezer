use super::*;
use ezer_shell::extensions::notification::HookAnnotationKind;
use ezer_shell::sampling::error::format_rate_limited_user_message;
...
pub(super) fn failed_hook_line(
    event_name: &str,
    run: &ezer_shell::extensions::notification::HookRunEntryDto,
) -> Option<String> {
    use ezer_hooks::config::HookDisplayName;
    use ezer_shell::extensions::notification::HookRunStatusDto;
    ...
    let subject = match ezer_hooks::config::hook_display_label(&run.name) {
    ...
    let changed = match session_notif.update {
        ...
        XaiSessionUpdate::SessionSummaryGenerated { session_summary } => {
            let title_is_manual = session_notif.meta.as_ref().and_then(|v| {
                v.get(ezer_shell::extensions::notification::TITLE_IS_MANUAL_META_KEY)
                    .and_then(|v| v.as_bool())
            });
            match title_is_manual {
                Some(true) => {
                    if let Some(clean) =
                        ezer_shell::session::persistence::sanitize_and_cap_title(
                            &session_summary,
                        )
                    {
                ...
                other => {
                    ...
                    if let Some(clean) =
                        ezer_shell::session::persistence::sanitize_and_cap_title(&decoded)
                    {
                        agent.generated_session_title = Some(clean);
        ...
        XaiSessionUpdate::ModelChanged {
            ...
            if !agent.session.models.available.contains_key(&new_model_id) {
                if ezer_shell::agent::chat_modes::process_chat_mode_enabled() {
        ...
        XaiSessionUpdate::MemoryFiles {
            ...
            let listing = ezer_shell::extensions::memory::MemoryListing {
                files,
                enabled,
                disabled_reason,
                capture_enabled,
...
fn apply_compaction_or_retry_update(
    ...
    use ezer_shell::extensions::notification::MODEL_FAMILY_SWITCH_COMPACT_BANNER;
...
pub(super) fn apply_image_compressed(
    agent: &mut AgentView,
    images: &[ezer_shell::extensions::notification::ImageCompressedEntry],
    message: &str,
) -> bool {
    if images.is_empty() {
...
pub(super) fn apply_retry_state(
    retry: &ezer_shell::extensions::notification::RetryState,
    session: &mut AgentSession,
    scrollback: &mut crate::scrollback::state::ScrollbackState,
    is_api_key_auth: bool,
) {
    ...
    use ezer_shell::extensions::notification::RetryState;
    match retry {
        RetryState::Retrying {
            attempt,
            max_retries,
        ...
        RetryState::Exhausted {
            ...
            if *rate_limited {
                ezer_telemetry::session_ctx::log_event(
                    ezer_telemetry::events::RateLimitHit {
                        model_id: session
                            .models
                            .current
                            .as_ref()
            ...
            let is_free_usage = *rate_limited
                && ezer_shell::sampling::error::is_free_usage_exhausted_error(reason);
            if is_credit_limit {
    ...
    if is_credit_limit {
        ezer_telemetry::session_ctx::log_event(ezer_telemetry::events::CreditLimitHit {
            model_id: session
                .models
                .current
                .as_ref()
...
pub(crate) fn detect_plan_mode_change_replayed(
    ...
    use ezer_tools::types::SessionMode;
