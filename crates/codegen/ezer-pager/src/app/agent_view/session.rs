...
use ezer_telemetry::events::{CancellationCompleted, CancellationScope};
...
impl AgentView {
    ...
    pub fn new(session: AgentSession, scrollback: ScrollbackState) -> Self {
        ...
        let mut view = Self {
            ...
            session_mode: ezer_tools::types::SessionMode::Default,
    ...
    pub fn apply_full_context_info(&mut self, next: ezer_shell::session::ContextInfo) {
        if self.chat_kind {
    ...
    pub fn apply_context_used(&mut self, used: u64, total: u64) {
        ...
        match self.context_state.as_mut() {
            ...
            None => {
                self.context_state = Some(ezer_shell::session::ContextInfo::from_notification(
                    used, total,
                ));
            }
        }
