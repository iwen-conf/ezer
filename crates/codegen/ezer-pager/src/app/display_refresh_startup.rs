...
/// Inclusive bounds for motion cadence env knobs (`EZER_MIN_DRAW_MS`, `EZER_SCROLL_CADENCE_MS`).
...
pub fn start(
    ...
    let (min_draw_env_set, min_draw_env_ms) =
        cadence_ms_from_env("EZER_MIN_DRAW_MS", default_cadence_ms);
    let (scroll_env_set, scroll_env_ms) =
        cadence_ms_from_env("EZER_SCROLL_CADENCE_MS", default_cadence_ms);
...
fn spawn_terminal_and_display_refresh_telemetry(tel: StartupTel) {
    tokio::task::spawn_blocking(move || {
        ...
        ezer_telemetry::session_ctx::log_event(t.clone());
        ...
        ezer_telemetry::session_ctx::log_event(
            ezer_telemetry::events::DisplayRefreshProbe {
                terminal: t,
