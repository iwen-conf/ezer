...
use ezer_telemetry::startup::{self, StartupPhase};
use actions::{
...
pub(crate) async fn discover_mcp_servers(
    ...
    let servers = tokio::task::spawn_blocking(move || ezer_shell::util::config::load_mcp_servers(
            &cwd,
...
async fn create_session_in_backend_rpc(
    ...
    let rpc_span = match startup::current_phase_span().as_ref() {
        Some(parent) => {
            ezer_telemetry::region!(
            "startup.session_create.backend_rpc",
            ezer_telemetry::region::Parent::Explicit(parent)
        )
        }
        None => {
            ezer_telemetry::region!(
...
pub(crate) fn execute(
    ...
    match effect {
        Effect::RegisterActiveSession { session_id, cwd } => {
            ...
            if let Err(e) = ezer_active_sessions::register(ezer_active_sessions::ActiveSession {
                session_id,
                pid: std::process::id(),
                cwd,
            ...
            let abort_handle = tasks
                .spawn(async move {
                    ...
                    let summaries = tokio::task::spawn_blocking(move || {
                            let _permit = permit;
                            ezer_foreign_sessions::scan_foreign_sessions(
                                &cwd,
                                enabled,
                            )
                        })
        ...
        Effect::FetchDashboardSessions => {
            ...
            tasks
                .spawn(async move {
                    ...
                    "cwd": cwd.to_string_lossy(),
                    "limit": 30,
                    "headless": ezer_shell::session::unified_list::HeadlessPolicy::Exclude
                        .as_wire_str(),
                });
                    let request = acp::ExtRequest::new(
                        "ezer/session/list",
            ...
            tasks
                .spawn(async move {
                    match tokio::task::spawn_blocking(move || {
                            let store = ezer_dashboard_store::WorkspaceStore::open(
                                &db_path,
                            )?;
                            let snapshot = store.snapshot()?;
                            Ok::<
                                _,
                                ezer_dashboard_store::StoreError,
                            >((store, snapshot))
                        })
                        .await
                    {
                        ...
                        Ok(Err(error)) => {
                            let retryable = matches!(&error, ezer_dashboard_store::StoreError::Busy { .. });
                                        ...
                                        if let Err(error) = store.insert_member(member) {
                                            failures
                                                .push(WorkspaceMutationFailure {
                                                    ...
                                                    retryable: matches!(
                                            &error,
                                            ezer_dashboard_store::StoreError::Busy { .. }
                                        ),
                                                });
                                        }
                                    }
        ...
        Effect::RestoreAndLoadSession { agent_id, session_id, session_cwd: _ } => {
            ...
            use ezer_shell::session::restore::restore_session_with_storage;
            let setup_started = std::time::Instant::now();
            let raw_config = ezer_shell::config::load_effective_config();
            let setup = raw_config
                .ok()
                .and_then(|raw| {
                    let cfg = ezer_shell::agent::config::Config::new_from_toml_cfg(
                            &raw,
                        )
                        .ok()?;
                    ...
                    let storage = ezer_shell::credential_factory::build_storage_client_for_proxy(
                        &proxy_base,
                        deployment_key,
                        alpha_test_key,
            ...
            tasks
                .spawn(async move {
                    ...
                    let progress: Option<
                        ezer_shell::session::restore::ProgressCallback,
                    > = {
                        use ezer_shell::session::restore::{PhaseStep, RestorePhase};
                        Some(
                            Box::new(move |event| {
                    ...
                    match restore_session_with_storage(
                            ...
                            &session_id,
                            &cwd_str,
                            ezer_shell::session::restore::RestoreSessionOpts {
                                turn_override: None,
                                progress,
                                restore_code: true,
                            },
            ...
            tasks
                .spawn(async move {
                    ...
                    let detail = tokio::task::spawn_blocking(move || {
                            let info = ezer_shell::session::info::Info {
                                id: acp::SessionId::new(session_id),
                                cwd,
                            };
                            let history_path = ezer_shell::session::persistence::session_dir(
                                    &info,
                                )
        ...
        Effect::SendBashCommand { agent_id, session_id, command, prompt_id } => {
            ...
            tasks
                .spawn(async move {
                    use ezer_shell::extensions::prompt_meta::PromptBlockMeta;
                    ulog::info(
                        "prompt.acp_send.start",
                        Some(&session_id.0),
                        Some(
        ...
        Effect::KillBgTask { session_id, task_id, source } => {
            ...
            tasks
                .spawn(async move {
                    let params = ezer_shell::extensions::task::KillTaskRequest {
            ...
            tasks
                .spawn(async move {
                    ...
                    let result = acp_send(req, &tx)
                        ...
                        .map(|_| ())
                        .map_err(|e| {
                            use ezer_shell::agent::config::ModelSwitchIncompatibleAgentError;
            ...
            tasks
                .spawn(async move {
                    let result = tokio::task::spawn_blocking(move || {
                            ezer_feedback::read_regular_capped(
                                    &path,
                                    ezer_shell::session::MAX_FEEDBACK_IMAGE_BYTES,
                                )
                                .map_err(|error| error.to_string())
                        })
                        .await
            ...
            tasks
                .spawn(async move {
                    let changelog = tokio::task::spawn_blocking(|| {
                            ezer_shell::util::changelog::ChangelogManager::new()
                                .fetch()
                        })
                        .await
                        .unwrap_or_else(|e| {
                            tracing::warn!(error = %e, "changelog fetch task failed");
                            ezer_shell::util::changelog::Changelog {
                                markdown: None,
                                entries: None,
                            }
                        });
        ...
        Effect::PersistAnnouncementsHidden { hidden_ids } => {
            tasks
                .spawn(async move {
                    ezer_announcements::write_hidden_announcement_ids(&hidden_ids)
                        .await;
        ...
        Effect::PersistPrivacyBannerAcked { acked_at } => {
            tasks
                .spawn(async move {
                    if let Err(e) = ezer_shell::util::config::set_privacy_banner_acked(
                            acked_at,
                        )
                        .await
                    {
        ...
        Effect::PersistPluginCtaDismissed { plugin_id } => {
            tasks
                .spawn(async move {
                    if let Err(e) = ezer_shell::config::run_add_dismissed_plugin_cta(
                            plugin_id,
                        )
                        .await
                    {
        ...
        Effect::PersistConsentAnswer { account, notice_id, version, acked } => {
            tasks
                .spawn(async move {
                    match ezer_shell::util::config::set_consent_answer(
                            account,
                            notice_id,
                            version,
                            acked,
        ...
        Effect::PersistPreferredModel { model_id, reasoning_effort } => {
            ...
            tasks
                .spawn(async move {
                    let result = ezer_shell::util::config::persist_models_default(
                            Some(model_id_str),
                            reasoning_effort,
                        )
                        .await
        ...
        Effect::PersistFeatureOverride { feature, saved } => {
            tasks
                .spawn(async move {
                    let result = ezer_shell::util::config::set_feature_override(
                            feature,
                            saved,
                        )
                        .await
        ...
        Effect::FetchMemoryList { agent_id, session_id } => {
            use ezer_shell::extensions::memory::{
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            serde_json::from_value::<
                                Vec<
                                    ezer_tools::implementations::skills::types::SkillInfo,
                                >,
        ...
        Effect::CheckMarketplaceUpdates { agent_id, session_id } => {
            if ezer_workspace::permission::resolution::managed_settings()
                .plugin_auto_update
                .is_disabled()
            {
            ...
            tasks
                .spawn(async move {
                    ...
                    struct McpUpsertRequest {
                        ...
                        server_name: String,
                        #[serde(flatten)]
                        config: ezer_shell::util::config::McpServerConfig,
                    }
        ...
        Effect::ShareSession { agent_id, session_id } => {
            use ezer_shell::session::{ShareSessionRequest, ShareSessionResponse};
            let tx = acp_tx.clone();
            tasks
                .spawn(async move {
                    let request = acp::ExtRequest::new(
        ...
        Effect::ShowSessionInfo { agent_id, session_id, show_resolved_model, nonce } => {
            let is_api_key_auth = session_flags.is_api_key_auth;
            let api_key_env_set = ezer_shell::agent::auth_method::has_xai_api_key_env();
            let tx = acp_tx.clone();
            tasks
                .spawn(async move {
                    match fetch_session_info(&session_id, &tx).await {
        ...
        Effect::SendFeedback {
            ...
            use ezer_shell::session::ClientType;
            use ezer_shell::session::acp_types::{
                        ...
                        serde_json::value::to_raw_value(
                            &FeedbackDraftSendRequest {
                                ...
                                edited_body: FeedbackDraftEditedBody {
                                    input: ezer_feedback::FeedbackDraftInput {
                                        title: draft.title,
                        ...
                        .await
                    {
                        Err(_elapsed) => {
                            TaskResult::FeedbackComplete {
                                agent_id,
                                origin,
                                outcome: ezer_shell::session::FeedbackOutcome::OutcomeUnknown,
                            ...
                            match send {
                                Ok(response) => {
                                    match serde_json::from_str::<
                                        ezer_shell::session::FeedbackResponse,
                                    >(response.0.get()) {
                                        Ok(response) => {
                                            TaskResult::FeedbackComplete {
                                                agent_id,
                                                ...
                                                outcome: response
                                                    .outcome
                                                    .unwrap_or(
                                                        ezer_shell::session::FeedbackOutcome::Submitted,
                                                    ),
                ...
                .spawn(async move {
                    match request {
                                    ...
                                    send.map_err(|error| sanitize_user_error(
                                        ...
                                        .and_then(|response| {
                                            ...
                                            struct DraftListResponse {
                                                drafts: Vec<ezer_feedback::FeedbackDraft>,
                                            }
                                    ...
                                    send.map_err(|error| sanitize_user_error(
                                        ...
                                        .and_then(|response| {
                                            ...
                                            struct DraftGetResponse {
                                                draft: ezer_feedback::FeedbackDraft,
                                            }
                        ...
                        crate::views::feedback_modal::FeedbackDraftRequest::Update(
                            ...
                            let raw_params = match serde_json::value::to_raw_value(
                                &ezer_shell::session::FeedbackDraftUpdateRequest {
                                    session_id: session_id.0.to_string(),
                ...
                .spawn(async move {
                    let result = tokio::task::spawn_blocking(move || {
                            let mode = match pinned_mode {
                                Some(mode) => mode,
                                None => ezer_shell::config::load_memory_mode()?,
                            };
                            let storage = ezer_shell::session::memory::MemoryStorage::new_for_mode(
                                &cwd,
                                None,
                                mode,
                            );
                            ...
                            Ok(Ok(resp)) => {
                                ...
                                if let Some(hits) = payload.get("results") {
                                    results = serde_json::from_value::<
                                        Vec<
                                            ezer_shell::extensions::session_search::SearchSessionHit,
                                        >,
                                    >(hits.clone())
                                        .unwrap_or_default();
                                }
        ...
        Effect::HydrateSessionMetaFromDisk {
            ...
            tasks
                .spawn(async move {
                    ...
                    let path = ezer_shell::session::persistence::session_dir(&info)
                        .join("summary.json");
                    ...
                    let (title, last_turn_summary) = tokio::task::spawn_blocking(move || -> Option<
                        ...
                        > {
                            ...
                            let summary: ezer_shell::session::persistence::Summary = serde_json::from_str(
                                    &raw,
                                )
                                .ok()?;
            ...
            tasks
                .spawn(async move {
                    let settings = tokio::task::spawn_blocking(|| {
                            ...
                            ezer_shell::remote::fetch_settings_blocking(
                                    &proxy_base,
                                    &auth,
                                    None,
                                )
...
async fn fetch_session_usage(
    ...
    let parsed: ezer_shell::extensions::usage::SessionUsageResponse = serde_json::from_str(
            resp.0.get(),
        )
        .map_err(|e| {
...
async fn lookup_session_title_in(
    ...
    use ezer_shell::session::storage::{JsonlStorageAdapter, StorageAdapter};
    let info = ezer_shell::session::info::Info {
        id: session_id.clone(),
        cwd: cwd.to_string(),
    };
...
fn session_info_fields(
    ...
    push(
        "Shell version",
        ezer_version::display_version(ezer_update::channel_label()),
        false,
    );
    ...
    let model_display = ezer_shell::session::model_display_name(
    ...
    if let Some(profile) = ezer_sandbox::profile_name() {
