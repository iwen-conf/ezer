...
use ezer_shell::extensions::billing::{BillingConfig, Cent, UsagePeriod};
...
fn parse_kill_outcome_reads_result_envelope() {
    use ezer_tools::types::KillOutcome;
...
fn parse_worktree_restore_payload_full() {
    use ezer_workspace::session::git::RestoreDegree;
...
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use ezer_active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
            ezer_active_sessions::list_in(dir.path())
                .expect("list")
                .is_empty(),
...
async fn deep_search_sessions_echoes_routing_and_policy() {
    ...
    execute(
        Effect::DeepSearchSessions {
            ...
            query: "abc".into(),
            seq: 4,
            headless_policy: ezer_shell::session::unified_list::HeadlessPolicy::Only,
        },
        &mut tasks,
        &tx,
        Path::new("."),
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> ezer_shell::session::SessionInfoResponse {
    use ezer_shell::session::acp_types::{ContextInfo, SessionInfoData};
    ezer_shell::session::SessionInfoResponse {
...
fn session_picker_summary_strips_skill_xml() {
    use ezer_tools::implementations::skills::skill::extract_skill_display_text;
...
fn sanitize_user_error_rewrites_shared_service_names() {
    for (pattern, replacement) in ezer_shell::sampling::error::SERVICE_NAME_REWRITES {
...
async fn hydrate_team_capability_adapter_maps_every_reply_to_the_asking_identity() {
    use std::sync::Arc;
    use xai_acp_lib::AcpAgentMessage;
    use ezer_shell::extensions::auth::HydrateTeamCapabilityResponse;
    ...
    for (reply, expected) in replies {
        ...
        tokio::spawn(async move {
            ...
            assert_eq!(args.request.method.as_ref(), "ezer/auth/hydrate_team_capability");
            assert_eq!(
                    args.request.params.get(),
