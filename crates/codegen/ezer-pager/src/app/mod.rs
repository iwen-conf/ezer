...
pub(crate) fn resolve_dock_enabled(remote: Option<bool>) -> bool {
    use ezer_shell::agent::config::{Feature, FeatureSources};
    let mut sources = FeatureSources::from_process_env(Feature::Dock);
    if sources.env.is_none() {
        sources.env = ezer_config::env_bool("EZER_DOCK_V2");
    }
...
pub(crate) fn resolve_voice_mode_enabled(
    ...
    use ezer_shell::agent::config::{ConfigSource, Feature, FeatureSources};
...
impl ScreenMode {
    ...
    /// Values are pinned by the telemetry allowlist (`ezer-telemetry`'s `KNOWN_SCREEN_MODES`).
...
fn seed_remote_ui_caches(remote_settings: Option<&ezer_shell::util::config::RemoteSettings>) {
    ezer_shell::util::config::cache_remote_auto_mode(
        remote_settings.and_then(|s| s.auto_mode.clone()),
    );
    ezer_shell::util::config::cache_remote_prompt_suggestions(
...
pub fn resolve_leader_mode<'p>(
    ...
    _remote_settings: Option<&ezer_shell::util::config::RemoteSettings>,
    eligible: bool,
...
async fn bounded_connect(
    ...
    let context = || startup_failure::Context {
        target,
        attempt,
        version: ezer_version::display_version_with_commit(
            ezer_version::full_version(),
            ezer_update::channel_label(),
        ),
        log_path: ezer_telemetry::unified_log::path(),
    };
    ...
    loop {
        ...
        tokio::select! {
            ...
            () = tokio::time::sleep(slice) => {
                let profile = ezer_shell::managed_config::startup_profile();
...
pub async fn run(
    ...
    if let ezer_login::PreTuiLoginOutcome::SignedIn(auth) =
        ...
        .await?
    {
        ezer_shell::agent::init::apply_post_login_config(*auth).await?;
    ...
    let remote_settings = if had_prefetch {
        ...
        let wait = {
            ...
            ezer_shell::agent::remote_config::settings_get::await_startup_settings(
                settings_query,
                EARLY_PREFETCH_WAIT,
    ...
    let raw_config = ezer_shell::config::load_effective_config()
        .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
    ezer_shell::config::cache_standalone_memory_mode(
        ezer_shell::config::MemoryConfig::resolve(
    ...
    if let Some(reason) = policy_disable_reason {
        tokio::spawn(ezer_shell::leader::kill_stale_reachable_leaders(reason));
    }
    if let Some(err) =
    ...
    {
        let summaries = ezer_shell::session::persistence::list_summaries(None).await?;
    ...
    let connect_ui_timeout = connect_timeout::resolve(
        ...
        ezer_shell::managed_config::startup_profile(),
    );
    ...
    ezer_telemetry::external::init(
        ezer_shell::agent::config::resolve_external_otel_config(
            ezer_telemetry::external::config::ExternalClientInfo {
                service_version: ezer_version::full_version().to_owned(),
    ...
    let pending_startup = ezer_telemetry::startup::PendingStartup::new();
    let timer = ezer_telemetry::startup::begin(crate::acp::Owner::Client);
    ...
    crate::unified_log::flush_blocking().await;
    drop(agent_guard);
    ezer_telemetry::session_ctx::drain_at_process_exit().await;
