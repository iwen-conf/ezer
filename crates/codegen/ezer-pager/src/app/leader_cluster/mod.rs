...
//!
//! ```bash
//! cargo test -p ezer-pager --lib -- app::leader_cluster --ignored --test-threads=1
//! ```
//!
...
impl PagerLeaderCluster {
    ...
    /// Callers MUST be `#[serial_test::serial(EZER_HOME)]` (env mutation) and run inside [`run_cluster_scenario`],
    ...
    async fn start(agent_keepalives: AgentKeepalives) -> Self {
        ...
        let env = vec![
            ...
            // Keep the flock's acquire slot inside the sandbox rather than the host's `/tmp/ezer-file-lock-<uid>`
            crate::test_util::EnvVarGuard::set(SLOT_DIR_ENV, ezer_home.path().join("lock-slots")),
        ];
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        let control_state = LeaderServerControlState::new(LeaderServerMetadata {
            ...
            // MUST be the client-side comparison source (ezer_version), not this crate's version
        ...
        let agent = ezer_shell::leader::in_process::spawn_agent(acp_rx, response_tx).await;
