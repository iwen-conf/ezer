...
//!
//! The shipped CLI profiles build with `panic = "abort"`, so the `catch_unwind` inside [`ezer_mermaid::render_checked`] is a no-op there.
...
//!
//! 1. The pager re-execs itself as `ezer-pager __mermaid-render` (see [`maybe_run_render_subprocess`]).
...
//! 2. The worker spawns that child with a wall-clock budget ([`RENDER_TIMEOUT`]) via [`ezer_mermaid::run_with_timeout`].
...
//!    Those are the source-size limit ([`ezer_mermaid::RenderLimits`]) and the raster megapixel/height caps (see `ezer-mermaid`).
...
fn render_source_to_png(
    ...
    target_width_px: u32,
    quality: MermaidRenderQuality,
) -> Result<RenderedDiagram, ezer_mermaid::MermaidError> {
...
impl AgentView {
    ...
    fn mermaid_out_path(&self, key: &MermaidCacheKey) -> Option<PathBuf> {
        // Test override: a hermetic per-test cache dir (no `EZER_HOME` mutation)
        #[cfg(test)]
...
mod tests {
    ...
    fn per_theme_renders_land_in_distinct_files() {
        ...
        let light_key =
            MermaidCacheKey::derive(src, ThemeKind::EzerDay, 80, MermaidRenderQuality::Terminal);
        assert_ne!(
    ...
    /// Point this test's session `mermaid/` cache dir at a private tempdir: hermetic, with no process-global `EZER_HOME` mutation.
    /// The `TempDir` lives in the [`TEST_MERMAID_DIR`] thread-local for the test thread's lifetime (so the dir outlives the view).
    /// Each parallel test gets its own dir, so there is no cross-test contamination and no `ezer_home()` cache race.
    ...
    fn mermaid_is_rendering_matches_by_source_across_theme_width_change() {
        ...
        let click_key =
            MermaidCacheKey::derive(src, ThemeKind::EzerNight, 80, MermaidRenderQuality::Open);
        let mut rt = MermaidRuntime::new();
        rt.pending.push(PendingMermaidAction {
            key: click_key.clone(),
            action: MermaidClickAction::Open,
        ...
        let live_key =
            MermaidCacheKey::derive(src, ThemeKind::EzerDay, 240, MermaidRenderQuality::Open);
        assert_ne!(
            click_key, live_key,
