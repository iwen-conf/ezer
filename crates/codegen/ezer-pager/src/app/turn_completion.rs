...
//! One is the fire-and-forget `ezer/session/prompt_complete` broadcast, kept for one release so leaders that have not yet upgraded still work.
...
pub(crate) const HOOK_DENIED_CATEGORY: &str =
    ezer_shell::session::commands::HOOK_DENIED_CATEGORY;
...
pub(crate) const COMPLETION_KIND_KEY: &str = ezer_shell::session::commands::COMPLETION_KIND_KEY;
/// `_meta.completionKind` of [`ezer_shell::session::commands::PromptCompletionKind::RemovedFromQueue`].
pub(crate) const REMOVED_FROM_QUEUE_KIND: &str =
    ezer_shell::session::commands::REMOVED_FROM_QUEUE_KIND;
...
impl From<&str> for TurnStopReason {
    fn from(s: &str) -> Self {
        match s {
            ...
            ezer_shell::session::interrupted_turn::INTERRUPTED_STOP_REASON => Self::Interrupted,
            _ => Self::Unknown,
        }
    }
}
...
pub(crate) fn terminal_marker(input: TerminalMarkerInput<'_>) -> Option<SessionEvent> {
    ...
    match input.stop {
        ...
        TurnStopReason::Interrupted => Some(SessionEvent::TurnFailed {
            error: input
                .agent_result
                .unwrap_or(ezer_shell::session::interrupted_turn::INTERRUPTED_MESSAGE)
                .to_string(),
            elapsed,
        }),
    }
...
pub(super) fn note_hook_blocked_turn(
    ...
    match requeued {
        Some((row_id, text)) => {
            ...
            let ctx: Option<ezer_shell::session::commands::CancellationContext> =
...
fn open_prompt_blocked_card(
    ...
    let short_hook_name = ezer_hooks::config::hook_display_name(hook_name);
