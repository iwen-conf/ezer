...
pub use ezer_telemetry::startup::{
...
pub struct ConnectFlags {
    ...
    pub remote_settings: Option<ezer_shell::util::config::RemoteSettings>,
...
pub async fn connect(cancel: &CancellationToken, flags: ConnectFlags) -> Result<AcpConnection> {
    ...
    let raw_config = {
        let _t = ezer_telemetry::instrumentation::timer("startup.config_load.merge_layers");
        ezer_shell::config::load_effective_config()
            .map_err(|e| anyhow::anyhow!("Failed to load config: {}", e))?
    };
    let mut agent_config = {
        let _t = ezer_telemetry::instrumentation::timer("startup.config_load.parse");
        AgentConfig::new_from_toml_cfg(&raw_config)
            .map_err(|e| anyhow::anyhow!("Failed to create agent config: {}", e))?
    };
    {
        let _t = ezer_telemetry::instrumentation::timer("startup.config_load.resolve");
        agent_config.resolve_runtime_fields(
            &ezer_shell::agent::config::RuntimeResolutionContext {
...
pub async fn connect_via_leader(
    ...
    apply_config_writes(&flags);
    startup::enter(StartupPhase::ConfigLoad);
    startup::set_auth_mode(ezer_shell::managed_config::classify_auth_mode());
    ...
    let conn = {
        let _t = ezer_telemetry::instrumentation::timer("startup.leader_connect.connect");
        connect_or_spawn(
            client_type,
            ClientMode::Stdio,
            &env_urls,
    ...
    ezer_shell::agent::init::update_telemetry_config(&agent_config, &auth_manager);
...
fn client_capabilities_meta(flags: &ConnectFlags) -> serde_json::Value {
    ...
    if let Some(obj) = meta.as_object_mut() {
        obj.insert(
            ezer_shell::session::USER_MESSAGE_ECHO_CAPABILITY.into(),
            true.into(),
        );
        obj.insert(
            ezer_status_line::STATUS_LINE_CAPABILITY.into(),
...
mod tests {
    ...
    fn shell_built_auth_methods_for_byok_user_skip_login_screen() {
        use ezer_shell::agent::auth_method::{AuthMethodsBuildInputs, build_auth_methods};
    ...
    fn startup_auth_xai_api_key_not_first_still_requires_login() {
        use ezer_shell::agent::auth_method::{EZER_COM_METHOD_ID, XAI_API_KEY_METHOD_ID};
        let methods = vec![
    ...
    fn client_capabilities_meta_always_requests_user_message_echo() {
        ...
        assert_eq!(
            meta.get(ezer_shell::session::USER_MESSAGE_ECHO_CAPABILITY)
                .and_then(|v| v.as_bool()),
            Some(true)
        );
    }
    ...
    fn client_capabilities_meta_advertises_the_status_line_the_config_asked_for() {
        let key = ezer_status_line::STATUS_LINE_CAPABILITY;
