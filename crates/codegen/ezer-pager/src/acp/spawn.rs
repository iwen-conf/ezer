...
pub(super) fn shutdown_worker_runtime(rt: tokio::runtime::Runtime) {
    let shutdown_span = ezer_telemetry::region::Region::from_span(tracing::info_span!(
...
fn join_agent_thread(handle: thread::JoinHandle<Result<()>>, timeout: Duration) -> JoinOutcome {
    ...
    let span = ezer_telemetry::session_end::join_span();
    ...
    ezer_telemetry::session_end::record_join(&span, outcome_label, elapsed_ms, notice_shown);
...
pub async fn spawn_ezer_shell(
    ...
    let boot = ezer_shell::agent::init::resolve_boot_startup_settings(
        &mut agent_config,
        cancel,
        true,
    ...
    let (agent_config, models_manager) = run_cancellable_blocking(bootstrap_cancel, move || {
        ezer_shell::agent::init::bootstrap_with_cancel(
            &agent_config,
            &bootstrap_auth,
            None,
            &worker_cancel,
    ...
    let spawn_fn: Box<dyn FnOnce(AcpClientTx) -> Result<Rc<MvpAgent>> + Send + 'static> = {
        Box::new(move |client_tx| {
            ...
            let _t = ezer_telemetry::instrumentation::timer("startup.worker_spawn.agent_build");
            let mut agent =
...
async fn spawn_agent_thread_direct(
    ...
    spawn_runtime_thread("acp-agent-worker", move |rt| {
        ...
        let result = local.block_on(&rt, async move {
            ...
            tokio::join!(
                ezer_shell::upload::drain_pending_uploads(UPLOAD_DRAIN_AT_CANCEL),
                ezer_telemetry::session_ctx::drain_at_process_exit(),
            );
            anyhow::Result::Ok(())
        });
