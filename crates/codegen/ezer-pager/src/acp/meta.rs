...
pub struct NotificationMeta {
    ...
    /// Monotonic per-process sequence parsed from `eventId` (`"{sessionId}-{counter}"`, see `ezer-shell
...
pub mod user_message_chunk_meta {
    ...
    /// See [`ezer_shell::session::PromptOrigin::hide_user_echo_from_scrollback`].
    ...
    pub const INTERJECTION: &str = ezer_shell::session::storage::INTERJECTION_META_KEY;
...
mod tests {
    ...
    fn parse_missing_new_fields() {
        // Simulate old ezer-shell that doesn't send streamStartMs/turnStartMs
        let meta_json = json!({
            "totalTokens": 1000u64,
            "agentTimestampMs": 1700000000000i64,
        });
