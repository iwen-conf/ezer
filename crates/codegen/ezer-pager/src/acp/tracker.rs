...
use std::rc::Rc;
use tracing::debug;
use ezer_shell::session::storage::chunk_meta_flag;
use ezer_tools::types::output::{BashOutput, ToolOutput};
use ezer_tools::types::output::{ReadFileOutput, SearchToolOutput, WebFetchOutput};
use ezer_tools::util::strip_redundant_session_cd;
...
impl WritingToolCall {
    ...
    pub fn label(&self) -> String {
        ...
        match self.tool_name.as_deref() {
            Some(name) if ezer_tools::is_task_tool_id(name) => {
                format!("Writing subagent prompt{ordinal}…")
            }
            Some(ezer_tools::USE_TOOL_NAME) => {
                format!("Preparing MCP tool{ordinal}…")
            }
            Some(ezer_tools::SEARCH_TOOL_NAME) => {
                format!("Searching MCP tools{ordinal}…")
            }
            Some(name) => {
                use ezer_tools::types::tool::ToolKind;
                let copy =
                    ezer_tools::tool_taxonomy::writing_tool_kind(name).and_then(|kind| {
                        match kind {
                ...
                match copy {
                    ...
                    None => {
                        let name =
                            ezer_workspace::permission::mcp_pretty_name_if_qualified(name);
...
impl AcpUpdateTracker {
    ...
    fn merge_edit_entries(
        ...
        if let Some(entry) = scrollback.get_by_id_mut(survivor) {
            if let RenderBlock::ToolCall(ToolCallBlock::Edit(edit)) = &mut entry.block {
                ...
                edit.set_hunks(ezer_pager_diff::stitch_overlapping_hunks(hunks));
    ...
    fn handle_user_message(
        ...
        let mut block = if chunk_meta_flag(&chunk, user_message_chunk_meta::INTERJECTION) {
            ...
            let skill_display =
                ezer_tools::implementations::skills::skill::extract_skill_display_text(&text);
...
fn user_message_hidden_from_scrollback(
    ...
    if let Some(pid) = meta.prompt_id.as_deref()
        && ezer_shell::session::PromptOrigin::from_prompt_id(pid)
    ...
    match tc.kind {
        ...
        acp::ToolKind::Edit => {
            ...
            let mut block = if success {
                let (hunks, _count) = ezer_pager_diff::extract_edit_hunks(tc);
...
fn canonical_tool_name(tc: &acp::ToolCall) -> Option<&str> {
    tc.meta
        .as_ref()?
        .get(ezer_tools::tool_taxonomy::TOOL_META_KEY)?
        .get("name")?
        .as_str()
}
...
fn is_task_tool(tc: &acp::ToolCall) -> bool {
    ezer_tools::is_task_tool_id(&tc.title) || is_task_variant(extract_variant(tc))
}
...
fn extract_edit_error(tc: &acp::ToolCall) -> String {
    use ezer_tools::types::output::SearchReplaceOutput;
    if let Some(ref raw) = tc.raw_output
...
fn extract_listdir_content(raw: &Option<serde_json::Value>) -> Option<String> {
    let val = raw.as_ref()?;
    match serde_json::from_value::<ToolOutput>(val.clone()) {
        Ok(ToolOutput::ListDir(ezer_tools::types::output::ListDirOutput::Content(c))) => {
            Some(c.content)
        }
        _ => None,
    }
...
fn extract_use_tool_output(raw: &Option<serde_json::Value>) -> Option<String> {
    ...
    if let Ok(output) = serde_json::from_value::<ToolOutput>(val.clone()) {
        let text = match output {
            ToolOutput::MCP(mcp) => {
                use ezer_tools::types::output::MCPOutputDetails;
                match mcp.output() {
