//! `ezer plugin` CLI subcommand: manage plugins and marketplace sources.
//!
...
//!
//! Business logic lives in `ezer_shell::plugin` and lower crates (`ezer-agent`, `ezer-plugin-marketplace`).
...
use ezer_agent::plugins::install_registry::{InstallKind, InstallRegistry};
use ezer_agent::plugins::manifest::{ManifestLoadResult, PluginManifest, load_manifest};
use ezer_plugin_marketplace::SourceKind;
use ezer_plugin_marketplace::git::SourceCacheLease;
use ezer_shell::plugin::{self, RepoUpdateOutcome, UninstallError};

// ── JSON output types ───────────────────────────────────────────────

/// Typed entry for `ezer plugin list --json`.
...
/// Typed entry for `ezer plugin marketplace list --json`.
#[derive(Serialize)]
...
pub enum PluginCommand {
    ...
    Uninstall {
        /// Plugin name (as shown by `ezer plugin list`).
        name: String,
...
fn trust_prompt(subject: &str, source_arg: &str) -> String {
    format!(
        "Installing {subject} requires confirmation.\n\
         ...
         \n\
         To proceed, re-run with --trust:\n  ezer plugin install {source_arg} --trust"
    )
}
...
fn cmd_list(json: bool, available: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("No plugins installed. Run `ezer plugin install --help` to get started.");
    } else {
...
fn installed_plugins(
    repos: &[(
        &str,
        &ezer_agent::plugins::install_registry::InstalledRepo,
    )],
) -> Vec<PluginEntry> {
    repos
        .iter()
...
fn available_plugins(registry: &InstallRegistry) -> Vec<PluginEntry> {
    ...
    let sources = ezer_shell::plugin::load_filtered_marketplace_sources();
    ...
    for source in &sources {
        ...
        for plugin in ezer_plugin_marketplace::scan_marketplace(&root).entries {
            let already_installed =
                ezer_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    registry,
                    &identity,
...
fn resolve_marketplace_root(
    source: &ezer_plugin_marketplace::MarketplaceSource,
    ...
    match &source.kind {
        ...
        SourceKind::Git { url, branch } => {
            let cache = ezer_plugin_marketplace::git::default_cache_root();
            ezer_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache,
                ezer_plugin_marketplace::git::SyncMode::UseTtl,
            )
...
fn install_kind(is_git: bool) -> ezer_telemetry::events::InstallKind {
    if is_git {
        ezer_telemetry::events::InstallKind::Git
    } else {
        ezer_telemetry::events::InstallKind::Local
    }
}

fn log_plugin_installed(
    install_kind: ezer_telemetry::events::InstallKind,
    success: bool,
    error_category: Option<String>,
) {
    ezer_telemetry::session_ctx::log_event(ezer_telemetry::events::PluginInstalled {
        install_kind,
        success,
        trust: true,
        error_category,
...
fn cmd_install(source: &str, trust: bool) -> Result<()> {
    if let Some(mref) = ezer_plugin_marketplace::install_resolve::parse_marketplace_ref(source)
    {
    ...
    if !trust {
        use ezer_agent::plugins::git_install::{self, InstallSource};
...
fn cmd_install_marketplace(
    source: &str,
    mref: &ezer_plugin_marketplace::install_resolve::MarketplaceRef,
    trust: bool,
) -> Result<()> {
    if !trust {
            ...
            if outcome.already_installed {
                ...
                println!(
                    "Plugin \"{}\" is already installed from {}. \
                     Run `ezer plugin update {}` to update it.",
    ...
    match plugin::uninstall_plugin(name, confirm, keep_data) {
        Ok(outcome) => {
            ezer_telemetry::session_ctx::log_event(
                ezer_telemetry::events::PluginUninstalled {
                    confirmed: true,
                    success: true,
                },
            );
        ...
        Err(UninstallError::NeedsConfirm {
            ...
            "Plugin \"{name}\" belongs to repo \"{repo_key}\" which also contains:\n\
             ...
             Uninstalling will remove all {total} plugin(s). To proceed:\n\
               ezer plugin uninstall {name} --confirm",
            other_plugins
                .iter()
                .map(|p| format!("  - {p}"))
                .collect::<Vec<_>>()
...
fn cmd_enable(name: &str) -> Result<()> {
    ...
    if let Err(e) = ezer_shell::config::remove_disabled_plugin(name) {
        tracing::warn!("failed to remove from disabled list: {e}");
    }
    ezer_shell::config::add_enabled_plugin(name)
...
fn cmd_disable(name: &str) -> Result<()> {
    ...
    if let Err(e) = ezer_shell::config::remove_enabled_plugin(name) {
        tracing::warn!("failed to remove from enabled list: {e}");
    }
    ezer_shell::config::add_disabled_plugin(name)
...
fn cmd_validate(path: &str) -> Result<()> {
    ...
    match load_manifest(&root) {
        ...
        Ok(ManifestLoadResult::NotFound) => {
            println!(
                "No plugin.json found. ezer discovers skills, agents, and hooks \
...
fn cmd_tag(path: &str, push: bool, force: bool, dry_run: bool) -> Result<()> {
    ...
    let version = match load_manifest(&root) {
        Ok(ManifestLoadResult::Found(m)) => m.version.ok_or_else(|| {
            anyhow::anyhow!(
                "No `version` field in plugin.json. Set a version to use `ezer plugin tag`."
            )
        })?,
...
async fn run_marketplace(cmd: MarketplaceCommand) -> Result<()> {
    ...
    match cmd {
        ...
        MarketplaceCommand::Remove { source } => {
            marketplace_remove(&ezer_shell::plugin::load_marketplace_sources(), &source)
        }
...
fn marketplace_list(
    sources: &[ezer_plugin_marketplace::MarketplaceSource],
    json: bool,
) -> Result<()> {
    if json {
        ...
        println!(
            "No marketplace sources configured.\n\
             Run `ezer plugin marketplace add --help` to get started."
        );
    } else {
        for s in sources {
...
fn marketplace_add(url: &str, force: bool) -> Result<()> {
    use ezer_shell::plugin::MarketplaceAddInput;
    ...
    let existing = ezer_shell::plugin::load_marketplace_sources();
    let already_configured = match &input {
        MarketplaceAddInput::GitUrl(git_url) => {
            use ezer_workspace::permission::resolution::normalize_git_url;
    ...
    if !force && let MarketplaceAddInput::GitUrl(git_url) = &input {
        ezer_plugin_marketplace::git::probe_git_remote(git_url).map_err(|e| {
            anyhow::anyhow!(
    ...
    let is_official = matches!(&input, MarketplaceAddInput::GitUrl(u)
        if ezer_plugin_marketplace::is_official_source_url(u));
    let name = if is_official {
        ezer_plugin_marketplace::OFFICIAL_SOURCE_NAME.to_string()
    } else {
        match &input {
...
fn find_removal_source<'a>(
    sources: &'a [ezer_plugin_marketplace::MarketplaceSource],
    input: &str,
    cwd: &Path,
) -> Result<&'a ezer_plugin_marketplace::MarketplaceSource, String> {
    ...
    let local_input = match plugin::classify_marketplace_add_input(input, cwd) {
        ezer_shell::plugin::MarketplaceAddInput::LocalPath(p) => Some(p),
        _ => None,
    };
...
fn marketplace_update(
    ...
    marketplace_update_with_cache_root(
        sources,
        name,
        &ezer_plugin_marketplace::git::default_cache_root(),
    )
}
...
fn marketplace_update_with_cache_root(
    ...
    for source in sources {
        ...
        if let SourceKind::Git { url, branch } = &source.kind {
            match ezer_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                cache_root,
            ) {
...
mod tests {
    use super::*;
    use ezer_plugin_marketplace::MarketplaceSource;

    /// `ezer plugin update` must exit nonzero when any update failed (e.g.
    ...
    fn trust_prompt_marketplace_has_no_error_framing() {
        ...
        assert!(msg.contains(
            "To proceed, re-run with --trust:\n  ezer plugin install sentry@xai-org/plugin-marketplace --trust"
        ));
    ...
    fn trust_prompt_git_and_local_subjects() {
        ...
        assert!(git.ends_with("  ezer plugin install u/r --trust"), "{git}");
    ...
    fn marketplace_update_force_syncs_fresh_git_cache() {
        ...
        let cache_dir = ezer_plugin_marketplace::git::sync_source_cache(
            &url,
            Some("main"),
            cache_root.path(),
        )
    ...
    fn current_head(repo: &Path) -> String {
        let output = ezer_plugin_marketplace::git::git_command()
            .current_dir(repo)
            .args(["rev-parse", "HEAD"])
            .output()
            .unwrap();
