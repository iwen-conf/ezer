//! Decodes the shell's `ezer/*` extension notifications into the headless [`ExtEvent`] the orchestrator dispatches.
...
pub(crate) fn reply_headless_ext_method(args: AcpArgsBox<acp::ExtRequest>) {
    ...
    let response = match method {
        ...
        "ezer/ask_user_question" => ext_response_from(&AskUserQuestionExtResponse::Cancelled),
        "ezer/mcp/elicit" => {
        ...
        "ezer/exit_plan_mode" => ext_response_from(&ExitPlanModeExtResponse {
...
pub(crate) fn handle_ext_notification(
    ...
    match method {
        "ezer/task_backgrounded" => decode_task_backgrounded(method, params),
        "ezer/task_completed" => decode_task_completed(method, params),
        "ezer/monitor_event" => ExtEvent::MonitorEvent,
        "ezer/leader/version_mismatch" => {
            match crate::acp::version_mismatch_banner(params) {
                Some(banner) => tracing::warn!(%banner, "ezer/leader/version_mismatch"),
                None => {
    ...
    match serde_json::from_str::<TaskBgEnvelope>(params) {
        Ok(env) => match env.update {
            ...
            TaskBgUpdate::Other => {
                tracing::error!(
                    method,
                    payload = params,
                    "headless: ezer/task_backgrounded with mismatched sessionUpdate \
        ...
        Err(e) => {
            tracing::error!(
                ...
                error = %e,
                payload = params,
                "headless: undecodable ezer/task_backgrounded notification; \
    ...
    match serde_json::from_str::<TaskDoneEnvelope>(params) {
        Ok(env) => match env.update {
            ...
            TaskDoneUpdate::Other => {
                tracing::error!(
                    method,
                    payload = params,
                    "headless: ezer/task_completed with mismatched sessionUpdate \
        ...
        Err(e) => {
            tracing::error!(
                ...
                error = %e,
                payload = params,
                "headless: undecodable ezer/task_completed notification; \
...
fn decode_session_notification(method: &str, params: &str) -> ExtEvent {
    ...
    enum XaiUpdate {
        ...
        ResponseCompleted {
            ...
            #[serde(default)]
            usage: Option<ezer_shell::extensions::notification::ResponseUsage>,
            #[serde(default)]
            signature: Option<String>,
            #[serde(default)]
                ...
                && matches!(tag.as_str(), "task_backgrounded" | "task_completed")
            {
                tracing::error!(
                    ...
                    "headless: background-task lifecycle tag on a session notification \
                     (expected the dedicated ezer/task_backgrounded|task_completed method); \
