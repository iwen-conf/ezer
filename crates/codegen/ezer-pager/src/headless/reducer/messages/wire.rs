...
/// Map a ezer permission mode to the Messages `permissionMode` enum; ezer-only values become `default`.
...
/// The `usage.server_tool_use` sub-object (ezer counts backend `web_search` only).
#[derive(Clone, Default, Serialize, Deserialize)]
