...
pub fn parse_permission_rules_strict(
    allow: &[String],
    deny: &[String],
) -> anyhow::Result<Vec<ezer_workspace::permission::types::PermissionRule>> {
...
pub(crate) fn parse_permission_rules_inner(
    ...
    use ezer_workspace::permission::types::RuleAction;
...
pub(crate) fn parse_cli_agents(
    json: &str,
) -> anyhow::Result<Vec<ezer_shell::agent::config::AgentDefinition>> {
    ...
    for (name, mut value) in map {
        ...
        let mut def = ezer_shell::agent::config::AgentDefinition::from_json(&value)
...
pub(crate) fn apply_agent_flag(
    agent: &Option<String>,
    config: &mut ezer_shell::agent::config::Config,
) {
