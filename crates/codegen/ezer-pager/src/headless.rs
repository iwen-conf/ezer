...
use crate::acp::spawn::{AgentShutdownGuard, SpawnedAgent, spawn_ezer_shell};
...
use ezer_shell::agent::auth_method::AuthMethodKind;
use ezer_shell::agent::config::Config as AgentConfig;
use ezer_shell::extensions::memory::MemoryFlushResponse;
use ezer_shell::extensions::task::{CancelSubagentRequest, KillTaskRequest};
...
use ezer_shell::util::config as cli_config;
...
pub(crate) fn attach_result_usage(result: &mut serde_json::Value, usage: &serde_json::Value) {
    ezer_shell::extensions::notification::attach_result_usage_fail_closed(result, usage);
}
...
async fn spawn_agent(
    ...
    let _ = options;
    spawn_ezer_shell(agent_config, cancel, memory_config).await
}
...
async fn authenticate(
    ...
    let method_id = crate::acp::select_eager_auth_method(auths, default_auth_method_id)
        .ok_or_else(|| {
            ...
            let interactive = std::io::stdin().is_terminal()
                && !ezer_shell::util::clipboard::is_remote_session();
    ...
    if kind.needs_interactive_login() {
        use std::io::IsTerminal;
        let interactive =
            std::io::stdin().is_terminal() && !ezer_shell::util::clipboard::is_remote_session();
...
async fn open_session(
    ...
    let mcp_servers =
        cli_config::load_mcp_servers(cwd, &ezer_tools::types::compat::CompatConfig::default());
...
async fn apply_headless_model_and_effort(
    ...
    .map_err(|e| {
        if let Some(name) = model_name {
            anyhow::anyhow!(
                "Couldn't set model '{}': {}. Run 'ezer models' to see available models.",
                name,
                e
            )
        } else {
...
pub async fn run_single_turn(
    ...
    ezer_shell::http::set_process_client_mode_headless();
    ...
    agent_config.mode = ezer_shell::agent::config::AgentMode::Headless;
    ...
    agent_config.cli_agent_overrides = ezer_shell::agent::config::CliAgentOverrides {
    ...
    ezer_telemetry::startup::enter(crate::acp::StartupPhase::AcpInitialize);
    ...
    ezer_telemetry::startup::enter(crate::acp::StartupPhase::EagerAuth);
    ...
    ezer_telemetry::startup::enter(crate::acp::StartupPhase::SessionCreate);
    ...
    if track_active {
        let _ = ezer_active_sessions::register(ezer_active_sessions::ActiveSession {
    ...
    let session_models = if needs_fresh_catalog {
        match ezer_shell::cli_models::fetch_model_state(&acp_tx).await {
    ...
    let outcome: Result<()> = match prompt_result {
        ...
        Some(Ok(resp)) => {
            ...
            let is_max_turns = resp
                ...
                .and_then(|v| v.as_str())
                == Some(ezer_shell::session::commands::MAX_TURNS_REACHED_CATEGORY);
            if is_max_turns {
        ...
        Some(Err(err)) => {
            ...
            if let Some(usage) = ezer_shell::sampling::error::prompt_usage_from_error(&err) {
            ...
            let stop_reason_override =
                (ezer_shell::sampling::error::stop_reason_for_turn_error(&err) == "MaxTokens")
    ...
    ezer_shell::upload::drain_pending_uploads_at_exit().await;
    ...
    let (method, params) = match work {
        ...
        BackgroundWork::Task(id) => (
            ...
            serde_json::value::to_raw_value(&KillTaskRequest {
                session_id: session_id.0.to_string(),
                task_id: id.clone(),
                source: ezer_shell::extensions::task::TaskKillSource::Teardown,
            })?,
        ),
    };
