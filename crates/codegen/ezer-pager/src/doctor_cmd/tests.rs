...
fn mixed_report() -> DiagnosticReport {
    ...
    report.findings = vec![
        ...
        DiagnosticFinding {
            ...
            message: "Use local SSH wrapping".to_owned(),
            remediation: Some(ManualRemediation {
                fix: "ezer wrap ssh <host>".to_owned(),
                config_path: None,
            }),
            automatic_remediation: Some(crate::diagnostics::ssh_wrap_automatic_remediation()),
            note: None,
...
fn human_mixed_fixture_is_exact() {
    assert_eq!(
        ...
        concat!(
            ...
            "  · ssh                          yes\n",
            "  · color                        256\n",
            "  · themes                       3/6: ezernight, ezerday, terminal\n",
            ...
            "    → Automatic setup: `ezer doctor fix tmux-clipboard`\n",
            "    → Add `set -g set-clipboard on` to ~/.tmux.conf\n",
            "      Reload tmux after editing.\n",
            "  i terminal.ssh-wrap            Use local SSH wrapping\n",
            "    → Automatic setup: `ezer doctor fix ssh-wrap`\n",
            "    → One-off: `ezer wrap ssh <host>`\n",
            "\n",
            ...
            "  Some checks only run in ezer. Start ezer and run /doctor.\n",
            "\n",
...
fn fix_preview_contains_exact_change_and_caveats() {
    ...
    assert!(preview.contains("To use once without changing config: `ezer wrap ssh <host>`"));
...
fn json_contract_is_structural_stable_ordered_and_ansi_free() {
        ...
        serde_json::json!({
            ...
            "facts": {
                ...
                "color": {
                    ...
                    "availableThemes": ["ezernight", "ezerday", "terminal"],
                    "totalThemes": 6
                },
            ...
            "findings": [
                ...
                },
                {
                    ...
                    "remediation": {"fix": "ezer wrap ssh <host>", "configPath": null},
                    "automaticRemediation": {
                        "fixId": "terminal.ssh-wrap",
                        "command": "ezer doctor fix terminal.ssh-wrap"
                    },
                    "note": null
                }
            ],
    ...
    assert!(!text.contains("ezer Doctor"));
}
...
fn new_named_findings_extend_json_without_schema_changes() {
    ...
    report.facts.clipboard.fix = Some("ezer wrap <ssh command> or /minimal".to_owned());
    report.findings.push(DiagnosticFinding {
