...
//!
//! A dependency on `ezer-pager-minimal` would be a cargo cycle: that crate reads this crate's `AppView`, `views::*` widgets, and `scrollback`.
...
//!
//! The `ezer-pager-bin` binary calls `ezer_pager_minimal::install()` once at startup.
...
/// Renders one minimal-mode frame; the installed implementation is `ezer_pager_minimal::draw`.
