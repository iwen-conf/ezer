...
use ezer_agent::config::{AgentDefinition, AgentScope, BuiltinAgentName};
use ezer_shell::agent::config::AgentSelectionConfig;
use ezer_tools::implementations::skills::discovery::extract_first_paragraph;
...
/// User-level vs project-level config files (`~/.ezer` vs `{cwd}/.ezer`).
...
pub struct AgentsModalState {
    ...
    bundle: BundleState,
    /// Resolved startup agent name (same chain as the shell: `[agent]`, `EZER_AGENT`, model `agentType`, then `ezer-build`).
    ...
    plugin_registry: Option<ezer_agent::plugins::PluginRegistry>,
...
pub fn build_agent_list(
    cwd: &Path,
    toggle: &HashMap<String, bool>,
    plugins: Option<&ezer_agent::plugins::PluginRegistry>,
) -> Vec<AgentListEntry> {
    ...
    let subagent_names: Vec<String> = BuiltinAgentName::subagent_variants()
        ...
        .collect();
    let discovered = ezer_agent::discovery::discover(cwd);
    ...
    if let Some(registry) = plugins {
        for agent in ezer_agent::discovery::plugin_agents(registry) {
...
/// Union bundled personas with local `~/.ezer/personas` and `{cwd}/.ezer/personas`.
///
...
pub fn load_agent_toggle() -> HashMap<String, bool> {
    let root = match ezer_shell::config::load_effective_config() {
        Ok(r) => r,
...
/// True when `path` is a deletable local persona file (user or project `.ezer/personas`).
pub fn persona_path_is_deletable(path: &Path) -> bool {
    config_path_is_user_or_project(path, "personas")
}
/// Shared guard: canonical path under `~/.ezer/{subdir}` or `{cwd}/.ezer/{subdir}`, not bundled.
...
pub(crate) fn load_agent_selection_config() -> AgentSelectionConfig {
    ezer_shell::config::load_effective_config()
        .ok()
        .and_then(|root| ezer_shell::agent::config::Config::new_from_toml_cfg(&root).ok())
        .map(|cfg| cfg.agent)
        .unwrap_or_default()
}
...
/// Mirrors `MvpAgent::resolve_agent_definition` in ezer-shell.
pub fn resolve_default_agent_name(cwd: &Path, model_agent_type: Option<&str>) -> String {
    ...
    ezer_shell::agent::mvp_agent::MvpAgent::resolve_agent_definition(
        cwd,
        None,
        &agent_config,
        None,
...
pub fn format_agent_detail(entry: &AgentListEntry) -> Vec<String> {
    ...
    let mode_label = match def.prompt_mode {
        ezer_agent::config::PromptMode::Extend => "extend",
        ezer_agent::config::PromptMode::Full => "full",
    };
...
mod tests {
    use super::*;
    use ezer_shell::agent::config::DEFAULT_AGENT_TYPE;
    #[test]
    ...
    fn merge_persona_lists_appends_local_only() {
        ...
        let personas_dir = dir.path().join(".ezer").join("personas");
    ...
    fn persona_is_deletable_local_vs_bundled() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local = dir.path().join(".ezer").join("personas").join("p.toml");
        ...
        let bundled_detail = PersonaDetail {
            ...
            has_inputs: false,
            has_outputs: false,
            source_path: Some("/home/user/.ezer/bundled/personas/b.toml".into()),
            scope_label: None,
        };
    ...
    fn delete_persona_file_allows_project_persona() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join(".ezer").join("personas").join("gone.toml");
    ...
    fn plugin_registry_with_reviewer(
        plugin_root: &Path,
    ) -> ezer_agent::plugins::PluginRegistry {
        use ezer_agent::plugins::discovery::PluginId;
        use ezer_agent::plugins::{
