...
use ezer_shell::tools::{TodoItem, TodoStatus};
...
mod tests {
    ...
    fn render_per_model_breakdown_shows_each_model() {
        let mut goal = make_goal();
        goal.live_tokens_by_model = vec![("test-model-4".into(), 12_300), ("test-model-3-mini".into(), 8_000)];
        let text = render_to_text(&goal);
        assert!(text.contains("test-model-4"), "first model must render:\n{text}");
        assert!(
            text.contains("test-model-3-mini"),
