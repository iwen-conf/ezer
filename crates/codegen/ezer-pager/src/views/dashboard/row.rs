...
pub(crate) fn workspace_member_is_pinned(member: &ezer_dashboard_store::Member) -> bool {
...
pub(crate) fn build_rows_with_workspace(
    ...
    let mut rows = inputs
        ...
        .filter(|member| matches!(member.kind, ezer_dashboard_store::MemberKind::Build))
        .filter_map(|member| {
...
pub(crate) fn workspace_member_is_visible(
    member: &ezer_dashboard_store::Member,
    live_agent: Option<&AgentView>,
    pinned: bool,
) -> bool {
    if !matches!(member.kind, ezer_dashboard_store::MemberKind::Build) {
        return false;
    }
    if pinned {
        return true;
...
fn workspace_member_has_content(member: &ezer_dashboard_store::Member) -> bool {
...
mod tests {
    ...
    fn workspace_rows_with_provisional(
        agents: &IndexMap<AgentId, AgentView>,
        snapshot: Option<&ezer_dashboard_store::WorkspaceSnapshot>,
        provisional: &[AgentId],
    ) -> Vec<DashboardRow> {
    ...
    fn workspace_rows_ignore_non_build_members() {
        ...
        conversation.kind = ezer_dashboard_store::MemberKind::Conversation;
    ...
    fn pinned_workspace_order_survives_session_loading() {
        for grouping in [
            ezer_dashboard_store::Grouping::State,
            ezer_dashboard_store::Grouping::Directory,
        ] {
    ...
    fn append_roster_rows_uses_model_id_as_secondary_line() {
        ...
        let entry = RosterEntry {
            model_id: Some("test-model-4.5".to_string()),
            ..roster_entry_with("m", Some("Fix the bug"), RosterActivity::Dormant)
        };
        let rows = collect_roster(&[entry], &empty);
        assert_eq!(rows.len(), 1);
        assert_eq!(nth(&rows, 0).secondary_line.as_deref(), Some("test-model-4.5"));
    }
    ...
    fn idle_local_agent_without_message_has_blank_secondary() {
        let agent = make_idle_agent_with_model(Some("test-model-4.5"));
    ...
    fn workspace_manual_order_is_global_and_survives_filtering() {
        ...
        a.order_rank = Some(2 * ezer_dashboard_store::RANK_GAP);
        let b = workspace_member("b", "Beta", None);
        let mut c = workspace_member("c", "Charlie", None);
        c.order_rank = Some(ezer_dashboard_store::RANK_GAP);
