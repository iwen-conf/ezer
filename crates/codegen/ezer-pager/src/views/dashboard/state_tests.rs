...
fn rename_at_cap_drops_extra_char() {
    assert_eq!(
        MAX_RENAME_SCALARS,
        ezer_shell::session::persistence::MAX_TITLE_SCALARS
    );
    assert_eq!(ezer_shell::session::persistence::MAX_TITLE_SCALARS, 100);
...
fn search_mode_mouse_does_not_edit_a_hidden_peek() {
    ...
    state.peek_reply.file_search.set_test_state(
        ctx,
        vec![ezer_workspace::file_system::FuzzyMatchResult {
    ...
    state.dispatch.file_search.set_test_state(
        dispatch_ctx,
        vec![
            ezer_workspace::file_system::FuzzyMatchResult {
...
fn workspace_resolver_roundtrips_unloaded_member_through_existing_format() {
    let snapshot = ezer_dashboard_store::WorkspaceSnapshot {
...
fn workspace_identity_keeps_removed_selection_as_repair_anchor() {
    ...
    old_snapshot.members.push(ezer_dashboard_store::Member {
        session_id: ezer_dashboard_store::SessionId::new("removed").unwrap(),
        kind: ezer_dashboard_store::MemberKind::Build,
        origin: ezer_dashboard_store::MemberOrigin::Local,
        cwd: Some("/tmp".into()),
