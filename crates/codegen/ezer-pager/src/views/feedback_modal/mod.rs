...
pub use ezer_feedback::{
    FeedbackDraft, FeedbackDraftId, FeedbackFailureMode, FeedbackTaskCategory, FeedbackTaxonomy,
    FeedbackType,
};
use ezer_feedback::{FeedbackSource, structured_feedback};
pub use ezer_shell::session::FeedbackTraceUploadIntent;
...
impl FeedbackModalState {
    ...
    fn report_trace_outcome(
        &mut self,
        choice: ezer_telemetry::events::FeedbackTraceConsentChoice,
    ) {
        if !self.trace_outcome_reported {
            ezer_telemetry::session_ctx::log_event(
                ezer_telemetry::events::FeedbackTraceConsentSelected {
                    choice,
    ...
    pub(crate) fn report_confirmed_trace_choice(&mut self, choice: FeedbackTraceChoice) {
        let choice = match choice {
            FeedbackTraceChoice::SendThisSession => {
                ezer_telemetry::events::FeedbackTraceConsentChoice::SendThisSession
            }
            FeedbackTraceChoice::FeedbackOnly => {
                ezer_telemetry::events::FeedbackTraceConsentChoice::NoUpload
            }
            FeedbackTraceChoice::NeverAsk => {
                ezer_telemetry::events::FeedbackTraceConsentChoice::NeverAsk
            }
        };
    ...
    fn report_dismissed_trace_card(&mut self) {
        if self.in_trace_step() {
            self.report_trace_outcome(
                ezer_telemetry::events::FeedbackTraceConsentChoice::Dismissed,
            );
        }
    }
