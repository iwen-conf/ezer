...
impl RenderBlock {
    ...
    pub fn memory_capture(
        from_turn: u32,
        through_turn: u32,
        entries: Vec<ezer_shell::extensions::notification::MemoryCaptureDebugEntry>,
    ) -> Self {
    ...
    pub fn context_info(
        snapshot: ezer_shell::session::ContextInfo,
    ...
    pub fn with_hyperlinks<R>(
        &self,
        f: impl FnOnce(&[ezer_markdown::HyperlinkTarget]) -> R,
    ) -> R {
        match self {
    ...
    pub fn with_table_copy_meta<R>(
        &self,
        f: impl FnOnce(&[ezer_markdown::TableCopyMeta]) -> R,
    ) -> R {
        match self {
...
mod searchable_text_tests {
    ...
    use ezer_shell::session::ContextInfo;
    ...
    fn subagent_failed_indexes_metadata_and_error() {
        ...
        assert!(text.contains("ezer-test"), "got: {text:?}");
