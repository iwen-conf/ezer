...
pub enum NotificationProtocol {
    ...
    Osc9,
    /// Kitty: `\x1b]99;i=ezer;{message}\x1b\\`
    Osc99,
...
fn notification_sequence(
    ...
    Some(match protocol {
        ...
        // OSC 777 already uses the tab title as subtitle, so keep "ezer".
        NotificationProtocol::Osc9 => format!("\x1b]9;{body} \u{b7} {title}\x07").into(),
        NotificationProtocol::Osc99 => format!("\x1b]99;i=ezer;{body} \u{b7} {title}\x1b\\").into(),
        NotificationProtocol::Osc777 => format!("\x1b]777;notify;ezer;{body}\x1b\\").into(),
...
mod tests {
    ...
    fn emit_enqueues_bytes_on_writer_queue() {
        ...
        assert_eq!(payload.data(), "\x1b]777;notify;ezer;body\x1b\\".as_bytes());
    }
