...
use ezer_shell::session::repo_changes::UploadMethod;
use ezer_shell::upload::trace_turns::{
...
pub struct TraceArgs {
    ...
    #[arg(long)]
    pub local: bool,
    /// Output path (default: $EZER_HOME/trace-exports/<session-id>.tar.gz)
    #[arg(short, long)]
...
pub fn build_session_tar(
    ...
    let mut file_count: u32 = 0;
    {
        ...
        let metadata = ExportMetadata {
            ...
            ezer_version: ezer_version::full_version().to_owned(),
...
pub(crate) fn find_session_dir(session_id: &str) -> Result<PathBuf> {
    ezer_shell::session::persistence::find_session_dir_by_id(session_id).with_context(|| {
        format!(
            "Session '{session_id}' not found under {}",
            crate::util::display_user_ezer_path("sessions")
        )
    })
}

pub fn trace_exports_dir() -> PathBuf {
    ezer_home().join("trace-exports")
}
...
async fn run_upload(
    ...
    let upload_method = match resolve_upload_gate(agent_config).await {
        ...
        UploadGate::NoCredentials => {
            if !json {
                eprintln!(
                    "No upload credentials for this account (run `ezer login` or set a deployment \
    ...
    if bucket_url.is_none()
        && matches!(
            upload_method,
            ezer_shell::session::repo_changes::UploadMethod::Direct { .. }
        )
    {
        anyhow::bail!(
    ...
    let upload_config = ezer_shell::session::repo_changes::TraceExportConfig {
...
impl UploadAttempt<'_> {
    ...
    fn write_debug_log(&self, error: &anyhow::Error, output_dir: &Path) -> PathBuf {
        ...
        let _ = writeln!(log, "ezer version: {}", ezer_version::full_version());
        let _ = writeln!(
            log,
            "OS:           {} {}",
            std::env::consts::OS,
...
async fn upload_with_retries(
    config: &ezer_shell::session::repo_changes::TraceExportConfig,
    object_path: &str,
    archive: &[u8],
...
pub(crate) async fn resolve_upload_gate(agent_config: &AgentConfig) -> UploadGate {
    ...
    let auth = ezer_login::ensure_authenticated_or_noninteractive(
    ...
    let auth_token = auth.filter(EzerAuth::is_xai_auth).map(|auth| auth.key);
