//! Local, ACP, and PTY terminal runners for the ezer shell.
//!
//! `ezer-shell` re-exports this crate as `ezer_shell::terminal`.
...
/// Resolved absolute path to bash. On Unix uses the `ezer_config` cascade (`$EZER_SHELL` > `$SHELL` > `which` > common dirs > `/bin/bash`), cached process-wide. On non-Unix returns `"/bin/bash"`.
...
pub(crate) fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        ezer_config::shell::unix_shell_path(ezer_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
pub use ezer_tools::util::pager_env;
