//! Tracks open TUI sessions in `~/.ezer/active_sessions.json` for crash recovery.
...
// -- Public API (delegates to `_in` variants with default ezer home) --------

/// Register a session as active (idempotent by session_id).
pub fn register(session: ActiveSession) -> io::Result<()> {
    register_in(&ezer_config::ezer_home(), session)
}
