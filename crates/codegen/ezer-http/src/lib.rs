...
//! process-wide clients owned by `ezer_sampler::shared_http`. TLS policy
//! (backend pin, roots, provider) lives in `ezer_extra_ca`.

#![deny(clippy::indexing_slicing)]

use std::sync::OnceLock;

use ezer_workspace::permission::ClientType;
...
macro_rules! startup_timer {
    ($name:literal) => {{
        use ezer_telemetry::instrumentation::{
...
pub use ezer_sampler::OriginClientInfo;
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn ezer_auth::AuthCredentialProvider>,
) -> reqwest_middleware::ClientWithMiddleware {
    reqwest_middleware::ClientBuilder::new(client)
        .with(ezer_auth::AuthRetryMiddleware::new(credentials, 1))
        .build()
}
...
pub fn shared_startup_blocking_client() -> reqwest::blocking::Client {
    ...
    BLOCKING_CLIENT
        .get_or_init(|| {
            ...
            ezer_extra_ca::build_blocking_reqwest_client(|builder| {
                builder
