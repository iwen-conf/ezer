...
use ezer_update::auto_update::{download_silent, download_with_progress};
use ezer_update::version::fetch_gcs_version_from_base;
...
async fn download_silent_writes_body_to_dest() {
    ...
    Mock::given(method("GET"))
        .and(path("/ezer-0.1.181-macos-aarch64"))
    ...
    let url = format!("{}/ezer-0.1.181-macos-aarch64", server.uri());
...
async fn download_silent_publishes_executable() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let dest = tmp.path().join("ezer-0.1.181-linux-x86_64");
    download_silent(&format!("{}/bin", server.uri()), &dest)
        .await
        .unwrap();
