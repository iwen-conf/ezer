...
fn test_tmp_download_path_is_unique_per_version_and_per_attempt() {
    // The old `with_extension("tmp")` collapsed every 0.1.x versioned name onto a single `ezer-0.1.tmp`
    ...
    let dest_181 = std::path::Path::new("/home/u/.ezer/downloads/ezer-0.1.181-linux-x86_64");
    let dest_182 = std::path::Path::new("/home/u/.ezer/downloads/ezer-0.1.182-linux-x86_64");
    ...
    assert!(
        name.starts_with("ezer-0.1.181-linux-x86_64."),
    ...
    assert_eq!(
        a.parent(),
        std::path::Path::new("/home/u/.ezer/downloads").into(),
...
async fn test_atomic_symlink_swap_replaces_regular_file() {
    ...
    let link = dir.path().join("ezer");
    // Simulate an old installation where ezer is a regular file.
...
async fn test_reconcile_agent_repoints_diverged_agent() {
    ...
    std::fs::write(downloads.join("ezer-0.2.101-macos-aarch64"), "new").unwrap();
    std::fs::write(downloads.join("ezer-0.1.199-macos-aarch64"), "old").unwrap();

    std::os::unix::fs::symlink("../downloads/ezer-0.2.101-macos-aarch64", bin.join("ezer"))
        .unwrap();
    std::os::unix::fs::symlink("../downloads/ezer-0.1.199-macos-aarch64", bin.join("agent"))
        .unwrap();

    reconcile_agent_to_ezer(&bin).await;

    assert_eq!(
        std::fs::read_link(bin.join("agent")).unwrap(),
        std::path::PathBuf::from("../downloads/ezer-0.2.101-macos-aarch64"),
    );
    assert_eq!(std::fs::read_to_string(bin.join("agent")).unwrap(), "new");
    assert!(downloads.join("ezer-0.1.199-macos-aarch64").exists());
}
...
async fn test_reconcile_agent_heals_legacy_unversioned_agent() {
    let (_dir, bin, downloads) = managed_layout();
    std::fs::write(downloads.join("ezer-0.2.101-macos-aarch64"), "new").unwrap();
    std::fs::write(downloads.join("ezer-macos-aarch64"), "legacy").unwrap();

    std::os::unix::fs::symlink("../downloads/ezer-0.2.101-macos-aarch64", bin.join("ezer"))
        .unwrap();
    std::os::unix::fs::symlink("../downloads/ezer-macos-aarch64", bin.join("agent")).unwrap();
...
async fn test_reconcile_agent_noop_when_consistent() {
    let (_dir, bin, downloads) = managed_layout();
    std::fs::write(downloads.join("ezer-0.2.101-macos-aarch64"), "new").unwrap();
    let target = "../downloads/ezer-0.2.101-macos-aarch64";
    std::os::unix::fs::symlink(target, bin.join("ezer")).unwrap();
...
#[cfg(unix)]
#[tokio::test]
async fn test_reconcile_agent_skips_when_ezer_dangling() {
    let (_dir, bin, downloads) = managed_layout();
    std::os::unix::fs::symlink("../downloads/ezer-0.2.101-macos-aarch64", bin.join("ezer"))
        .unwrap();
    std::fs::write(downloads.join("ezer-0.1.199-macos-aarch64"), "old").unwrap();
    ...
    assert_eq!(
        std::fs::read_link(bin.join("agent")).unwrap(),
        std::path::PathBuf::from("../downloads/ezer-0.1.199-macos-aarch64"),
    );
}

#[cfg(unix)]
#[tokio::test]
async fn test_reconcile_agent_skips_when_ezer_not_symlink() {
    let (_dir, bin, downloads) = managed_layout();
    std::fs::write(bin.join("ezer"), "copy-binary").unwrap();
    std::fs::write(downloads.join("ezer-0.1.199-macos-aarch64"), "old").unwrap();
    std::os::unix::fs::symlink("../downloads/ezer-0.1.199-macos-aarch64", bin.join("agent"))
        .unwrap();
...
async fn test_sweep_stale_tmp_links_removes_stale_keeps_fresh_and_active() {
    ...
    let leftover_old = dir.path().join("ezer.tmp-link");
    let leftover_new = dir.path().join("ezer.123-0.tmp-link");
    std::os::unix::fs::symlink(&target, &leftover_old).unwrap();
    std::os::unix::fs::symlink(&target, &leftover_new).unwrap();

    // max_age = ZERO: every leftover is stale and removed; the active `ezer` link (no `.tmp-link` suffix) is untouched
    ...
    let fresh = dir.path().join("ezer.999-9.tmp-link");
...
async fn test_atomic_symlink_swap_with_relative_target() {
    ...
    std::fs::write(downloads.join("ezer-0.1.203"), "v203").unwrap();

    let rel_target = std::path::Path::new("../downloads/ezer-0.1.203");
    ...
    assert_eq!(
        ...
        std::path::PathBuf::from("../downloads/ezer-0.1.203")
    );
...
fn test_relative_symlink_target_sibling_dirs() {
    // bin/ezer -> ../downloads/ezer-0.1.203
    let target = std::path::Path::new("/home/alice/.ezer/downloads/ezer-0.1.203");
    let link = std::path::Path::new("/home/alice/.ezer/bin/ezer");
...
fn test_relative_symlink_target_same_dir() {
    // downloads/ezer-latest -> ezer-0.1.203 (same directory)
    let target = std::path::Path::new("/home/alice/.ezer/downloads/ezer-0.1.203");
    let link = std::path::Path::new("/home/alice/.ezer/downloads/ezer-latest");
    let result = relative_symlink_target(target, link);
    assert_eq!(result, std::path::PathBuf::from("ezer-0.1.203"));
}

#[cfg(unix)]
#[test]
fn test_relative_symlink_target_cross_tree_stays_absolute() {
    // /usr/local/bin/ezer -> /home/alice/.ezer/downloads/ezer-0.1.203
    // Different grandparents, so the target should stay absolute
    let target = std::path::Path::new("/home/alice/.ezer/downloads/ezer-0.1.203");
    let link = std::path::Path::new("/usr/local/bin/ezer");
    let result = relative_symlink_target(target, link);
    assert_eq!(
        result,
        std::path::PathBuf::from("/home/alice/.ezer/downloads/ezer-0.1.203")
    );
}
...
async fn test_relative_symlink_survives_directory_move() {
    // Simulates Docker bind-mount: create ~/.ezer/ layout at path A,
    // then move it to path B and verify the symlink still resolves.
    let dir = tempfile::tempdir().unwrap();

    // Create alice's layout
    let alice = dir.path().join("alice").join(".ezer");
    ...
    let bob_link = bob.join("bin").join("ezer");
...
async fn test_cleanup_old_downloads_keeps_current_plus_one() {
    ...
    // Simulate 5 old ezer binaries in downloads dir.
    for v in ["0.1.140", "0.1.141", "0.1.142", "0.1.143", "0.1.144"] {
        std::fs::write(d.join(format!("ezer-{}-macos-aarch64", v)), v).unwrap();
    }
    std::fs::write(d.join("ezer-0.1.145-macos-aarch64"), "current").unwrap();

    make_all_stale(d);

    cleanup_old_downloads(d, "ezer", "0.1.145").await;
    ...
    assert!(
        !d.join("ezer-0.1.143-macos-aarch64").exists(),
        "0.1.143 should be deleted"
    );
    assert!(
        !d.join("ezer-0.1.142-macos-aarch64").exists(),
        "0.1.142 should be deleted"
    );
    assert!(
        !d.join("ezer-0.1.141-macos-aarch64").exists(),
        "0.1.141 should be deleted"
    );
    assert!(
        !d.join("ezer-0.1.140-macos-aarch64").exists(),
...
async fn test_cleanup_old_downloads_does_not_touch_other_binaries() {
    ...
    // ezer and ezer-pager should not interfere with each other.
    std::fs::write(d.join("ezer-0.1.140-macos-aarch64"), "old-ezer").unwrap();
    std::fs::write(d.join("ezer-0.1.141-macos-aarch64"), "current-ezer").unwrap();
    std::fs::write(d.join("ezer-0.1.140-macos-aarch64"), "old-pager").unwrap();
    std::fs::write(d.join("ezer-0.1.141-macos-aarch64"), "current-pager").unwrap();

    // Cleanup only ezer; pager files must be untouched
    make_all_stale(d);

    cleanup_old_downloads(d, "ezer", "0.1.141").await;

    assert!(d.join("ezer-0.1.141-macos-aarch64").exists());
    assert!(d.join("ezer-0.1.140-macos-aarch64").exists()); // only old, kept as N-1
    assert!(
        d.join("ezer-0.1.140-macos-aarch64").exists(),
        "pager untouched"
    );
...
async fn test_cleanup_old_downloads_removes_stale_tmp_keeps_fresh_tmp() {
    ...
    std::fs::write(d.join("ezer-0.1.140-macos-aarch64.tmp"), "partial").unwrap();
    make_stale(&d.join("ezer-0.1.140-macos-aarch64.tmp"));
    // Fresh tmp: a concurrent updater's in-flight download is kept, or its atomic rename would fail with ENOENT
    std::fs::write(d.join("ezer-0.1.142-macos-aarch64.77-0.tmp"), "inflight").unwrap();
    std::fs::write(d.join("ezer-0.1.141-macos-aarch64"), "current").unwrap();

    cleanup_old_downloads(d, "ezer", "0.1.141").await;

    assert!(
        !d.join("ezer-0.1.140-macos-aarch64.tmp").exists(),
        "stale tmp cleaned up"
    );
    assert!(
        d.join("ezer-0.1.142-macos-aarch64.77-0.tmp").exists(),
...
async fn test_cleanup_old_downloads_keeps_fresh_versioned_binary() {
    ...
    std::fs::write(d.join("ezer-0.1.138-macos-aarch64"), "in-flight").unwrap();

    cleanup_old_downloads(d, "ezer", "0.1.141").await;

    assert!(d.join("ezer-0.1.141-macos-aarch64").exists(), "current");
    assert!(d.join("ezer-0.1.140-macos-aarch64").exists(), "N-1 kept");
    assert!(
        d.join("ezer-0.1.138-macos-aarch64").exists(),
        "fresh just-renamed binary must NOT be deleted"
    );
    assert!(
        !d.join("ezer-0.1.139-macos-aarch64").exists(),
...
async fn test_cleanup_old_downloads_skips_symlinks() {
    ...
    // ezer-latest is a symlink, so it must be skipped
    let target = d.join("ezer-0.1.141-macos-aarch64");
    std::fs::write(&target, "current").unwrap();
    std::os::unix::fs::symlink(&target, d.join("ezer-latest")).unwrap();
...
async fn test_cleanup_old_downloads_version_prefix_collision() {
    ...
    std::fs::write(d.join("ezer-0.1.14-macos-aarch64"), "current").unwrap();
    std::fs::write(d.join("ezer-0.1.140-macos-aarch64"), "old-140").unwrap();
    std::fs::write(d.join("ezer-0.1.141-macos-aarch64"), "old-141").unwrap();
    std::fs::write(d.join("ezer-0.1.13-macos-aarch64"), "old-13").unwrap();

    make_all_stale(d);

    cleanup_old_downloads(d, "ezer", "0.1.14").await;

    // Current must survive.
    assert!(
        d.join("ezer-0.1.14-macos-aarch64").exists(),
        "current 0.1.14"
    );
    assert!(
        d.join("ezer-0.1.141-macos-aarch64").exists(),
    ...
    assert!(
        !d.join("ezer-0.1.13-macos-aarch64").exists(),
...
async fn test_cleanup_old_downloads_pager_multi_version() {
    // Verify cleanup works for ezer-pager with multiple old versions.
    let dir = tempfile::tempdir().unwrap();
    let d = dir.path();

    for v in ["0.1.148", "0.1.149", "0.1.150"] {
        std::fs::write(d.join(format!("ezer-{}-linux-x64", v)), v).unwrap();
    }
    std::fs::write(d.join("ezer-0.1.151-linux-x64"), "current").unwrap();

    make_all_stale(d);

    cleanup_old_downloads(d, "ezer", "0.1.151").await;

    assert!(d.join("ezer-0.1.151-linux-x64").exists(), "current");
    assert!(d.join("ezer-0.1.150-linux-x64").exists(), "N-1 kept");
    assert!(
        !d.join("ezer-0.1.149-linux-x64").exists(),
        "0.1.149 deleted"
    );
    assert!(
        !d.join("ezer-0.1.148-linux-x64").exists(),
        "0.1.148 deleted"
    );
}
...
async fn test_cleanup_old_downloads_npm_layout() {
    // npm layout: files are just `ezer-{version}` (no platform suffix).
    let dir = tempfile::tempdir().unwrap();
    let d = dir.path();

    for v in ["0.1.138", "0.1.139", "0.1.140"] {
        std::fs::write(d.join(format!("ezer-{}", v)), v).unwrap();
    }
    std::fs::write(d.join("ezer-0.1.141"), "current").unwrap();

    make_all_stale(d);

    cleanup_old_downloads(d, "ezer", "0.1.141").await;

    assert!(d.join("ezer-0.1.141").exists(), "current");
    assert!(d.join("ezer-0.1.140").exists(), "N-1 kept");
    assert!(!d.join("ezer-0.1.139").exists(), "0.1.139 deleted");
    assert!(!d.join("ezer-0.1.138").exists(), "0.1.138 deleted");
}

#[tokio::test]
async fn test_cleanup_old_downloads_alpha_versions() {
    // Alpha version filenames include pre-release tags:
    //   ezer-0.1.150-alpha.1-macos-aarch64
    let dir = tempfile::tempdir().unwrap();
    let d = dir.path();

    std::fs::write(d.join("ezer-0.1.148-alpha.1-macos-aarch64"), "alpha-148-1").unwrap();
    std::fs::write(d.join("ezer-0.1.148-alpha.2-macos-aarch64"), "alpha-148-2").unwrap();
    std::fs::write(d.join("ezer-0.1.149-alpha.1-macos-aarch64"), "alpha-149-1").unwrap();
    // Current version is the newest alpha.
    std::fs::write(d.join("ezer-0.1.150-alpha.1-macos-aarch64"), "current").unwrap();

    make_all_stale(d);

    cleanup_old_downloads(d, "ezer", "0.1.150-alpha.1").await;

    // Current must survive.
    assert!(
        d.join("ezer-0.1.150-alpha.1-macos-aarch64").exists(),
        "current alpha"
    );
    // Newest old (0.1.149-alpha.1) kept as N-1.
    assert!(
        d.join("ezer-0.1.149-alpha.1-macos-aarch64").exists(),
        "N-1 alpha"
    );
    // Older alphas deleted.
    assert!(
        !d.join("ezer-0.1.148-alpha.2-macos-aarch64").exists(),
        "0.1.148-alpha.2 deleted"
    );
    assert!(
        !d.join("ezer-0.1.148-alpha.1-macos-aarch64").exists(),
...
async fn test_cleanup_old_downloads_mixed_stable_and_alpha() {
    ...
    std::fs::write(d.join("ezer-0.1.148-macos-aarch64"), "stable-148").unwrap();
    std::fs::write(d.join("ezer-0.1.149-alpha.1-macos-aarch64"), "alpha-149").unwrap();
    std::fs::write(d.join("ezer-0.1.149-macos-aarch64"), "stable-149").unwrap();
    // Current is a stable release.
    std::fs::write(d.join("ezer-0.1.150-macos-aarch64"), "current").unwrap();

    make_all_stale(d);

    cleanup_old_downloads(d, "ezer", "0.1.150").await;

    // Current must survive.
    assert!(d.join("ezer-0.1.150-macos-aarch64").exists(), "current");
    // Newest old is 0.1.149 stable (semver: 0.1.149 > 0.1.149-alpha.1).
    assert!(
        d.join("ezer-0.1.149-macos-aarch64").exists(),
    ...
    assert!(
        !d.join("ezer-0.1.148-macos-aarch64").exists(),
...
fn test_reinstall_hint_gh_release_mentions_gh_command() {
    ...
    assert!(
        hint.contains("xai-org-shared/ezer-build"),
...
fn test_reinstall_hint_internal_alpha_sets_channel() {
    ...
    if cfg!(windows) {
        assert!(
            hint.contains("$env:EZER_CHANNEL='alpha'"),
            "alpha should set EZER_CHANNEL: {hint}"
        );
    } else {
        assert!(
            hint.contains("| EZER_CHANNEL='alpha' bash"),
            "alpha must set EZER_CHANNEL on bash (the process running \
...
fn test_reinstall_hint_enterprise_uses_enterprise_script() {
    // Enterprise ships via its own bootstrap script (channel hardcoded there), never install.sh with EZER_CHANNEL
...
async fn test_cleanup_old_downloads_invalid_current_version_is_no_op() {
    let dir = tempfile::tempdir().unwrap();
    let d = dir.path();
    std::fs::write(d.join("ezer-0.1.140-macos-aarch64"), "v140").unwrap();
    std::fs::write(d.join("ezer-0.1.141-macos-aarch64"), "v141").unwrap();

    // An invalid version string makes cleanup early-return without deleting
    make_all_stale(d);

    cleanup_old_downloads(d, "ezer", "not-a-version").await;
    assert!(d.join("ezer-0.1.140-macos-aarch64").exists());
    assert!(d.join("ezer-0.1.141-macos-aarch64").exists());
}
#[tokio::test]
async fn test_cleanup_old_downloads_files_with_non_digit_suffix_skipped() {
    let dir = tempfile::tempdir().unwrap();
    let d = dir.path();
    // Files matching prefix but with a non-digit-leading suffix must be ignored (e.g. ezer-latest, ezer-pager-* when prefix is ezer).
    std::fs::write(d.join("ezer-latest"), "alias").unwrap();
    std::fs::write(d.join("ezer-0.1.141-macos-aarch64"), "pager").unwrap();
    std::fs::write(d.join("ezer-0.1.140-macos-aarch64"), "v140").unwrap();
    std::fs::write(d.join("ezer-0.1.141-macos-aarch64"), "current").unwrap();
    ...
    // ezer-latest and ezer-pager-* must be untouched.
    assert!(d.join("ezer-latest").exists());
    assert!(d.join("ezer-0.1.141-macos-aarch64").exists());
}
...
async fn test_cleanup_old_downloads_unparseable_version_skipped() {
    ...
    std::fs::write(d.join("ezer-9garbage-macos-aarch64"), "junk").unwrap();
    std::fs::write(d.join("ezer-0.1.141-macos-aarch64"), "current").unwrap();
    ...
    assert!(
        d.join("ezer-9garbage-macos-aarch64").exists(),
...
async fn test_cleanup_old_downloads_multiplatform_in_same_dir() {
    ...
    std::fs::write(d.join("ezer-0.1.141-macos-aarch64"), "mac").unwrap();
    std::fs::write(d.join("ezer-0.1.141-linux-x86_64"), "linux").unwrap();
    std::fs::write(d.join("ezer-0.1.140-macos-aarch64"), "old-mac").unwrap();
    std::fs::write(d.join("ezer-0.1.139-macos-aarch64"), "older-mac").unwrap();
    ...
    assert!(d.join("ezer-0.1.141-macos-aarch64").exists());
    assert!(d.join("ezer-0.1.141-linux-x86_64").exists());
    // N-1 (0.1.140) kept, older deleted.
    assert!(d.join("ezer-0.1.140-macos-aarch64").exists());
    assert!(!d.join("ezer-0.1.139-macos-aarch64").exists());
}
...
async fn test_cleanup_old_downloads_tmp_files_deleted_even_when_unparseable() {
    ...
    std::fs::write(d.join("ezer-junk.tmp"), "partial").unwrap();
    make_stale(&d.join("ezer-junk.tmp"));
    std::fs::write(d.join("ezer-0.1.140-macos-aarch64.tmp"), "partial2").unwrap();
    make_stale(&d.join("ezer-0.1.140-macos-aarch64.tmp"));
    std::fs::write(d.join("ezer-0.1.141-macos-aarch64"), "current").unwrap();
    ...
    assert!(!d.join("ezer-junk.tmp").exists(), "junk tmp deleted");
    assert!(
        !d.join("ezer-0.1.140-macos-aarch64.tmp").exists(),
...
async fn test_cleanup_old_downloads_darwin_platform_recognized() {
    // The `darwin` alias for macOS is in PLATFORM_OS; versions on ezer-X.Y.Z-darwin-* layouts must split correctly
    let dir = tempfile::tempdir().unwrap();
    let d = dir.path();
    std::fs::write(d.join("ezer-0.1.140-darwin-arm64"), "v140").unwrap();
    std::fs::write(d.join("ezer-0.1.141-darwin-arm64"), "current").unwrap();

    make_all_stale(d);

    cleanup_old_downloads(d, "ezer", "0.1.141").await;

    assert!(d.join("ezer-0.1.141-darwin-arm64").exists(), "current");
    assert!(d.join("ezer-0.1.140-darwin-arm64").exists(), "N-1");
}
...
// serially. EZER_INSTALLER (npm | internal | gh-release | gh); EZER_MANAGED_BY_NPM → npm; EZER_MANAGED_BY_INTERNAL →
// internal. ──────────────────────────────────────────────────────────────────────
...
fn test_env_installer_explicit_internal() {
    let _g = InstallerEnvGuard::isolate();
    unsafe { std::env::set_var("EZER_INSTALLER", "internal") };
...
fn test_env_installer_explicit_gh_alias() {
    ...
    let _g = InstallerEnvGuard::isolate();
    unsafe { std::env::set_var("EZER_INSTALLER", "gh") };
    assert_eq!(env_installer(), Some("gh-release"));
}
...
fn test_env_installer_explicit_unknown_value_returns_none() {
    ...
    // So `EZER_INSTALLER=brew` disables the env-installer detection entirely
    let _g = InstallerEnvGuard::isolate();
    unsafe { std::env::set_var("EZER_INSTALLER", "brew") };
    // Even if MANAGED_BY_NPM is also set, the explicit var wins (and rejects).
    unsafe { std::env::set_var("EZER_MANAGED_BY_NPM", "1") };
    assert_eq!(
        env_installer(),
        None,
        "explicit EZER_INSTALLER=brew must early-return None, not fall through"
    );
}
...
fn test_env_installer_explicit_empty_returns_none() {
    let _g = InstallerEnvGuard::isolate();
    unsafe { std::env::set_var("EZER_INSTALLER", "") };
    assert_eq!(env_installer(), None);
}
...
fn test_env_installer_managed_by_npm_any_value() {
    ...
    let _g = InstallerEnvGuard::isolate();
    unsafe { std::env::set_var("EZER_MANAGED_BY_NPM", "") };
    assert_eq!(env_installer(), Some("npm"));
}
...
fn test_env_installer_managed_by_internal() {
    let _g = InstallerEnvGuard::isolate();
    unsafe { std::env::set_var("EZER_MANAGED_BY_INTERNAL", "1") };
    assert_eq!(env_installer(), Some("internal"));
}
...
fn test_env_installer_explicit_internal_wins_over_npm_managed() {
    // EZER_INSTALLER=internal must override an inherited MANAGED_BY_NPM.
    let _g = InstallerEnvGuard::isolate();
    unsafe {
        std::env::set_var("EZER_INSTALLER", "internal");
        std::env::set_var("EZER_MANAGED_BY_NPM", "1");
    }
    assert_eq!(env_installer(), Some("internal"));
}
...
async fn test_windows_replace_exe_creates_dest_when_missing() {
    ...
    let dest = dir.path().join("ezer.exe");
...
async fn test_windows_replace_exe_cleans_stale_old_backup() {
    ...
    let old = dir.path().join("ezer.exe.old");
...
async fn test_windows_replace_exe_rollback_on_copy_failure() {
    // No stale .old: the aside IS ezer.exe.old, so this pins the non-diverted rollback branch (rename .old back onto dest)
    use std::os::windows::fs::OpenOptionsExt;
    const FILE_SHARE_READ: u32 = 0x00000001;
    const FILE_SHARE_DELETE: u32 = 0x00000004;
...
async fn test_windows_replace_exe_locked_stale_old_does_not_block_update() {
    ...
    let asides: Vec<std::path::PathBuf> = std::fs::read_dir(dir.path())
        ...
        .filter(|p| {
            p.file_name()
                .and_then(|n| n.to_str())
                .is_some_and(|n| n.starts_with("ezer.exe.old.") && n.ends_with(".old"))
        })
        .collect();
    assert_eq!(
        asides.len(),
...
async fn test_windows_replace_exe_rollback_restores_from_diverted_aside() {
    ...
    let leftover_asides = std::fs::read_dir(dir.path())
        ...
        .filter(|e| {
            ...
            name.starts_with("ezer.exe.old.") && name.ends_with(".old")
        })
        .count();
...
async fn test_windows_replace_exe_sweeps_accumulated_asides() {
    ...
    let aside_a = dir.path().join("ezer.exe.old.1234-0.old");
    let aside_b = dir.path().join("ezer.exe.old.1234-1.old");
...
async fn download_and_decode_round_trips_each_codec() {
    ...
    let payload = b"\x7fELF ezer binary payload".to_vec();
    ...
    for (suffix, codec, body) in [("zst", Codec::Zstd, zst), ("gz", Codec::Gzip, gz)] {
        ...
        Mock::given(method("GET"))
            .and(path(format!("/ezer-1.2.3-linux-x86_64.{suffix}")))
            .respond_with(ResponseTemplate::new(200).set_body_bytes(body))
            .mount(&server)
            .await;

        let dir = tempfile::tempdir().unwrap();
        let dest = dir.path().join("ezer-1.2.3-linux-x86_64");
        let url = format!("{}/ezer-1.2.3-linux-x86_64.{suffix}", server.uri());
...
async fn download_and_decode_errs_on_corrupt() {
    ...
    let server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(path("/ezer-1.2.3-linux-x86_64.zst"))
    ...
    let dir = tempfile::tempdir().unwrap();
    let dest = dir.path().join("ezer-1.2.3-linux-x86_64");
    let url = format!("{}/ezer-1.2.3-linux-x86_64.zst", server.uri());
...
async fn download_cli_artifact_falls_back_to_plain() {
    ...
    Mock::given(method("GET"))
        .and(path("/ezer-1.2.3-linux-x86_64"))
    ...
    let dir = tempfile::tempdir().unwrap();
    let dest = dir.path().join("ezer-1.2.3-linux-x86_64");
    download_cli_artifact_from_gcs(&server.uri(), "ezer-1.2.3-linux-x86_64", &dest, false)
        .await
...
fn cli_object_candidates_try_windows_exe_first() {
    assert_eq!(
        cli_object_candidates("ezer-1.2.3-windows-x86_64", true),
        ["ezer-1.2.3-windows-x86_64.exe", "ezer-1.2.3-windows-x86_64"]
    );
    assert_eq!(
        cli_object_candidates("ezer-1.2.3-linux-x86_64", false),
        ["ezer-1.2.3-linux-x86_64"]
    );
}
...
fn npm_entry_is_recognized_by_the_binary_location() {
    ...
    let root = temp.path();
    let native = root.join("lib/node_modules/@ezer/ezer/bin/ezer-native");
    std::fs::create_dir_all(native.parent().unwrap()).unwrap();
    std::fs::write(&native, "bin").unwrap();
    let path_entry = root.join("prefix-bin/ezer");
    ...
    assert!(!super::is_under_node_modules(&root.join("home/bin/ezer")));
}
