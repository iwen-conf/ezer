...
fn test_tmp_download_path_is_unique_per_version_and_per_attempt() {
    ...
    let dest_181 = std::path::Path::new("/home/u/.ezer/downloads/grok-0.1.181-linux-x86_64");
    let dest_182 = std::path::Path::new("/home/u/.ezer/downloads/grok-0.1.182-linux-x86_64");
...
async fn test_reconcile_agent_repoints_diverged_agent() {
    ...
    std::os::unix::fs::symlink("../downloads/grok-0.2.101-macos-aarch64", bin.join("ezer"))
        .unwrap();
...
fn test_relative_symlink_target_sibling_dirs() {
    ...
    let target = std::path::Path::new("/home/alice/.ezer/downloads/grok-0.1.203");
...
fn test_relative_symlink_target_cross_tree_stays_absolute() {
    ...
    assert_eq!(
        result,
        std::path::PathBuf::from("/home/alice/.ezer/downloads/grok-0.1.203")
    );
}
...
async fn test_cleanup_old_downloads_does_not_touch_other_binaries() {
    ...
    std::fs::write(d.join("grok-0.1.140-macos-aarch64"), "old-ezer").unwrap();
    std::fs::write(d.join("grok-0.1.141-macos-aarch64"), "current-ezer").unwrap();
