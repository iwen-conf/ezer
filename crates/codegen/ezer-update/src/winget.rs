...
/// What `ezer update` prints for a WinGet install. `channel` is the requested or configured channel, which WinGet
...
pub(crate) fn hand_off_message(target: Target<'_>, channel: &str) -> String {
    ...
    format!(
        "ezer Build was installed with WinGet, so WinGet manages its updates.\n\
         {channel_note}\
...
/// Follows "A new version of ezer Build is available" in headless runs and `ezer update --check`.
pub(crate) fn update_available_note(target: Target<'_>) -> String {
    format!(
        "Installed with WinGet: quit ezer and run `{}` \
         (new releases can take a few days to reach WinGet).",
        target.command()
    )
}
