...
//! # Wiring
//!
//! `ezer-pager` (the lib) does **not** depend on this crate: this crate reads deeply into the pager's [`AppView`] / view model.
//! A reverse dependency would be a cargo cycle.
//! Instead the pager exposes function-pointer hooks ([`ezer_pager::minimal_hook`]).
//! The composition-root binary (`ezer-pager-bin`) calls [`install`] once at startup to register this crate's [`draw`] entry point.
...
pub fn draw(app: &mut AppView, terminal: &mut PagerTerminal) {
    let ctx = ezer_pager::terminal::terminal_context();
    if ezer_pager::terminal::should_emit_synchronized_output(ctx) {
...
/// Register the minimal-mode render hooks with `ezer-pager`. It installs the function-pointer hooks so the
/// pager's `ScreenMode::Minimal` branches dispatch into this crate.
pub fn install() {
    ezer_pager::minimal_hook::install(ezer_pager::minimal_hook::MinimalHooks { draw });
}
