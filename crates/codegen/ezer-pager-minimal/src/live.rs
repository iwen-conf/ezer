...
use ezer_pager::app::PagerTerminal;
use ezer_pager::app::app_view::{ActiveView, AppView};
use ezer_pager::minimal_api;
use ezer_pager::render::Renderable;
use ezer_pager::scrollback::state::ScrollbackState;
use ezer_pager::scrollback::wrappers::EntryRenderer;
use ezer_pager::terminal::TerminalContext;
use ezer_pager::theme::Theme;
use ezer_pager::views::prompt_widget::{PromptBg, PromptStyle};
use ezer_pager::views::turn_status;
...
/// edge.
pub(super) fn live_left_inset(_appearance: &ezer_pager::appearance::AppearanceConfig) -> u16 {
    0
}
...
fn clear_btw_geometry(agent: &mut ezer_pager::app::agent_view::AgentView) {
    agent.last_btw_selection_model =
        ezer_pager::scrollback::text_selection::ResolvedSelectionModel::default();
...
pub(super) fn prompt_style(
    appearance: &ezer_pager::appearance::AppearanceConfig,
    input_mode: ezer_pager::app::agent_view::PromptInputMode,
    theme: &Theme,
    multiline: bool,
) -> PromptStyle {
    PromptStyle {
...
pub fn draw_live(app: &mut AppView, terminal: &mut PagerTerminal, ctx: &TerminalContext) {
    ...
    ezer_pager::render::draw::draw_frame(terminal, cursor, ctx, |frame, _link_spans| {
        let area = frame.area();
        ...
        agent.active_pane = ezer_pager::app::agent_view::AgentPane::Prompt;
        ...
        if minimal_api::extensions_modal(agent).is_some() {
            ...
            if let Some(state) = minimal_api::extensions_modal_mut(agent) {
                ezer_pager::views::extensions_modal::render_extensions_modal(
                    frame.buffer_mut(),
                    area,
                    state,
                    None,
        ...
        let raw_btw = if minimal_api::minimal_btw_surface_available(agent) {
            ezer_pager::views::btw_overlay::btw_panel_height(
        ...
        if let (Some(btw), Some(btw_area)) = (agent.btw_state.as_ref(), btw_area) {
            ...
            ezer_pager::views::btw_overlay::render_btw_panel(
                frame.buffer_mut(),
                btw,
                btw_area,
                tick,
        ...
        (
            ...
            result
                .post_flush_escapes
                .map(ezer_pager::terminal::overlay::PostFlush::from),
        )
    });
}
fn live_tail_renderer<'a>(
    entry: &'a ezer_pager::scrollback::entry::ScrollbackEntry,
    theme: &'a Theme,
    appearance: &ezer_pager::appearance::AppearanceConfig,
    cwd: &'a std::path::Path,
    tick: u64,
...
fn minimal_advance_phase_timer(
    agent: &mut ezer_pager::app::agent_view::AgentView,
) -> Option<ezer_pager::acp::tracker::TurnActivity> {
...
fn render_minimal_status(
    buf: &mut Buffer,
    area: Rect,
    agent: &ezer_pager::app::agent_view::AgentView,
    activity: &Option<ezer_pager::acp::tracker::TurnActivity>,
...
fn render_config_status_line(
    ...
    area: Rect,
    agent: &mut ezer_pager::app::agent_view::AgentView,
    frame: &ezer_pager::views::status_line::StatusLineFrame,
    theme: &Theme,
) {
    if area.height == 0 || area.width == 0 {
        return;
    ...
    if let Some(width) = minimal_api::status_line_inner_width(area.width, padding) {
        agent.last_status_line_size = Some(ezer_pager::views::status_line::RowSize {
            cols: width,
            lines: area.height,
        });
    }
    if let Some(display) = frame.display() {
        let _ = ezer_pager::views::status_line::render_status_line(
...
fn render_idle_hint(buf: &mut Buffer, area: Rect, theme: &Theme) {
    ...
    buf.set_style(area, style);
    let auto = ezer_pager::app::minimal_auto_set_for_mouse_leak();
    let switch_back = ezer_pager::app::minimal_show_switch_back_to_fullscreen();
...
fn render_prompt_info(
    ...
    use ezer_pager::views::context_bar::fmt_tokens;
...
fn minimal_pending_hint(
    pending: &Option<ezer_pager::app::app_view::PendingAction>,
) -> Option<String> {
...
mod tests {
    use super::*;
    fn agent() -> ezer_pager::app::agent_view::AgentView {
    ...
    fn config_status_line_paints_and_records_the_script_size() {
        use std::sync::Arc;
        use ezer_pager::views::status_line::{
    ...
    fn tail_height_uses_owning_session_cwd_for_tool_paths() {
        use ezer_pager::app::agent::AgentState;
        use ezer_pager::scrollback::RenderBlock;
        use ezer_pager::scrollback::entry::ScrollbackEntry;
        use ezer_pager::scrollback::types::DisplayMode;
        ...
        let appearance = super::super::commit::committed_appearance(
            &ezer_pager::appearance::AppearanceConfig::default(),
        );
        let theme = Theme::current();
    ...
    fn minimal_status_shows_rich_activity_and_idle_hint() {
        use ezer_pager::acp::tracker::TurnActivity;
        use ezer_pager::app::agent::AgentState;
        let theme = Theme::current();
        ...
        ezer_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(false);
        let a = agent();
        let mut buf = Buffer::empty(area);
        ...
        ezer_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(true);
        let mut buf = Buffer::empty(area);
        render_minimal_status(&mut buf, area, &a, &None, None, &theme);
    ...
    fn minimal_status_shows_idle_watching_cue() {
        ...
        a.session.scheduled_tasks.insert(
            "loop-1".to_string(),
            ezer_pager::app::agent::ScheduledTaskInfo {
    ...
    fn prompt_style_bash_mode_shows_bang_prefix() {
        use ezer_pager::app::agent_view::PromptInputMode;
        use ezer_pager::appearance::AppearanceConfig;
    ...
    fn prompt_info_renders_model_context_and_queued() {
        let mut a = agent();
        a.context_state = Some(ezer_shell::session::ContextInfo {
            used: 276_000,
            total: 2_000_000,
            ..Default::default()
        });
    ...
    fn prompt_info_shows_session_mode_flag() {
        ...
        let render = |a: &ezer_pager::app::agent_view::AgentView| -> String {
            let mut buf = Buffer::empty(area);
    ...
    fn pending_hint_formats_press_again() {
        ...
        use ezer_pager::app::actions::Action;
        use ezer_pager::app::app_view::PendingAction;
        use ezer_pager::input::key::KeyShortcut;
