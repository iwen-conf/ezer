...
//!
//! The TUI ([`ezer_pager::prompt_images`]) and the server-side ingestion path ([`crate::session::acp_session`]) need to recover image bytes.
...
//!
//! Wire format: `[Image #<n>: <absolute_path>]`; the producer is [`ezer_pager::prompt_images::display_text`].
...
/// The server resolves `[Image #N]` tokens by this number rather than by list position (the two diverge; see `AttachedImages` in `ezer-tools`).
...
/// See `ezer_tools::types::resources::AttachedImages`.
///
...
///
/// * The producer emits exactly `": "` (colon, single space) as the separator; see [`ezer_pager::prompt_images::display_text`].
...
/// That is strictly stronger than the legacy [`ezer_pager::prompt_images::read_image_at_path`], which has no prefix allowlist at all.
...
fn decode_image_mime(data: &[u8]) -> Option<&'static str> {
    ezer_image::validate_image_bytes_with(data, false)
        .ok()
        .map(|(_, _, mime)| mime)
}
...
mod tests {
    ...
    /// The placeholder text wire format is the unencoded path produced by `ezer_pager::prompt_images::display_text`.
