//! Bounded advisory file locks for files under the ezer home. Every attempt runs behind a
...
//! - Slot storage is local by construction: `EZER_FILE_LOCK_SLOT_DIR` or
//!   `/tmp/ezer-file-lock-<euid>`, never `$HOME`, `$TMPDIR`, or `$XDG_RUNTIME_DIR`.
