...
pub const SLOT_DIR_ENV: &str = "EZER_FILE_LOCK_SLOT_DIR";
...
mod unix {
    ...
    /// Pure: an absolute `override_dir` (refused when loose), else `/tmp/ezer-file-lock-<euid>`
    ...
    pub(crate) fn resolve_slot_dir(override_dir: Option<&Path>, euid: u32) -> (PathBuf, LooseMode) {
        match override_dir {
            ...
            Some(_) | None => (
                PathBuf::from(format!("/tmp/ezer-file-lock-{euid}")),
                LooseMode::Tighten,
            ),
        }
    }
    ...
    /// Resolve, create (mode 0700), and verify the slot directory: `$EZER_FILE_LOCK_SLOT_DIR` when
    /// set and absolute, else `/tmp/ezer-file-lock-<euid>`. `None` means no usable directory; the
