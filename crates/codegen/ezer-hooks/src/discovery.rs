...
/// `ezer_workspace::folder_trust::repo_configs_present`, which probes the same paths.
pub fn discover_hook_source_paths(
    git_root: Option<&Path>,
    compat: &ezer_tools::types::compat::CompatConfig,
    claude_import_marked: bool,
) -> HookSourcePaths {
    let grok = ezer_config::user_ezer_home();
...
fn load_hooks_from_directory(dir: &Path) -> (Vec<HookSpec>, Vec<HookError>) {
    ...
    // (Sandbox fail-closed listing lives in ezer_config.)
    ...
    for entry in entries {
        ...
        if !ezer_config::is_direct_hook_json_name(name) || !path.is_file() {
            continue;
        }
...
fn is_valid_hook_file(path: &Path) -> bool {
    ...
    ezer_config::is_direct_hook_json_name(name) && path.is_file()
}
...
mod tests {
    ...
    fn dedup_keeps_the_managed_policy_copy_regardless_of_order() {
        ...
        // The signed cloud cache outranks the user-writable `$EZER_HOME` tiers it shares a directory with, and yields to root-owned policy
        let registry = registry_from_specs_deduped(vec![
