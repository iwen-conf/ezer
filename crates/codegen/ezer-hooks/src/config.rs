...
pub use ezer_config::HookProvenance;
...
mod tests {
    ...
    fn hook_display_name_tracks_stamped_names() {
        ...
        for (source_name, provenance, expected) in tiers {
            let layer = ezer_config::HookConfigLayer::new(
                provenance,
                source_name,
    ...
    fn config_layer(source_name: &str, toml_src: &str) -> ezer_config::HookConfigLayer {
        let value: toml::Value = toml::from_str(toml_src).unwrap();
        let hooks = value.get("hooks").cloned().unwrap();
        ezer_config::HookConfigLayer::new(
            ezer_config::HookProvenance::Managed,
            source_name,
            hooks,
        )
    }
    ...
    fn config_layers_additive_and_dedup_keeps_higher_authority() {
        ...
        use ezer_config::HookProvenance::{Managed, User};
