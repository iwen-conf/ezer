...
use ezer_tools::util::ProcessGroup;
...
pub async fn run_command_hook(
    ...
    let mut cmd = if is_shell_command {
        ...
        #[cfg(not(unix))]
        {
            ...
            let inv = ezer_config::shell::shell_command_argv(command_str.as_ref());
    ...
    #[cfg(not(unix))]
    let env_root = {
        use ezer_config::shell::{WindowsShell, detect_windows_shell};
...
mod tests {
    ...
    fn make_scoped_ctx(scope: ezer_tools::util::ProcessScope) -> RunContext<'static> {
        RunContext {
    ...
    async fn command_hook_session_close_reaps_whole_group() {
        ...
        let envelope = make_envelope();
        let scope = ezer_tools::util::ProcessScope::new();
