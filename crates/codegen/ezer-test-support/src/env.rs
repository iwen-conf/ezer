...
static EZER_BINARY_OVERRIDE: OnceLock<PathBuf> = OnceLock::new();

pub fn set_ezer_binary_override(path: PathBuf) {
    let _ = EZER_BINARY_OVERRIDE.set(path);
}
...
pub fn ezer_binary() -> PathBuf {
    if let Some(path) = EZER_BINARY_OVERRIDE.get() {
        return path.clone();
    }
    ...
    ensure_cargo_bin("ezer-pager-bin", "ezer-pager")
}
