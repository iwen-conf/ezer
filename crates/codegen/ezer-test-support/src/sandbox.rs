...
fn baseline_env_from_parent(
    ...
    // Leader-lock acquire slots stay inside the sandbox instead of the developer's `/tmp/ezer-file-lock-<uid>`.
    // Literal on purpose: `ezer_file_lock::SLOT_DIR_ENV` lives in a crate this one does not depend on.
    env.insert(
        "EZER_FILE_LOCK_SLOT_DIR".into(),
...
mod tests {
    ...
    fn baseline_is_hermetic_and_network_quiet() {
        ...
        assert_eq!(
            env_value(&sandbox, "EZER_FILE_LOCK_SLOT_DIR"),
