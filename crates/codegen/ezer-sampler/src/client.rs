...
pub use ezer_sampling_types::ApiBackend;
...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = ezer_sampling_types::reported_cost_ticks(
        value
...
struct ClientDefaults {
    ...
    reasoning_summary: Option<ezer_sampling_types::ReasoningSummary>,
    extra_response_includes: Vec<String>,
    doom_loop_recovery: Option<ezer_sampling_types::DoomLoopRecoveryPolicy>,
}
...
impl SamplingClient {
    ...
    pub async fn create_response(
        ...
        ezer_sampling_types::patch_reasoning_text_types(&mut request_body);
        self.prepare_bearer().await;
        let SentRequest {
            builder,
            sent_bearer,
    ...
    /// Honors the request's [`LengthPolicy`](ezer_sampling_types::LengthPolicy) like the actor path.
...
/// Applies the request's [`ezer_sampling_types::LengthPolicy`] to a collected response.
...
pub(crate) fn apply_length_policy(
    policy: ezer_sampling_types::LengthPolicy,
    response: ezer_sampling_types::ConversationResponse,
) -> Result<ezer_sampling_types::ConversationResponse> {
    use ezer_sampling_types::LengthVerdict;
...
mod tests {
    ...
    use ezer_sampling_types::ApiErrorCode;
    use ezer_sampling_types::types::ChatRequestMessage;
    ...
    async fn capture_request(
        ...
        use ezer_sampling_types::{ContentPart, ConversationItem, UserItem};
    ...
    async fn stream_span_adopts_request_traceparent_on_every_backend() {
        ...
        let request = || ConversationRequest {
            items: vec![ezer_sampling_types::ConversationItem::user("hi")],
    ...
    fn user_agent_collapses_when_origin_matches_agent() {
        let agent_version = ezer_version::VERSION.to_string();
        let origin = OriginClientInfo {
            product: AGENT_PRODUCT.to_string(),
    ...
    fn client_with_summary(
        summary: Option<ezer_sampling_types::ReasoningSummary>,
    ) -> SamplingClient {
    ...
    fn reasoning_summary_none_omits_the_field_but_keeps_effort() {
        let client = client_with_summary(Some(ezer_sampling_types::ReasoningSummary::None));
        let mut request = built_response_request();
        client.apply_response_defaults(&mut request).unwrap();
    ...
    fn reasoning_summary_override_replaces_the_built_value() {
        let client = client_with_summary(Some(ezer_sampling_types::ReasoningSummary::Detailed));
        let mut request = built_response_request();
        client.apply_response_defaults(&mut request).unwrap();
        assert_eq!(
    ...
    fn reasoning_summary_adds_a_reasoning_block_only_when_there_is_something_to_send() {
        let with_summary =
            client_with_summary(Some(ezer_sampling_types::ReasoningSummary::Auto));
        ...
        let without = client_with_summary(Some(ezer_sampling_types::ReasoningSummary::None));
        let mut request = CreateResponseWrapper::new(rs::CreateResponse::default());
