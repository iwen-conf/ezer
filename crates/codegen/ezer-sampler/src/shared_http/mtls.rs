...
pub(crate) fn client(
    cert_dir: &Path,
    force_http1: bool,
) -> ezer_sampling_types::Result<reqwest::Client> {
...
fn identity_paths(cert_dir: &Path) -> ezer_sampling_types::Result<(PathBuf, PathBuf)> {
...
fn try_path_exists(path: &Path) -> ezer_sampling_types::Result<bool> {
...
fn read_identity_file(path: &Path) -> ezer_sampling_types::Result<Vec<u8>> {
