    ...
    assert_eq!(
        ...
        [
            (
                ...
                "ezer doctor fix terminal.tmux-clipboard",
            ),
            (
                crate::diagnostics::DCS_PASSTHROUGH_ID,
                "ezer doctor fix terminal.dcs-passthrough",
            ),
            (
                crate::diagnostics::TMUX_EXTENDED_KEYS_ID,
                "ezer doctor fix terminal.tmux-extended-keys",
            ),
        ]
    );
...
fn clipboard_delivery_findings_own_remediation_while_fix_fact_stays_compatible() {
    let cases = [
        (
            ...
            "ezer wrap <ssh command> or /minimal",
        ),
        (
            TerminalContext {
