...
fn auth_required_message(interactive: bool) -> String {
    if interactive {
        "Not signed in. Set EZER_API_KEY / OPENAI_API_KEY or add api_key \
         to ~/.ezer/config.toml for BYOK (Responses-first)."
            .to_string()
    } else {
        "Not signed in. Set EZER_API_KEY / OPENAI_API_KEY / XAI_API_KEY, \
         or add api_key under [model.<id>] in ~/.ezer/config.toml."
            .to_string()
    }
}
