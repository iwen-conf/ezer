...
impl TitleManager {
    ...
    pub fn reset(&mut self) -> String {
        let esc = build_title_escape("ezer");
        self.last_title.clear();
        self.last_title.push_str("ezer");
...
mod tests {
    ...
    fn session_name_and_grok_joined_with_separator() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "my project - ezer");
    }
    ...
    fn spinner_only_shown_when_active() {
        ...
        mgr.update(&state);
        assert!(mgr.last_title.contains(" - ezer"));
    ...
    fn activity_shows_waiting_when_busy_without_activity() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "Waiting - ezer");
    }
    ...
    fn activity_prefers_specific_activity_over_waiting() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "Thinking - ezer");
    }
    ...
    fn turn_timer_shown_when_above_one_second() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "42s - ezer");
    }
    ...
    fn default_config_active_turn_with_permissions() {
        ...
        for t in [&t1, &t2] {
            assert!(t.contains("ezer"), "title missing 'ezer': {t}");
