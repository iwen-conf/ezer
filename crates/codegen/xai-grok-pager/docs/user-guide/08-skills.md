...
|----------|-------|----------|-------|
| `./.ezer/skills/`, `./.grok/commands/` | Local (CWD) | Highest | Current directory skills / legacy command markdown |
...
Grok deduplicates skills by name -- a higher-priority location overrides a lower one. Grok also scans `.agents/skills/` (and `commands/`) at each tier (alongside `.ezer/`) and walks every directory between your working directory and the repo root.
...
3. **Creates the skill directory.** Grok creates the `<scope>/.ezer/skills/<name>/` directory, plus `scripts/` or `references/` subdirectories when the skill needs them.
...
- **Project** (`<repo_root>/.ezer/skills/<name>/`) -- available only in this repository and shareable with teammates through version control. Grok recommends this scope inside a git repository.
...
To browse your skills, type `/` to open the slash-command menu. Grok lists every built-in command and skill and filters them as you type. To list skills from the command line instead, run `ezer inspect` (see [Viewing Skill Details](#viewing-skill-details)).
...
Run `ezer inspect` to see every skill Grok discovers, along with the rest of your configuration:
...
Grok distributes platform skills separately from your personal skills. Bundled skills are cached under `~/.ezer/bundled/skills/`; Grok never writes them into `~/.ezer/skills/`. A same-named local, repo, or user skill overrides the bundled copy. `ezer inspect` labels each definition by its actual source. (A plugin skill of the same name does not override a native skill; it stays available under its qualified `plugin:name` form.)
