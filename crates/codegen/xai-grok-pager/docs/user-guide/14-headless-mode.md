...
ezer stores data in `~/.ezer` (override with `GROK_HOME`; see [Environment Variables for Headless](#environment-variables-for-headless)):
...
unless it runs from the managed install (`$GROK_HOME/bin/ezer`).
