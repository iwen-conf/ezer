...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.ezer/agents/` or `~/.ezer/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.grok/personas/`; applied during subagent resolution |
