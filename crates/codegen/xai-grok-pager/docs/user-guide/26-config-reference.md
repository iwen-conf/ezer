...
1. Compiled defaults.
2. `/etc/grok/managed_config.toml`, then `$EZER_HOME/managed_config.toml` (fleet defaults; console-synced).
...
6. `$EZER_HOME/requirements.toml`, then `/etc/grok/requirements.toml`, then macOS MDM `ai.x.grok`. Admin layer. Keys marked `pin` in the table cannot be overridden; keys marked `yes` are also valid in this file.
...
| `cli.grove` | `boolean` or `grove` / `grove-fuse` / `grove-nfs` / `nfs` / `all` / `copy` / `true` / `false` / `1` / `0` / `on` / `off` | `yes` | `user` | Convenience that turns **both** `ezer clone` and session / `-w` Grove on when the specific knobs are unset. Also `GROK_GROVE`. `false` / `copy` / `off` means enable-all is off (fall through); it does not force both surfaces off. `[cli] grove_worktree` and `GROK_WORKTREE_TYPE` still win for worktrees; `GROK_CLONE` still wins for clone. Remote `grove_worktree = false` still kills worktrees only. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `diagnostics.crash_handler` | `boolean` | `yes` | `user` | Write a panic report under `$EZER_HOME/crash/`. Also GROK_CRASH_HANDLER. |
...
Grok Build reads `/etc/grok/managed_config.toml` first, then `$EZER_HOME/managed_config.toml`, which the console keeps in sync. Values in the second replace values in the first.
...
`requirements.toml` is an admin-enforced file. Locations: `$EZER_HOME/requirements.toml` (signed cache) then `/etc/grok/requirements.toml`, then macOS MDM `ai.x.grok`. The **Requirements** column on the `config.toml` tables lists every `config.toml` key this file accepts (`pin` or `yes`). Omitted keys stay unconstrained.
