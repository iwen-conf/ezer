...
ezer supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.ezer/hooks/`
- **Project hooks** — from `.ezer/hooks/` in your repo
...
Plugin hooks automatically receive `EZER_PLUGIN_ROOT` and `EZER_PLUGIN_DATA` environment variables (see the [Plugins guide](../user-guide/09-plugins.md#environment-variables-in-plugin-hooks)).
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.ezer/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `ezer plugin marketplace add <source>`)
...
Sources land in `~/.ezer/config.toml`:
...
Or in `~/.ezer/settings.json` / `~/.claude/settings.json`:
