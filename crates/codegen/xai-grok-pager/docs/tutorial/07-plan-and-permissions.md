...
ezer asks before doing anything risky — and can plan before it codes.

## Permissions

When ezer wants to run a risky command or edit a file, it pauses and asks:
...
For bigger or more ambiguous tasks, use **plan mode**: ezer explores the
...
`s` requests changes — ezer iterates until you're happy, then implements.
...
background — ezer keeps working and you're notified when it finishes
