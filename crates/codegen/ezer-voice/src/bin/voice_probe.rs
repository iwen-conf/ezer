...
//! ```bash
//! export XAI_API_KEY=...
//! cargo run -p ezer-voice --bin voice-probe -- --seconds 5
//! ```
...
fn main() -> anyhow::Result<()> {
    ...
    if let Some(code) = ezer_voice::maybe_run_capture_subprocess() {
...
async fn run() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,ezer_voice=debug".into()),
        )
        .init();
    ...
    if args.mic_only {
        ...
        {
            let (bytes, chunks) =
                ezer_voice::run_mic_only_probe(config.sample_rate, args.seconds)?;
...
Environment:
  ...
  RUST_LOG        optional (default info,ezer_voice=debug)
