...
fn body_text_is_every_kept_body_in_order() {
    ...
    let first = logs_body("ezer.session_start");
    let second = logs_body("ezer.user_prompt");
...
fn any_fault_fails_body_text_even_with_every_body_kept() {
    ...
    recorder.record_protobuf(OtelSignal::Logs, &logs_body("ezer.session_start"));
...
async fn wait_for_signals_needs_every_signal_and_resolves_with_every_event() {
    ...
    let (events, ()) = tokio::join!(
        ...
        async {
            ...
            recorder.record_protobuf(OtelSignal::Metrics, &metrics_body("ezer.token.usage"));
        },
    );

    assert_eq!(
        Ok(vec![
            logs_event("ezer.session_start"),
            metrics_event("ezer.token.usage"),
        ]),
        events
    );
}
...
async fn wait_for_events_times_out_reporting_the_distinct_event_names_recorded() {
    ...
    assert_eq!(
        Err(OtelRecorderError::Timeout {
            ...
            events_recorded: 3,
            event_names: vec![
                "ezer.session_start".to_owned(),
                "ezer.user_prompt".to_owned()
            ],
        }),
        outcome
    );
...
async fn wait_for_silence_sees_only_events_recorded_after_the_call() {
    ...
    let (broken, ()) = tokio::join!(
        ...
        async { recorder.record_protobuf(OtelSignal::Logs, &logs_body("ezer.user_prompt")) },
    );
    ...
    assert_eq!(
        (
            ...
            Err(OtelRecorderError::SilenceBroken {
                window,
                events: vec![logs_event("ezer.user_prompt")],
            })
        ),
        (held, broken)
    );
