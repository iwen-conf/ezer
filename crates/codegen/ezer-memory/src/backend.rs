...
use ezer_tools::types::memory_backend::{MemoryBackend, MemorySearchResult};
...
pub struct EndpointScopedCredentials {
    ...
    auth_credentials: Option<Arc<dyn ezer_auth::AuthCredentialProvider>>,
...
impl EndpointScopedCredentials {
    ...
    fn auth_credentials(&self) -> Option<&Arc<dyn ezer_auth::AuthCredentialProvider>> {
        self.auth_credentials.as_ref()
    }

    fn api_key_provider(&self) -> Option<&ezer_tools::types::SharedApiKeyProvider> {
...
pub struct MemoryBackendParams {
    ...
    pub embed_config: Option<ezer_config_types::MemoryEmbeddingConfig>,
    ...
    pub search_config: ezer_config_types::MemorySearchConfig,
...
async fn build_embedding_provider(
    config: Option<&ezer_config_types::MemoryEmbeddingConfig>,
...
pub struct MemoryBackendImpl {
    ...
    embed_config: Option<ezer_config_types::MemoryEmbeddingConfig>,
    ...
    search_config: ezer_config_types::MemorySearchConfig,
...
impl MemoryBackendImpl {
    ...
    pub fn new(db_path: PathBuf, storage: MemoryStorage) -> Self {
        Self {
            ...
            embed_api_key: None,
            search_config: ezer_config_types::MemorySearchConfig::default(),
            watcher: None,
            stale_claim_secs: 60,
    ...
    pub fn with_embedding(
        mut self,
        config: ezer_config_types::MemoryEmbeddingConfig,
        base_url: String,
    ...
    pub fn with_search_config(mut self, config: ezer_config_types::MemorySearchConfig) -> Self {
...
#[cfg(test)]
impl MemoryBackendImpl {
    pub fn search_config_for_test(&self) -> &ezer_config_types::MemorySearchConfig {
        &self.search_config
    }
}
...
impl MemoryBackend for MemoryBackendImpl {
    ...
    async fn search(
        ...
        let mut index = match super::index::MemoryIndex::open_or_create(
            &self.db_path,
            self.storage.clone(),
            ezer_config_types::MemoryIndexConfig::default(),
            embed_dims,
        ) {
            Ok(index) => index,
            Err(error) => {
...
mod factory_tests {
    ...
    use ezer_config_types::{MemoryEmbeddingConfig, MemorySearchConfig};
    ...
    fn test_factory_wires_non_overridable_search_config_fields() {
        ...
        let custom_search = MemorySearchConfig {
            max_results: 7,
            mmr: ezer_config_types::MmrConfig {
                enabled: true,
                lambda: 0.42,
            },
            temporal_decay: ezer_config_types::TemporalDecayConfig {
                enabled: true,
                half_life_days: 14.0,
            },
            ..Default::default()
    ...
    async fn make_embedding_provider_uses_async_api_key_resolution() {
        ...
        let probe: ezer_tools::types::SharedApiKeyProvider = Arc::new(AsyncProbe {
...
mod tests {
    ...
    async fn test_build_drops_credentials_when_request_url_differs() {
        let session: ezer_tools::types::SharedApiKeyProvider = Arc::new(PanicKey);
        ...
        let config = ezer_config_types::MemoryEmbeddingConfig {
    ...
    async fn test_trusted_endpoint_prefers_session_credential() {
        struct StubAuth;
        impl ezer_auth::HttpAuth for StubAuth {
            fn apply(
                &self,
        ...
        #[async_trait::async_trait]
        impl ezer_auth::AuthCredentialProvider for StubAuth {
            fn snapshot(&self) -> ezer_auth::CredentialSnapshot {
                ezer_auth::CredentialSnapshot::default()
            }
        ...
        let auth: Arc<dyn ezer_auth::AuthCredentialProvider> = Arc::new(StubAuth);
        let api_key: ezer_tools::types::SharedApiKeyProvider = Arc::new(PanicKey);
        let scoped = EndpointScopedCredentials::for_endpoint(
            "https://api.x.ai/v1",
            |_| true,
            Some(auth),
    ...
    fn endpoint_scoped_credentials_trust_gate_and_url_match() {
        struct AnyKey;
        impl ezer_tools::types::ApiKeyProvider for AnyKey {
            fn current_api_key(&self) -> Option<String> {
                None
            }
        }
        let key = || Arc::new(AnyKey) as ezer_tools::types::SharedApiKeyProvider;
        ...
        assert!(!scoped.is_empty(), "trusted endpoint keeps the credential");
        assert!(
            scoped.approved_for("https://API.ezer/v1"),
