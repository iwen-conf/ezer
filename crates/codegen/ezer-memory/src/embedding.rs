...
pub(super) fn build_middleware_client(
    ...
    ezer_http::with_auth_retry(ezer_http::shared_client(), credentials)
}
...
fn build_static_middleware_client(
    ...
    let provider: std::sync::Arc<dyn ezer_auth::AuthCredentialProvider> = std::sync::Arc::new(
        ezer_auth::StaticAuthCredentialProvider::new(Box::new(NoopHttpAuth), api_key),
    );
...
impl ezer_auth::HttpAuth for NoopHttpAuth {
    ...
    async fn embed_batch(
        ...
        for batch in texts.chunks(self.max_batch_size) {
            ...
            for attempt in 0..MAX_RETRIES {
                ...
                let request = ezer_http::shared_client()
