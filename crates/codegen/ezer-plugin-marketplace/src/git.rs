...
//! Cache root: `~/.ezer/marketplace-cache/<url-hash>/`
...
pub fn sync_source_cache_with_mode(
    ...
    let url = ezer_agent::plugins::git_install::validate_git_url(url)?;
    let branch = branch
        .map(ezer_agent::plugins::git_install::validate_git_ref)
        .transpose()?;
...
pub use ezer_agent::plugins::install_registry::acquire_file_lock;
