...
//!
//! ```text
//! cargo test -p ezer-sandbox --test read_write_trailing_glob_e2e -- --nocapture
//! ```
...
fn subprocess_entry() {
    ...
    let profile: ezer_sandbox::ProfileName = "cargo".parse().expect("profile name");
    let mut mgr = ezer_sandbox::SandboxManager::new(profile, &workspace);
