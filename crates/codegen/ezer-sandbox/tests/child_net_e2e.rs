...
fn spawn_probe(socket: &Path, filtered: bool) -> std::process::Output {
    ...
    if filtered {
        ...
        let filter = ezer_sandbox::child_net::prebuilt_child_network_filter();
        // SAFETY: the closure only runs prctl against the parent-built program.
        unsafe {
            cmd.pre_exec(move || ezer_sandbox::child_net::install_child_network_filter(filter));
        }
    }
...
fn restrict_child_network_std_arms_from_config_without_apply() {
    let manager = ezer_sandbox::SandboxManager::new(
        ezer_sandbox::ProfileName::ReadOnly,
        Path::new("/tmp"),
    );
    assert!(!manager.is_applied(), "no kernel apply in this test");
    manager.install();
    assert!(
        ezer_sandbox::should_restrict_child_network(),
    ...
    cmd.env(SOCKET_ENV, &sock)
        .args(["--ignored", "--exact", "--nocapture", "subprocess_entry"]);
    ezer_sandbox::child_net::restrict_child_network_std(&mut cmd);
