...
fn skip_if_enforcement_unavailable() -> bool {
    ...
    let support = ezer_sandbox::SandboxManager::support_info();
...
fn profile_from_env() -> ezer_sandbox::ProfileName {
    ezer_sandbox::ProfileName::Custom(std::env::var(PROFILE_ENV).expect(PROFILE_ENV))
}
...
fn subprocess_profile_and_bwrap_reexec(profile: &ezer_sandbox::ProfileName, workspace: &Path) {
    #[cfg(target_os = "linux")]
    {
        if !ezer_sandbox::is_inside_bwrap() {
            match ezer_sandbox::bwrap_reexec_for_profile(profile, workspace) {
                Some(mut cmd) => {
...
fn subprocess_block_deny(workspace: &Path) {
    ...
    let mut sandbox = ezer_sandbox::SandboxManager::new(profile.clone(), workspace);
...
fn subprocess_devbox_marker_spoof(workspace: &Path) {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let verified = ezer_sandbox::verify_data_write_deny_enforced(
            &ezer_sandbox::ProfileName::Devbox,
            workspace,
        );
        ...
        let mut sandbox =
            ezer_sandbox::SandboxManager::new(ezer_sandbox::ProfileName::Devbox, workspace);
        if let Err(e) = sandbox.apply(workspace) {
            eprintln!("sandbox apply failed: {e}");
            std::process::exit(3);
        }
...
fn subprocess_devbox_genuine(workspace: &Path) {
    let profile = ezer_sandbox::ProfileName::Devbox;
    subprocess_profile_and_bwrap_reexec(&profile, workspace);
    ...
    #[cfg(target_os = "linux")]
    if let Err(e) = ezer_sandbox::verify_data_write_deny_enforced(
        &ezer_sandbox::ProfileName::Devbox,
        workspace,
    ) {
...
fn subprocess_hook_write_deny(workspace: &Path, first_run: bool) {
    ...
    let trust_boundary_files: Vec<(&str, PathBuf)> = ezer_config::TRUST_BOUNDARY_FILENAMES
        .iter()
        .copied()
...
fn workspace_protects_direct_hook_sources() {
    ...
    for name in ezer_config::TRUST_BOUNDARY_FILENAMES {
