...
impl From<ezer_config::GlobalHookSourceError> for HookWriteDenyError {
    fn from(e: ezer_config::GlobalHookSourceError) -> Self {
...
fn reject_hardlinked_files(sources: &[GlobalHookSource]) -> Result<(), HookWriteDenyError> {
    use std::os::unix::fs::MetadataExt;
    use ezer_config::GlobalHookSourceKind;
    for s in sources {
...
fn capture_dir_json_snapshot(dir: &Path) -> Result<DirJsonSnapshot, HookWriteDenyError> {
    use ezer_config::{list_direct_hook_json_files, validate_direct_hook_json_file};
