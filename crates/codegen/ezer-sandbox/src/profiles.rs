...
use ezer_config::{GlobalHookSource, SANDBOX_CONFIG_FILENAME};
...
impl ProfileName {
        ...
        match std::fs::symlink_metadata(path) {
            Ok(meta) if meta.file_type().is_symlink() => {
                ...
                let default_sessions =
                    xai_dirs::home_dir().map(|user_home| user_home.join(".ezer").join("sessions"));
    ...
    pub(crate) fn capability_set_from_profile(
        ...
        if !profile.write_deny.is_empty() {
            ...
            {
                let files =
                    ezer_config::validated_hook_json_files_for_sources(&profile.write_deny)
