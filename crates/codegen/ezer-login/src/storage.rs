...
pub fn read_auth_json(auth_file: &Path) -> std::io::Result<AuthStore> {
    ...
    if let Err(e) = ezer_shell_base::util::secure_file::ensure_owner_only_permissions(auth_file)
    {
        tracing::warn!(
...
pub fn backup_corrupt_auth_file(path: &Path) -> Option<PathBuf> {
    ...
    match std::fs::rename(path, &backup) {
        Ok(()) => {
            ...
            let _ = ezer_shell_base::util::secure_file::ensure_owner_only_permissions(&backup);
            tracing::warn!(
...
fn write_store_to(path: &Path, auth_store: &AuthStore) -> std::io::Result<()> {
    use ezer_shell_base::util::secure_file::open_secure_file;
    ...
    if let Err(e) = ezer_shell_base::util::secure_file::ensure_owner_only_permissions(path) {
        tracing::warn!(
            error = %e,
            path = %path.display(),
...
fn restore_prior_bytes(auth_file: &Path, bytes: &[u8]) -> std::io::Result<()> {
    ...
    file.sync_all()?;
    ezer_shell_base::util::secure_file::ensure_owner_only_permissions(auth_file)?;
    Ok(())
}
...
mod write_fallback_tests {
    ...
    fn fake_truncate_then_fail(path: &Path, _: &AuthStore) -> std::io::Result<()> {
        ezer_shell_base::util::secure_file::open_secure_file(path)?; // truncates to 0 bytes
        Err(std::io::Error::from(std::io::ErrorKind::StorageFull))
    }
