        ...
        if !crate::is_expired(&disk_now) && disk_now.key != tried.key {
            ezer_telemetry::unified_log::info(
                ...
                Some(serde_json::json!({
                    "disk_key_prefix": ezer_auth::bearer_suffix(&disk_now.key),
                    "tried_key_prefix": ezer_auth::bearer_suffix(&tried.key),
                })),
            );
    ...
    async fn refresh(&self, reason: RefreshReason) -> RefreshOutcome {
            ...
            && self.auth.current().map(|a| a.key).as_deref() != Some(&d.key)
        {
            ezer_telemetry::unified_log::info(
                ...
                None,
                Some(serde_json::json!({
                    "disk_key_prefix": ezer_auth::bearer_suffix(&d.key),
                })),
            );
            self.note_refresh_progress();
        ...
        let pre_token = ezer_auth::bearer_suffix(&auth.key).to_owned();
...
fn spawn_diagnostic_upload(
    ...
    tokio::spawn(async move {
        ...
        let log_bytes = match tokio::task::spawn_blocking(
            ezer_telemetry::unified_log::snapshot_log,
        )
        .await
        {
            Ok(Some(bytes)) => bytes,
            Ok(None) => {
                ezer_telemetry::unified_log::debug("diagnostic snapshot empty", None, None);
