...
use ezer_test_support::{MockCanAdministerTeam, MockInferenceServer};

const USER_A: &str = "a@acme.test";

fn team_auth() -> EzerAuth {
    EzerAuth {
...
async fn denying_server() -> (MockInferenceServer, AuthManager, tempfile::TempDir) {
    ...
    let manager =
        AuthManager::new(dir.path(), EzerComConfig::default()).with_proxy_base_url(&server.url());
