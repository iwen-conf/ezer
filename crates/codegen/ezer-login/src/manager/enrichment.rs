...
async fn log_offloaded(lvl: LogLevel, msg: String, ctx: serde_json::Value) {
    ...
    let write = tokio::task::spawn_blocking(move || {
        ...
        ezer_telemetry::unified_log::emit(lvl, &msg, None, Some(ctx));
    });
    ...
    if disk.key != auth.key || disk.refresh_token != auth.refresh_token {
        ezer_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                ...
                "written_key_prefix": ezer_auth::bearer_suffix(&auth.key),
                "disk_key_prefix": ezer_auth::bearer_suffix(&disk.key),
            })),
        );
        return;
    }
...
pub(super) async fn hydrate_can_administer_team(
    ...
    let same_account = |live: &EzerAuth| {
        auth.email.is_some()
