...
use ezer_tools::implementations::ezer_build::image_gen::{ImageGenClient, ImageGenConfig};
use ezer_tools::implementations::ezer_build::media_bearer::SIDE_CALL_BEARER_ERROR_CODE;
use ezer_tools::types::api_key_provider::SideCallBearerError;

use super::{SharedAuthKeyProvider, is_xai_side_call_principal};
use crate::config::{EzerComConfig, XAI_OAUTH2_ISSUER};
use crate::{AuthManager, AuthMode, EzerAuth};
...
fn session(key: &str, mode: AuthMode, issuer: Option<&str>) -> EzerAuth {
    EzerAuth {
        key: key.to_owned(),
        auth_mode: mode,
...
fn static_key_guards() -> [EnvGuard; 3] {
    [
        ...
        EnvGuard::unset("EZER_CODE_XAI_API_KEY"),
        EnvGuard::unset("EZER_AUTH_PATH"),
    ]
}

fn manager(dir: &tempfile::TempDir, auth: Option<EzerAuth>) -> Arc<AuthManager> {
