...
fn resolve_device_flow(
    ...
    config: Option<bool>,
    remote: Option<bool>,
) -> ezer_config_types::Resolved<bool> {
    ezer_config_types::BoolFlag::env("EZER_LOGIN_DEVICE_FLOW")
...
async fn should_use_device_flow(
    ...
    let resolved = if login_override.as_cli_bool().is_some() {
        ...
        let env = ezer_config::env_bool("EZER_LOGIN_DEVICE_FLOW");
...
pub async fn run_external_auth_provider(
    ...
    let mut cmd = ezer_shell_base::util::subprocess::shell_c(command);
...
fn login_failure_event(err: &anyhow::Error) -> Option<LoginFailed> {
    ...
    Some(LoginFailed {
        error_kind: failure_kind(
            ezer_http::TransportFailure::classify(source).kind,
            source.is_decode(),
        ),
        os_error: ezer_http::find_os_error_code(source),
    })
}
...
async fn try_noninteractive_auth_no_mint_with(auth_manager: &Arc<AuthManager>) -> Option<EzerAuth> {
    ...
    .await
    {
        ...
        Err(_elapsed) => {
            tracing::warn!(
                timeout_secs = ezer_http::STARTUP_AUTH_REFRESH_TIMEOUT.as_secs(),
...
pub fn perform_logout(
    ...
    if was_logged_in {
        ezer_telemetry::external::set_identity(
            ezer_telemetry::external::IdentityAttrs::default(),
        );
...
async fn fetch_login_device_flow(cli_chat_proxy_base_url: &str) -> Option<bool> {
    let agent_id = tokio::task::spawn_blocking(ezer_telemetry::id::agent_id)
        .await
        .ok()?;
    ...
    let response = client
        ...
        .header(
            ...
            ezer_http::process_client_identifier(),
        )
        .header(
            ezer_http::CLIENT_MODE_HEADER,
            ezer_http::process_client_mode(),
        )
        .send()
        .await;
...
mod tests {
    ...
    use chrono::Utc;
    use std::path::Path;
    use ezer_shell_base::env::EnvVarGuard;
    /// `os_error` and the reqwest classification are covered in `ezer-http`.
    ...
    /// That breaks unrelated auth tests, so classification is covered in `ezer-http`.
    #[test]
        ...
        with_device_flow_env(None, || {
            assert_eq!(
                resolve_device_flow(LoginTransportOverride::Preresolved(true), None, Some(true))
                    .source,
                ezer_config_types::ConfigSource::Remote,
    ...
    fn device_flow_records_deciding_tier() {
        use ezer_config_types::ConfigSource;
        with_device_flow_env(Some(false), || {
            assert_eq!(
    ...
    async fn no_mint_readiness_auth_is_bounded() {
        ...
        assert!(
            elapsed >= ezer_http::STARTUP_AUTH_REFRESH_TIMEOUT,
            "expected a bounded refresh attempt (elapsed {elapsed:?})"
        );
        assert!(
            elapsed < ezer_http::STARTUP_AUTH_TIMEOUT,
    ...
    const _: () = assert!(
        ezer_http::STARTUP_AUTH_REFRESH_TIMEOUT.as_millis()
