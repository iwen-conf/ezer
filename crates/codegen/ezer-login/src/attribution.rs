...
//!
//! 1. [`ezer_telemetry::unified_log::warn`] for the local
...
//! # Cross-crate wiring
//!
//! [`ezer_sampler`] is intentionally decoupled from this crate.
//! It invokes the trait [`ezer_sampler::Auth401AttributionCallback`] at its six 401 arms.
//! This module provides [`ShellAttribution`], the concrete impl wired into [`ezer_sampler::SamplerConfig::attribution_callback`].
...
use ezer_sampler::{Auth401AttributionCallback, SamplingConsumer};
use ezer_tools::{Auth401AttributionCallback as ToolAuth401AttributionCallback, ToolConsumer};

use crate::{AuthManager, TOKEN_TTL};
use ezer_auth::bearer_suffix;
...
impl ShellAttribution {
    ...
    /// Returns `Arc<dyn Trait>` so callers can drop the value directly into [`ezer_sampler::SamplerConfig::attribution_callback`].
    ...
    /// Tool-side counterpart of [`Self::new`]: returns `Arc<dyn ezer_tools::Auth401AttributionCallback>` for the `with_attribution_callback(...)` builder on each tool HTTP client (`ImageGenClient`, `VideoGenClient`, `WebSearchClient`).
...
/// Tool-side hook: each tool client (image_gen, video_gen, web_search) in `ezer-tools` emits a 401 attribution event through this trait when its HTTP request returns UNAUTHORIZED.
...
pub enum ConsumerKind {
    ...
    IdleResumeModelRefresh,
    /// `ezer_tools::ToolConsumer::ImageGen`, the Imagine API (`POST /images/generations`).
    /// No per-op discriminator; consumer string is just `"ImageGen"`.
    ImageGen,
    /// `ezer_tools::ToolConsumer::VideoGenStart` and `VideoGenPoll`, the Video Generation API.
    /// The op string is `"start"` (`POST /videos/generations`) or `"poll"` (`GET /videos/{request_id}`).
    VideoGen,
    /// `ezer_tools::ToolConsumer::WebSearch`, web search via `POST /responses` with a `WebSearch` tool.
...
/// All 401 emit sites in `ezer-shell` go through this helper.
...
mod tests {
    ...
    /// `ShellAttribution` implements `ezer_tools::Auth401AttributionCallback` by routing each `ToolConsumer` variant to the right
