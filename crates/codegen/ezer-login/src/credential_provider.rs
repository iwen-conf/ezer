...
use ezer_auth::{AuthCredentialProvider, CredentialSnapshot, HttpAuth};
...
/// Sampler [`BearerResolver`](ezer_sampler::BearerResolver) over a live [`AuthManager`].
...
impl WireValidBearerResolver {
    ...
    pub fn shared(auth_manager: Arc<AuthManager>) -> ezer_sampler::SharedBearerResolver {
...
impl ezer_sampler::BearerResolver for WireValidBearerResolver {
...
pub fn embedding_session_credentials(
...
) -> ezer_memory::EndpointScopedCredentials {
    ...
    ezer_memory::EndpointScopedCredentials::for_endpoint(
        embed_base_url,
        ezer_shell_base::util::is_xai_api_bearer_url,
        auth_credentials,
        api_key_provider,
    )
}
...
pub fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        let mut attrs = ezer_telemetry::external::IdentityAttrs::from_snapshot(&snapshot);
        attrs.email = provider.oauth_gateway_email();
        ezer_telemetry::external::set_identity(attrs);
    }
}
...
mod tests {
    ...
    /// The shell half of the subagent-401 contract; the sampler half is pinned in ezer-sampler's resolver tests. Over a real `AuthManager` the resolver returns `None` when hard-expired (fail-closed).
    ...
    fn wire_valid_resolver_tracks_manager_across_expiry_and_refresh() {
        use ezer_sampler::BearerResolver;
    ...
    fn embedding_session_credentials_scopes_to_first_party() {
        ...
        let api_key_provider: ezer_tools::types::SharedApiKeyProvider =
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use ezer_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
