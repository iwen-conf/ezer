...
//!   Adding it here would force every consumer of `ezer-workspace-types` (including the eventual WASM browser SDK) to pull in tokio.
...
//! The runtime crate (`ezer-workspace`) wraps this in its own `Request<T>` that adds the cancellation token and extensions map.
//!
//! Splitting the envelope this way keeps `ezer-workspace-types` tokio-free.
