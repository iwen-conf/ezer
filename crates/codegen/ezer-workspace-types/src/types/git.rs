...
//! `ezer_shell::session::git` and `ezer_shell::extensions::git`
//! when the VCS subsystem moves into the workspace crate.

use serde::{Deserialize, Serialize};

/// TODO: align with `ezer_shell::session::git::VcsKind`.
...
/// TODO: align with `GitStatusData` in `ezer_shell::session::git`.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
...
/// TODO: align with `GitDiffsData` in `ezer_shell::session::git`.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
pub struct GitDiff {
    /// Unified-diff text.
    #[serde(default)]
