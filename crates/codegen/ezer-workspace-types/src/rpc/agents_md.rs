...
/// Claude.md / `.ezer/rules/*.md`) discovered from the workspace root up to
/// the git root, plus `~/.ezer` and compat dirs.
...
/// Mirrors the serde shape of `ezer-agent`'s `AgentConfigFile`.
...
/// Mirrors `ezer-agent`'s `InstructionSource`.
