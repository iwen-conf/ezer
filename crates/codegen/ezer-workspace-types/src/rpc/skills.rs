...
//!
//! SYNC: [`SkillInfo`] / [`SkillScope`] mirror the serde shape the server serializes in `ezer-tools/src/implementations/skills/types.rs`.
...
pub enum SkillScope {
    /// cwd/.ezer/skills
    Local,
    /// repo_root/.ezer/skills
    Repo,
    /// ~/.ezer/skills
    User,
    /// ~/.ezer/server-skills (synced from the skill store)
    Server,
...
mod tests {
    ...
    fn skill_info_deserializes_minimal_payload() {
        let raw = serde_json::json!({
            ...
            "path": "/workspace/.ezer/skills/my-skill/SKILL.md",
            "scope": "local",
        });
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.ezer/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.ezer/skills"},
