...
//!
//! Run: `cargo bench -p ezer-shell --bench session_list`
...
use ezer_shell::session::persistence::Summary;
use ezer_shell::session::storage::{JsonlStorageAdapter, StorageAdapter};
use ezer_shell::session::unified_list::{ListReq, UnifiedListResult, build_unified_list};
...
impl Fixture {
    ...
    fn assert_correct(&self, runtime: &tokio::runtime::Runtime) {
        let discovered = ezer_shell::session::worktree::candidate_worktree_cwds_for_same_repo(
...
fn create_same_repo_cwds(home: &Path) -> SameRepoTopology {
    ...
    let worktree_base = ezer_shell::session::worktree::worktree_base_dir(&main);
    ...
    for (index, (path, label)) in linked.iter().enumerate().take(DB_OVERLAP_WORKTREE_COUNT) {
        db.register(&WorktreeRecord {
            ...
            metadata: Some(ezer_shell::session::worktree::build_label_metadata(
                label, false,
            )),
        })
