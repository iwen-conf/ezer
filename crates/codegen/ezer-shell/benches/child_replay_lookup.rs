...
//! order as a fat local `~/.ezer/sessions`). Override with env:
//!
//! ```text
//! cargo bench -p ezer-shell --bench child_replay_lookup
//! CHILD_REPLAY_LOOKUP_CWDS=3000 CHILD_REPLAY_LOOKUP_PER_CWD=3 cargo bench ...
//! # read-only against a real store (no writes):
//! CHILD_REPLAY_LOOKUP_HOME=$HOME/.ezer cargo bench -p ezer-shell --bench child_replay_lookup
//! ```
...
use ezer_config::encode_cwd_dirname;
use ezer_shell::session::storage::{
