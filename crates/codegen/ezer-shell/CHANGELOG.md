...
- **`ezer clone` on Windows** no longer fails when the repository's git config contains backslashes or quotes.
...
- **Auto-update** no longer removes a ezer binary that is still in use by another process.
...
- **Fixed ezer -c attaching the wrong empty session** after starting without --continue then resuming.
...
- **New `ezer-workspaced daemon`** mode lets a long-lived process expose folders to the Computer Hub (separate from the Desktop-supervised sidecar).
...
- **Headless sessions** (`ezer -p`) now support the `--worktree` flag to run in a separate git worktree.
...
- **Fixed crashes** that occurred when the terminal pane was closed while ezer was exiting.
...
- **Enterprise policy files** are no longer deleted on startup when your team login is stored at a custom `EZER_AUTH_PATH`.
...
- **ezer inspect** and **ezer mcp doctor** now show managed policy details.
...
- **ezer usage <session-id>** now shows persisted per-turn token and cost data.
...
- **ezer inspect** now correctly shows Claude bypass locks as advisory rather than enforced.
...
- **--sandbox strict** now restricts writes to ~/.ezer/sessions only.
...
- **Windows users** can now correctly open ~/.ezer and worktree sessions.
...
- **MCP servers** now receive a ezer-cli User-Agent header so providers can attribute traffic.
...
- **ezer clone** now fetches only the selected branch tip by default, with --full-history for legacy full clones.
- **ezer clone** can now reuse a matching local checkout as a linked worktree instead of always fetching from the network.
...
- **ezer mcp add** now correctly treats bare http(s):// URLs as HTTP servers instead of stdio commands.
...
- **Interactive ezer sessions** now start in ask mode by default again instead of auto.
...
- Users hitting startup timeouts can now raise the connect budget with the `EZER_CONNECT_UI_TIMEOUT_SECS` environment variable.
...
- **ezer clone** can now fetch a repo into a content store and mount a projected working tree.
...
- **EZER_CONFIG** and **EZER_CONFIG_PATH** environment variables now let launchers override selected config settings without editing config.toml.
- **Worktrees** under ~/.ezer/worktrees are now automatically reclaimed when safe, with strong safeguards that never delete a user's last copy.
...
- **EZER_FORCE_LOGIN_TEAM_ID** environment variable now lets launchers restrict interactive login to one or more teams.
...
- **ezer inspect** no longer crashes when its output is piped into a command that closes the pipe early.
...
- Tool commands and MCP servers now receive a EZER_SESSION_ID environment variable matching the current session.
...
- **Keystrokes typed while ezer is starting** are now preserved in the composer instead of being lost.
...
- **`ezer du`** and worktree commands now work on Windows when only USERPROFILE is set.
...
- **Subagent spawning** is dramatically faster when you have many sessions in ~/.ezer.
...
- New `ezer du` command shows disk usage of ~/.ezer including worktrees and sessions.
...
- **ezer trace** exports now bundle memory trace files for easier debugging.
...
- **Failed alpha/enterprise updates** now suggest the matching EZER_CHANNEL reinstall command.
- **On Apple Silicon**, ezer now installs the native arm64 build even from a Rosetta shell or x86_64 updater.
...
- **Non-interactive sessions** (`ezer -p`) no longer fail when the agent asks for user input or plan approval.
...
- ezer skips the project-directory prompt when launched from home or other non-project directories
...
- Sandboxed ezer starts on large directories with many deny-glob matches
...
- ezer retries more server errors during outages
...
- **ezer doctor** now warns when tmux is reducing colors and can fix the config.
...
- **EZER_EXTRA_CA_BUNDLE** env var allows adding custom TLS root certificates.
...
- **ezer** no longer crashes on startup when the host machine has no free threads.
...
- **Invalid MCP server entries** in config.toml no longer prevent ezer from starting; problems are shown in `ezer inspect`.
...
- **Fixed credential loss** across multiple ezer processes sharing the same auth file.
...
- **New /tutorial slash command** opens an opt-in nine-topic onboarding tour of ezer.
...
- **`ezer doctor fix`** can now repair common tmux clipboard and passthrough problems.
- **Per-provider auth helpers** now work on Windows and can run from a configurable working directory.
- **/resume** now shows only native ezer sessions by default and shows a hint when external sessions are hidden.
- **`ezer --resume`** can now resume a session by its title as well as by ID.
...
- You can now run `ezer doctor fix` commands directly from inside the TUI instead of only from the CLI.
...
- ezer now automatically stops a turn that keeps repeating the exact same tool call many times in a row.
...
- When the auto-permission classifier times out or fails, ezer now shows a normal permission prompt instead of silently denying.
...
- **ezer doctor fix ssh-wrap** can set up `ezer wrap ssh` automatically for Bash, zsh, and fish.
...
- **ezer doctor** checks terminal, tmux, clipboard, and keyboard setup without opening the TUI.

## Bug Fixes

- **Image paste** over ezer wrap now works on headless remotes.
...
- **Added EZER_CLIPBOARD_NO_OSC52** env var to stop clipboard sequences from appearing as garbage in unsupported terminals.
...
- **/btw** now works inside `ezer --minimal`, showing answers in the live area and committing them to scrollback on Esc.
...
- **Default model** is now ezer 4.5 with high/medium/low reasoning effort and improved compaction settings.
...
- **Global rules** from ~/.ezer/rules and compatible vendor homes are now discovered correctly.
...
- **SSH sessions** now show a one-time tip recommending `ezer wrap ssh <host>` for clipboard and terminal restore.
...
- **ezer** and the agent binary now stay in sync even when no update is installed.
- **Copying** a multiline queued prompt now copies the complete text instead of a collapsed summary.
- **ezer wrap** now restores the terminal after SSH disconnects or other abrupt child exits.
- **Voice speech-to-text** now works with per-model API keys in config.toml without requiring `ezer login`.
...
- **ezer login** now requests ezer Projects scopes so workspace listing works after consent.
...
- **`ezer plugin install`** now accepts a marketplace's registered name as a qualifier.
...
- **Fixed background task tracking** when using ezer -p --no-wait-for-background so tasks are properly reaped on exit.
...
- **ezer inspect** now shows effective compatibility settings for Cursor, Claude, and Codex sessions.
...
- **Resume hint** after quitting minimal mode now prints the correct ezer --minimal --resume command.
...
- **ezer no longer crashes** when printing resume hints after the terminal pane has closed.
...
- **ezer setup --json** prints your team's managed configuration without installing it.
...
- **How-to Guides** modal now shows a tip linking to Ask ezer above the footer shortcuts.
...
- **`ezer worktree ls`** now works as a short alias for `ezer worktree list`.
- **MCP tool output truncation** can now be set per-repo in `.ezer/config.toml`.
...
- **File links and media** now work for worktree sessions under ~/.ezer/worktrees/.
...
- **ezer inspect** now lists skills from [skills].paths and correctly labels bundled vs user skills.
- **--minimal** and **--fullscreen** choices are now remembered for future plain ezer launches.

## Bug Fixes

- **Queued bash commands** promoted at turn end now render their output instead of disappearing.
- **Xcode / Foundation ACP clients** can now drive ezer agent stdio without silent parse drops on session/* calls.
...
- **ezer --minimal** now supports native drag-select on classic Windows conhost terminals.
...
- **ezer --minimal** now aligns the prompt, status bar, and messages flush-left with the welcome card.
...
- **ezer update** on Windows no longer fails when a previous .old executable is still running.
...
- **ezer models** banner now correctly reports per-model API keys and deployment keys.
...
- You can now add a local directory as a plugin marketplace source with `ezer plugin marketplace add`.
...
- **Plan files** now default to .ezer/plan.md to match ezer conventions.
...
- **--minimal** flag now shows in `ezer --help`.
...
- **Tool result previews** no longer paint opaque panels in `ezer --minimal`.
- **ezer wrap** now correctly handles quoted strings and shell aliases.
...
- **`ezer update`** downloads have a longer timeout.
...
- **Pasting images** from the local clipboard now works when running commands through `ezer wrap`.
...
- **ezer agent stdio** now checks for updates in the background like other modes.
...
- **ezer wrap** now shows copy success over SSH and suggests native drag-select when paste fails.
...
- **Added `ezer wrap`** to run any command with local clipboard support.
...
- **Fixed `ezer agent stdio` startup hangs** on Windows when used with persistent clients such as VS Code or ezer-desktop.
...
- You can now pass --json-schema to ezer -p and receive a validated JSON object instead of free text.
...
- **Agent-run terminal commands** now set `EZER_AGENT=1` so host tools can tell them apart from interactive shells.
...
- **ezer -w --ref <branch>** now creates worktrees based on the specified ref instead of HEAD.
...
- **--disable-web-search** now honored in ezer -p and ezer agent; auxiliary model routing respects catalog overrides.
...
- The legacy `agent` command is now kept in sync with `ezer` after running `ezer update`.
- Headless (`ezer -p`) runs now wait for background tasks and subagents to finish before exiting.
...
- **`ezer plugin install <name>`** now resolves plugins from registered marketplaces instead of only local paths.
...
- **ezer sessions delete <id>** command now lets you permanently remove a session from the CLI.
...
- **ezer quit** no longer hangs when background git or network tasks are slow.
...
- Session lists (welcome screen, /resume, ezer sessions list) are now sorted by the same activity time shown in the UI.
...
- **Added option** to fully disable the hunk tracker via --hunk-tracker-mode, EZER_HUNK_TRACKER, or config.
...
- **ezer** can now export usage metrics and events to your own OpenTelemetry collector when enabled.
...
- **`ezer dashboard`** now reliably opens the dashboard instead of silently falling through to a normal session.
...
- **`ezer mcp add`** now accepts positional arguments (e.g. `ezer mcp add filesystem -- npx ...`), supports --scope project, and adds -e/-H flags for env/headers.
...
- **`ezer update`** no longer downloads the same binary twice when multiple updaters or leader checks run concurrently.
...
- **Fixed git clone failures** on Windows when the CLI tries to clone marketplace plugins into ~/.ezer.
...
- **MCP servers menu** and slash commands now work when starting ezer outside a project directory.
...
- **ezer --resume** now correctly finds the real session instead of failing on empty image-only folders.
...
- **Fixed `ezer --resume`** failing on empty image-only session folders left by cross-directory pastes.
...
- **Ctrl+C** now cancels a blocking `ezer update` cleanly instead of leaving an orphaned download repainting the terminal.
...
- **`ezer --debug`** now produces per-session log files under ~/.ezer/debug/ even with a leader process.
...
- **`ezer update`** now restarts any older running leader so all clients get the new binary.
...
- **`ezer login`** now defaults to device code flow, which works reliably in SSH, WSL, VPN, and browser-restricted environments.
...
- **API-key users** can now run `ezer agent --leader` without forced interactive login or timeouts.
...
- **image_gen** can now be enabled via the harness model using [features] in config.toml or the EZER_IMAGE_GEN_HARNESS env var.
...
- **ezer inspect** now shows the resolved on/off state and source for every Claude/Cursor compatibility toggle.
- **Cursor MCP servers and hooks** are now discovered and can be disabled independently via EZER_CURSOR_MCPS_ENABLED / EZER_CURSOR_HOOKS_ENABLED.
...
- **Cursor skills and rules** are now discovered alongside ezer and Claude directories.
...
- **Background tasks** started inside subagents now continue running after the subagent session ends.
- **Image description** now reliably uses the ezer-build model instead of falling back to the active session model.
...
- **image_gen** now uses the higher-quality ezer-imagine-image-quality model.
