...
- The **root-owned** system layers (`/etc/ezer/requirements.toml`,
  `/etc/ezer/managed_config.toml`). Enforcement relies on OS file ownership, so
  deploy these files root-owned (or via MDM).
- The **signed** `$EZER_HOME/requirements.toml` the deployment sync writes.
...
Hooks in the other `$EZER_HOME` layers (`managed_config.toml`, `config.toml`)
