...
//! Those files include `.mcp.json`, project `.ezer/config.toml` (`[permission]` / `[mcp_servers]` / `[plugins].paths`), `.ezer/lsp.json`, `AGENTS.md` / `CLAUDE.md`, and `.ezer/skills`.
...
//!
//! The DECISION side lives in `ezer-workspace` (client-side).
//! It holds the workspace scan, the pure [`decide`] precedence, and the interactive prompt.
//! It also owns the durable [`ezer_workspace::trust::TrustStore`] reads/writes.
//! This module keeps the CONSUME/gating side: the `DECISIONS` cache, [`resolve_and_record`], and the loader filters.
//! The ordered trust precedence is documented canonically on [`ezer_workspace::folder_trust::decide`].
...
use ezer_workspace::trust::{is_unsafe_trust_root, workspace_key};

// Decision-side (scan/decide/prompt/store) lives in `ezer-workspace` (client crate)
...
use ezer_workspace::folder_trust::{
...
// NOTE: this folder-trust store (`~/.ezer/trusted_folders.toml`) is SEPARATE from the pre-existing per-plugin trust store (`ezer_agent::plugins::TrustStore` at `~/.ezer/trusted-plugins`, plus the hooks' own project-trust gating)
...
pub use ezer_workspace::folder_trust::grant_folder_trust;
...
pub(crate) fn revoke_folder_trust(cwd: &Path) -> bool {
    ...
    let was_trusted = ezer_workspace::folder_trust::revoke_folder_trust_store(cwd);
...
pub fn project_scope_allowed(cwd: &Path) -> bool {
    ...
    match cached {
        ...
        Some(false) => {
            if ezer_workspace::folder_trust::is_trusted_this_process(&key) {
                record(&key, true);
                true
            } else {
                false
...
/// Interactivity is forced `true` because the caller already confirmed the client can prompt (it advertised `ezer/folderTrust.interactive`). The TTY-based [`decide_inputs`] default is false under the ACP stdio transport.
...
/// Single-sourced from the SAME scan as the canonical gate ([`ezer_workspace::folder_trust::repo_config_kinds`] / [`repo_configs_present`]).
/// The prompt's reason list therefore cannot drift from what actually gated the folder (same markers, same cwd-to-git-root walk). So an `.ezer/lsp.json`-only repo still has a non-empty reason list. Only the post-grant *hot-reload* skips LSP; project LSP applies on the next session open (the backend is spawn-baked into the tool bridge).
pub(crate) fn detected_config_kinds(cwd: &Path) -> Vec<String> {
    ezer_workspace::folder_trust::repo_config_kinds(cwd)
        .into_iter()
        .map(str::to_string)
        .collect()
}
...
pub(crate) fn agent_inline_hooks_allowed(
    scope: ezer_agent::config::AgentScope,
    trusted: impl FnOnce() -> bool,
) -> bool {
    scope != ezer_agent::config::AgentScope::Project || trusted()
}
...
pub(crate) fn resolve_and_record(
    ...
    resolve_and_record_inner(
        &key,
        || ezer_workspace::folder_trust::is_trusted_this_process(&key),
...
pub(crate) fn resolve_and_record_from_scan(
    ...
    inputs.store_trusted = ezer_workspace::folder_trust::is_trusted_this_process(&key);
    resolve_and_record_inner(
        &key,
        || ezer_workspace::folder_trust::is_trusted_this_process(&key),
        || compute_from_inputs(&inputs, feature_enabled(remote), &key, allow_prompt),
...
/// It MUST enumerate every project MCP source the loaders read. Name-based (not `ConfigSource`-based) ON PURPOSE. Sources: project `.ezer/config.toml [mcp_servers]` (NOT the user-tier global config).
/// Also project `.mcp.json` (`cwd` up to the repo root, never `$HOME`), project `.cursor/mcp.json`, and `~/.claude.json projects.<cwd>.mcpServers`.
/// Edge case: a name declared in BOTH a project config and the global `~/.ezer/config.toml` is dropped when untrusted. This is intended — untrusted project content must not influence the command spawned for a shared name.
pub(crate) fn project_scoped_mcp_names(cwd: &Path) -> HashSet<String> {
    let mut names = HashSet::new();

    // `.ezer/config.toml [mcp_servers]` entries tagged project (the loader's key is the display name, matching `mcp_server_name` of the merged server)
...
/// Thin cwd-to-verdict wrapper over the shared [`ezer_tools::implementations::lsp::config::filter_project_lsp_when_untrusted`] predicate. Site B and the workspace build path therefore share one gate.
pub(crate) fn filter_untrusted_project_lsp(
    cwd: &Path,
    sourced: std::collections::BTreeMap<
        String,
        (
            ezer_tools::implementations::lsp::config::LspServerConfig,
            ezer_tools::types::config_source::ConfigSource,
        ),
    >,
) -> std::collections::BTreeMap<String, ezer_tools::implementations::lsp::config::LspServerConfig>
{
    ezer_tools::implementations::lsp::config::filter_project_lsp_when_untrusted(
        sourced,
...
mod tests {
    ...
    use ezer_workspace::folder_trust::repo_configs_present;
    use ezer_workspace::trust::TrustStore;
    ...
    fn revoke_folder_trust_downgrades_cache() {
        ...
        // EZER_HOME-isolated so the seed can't touch the real user file, and `#[serial]` because EZER_HOME is process-global
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("EZER_HOME", home.path());
        let tmp = repo_tmp();
    ...
    fn revoke_on_unrecordable_home_root_records_no_deny() {
        ...
        // HOME overridden so workspace_key sees the tempdir as home; EZER_HOME-isolated store EZER_FOLDER_TRUST unset so the default-on flag applies.
        let home = tempfile::tempdir().unwrap();
        ...
        std::fs::create_dir_all(home.path().join(".ezer").join("hooks")).unwrap();
    ...
    fn envrc_gate_drops_untrusted_then_loads_when_store_trusted() {
        ...
        // EZER_HOME-isolated so the trust store is empty; EZER_FOLDER_TRUST unset so the default-on feature flag applies
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("EZER_HOME", home.path());
        let _flag = EnvGuard::unset("EZER_FOLDER_TRUST");
        let tmp = repo_tmp();
        ...
        let env = ezer_workspace::envrc::load_envrc_or_empty(tmp.path());
        assert_eq!(
            env.get("GATED_ENVRC"),
            Some(&"1".to_string()),
    ...
    fn claude_env_gate_drops_project_env_when_untrusted() {
        ...
        // EZER_HOME-isolated so the trust store is empty; EZER_FOLDER_TRUST unset so the default-on feature flag applies
        use ezer_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("EZER_HOME", home.path());
        let _flag = EnvGuard::unset("EZER_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn claude_env_gate_drops_subdir_project_env_when_untrusted() {
        ...
        // The env loader walks cwd to repo-root, so detection MUST walk too (a git-root-only probe missed this) EZER_HOME-isolated so the trust store is empty
        use ezer_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("EZER_HOME", home.path());
        let _flag = EnvGuard::unset("EZER_FOLDER_TRUST");
    ...
    fn project_agent_inline_hooks_gated_when_untrusted_but_user_kept() {
        ...
        // Exercises real discovery and the exact call-site predicate used at mvp_agent/subagent EZER_HOME-isolated (empty store)
        use ezer_agent::config::AgentScope;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("EZER_HOME", home.path());
        let _flag = EnvGuard::unset("EZER_FOLDER_TRUST");
        let tmp = repo_tmp();
        let agents = tmp.path().join(".ezer").join("agents");
        ...
        .unwrap();
        let def = ezer_agent::discovery::by_name_in_cwd("explore", tmp.path())
    ...
    fn project_scope_allowed_denies_untrusted_repo_with_configs() {
        // Fail-closed (the dangerous case) The setup: a release-stamped build, the feature on by default, an untrusted folder shipping code-exec config (here `.ezer/hooks`) With no store grant that must be DENIED
        // That holds even though no verdict was recorded first (the gate re-resolves fail-closed rather than defaulting open) EZER_HOME-isolated (empty store); EZER_FOLDER_TRUST unset so the default-on flag applies
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("EZER_HOME", home.path());
        let _flag = EnvGuard::unset("EZER_FOLDER_TRUST");
    ...
    fn project_scope_allowed_allows_repo_without_configs() {
        ...
        // A naive `.unwrap_or(false)` cache peek would wrongly deny it Release-stamped and EZER_HOME-isolated so the verdict comes from `decide` rule 4 (no repo configs), not the inert short-circuit
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("EZER_HOME", home.path());
        let _flag = EnvGuard::unset("EZER_FOLDER_TRUST");
    ...
    fn project_scope_allowed_allows_store_trusted_repo() {
        ...
        // EZER_HOME-isolated so the seeded store is the temp one; EZER_FOLDER_TRUST unset so the default-on flag applies
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("EZER_HOME", home.path());
        let _flag = EnvGuard::unset("EZER_FOLDER_TRUST");
    ...
    fn project_scope_allowed_allows_inert_local_build() {
        ...
        // EZER_TEST_VERSION is unset so `is_local_build()` is genuinely true
        let _unset_ver = EnvGuard::unset(ezer_version::TEST_VERSION_ENV);
    ...
    fn project_scope_allowed_denies_untrusted_plugin_only_repo() {
        // A plugin-only untrusted repo (just `.ezer/plugins/<x>/`, no hooks/MCP/LSP, no store grant) is repo-controlled code-exec It must be DENIED
        // That is the verdict the shell plugin call sites feed into discover_plugins/build_for_cwd/reload EZER_HOME-isolated (empty store); EZER_FOLDER_TRUST unset so the default-on flag applies
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("EZER_HOME", home.path());
        let _flag = EnvGuard::unset("EZER_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".ezer").join("plugins").join("evil")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn project_scope_allowed_denies_untrusted_permission_only_repo() {
        // Bridge: a clone whose ONLY repo-local config is `.ezer/config.toml` `[permission]` (no MCP/hooks/plugins) must still gate
    ...
    async fn project_scope_allowed_denies_workspace_user_only_instructions() {
        ...
        .unwrap();
        let skill_dir = workspace_user.join(".ezer/skills/workspace-user-only-skill");
        ...
        let instructions = ezer_agent::prompt::agents_md::read_agents_config_with_paths(
            &cwd,
            ezer_agent::prompt::skills::CompatConfig::default(),
            &ezer_agent::prompt::paths::PathsConfig::default(),
            verdict,
        )
        .await;
        let skills = ezer_agent::prompt::skills::list_skills(
            Some(&cwd),
            &ezer_agent::prompt::skills::SkillsConfig::default(),
            ezer_agent::prompt::skills::CompatConfig::default(),
            verdict,
        )
        .await;
    ...
    fn kill_switch_allows_untrusted_repo_after_authoritative_resolve() {
        ...
        // The session spawn path resolves once with the real RemoteSettings before any gate read, so the gate cache-hits that verdict EZER_HOME-isolated (empty store); EZER_FOLDER_TRUST unset so the kill-switch is the only signal
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("EZER_HOME", home.path());
        let _flag = EnvGuard::unset("EZER_FOLDER_TRUST");
        ...
        let cold = repo_tmp();
        std::fs::create_dir_all(cold.path().join(".ezer").join("hooks")).unwrap();
        assert!(
    ...
    fn build_for_cwd_with_trust_verdict_gates_active_project_plugin() {
        ...
        // EZER_HOME-isolated so both the folder-trust store and the plugin trust store start empty (deterministic untrusted) EZER_FOLDER_TRUST unset so the default-on flag applies; `#[serial]` because both are process-global
        use ezer_agent::plugins::discovery::DiscoveryConfig;
        use ezer_agent::plugins::{PluginRegistry, SharedPluginRegistryHandle};
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("EZER_HOME", home.path());
        let _flag = EnvGuard::unset("EZER_FOLDER_TRUST");
        let tmp = repo_tmp();
        ...
        let plugin = tmp.path().join(".ezer").join("plugins").join("trustgate");
    ...
    fn discover_hooks_gates_then_loads_project_hook_via_trust_verdict() {
        ...
        // EZER_HOME-isolated so the grant writes to a temp store; EZER_FOLDER_TRUST unset so the default-on flag applies
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("EZER_HOME", home.path());
        let _flag = EnvGuard::unset("EZER_FOLDER_TRUST");
        let tmp = repo_tmp();
        let hooks_dir = tmp.path().join(".ezer").join("hooks");
        ...
        let has_project_hook = |reg: &ezer_hooks::discovery::HookRegistry| {
            reg.all_hooks()
                .iter()
        ...
        let git_root = ezer_workspace::session::git::find_git_root_from_path(tmp.path()).ok();
    ...
    fn filter_untrusted_project_lsp_drops_only_project() {
        ...
        use ezer_tools::implementations::lsp::config::LspServerConfig;
        use ezer_tools::types::config_source::ConfigSource;
    ...
    fn load_servers_sourced_tags_project_lsp_json() {
        use ezer_tools::implementations::lsp::config::load_servers_with_plugins_sourced;
        use ezer_tools::types::config_source::ConfigSource;

        // A `<cwd>/.ezer/lsp.json` server must be tagged `Project` so the gate can distinguish it from user/plugin servers
        // Asserts on the specific
        // key, so any real `~/.ezer/lsp.json` on the test host is irrelevant.
        let tmp = repo_tmp();
        let ezer = tmp.path().join(".ezer");
        std::fs::create_dir_all(&ezer).unwrap();
        std::fs::write(ezer.join("lsp.json"), r#"{"projlsp": {"command": "true"}}"#).unwrap();
    ...
    fn untrusted_workspace_drops_loaded_project_lsp() {
        ...
        // A project server loaded from `<cwd>/.ezer/lsp.json` is dropped once the workspace is untrusted
        let tmp = repo_tmp();
        let ezer = tmp.path().join(".ezer");
        std::fs::create_dir_all(&ezer).unwrap();
        std::fs::write(ezer.join("lsp.json"), r#"{"projlsp": {"command": "true"}}"#).unwrap();
    ...
    /// A git-init'd repo declaring two project-scoped MCP servers: `projjson` (`.mcp.json`) and `projtoml` (`.ezer/config.toml [mcp_servers]`).
    ...
    /// Pins the three known repo-local FILE sources of [`project_scoped_mcp_names`]. A project server declared in each of `.ezer/config.toml`, `.mcp.json`, and `.cursor/mcp.json` must appear in the returned set.
    ...
    fn cached_untrusted_is_upgraded_by_a_later_grant() {
        ...
        record(&key, false);
        assert!(!project_scope_allowed(tmp.path()));
        // Simulate a `ezer --trust` grant landing in the store after the untrusted verdict was cached
    ...
    fn explicit_child_untrust_survives_reload_despite_ancestor_trust() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let store_path = tmp.path().join(ezer_workspace::trust::TRUST_FILE_NAME);
    ...
    fn no_configs_trust_is_provisional_and_regated_when_configs_appear() {
        ...
        unsafe { std::env::set_var("EZER_FOLDER_TRUST", "1") };
        // Simulate a release-stamped build
        // An unstamped local/dev build (as in CI, no EZER_VERSION) auto-trusts, so the gate would never engage without this
        unsafe { std::env::set_var(ezer_version::TEST_VERSION_ENV, "0.0.0-sim") };
        let tmp = repo_tmp();
        ...
        unsafe { std::env::remove_var(ezer_version::TEST_VERSION_ENV) };
        unsafe { std::env::remove_var("EZER_FOLDER_TRUST") };
    }
    ...
    fn resolve_launch_dir_trust_matches_resolve_and_record() {
        ...
        // Isolate EZER_HOME so the store is empty/seeded in temp; `#[serial]` because both vars are process-global
        let _feature = EnvGuard::set("EZER_FOLDER_TRUST", "1");
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("EZER_HOME", home.path());
        ...
        let untrusted = repo_tmp();
        std::fs::create_dir_all(untrusted.path().join(".ezer").join("hooks")).unwrap();
        ...
        let trusted = repo_tmp();
        std::fs::create_dir_all(trusted.path().join(".ezer").join("hooks")).unwrap();
        let mut store = TrustStore::load();
    ...
    fn local_build_is_inert_launch_trust_auto_trusts() {
        ...
        // Assert the local branch ONLY when compiled unstamped (mirrors the workspace `is_local_build_honors_test_version_override`) EZER_TEST_VERSION is unset so `is_local_build()` is genuinely true; EZER_HOME-isolated so the real store is never touched
        let _sim = EnvGuard::unset(ezer_version::TEST_VERSION_ENV);
        if option_env!("EZER_VERSION").is_some() {
            return; // a release-stamped test binary is not a local build
        }
        let home = tempfile::tempdir().unwrap();
    ...
    fn prompt_warranted_true_for_untrusted_repo_with_configs() {
        ...
        // EZER_HOME-isolated so the store starts empty; `#[serial]` because EZER_HOME is process-global
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("EZER_HOME", home.path());
        let tmp = repo_tmp();
        ...
        // EZER_FOLDER_TRUST unset: env outranks the remote flag, so an ambient opt-out would otherwise false-fail the Prompt assertion
        let _sim = EnvGuard::set(ezer_version::TEST_VERSION_ENV, "0.0.0-sim");
        let _flag = EnvGuard::unset("EZER_FOLDER_TRUST");
        let remote = RemoteSettings {
    ...
    fn prompt_warranted_false_when_feature_disabled() {
        ...
        // Simulate a release build so the inert local-build path is not what's under test EZER_HOME-isolated and EZER_FOLDER_TRUST unset so the kill-switch is the only signal
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("EZER_HOME", home.path());
        let _flag = EnvGuard::unset("EZER_FOLDER_TRUST");
        let _sim = simulate_release_build();
    ...
    fn detected_config_kinds_summarizes_present_markers() {
        ...
        std::fs::write(tmp.path().join(".mcp.json"), "{}").unwrap();
        std::fs::create_dir_all(tmp.path().join(".ezer").join("hooks")).unwrap();
        std::fs::write(tmp.path().join(".ezer").join("lsp.json"), "{}").unwrap();
    ...
    fn detected_config_kinds_reports_lsp_only_repo() {
        // Regression for the "empty configKinds" bug: a repo gated SOLELY by `.ezer/lsp.json` must still produce a non-empty reason list
        let tmp = repo_tmp();
