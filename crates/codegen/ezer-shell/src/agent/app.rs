    ...
    *keepalive.borrow_mut() = Some(Rc::clone(&agent));
    ...
    tokio::task::spawn_local(
        GatewayReceiver::new(gw_rx, conn)
            .with_on_meta(ezer_otel::span_from_meta_traceparent)
            .run(),
    );
    handle_io
}
    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let workspace_user_dir = ezer_agent::prompt::workspace_user::optional_workspace_user_dir();
...
pub async fn run_headless(
    ...
    let (mut auth, did_browser_flow) = if reauthenticate {
        ...
        if crate::agent::auth_method::has_xai_api_key_env()
            ...
            && ezer_login::try_ensure_fresh_auth(ctx, auth_manager.proxy_base_url().to_string())
                .await
                .is_none()
        {
...
/// Another process is wedged inside its own open+flock of the leader lock (stalled ezer home). Only `tracing`
/// here: a socket probe, pid read, or log open under the ezer home could wedge this process too. Best effort: a
/// client-spawned leader's stderr is `$EZER_HOME/leader.log`, so even this line can park in `write(2)` there.
...
pub async fn run_leader(
    ...
    let control_state = LeaderServerControlState::new(LeaderServerMetadata {
        ...
        leader_binary_version: ezer_version::VERSION.to_string(),
    })
    ...
    local_set
        .run_until(async move {
            ...
            if session_pending {
                ...
                tokio::task::spawn_local(async move {
                    tokio::select! {
                        ...
                        minted = ezer_login::mint_session_noninteractive(&mint_auth_manager)
                            => match minted {
                            Some(session) => info!(
            ...
            let initial_auth_key_hash = ezer_config::user_ezer_home()
                ...
                .and_then(|store| {
                    ezer_login::lookup_auth(&store, &auth_scope)
