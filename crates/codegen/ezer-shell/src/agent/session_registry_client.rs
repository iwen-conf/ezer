...
impl SessionRegistryClient {
    ...
    pub fn with_auth(mut self, auth_manager: std::sync::Arc<ezer_login::AuthManager>) -> Self {
        let provider: std::sync::Arc<dyn ezer_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
    ...
    async fn send_authed(
        ...
        reqwest::Response,
        Option<ezer_auth::StampedBearerSuffix>,
    )> {
        let builder = ezer_otel::inject_trace_context_into_request(builder);
        let request = builder.build().context(op)?;
        ezer_auth::execute_with_stamp(&self.client, request)
            .await
            .map_err(|e| match e {
    ...
    fn check_response(
        ...
        stamp: Option<&ezer_auth::StampedBearerSuffix>,
        op: &str,
    ) -> anyhow::Error {
    ...
    /// `stamp` is what the middleware put on the wire; [`ezer_auth::StampedBearerSuffix`] explains why it is never re-resolved.
    fn record_401_attribution(&self, op: &str, stamp: Option<&ezer_auth::StampedBearerSuffix>) {
        if let Some(manager) = self.credentials.auth_manager() {
            ezer_login::attribution::record_consumer_401(
                manager.as_ref(),
                self.session_id.as_deref(),
                ezer_login::attribution::ConsumerKind::SessionRegistryClient,
                op,
