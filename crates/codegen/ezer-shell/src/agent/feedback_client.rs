...
impl FeedbackClient {
    ...
    fn build_middleware_client(
        ...
        reqwest_middleware::ClientBuilder::new(http.clone())
            .with(ezer_auth::AuthRetryMiddleware::new(provider, 0))
            .build()
    }
    ...
    fn make_auth_provider(
        ...
        if let Some(am) = credentials.auth_manager() {
            ...
            Arc::new(ezer_auth::StaticAuthCredentialProvider::new(
    ...
    fn record_401_attribution_if_needed(
            ...
            && let Some(am) = self.credentials.auth_manager()
        {
            ...
            ezer_login::attribution::record_consumer_401(
                ...
                ezer_login::attribution::ConsumerKind::FeedbackClient,
                op,
    ...
    pub(crate) async fn try_refresh_credentials(&self) -> bool {
        ...
        manager
            .try_recover_unauthorized(ezer_login::recovery::RecoverySource::Background)
            .await
    }
    ...
    fn add_common_headers(&self, builder: RequestBuilder) -> RequestBuilder {
        let builder = builder
            .header(CLIENT_VERSION_HEADER, ezer_version::VERSION)
            .header(
        ...
        if self.credentials.deployment_key.is_none() {
            builder.header("X-XAI-Token-Auth", "ezer-cli")
        } else {
            builder
        }
    }
    ...
    async fn send_json<T: DeserializeOwned>(
        ...
        let request = ezer_otel::inject_trace_context_into_request(request);
        let req = request.build().context(context)?;
        let (response, stamp) = ezer_auth::execute_with_stamp(&self.client, req)
            .await
            .context(context)?;
...
mod auth_refresh_tests {
    ...
    impl ezer_login::refresh::TokenRefresher for CountingRefresher {
        async fn refresh(
            &self,
