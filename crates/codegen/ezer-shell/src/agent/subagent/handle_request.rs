...
use ezer_telemetry::events::{SubagentModelOverrideRejected, SubagentModelRejectionReason};
...
static SUBAGENTS_ACTIVE: ezer_telemetry::activity::ActivityGauge =
    ezer_telemetry::activity::ActivityGauge::work(
        ezer_telemetry::activity::SUBAGENTS_ACTIVE_KEY,
    );
...
pub(super) fn agent_memory_scope_for_mode(
    scope: Option<ezer_agent::config::MemoryScope>,
    mode: crate::config::MemoryMode,
) -> Option<ezer_agent::config::MemoryScope> {
...
pub(super) async fn reparent_surviving_child_tasks(
    parent_tb: &std::sync::Arc<dyn ezer_tools::computer::types::TerminalBackend>,
    parent_notif_handle: &ezer_tools::notification::types::ToolNotificationHandle,
    parent_cmd_tx: Option<&mpsc::UnboundedSender<SessionCommand>>,
...
pub(crate) async fn run_shell_child(
    ...
    let spawn_timer = ezer_telemetry::subagent_spawn::SubagentSpawnTimer::new_shared();
    ...
    let mut effective_runtime = ezer_subagent_resolution::resolve_runtime_config(
        &request.subagent_type,
    ...
    if let Some(ref source) = resume_source {
        ...
        if let Err(e) = ezer_subagent_resolution::validate_resume_identity(
            &request.subagent_type,
        ...
        )
    {
        let message = match error {
            TaskModelAdmissionError::HiddenSelection => {
                ezer_telemetry::session_ctx::log_event(SubagentModelOverrideRejected {
    ...
    effective_runtime.capability_mode = ezer_subagent_resolution::intersect_capability_modes(
    ...
    ezer_subagent_resolution::apply_child_tool_policy(
        &mut definition,
        effective_runtime.capability_mode,
    ...
    {
        let reminder = ezer_sampling_types::conversation::ConversationItem::system_reminder(
    ...
    let child_cwd_abs = ezer_paths::AbsPathBuf::new(child_cwd).unwrap_or_else(|_| {
        ezer_paths::AbsPathBuf::new(std::env::current_dir().unwrap_or_default())
    ...
    let parent_traceparent = ezer_otel::current_traceparent();
    ...
    let attribution_callback: Option<ezer_sampler::SharedAttributionCallback> =
    ...
    let yolo_policy_block = ezer_workspace::permission::resolution::yolo_disabled_by_policy();
    ...
    if let Some(scope) = agent_memory_scope {
            ...
            && let Ok(content) = std::fs::read_to_string(&memory_md)
        {
            ...
            let truncated: String = content
                ...
                .join("\n");
            let truncated =
                ezer_tools::util::truncate::truncate_str(&truncated, MAX_BYTES).to_string();
        ...
        if is_plugin_agent {
            ...
            if !specs.is_empty() {
                let specs: Vec<_> = specs
                    .into_iter()
                    .map(|mut s| {
                        if s.event == ezer_hooks::event::HookEventName::Stop {
                            s.event = ezer_hooks::event::HookEventName::SubagentStop;
                        }
                        s
                    })
                    .collect();
    ...
    let agent_mcp_servers: Vec<_> = if definition.mcp_servers.is_empty() {
        ...
        definition
                ...
                .filter_map(|entry| match entry {
                    ezer_agent::config::McpServerRef::Named(name) => {
                    ...
                    ezer_agent::config::McpServerRef::Inline { name, config } => {
    ...
    if inherit_skills && ctx.parent_skills.is_none() {
        ...
        ctx.parent_skills = Some(
            ezer_agent::prompt::skills::list_skills_with_plugins(
                Some(&parent_cwd_str),
    ...
    ezer_telemetry::session_ctx::log_event(ezer_telemetry::events::SubagentLaunched {
    ...
    let spawn_result = session::spawn_session_on_thread(
        ...
        auto_compact_threshold_percent,
        ezer_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
        pins.mode,
        ...
        std::sync::Arc::new(parking_lot::Mutex::new(
            ezer_workspace::file_system::CodebaseIndexManager::new(),
        )),
        false,
        subagent_fs_watch,
        ...
        ctx.yolo_mode
            || matches!(
                ...
                ezer_agent::config::PermissionMode::BypassPermissions
            ),
        false,
        None,
        ...
        Vec::new(),
        ezer_agent::prompt::context::PromptAudience::Subagent,
        ...
        false,
        Some(ezer_telemetry::subagent_spawn::SpawnPhaseContext {
    ...
    let outcome = if result.success {
        ezer_telemetry::events::Outcome::Completed
    } else if result.cancelled {
        ezer_telemetry::events::Outcome::Cancelled
    } else {
        ezer_telemetry::events::Outcome::Error
    };
    let mut completed = ezer_telemetry::events::SubagentCompleted {
        subagent_id: request.id.clone(),
        parent_session_id: request.parent_session_id.clone(),
        owner: telemetry_owner_kind(&request),
        workflow_run_id: request.owner.workflow_run_id().map(str::to_string),
    ...
    ezer_telemetry::session_ctx::log_event(completed);
    match (
