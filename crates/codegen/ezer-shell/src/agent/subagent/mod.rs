...
//!
//! Lifecycle state and command scheduling live in the shared `ezer-tools` coordinator actor.
...
use ezer_agent::config::{McpInheritance, ModelOverride, PermissionMode};
...
pub(crate) struct SubagentSpawnContext {
    ...
    pub lsp: Option<std::sync::Arc<dyn ezer_tools::implementations::lsp::LspBackend>>,
    ...
    /// The Spawn arm of `handle_command` in ezer-tools task/coordinator.rs re-parents nested Spawn requests to the root parent. Every subagent therefore resolves back to the root session.
    ...
    pub sampling_config: ezer_sampler::SamplerConfig,
    #[cfg(test)]
    ...
    pub inherited_tool_overrides: Option<ezer_sampling_types::ToolOverrides>,
    pub yolo_mode: bool,
    pub subagent_event_tx: mpsc::UnboundedSender<SubagentEvent>,
    pub parent_depth: u32,
    pub subagents_max_depth: u32,
    pub workflow_max_concurrent_agents: usize,
    pub media_gen_batch_limits: ezer_tools::media_gen_limits::MediaGenBatchLimits,
    ...
    pub parent_terminal_backend: Option<Arc<dyn ezer_tools::computer::types::TerminalBackend>>,
    ...
    pub subagent_roles:
        std::collections::HashMap<String, ezer_subagent_resolution::config::SubagentRole>,
    /// Subagent personas config for persona/SOUL layering.
    pub subagent_personas:
        std::collections::HashMap<String, ezer_subagent_resolution::config::SubagentPersona>,
    ...
    pub plugin_registry: Option<std::sync::Arc<ezer_agent::plugins::PluginRegistry>>,
    ...
    pub file_tool_overrides: Option<Vec<ezer_tools::registry::types::ToolConfig>>,
    ...
    pub hook_registry: Option<std::sync::Arc<ezer_hooks::discovery::HookRegistry>>,
    pub permission_handle: Option<ezer_workspace::permission::PermissionHandle>,
    pub worktree_type: crate::util::config::WorktreeType,
    pub api_key_provider: Option<ezer_tools::types::SharedApiKeyProvider>,
    pub image_description_model: String,
    pub workspace_ops: ezer_workspace::WorkspaceOps,
    pub auth_manager: std::sync::Arc<ezer_login::AuthManager>,
    ...
    pub attribution_callback: Option<ezer_sampler::SharedAttributionCallback>,
    ...
    pub parent_tool_definitions: Option<Vec<ezer_sampling_types::ToolSpec>>,
    /// Pre-discovered skills from the parent session, captured at spawn time.
    pub parent_skills: Option<Vec<ezer_tools::implementations::skills::types::SkillInfo>>,
    /// Parent's skills config for the child's SkillManager.
    pub parent_skills_config: ezer_agent::prompt::skills::SkillsConfig,
    ...
    pub parent_compat: ezer_tools::types::compat::CompatConfig,
    /// Parent's `[paths]` config, inherited for the same reason as `parent_compat`.
    pub parent_paths_config: ezer_agent::prompt::paths::PathsConfig,
...
impl SubagentSpawnContext {
    ...
    fn apply_session_cli_overrides(&self, def: &mut ezer_agent::config::AgentDefinition) {
    ...
    pub(crate) fn snapshot_parent_compaction_pins(
        ...
        let current = parent_agent_name
            .and_then(|name| ezer_agent::discovery::by_name_in_cwd(name, parent_cwd))
    ...
    pub(crate) fn compaction_pins_for_child(
        &self,
        child_template: &ezer_agent::prompt::user_message::UserMessageTemplate,
    ) -> crate::session::CompactionPins {
        crate::session::cursor_compaction_pins(
...
async fn resolve_subagent_sampling_config(
    agent_name: &str,
    agent_model: &ezer_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (ezer_sampler::SamplerConfig, acp::ModelId) {
...
async fn resolve_effective_model_config(
    ...
    subagent_type: &str,
    definition_model: &ezer_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (ezer_sampler::SamplerConfig, acp::ModelId) {
...
fn log_subagent_model_resolution(
    agent_name: &str,
    priority: &str,
    resolved: &ezer_sampler::SamplerConfig,
    resolved_id: &acp::ModelId,
    parent: &ezer_sampler::SamplerConfig,
) {
...
fn session_bearer_resolver(
    ...
    base_url: &str,
) -> Option<ezer_sampler::SharedBearerResolver> {
    ...
    .then(|| {
        ezer_login::credential_provider::WireValidBearerResolver::shared(
...
async fn read_parent_sampling_config(
    ...
    if let Some(ref chat_state) = ctx.parent_chat_state {
        if let Some(cfg) = chat_state.get_sampling_config().await {
            ...
            let inherited = ezer_sampler::SamplerConfig {
...
fn resolve_model_override_to_config(
    model_id: &str,
    ctx: &SubagentSpawnContext,
) -> Option<(ezer_sampler::SamplerConfig, acp::ModelId)> {
...
struct InitialContext {
    ...
    conversation: Vec<ezer_sampling_types::conversation::ConversationItem>,
    force_compact: bool,
...
fn forked_initial_context(
    mut items: Vec<ezer_sampling_types::conversation::ConversationItem>,
) -> InitialContext {
    ...
    let (conversation, prefix_len) =
        ezer_subagent_resolution::context::normalize_forked_context(items);
    if prefix_len < 2 {
        return InitialContext {
            source: InitialContextSource::New,
...
fn conversation_tail_is_complete(
    items: &[ezer_sampling_types::conversation::ConversationItem],
) -> bool {
    matches!(
        items.last(),
...
fn verbatim_or_normalize_fork(
    items: Vec<ezer_sampling_types::conversation::ConversationItem>,
    ...
    let (conversation, prefix_len) =
        ezer_subagent_resolution::context::normalize_forked_context(filtered);
    InitialContext {
        source: InitialContextSource::Forked,
        copy_error: None,
        prefix_len: Some(prefix_len),
...
fn resolve_inherited_mcp_pool(
    ...
    inheritance: &ezer_agent::config::McpInheritance,
...
fn resolve_agent_definition(
    subagent_type: &str,
    ctx: &SubagentSpawnContext,
) -> Option<ezer_agent::config::AgentDefinition> {
    let cli_agents = ctx
        .agent_config
        .as_ref()
        .map(|config| config.cli_agents.as_slice())
        .unwrap_or_default();
    let resolution_context = ezer_subagent_resolution::DefinitionResolutionContext {
        cwd: &ctx.parent_cwd,
    ...
    let mut def = ezer_subagent_resolution::discover_agent_definition(
        subagent_type,
        &resolution_context,
    )?;
...
fn available_agent_names(ctx: &SubagentSpawnContext) -> Vec<String> {
    let cli_agents = ctx
        ...
        .map(|config| config.cli_agents.as_slice())
        .unwrap_or_default();
    ezer_subagent_resolution::available_agent_names(
        &ezer_subagent_resolution::DefinitionResolutionContext {
            cwd: &ctx.parent_cwd,
            plugins: ctx.plugin_registry.as_deref(),
            cli_agents,
            toggles: &ctx.subagent_toggle,
...
pub(crate) struct SubagentValidationContext {
    pub parent_cwd: PathBuf,
    pub plugin_registry: Option<Arc<ezer_agent::plugins::PluginRegistry>>,
...
pub(crate) fn validate_subagent_type(
    ...
    let context = ezer_subagent_resolution::DefinitionValidationContext {
        cwd: &ctx.parent_cwd,
        plugins: ctx.plugin_registry.as_deref(),
    ...
    match ezer_subagent_resolution::validate_agent_name(subagent_type, &context) {
        Ok(()) => SubagentValidateTypeOutcome::Ok,
        Err(ezer_subagent_resolution::ResolutionError::Unknown { available, .. }) => {
            SubagentValidateTypeOutcome::Unknown { available }
        }
        Err(ezer_subagent_resolution::ResolutionError::Disabled { .. }) => {
            SubagentValidateTypeOutcome::Disabled
        }
        Err(ezer_subagent_resolution::ResolutionError::NotAllowed { allowed, .. }) => {
            SubagentValidateTypeOutcome::NotAllowed { allowed }
        }
        Err(
            ezer_subagent_resolution::ResolutionError::PersonaResolution(_)
            | ezer_subagent_resolution::ResolutionError::ResumeValidation(_),
...
fn gate_subagent_type(
    ...
    match ezer_subagent_resolution::gate_agent_definition(subagent_type, &resolution_context) {
        Ok(()) => SubagentValidateTypeOutcome::Ok,
        Err(ezer_subagent_resolution::ResolutionError::Disabled { .. }) => {
            SubagentValidateTypeOutcome::Disabled
        }
        ...
        Err(
            ezer_subagent_resolution::ResolutionError::Unknown { .. }
            | ezer_subagent_resolution::ResolutionError::PersonaResolution(_)
            | ezer_subagent_resolution::ResolutionError::ResumeValidation(_),
        ) => SubagentValidateTypeOutcome::ValidationUnavailable,
    }
...
pub(crate) fn subagent_harness_flavor_is_representable(agent_type: &str) -> bool {
    ezer_subagent_resolution::subagent_harness_flavor_is_representable(agent_type)
}
...
fn resolve_subagent_toolset(
    ...
    ctx: &SubagentSpawnContext,
    definition: &mut ezer_agent::config::AgentDefinition,
) {
    let resolution_context = ezer_subagent_resolution::HarnessToolsetContext {
    ...
    ezer_subagent_resolution::apply_harness_toolset(
        subagent_type,
        &resolution_context,
        definition,
    );
...
fn summarize_tool_config(
    config: &ezer_tools::registry::types::ToolServerConfig,
) -> SubagentTypeSummary {
...
fn resolve_subagent_permission_mode(
    requested: ezer_agent::config::PermissionMode,
    is_plugin: bool,
    policy_block: Option<&'static str>,
) -> ezer_agent::config::PermissionMode {
    if is_plugin {
...
fn resolve_subagent_source_repo(ctx: &SubagentSpawnContext) -> std::path::PathBuf {
    ...
    ezer_workspace::session::git::find_main_repo_root_from_path(&source_cwd)
...
fn cancellation_error_message(
    category: Option<ezer_session_events::types::CancellationCategory>,
...
fn telemetry_owner_kind(
    request: &SubagentRequest,
) -> ezer_telemetry::events::SubagentOwnerKind {
    if request.owner.is_workflow() {
        ezer_telemetry::events::SubagentOwnerKind::Workflow
    } else if request.from_scheduler_loop() {
        ezer_telemetry::events::SubagentOwnerKind::SchedulerLoop
    } else {
        ezer_telemetry::events::SubagentOwnerKind::Task
    }
}
