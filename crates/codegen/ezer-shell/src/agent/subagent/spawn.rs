...
//!    `spawn_subagent_coordinator` drives the coordinator (living in `ezer-tools`).
...
fn log_limit_notice(notice: coordinator::SubagentLimitNotice) {
    ...
    ezer_telemetry::session_ctx::log_event(SubagentLimitHit::session_concurrent(
...
pub(crate) fn inject_subagent_completed_prompt(params: InjectParams) {
    ...
    let wrapped = ezer_tools::reminders::wrap_reminder(&message);
