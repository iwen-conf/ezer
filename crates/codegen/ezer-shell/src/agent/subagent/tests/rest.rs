...
fn normalize_forked_context_strips_project_layout() {
    ...
    let (conv, _) = ezer_subagent_resolution::context::normalize_forked_context(
        items,
    );
...
fn end_to_end_normalized_conversation_shape() {
    ...
    let (mut conv, prefix_len) = ezer_subagent_resolution::context::normalize_forked_context(
        parent_conv,
    );
    assert_eq!(prefix_len, 2);
...
fn last_user_message_is_task_after_normalization() {
    ...
    let (mut conv, _) = ezer_subagent_resolution::context::normalize_forked_context(
        parent_conv,
    );
            ...
            if let ConversationItem::User(u) = i {
                u.content
                    ...
                    .any(|p| {
                        matches!(
                    p,
                    ezer_sampling_types::conversation::ContentPart::Text { text } if text.contains("<background_context>")
                )
                    })
            } else {
                false
...
fn token_estimation_accounts_for_images() {
    use ezer_sampling_types::conversation::{ContentPart, ConversationItem, UserItem};
...
async fn read_parent_sampling_config_keeps_auto_when_catalog_has_slug_key_only() {
    ...
    crate::util::config::cache_remote_accept_request_encodings(
        parent_base_url,
        &[ezer_config_types::RemoteRequestEncoding::Zstd],
    );
    let (config, model_id) = read_parent_sampling_config(&ctx).await;
    ...
    assert_eq!(config.request_compression, ezer_sampler::RequestCompression::Zstd);
}
#[tokio::test]
...
async fn subagent_override_provider_model_spawns_cache_only_credentials() {
    ...
    let provider = ezer_login::test_counting_provider(
        "test-subagent-spawn",
        dir.path(),
    );
...
fn non_cursor_persona_injected_as_system_reminder() {
    use ezer_sampling_types::conversation::{ConversationItem, SyntheticReason};
...
fn filter_inheritance_all_passes_everything_through() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &ezer_agent::config::McpInheritance::All,
    );
...
fn filter_inheritance_none_returns_none() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &ezer_agent::config::McpInheritance::None,
    );
...
fn filter_inheritance_named_selects_specific_servers() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &ezer_agent::config::McpInheritance::Named(
...
fn filter_inheritance_except_excludes_specific_servers() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &ezer_agent::config::McpInheritance::Except(
...
fn filter_inheritance_named_empty_list_gives_empty_pool() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &ezer_agent::config::McpInheritance::Named(vec![]),
    );
...
fn filter_inheritance_except_empty_list_keeps_all() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &ezer_agent::config::McpInheritance::Except(vec![]),
    );
    let result = result.expect("Except([]) should return Some");
...
fn filter_inheritance_except_nonexistent_servers_ignored() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &ezer_agent::config::McpInheritance::Except(vec!["nonexistent".into()]),
    );
    let result = result.expect("Except should return Some");
    assert_eq!(pool_names(&result), vec!["github", "linear"]);
}
...
fn filter_inheritance_named_all_nonexistent_gives_empty() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &ezer_agent::config::McpInheritance::Named(vec!["foo".into(), "bar".into()]),
    );
    let result = result.expect("Named should return Some");
    assert_eq!(result.server_names().count(), 0);
}
...
fn resolve_inherited_pool_named_filters() {
    ...
    let result = super::resolve_inherited_mcp_pool(
            Some(pool),
            &ezer_agent::config::McpInheritance::Named(vec!["atlassian".into()]),
        )
...
fn make_test_skill(
    ...
    ezer_tools::implementations::skills::types::SkillInfo {
        ...
        scope: ezer_tools::implementations::skills::types::SkillScope::Local,
        enabled: true,
        user_invocable: true,
