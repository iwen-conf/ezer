...
fn v2_disables_legacy_agent_memory_scope() {
    let scope = Some(ezer_agent::config::MemoryScope::Project);
    assert_eq!(
...
fn wedged_child_handle() -> (
    ...
    let handle = SessionHandle {
        ...
        tool_context: crate::tools::ToolContext::new_local_context(
            ezer_paths::AbsPathBuf::new(PathBuf::from("/tmp")).unwrap(),
            std::sync::Arc::new(
                ezer_workspace::file_system::LocalFs::new(PathBuf::from("/tmp")),
            ),
...
impl ezer_tools::computer::types::TerminalBackend for StarvedTerminal {
    async fn run(
        &self,
        _request: ezer_tools::computer::types::TerminalRunRequest,
    ) -> Result<
        ezer_tools::computer::types::TerminalRunResult,
        ezer_tools::computer::types::ComputerError,
    > {
    ...
    async fn run_background(
        ...
        ezer_tools::computer::types::BackgroundHandle,
        ezer_tools::computer::types::ComputerError,
    > {
        std::future::pending().await
    }
    async fn get_task(
        &self,
        _task_id: &str,
    ) -> Option<ezer_tools::computer::types::TaskSnapshot> {
        std::future::pending().await
    }
    async fn kill_task(
        &self,
        _task_id: &str,
    ) -> ezer_tools::computer::types::KillOutcome {
        std::future::pending().await
    }
    ...
    async fn list_tasks(&self) -> Vec<ezer_tools::computer::types::TaskSnapshot> {
        std::future::pending().await
    }
    async fn reparent_notifications(
        ...
        _new_handle: ezer_tools::notification::types::ToolNotificationHandle,
        _backend_weak: std::sync::Weak<
            dyn ezer_tools::computer::types::TerminalBackend,
        >,
    ) {
        std::future::pending().await
    }
...
async fn reparent_is_bounded_when_terminal_actor_never_answers() {
    ...
    let notif_handle = ezer_tools::notification::types::ToolNotificationHandle::noop();
    let (cmd_tx, mut cmd_rx) = mpsc::unbounded_channel();
    tokio::time::timeout(
            20 * PARENT_ACK_TIMEOUT,
...
async fn subagent_inherits_session_cli_overrides() {
    use ezer_agent::config::{AgentDefinition, PermissionMode};
...
fn subagent_bypass_permission_mode_gated_by_policy_pin() {
    use ezer_agent::config::PermissionMode;
    const PIN: &str = ezer_workspace::permission::resolution::YoloPinReason::DisableBypassPermissionsMode
        .message();
    assert_eq!(
...
fn partial_override_fills_from_role() {
    ...
    let role = ezer_subagent_resolution::config::SubagentRole {
...
fn persona_resolved_from_config() {
    ...
    personas
        .insert(
            ...
            ezer_subagent_resolution::config::SubagentPersona {
...
fn model_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_model = ezer_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
...
fn reasoning_effort_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_re = ezer_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
    let r = resolve_effective_overrides(
...
fn forked_initial_context_inherits_parent_across_reasoning() {
    ...
    let items = vec![
            ...
            ConversationItem::Reasoning(ezer_sampling_types::synthesized_reasoning_item(
                "deliberating",
            )),
...
fn verbatim_fork_falls_back_to_summary_when_oversize() {
    use ezer_sampling_types::conversation::{ContentPart, ConversationItem};
    let items = vec![
            ConversationItem::system("parent system"),
...
async fn bootstrap_in_place_resume_reads_existing_transcript() {
    ...
    let source = ezer_subagent_resolution::ResumeSourceData {
                    ...
                    ConversationItem::User(u) => {
                        Some(
                            u
                                ...
                                .iter()
                                .filter_map(|p| match p {
                                    ezer_sampling_types::conversation::ContentPart::Text {
                                        text,
                                    } => Some(text.as_ref()),
                                    _ => None,
                                })
...
fn summarize_tool_config_uses_name_override_and_strips_namespace() {
    use ezer_tools::registry::types::{ToolConfig, ToolServerConfig};
...
fn subagent_keeps_default_flavor_when_parent_model_is_non_strict() {
    use ezer_agent::config::BuiltinAgentName;
    let mut ctx = ctx_with_toggle(HashMap::new());
...
fn fresh_tool_model_accepts_visible_key_and_internal_id() {
    ...
    assert_eq!(
            admit_explicit_tool_model("test-model-3", TaskModelSelection::Inherited, &models, false),
...
fn normalize_forked_context_empty_parent() {
    ...
    let (conv, prefix_len) = ezer_subagent_resolution::context::normalize_forked_context(
        items,
    );
...
fn test_sampling_config(model_slug: &str) -> ezer_sampling_types::SamplingConfig {
