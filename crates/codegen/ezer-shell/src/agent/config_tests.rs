...
fn first_run_byok_template_parses_as_responses_gateway() {
    ...
    assert_eq!(
        ...
        Some(ezer_config::DEFAULT_GATEWAY_BASE_URL)
    );
    let resolved = resolve_model_list(&cfg, None);
    let model = resolved
        .get(ezer_config::DEFAULT_GATEWAY_MODEL_KEY)
        .expect("starter model");
    assert_eq!(model.info.api_backend, ApiBackend::Responses);
    assert_eq!(
        model.info.base_url,
        ezer_config::DEFAULT_GATEWAY_BASE_URL
    );
    assert_eq!(model.info.model, ezer_config::DEFAULT_GATEWAY_MODEL_ID);
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(ezer_config::DEFAULT_GATEWAY_API_KEY)
    );
    assert_eq!(
        cfg.session_summary_model.as_deref(),
        Some(ezer_config::DEFAULT_GATEWAY_MODEL_KEY)
    );
    for id in ezer_config::DEFAULT_GATEWAY_OPTIONAL_MODELS {
...
fn auto_mode_config_parses_from_toml_and_json_equivalently() {
    use ezer_workspace::permission::ClassifierPromptType;
    let toml_src = r#"
enabled = true
...
fn session_resolver_is_not_stamped_onto_third_party_samplers() {
    ...
    impl ezer_sampler::BearerResolver for SessionResolver {
...
fn shell_environment_policy_known_keys_track_the_policy_struct() {
    let ezer_tools::util::ShellEnvironmentPolicy {
        inherit: _,
        ignore_default_excludes: _,
        exclude: _,
        set: _,
        include_only: _,
    } = ezer_tools::util::ShellEnvironmentPolicy::default();
...
fn parses_auth_provider_tables_and_model_reference() {
    ...
    assert_eq!(
        ...
        Some(&ezer_login::AuthProviderConfig {
...
fn prefetched_entry_provider_config_comes_from_trusted_tables_only() {
    ...
    let smuggled: ezer_login::AuthProviderRef =
    ...
    assert_eq!(
        provider.config,
        ezer_login::AuthProviderConfig::default()
    );
...
fn x_api_key_auth_scheme_flows_from_config_to_sampler() {
    ...
    assert_eq!(config.api_backend, ApiBackend::Messages);
    let client = ezer_sampler::SamplingClient::new(config).expect("client should build");
...
fn user_override_parses_compaction_at_tokens_from_toml() {
    use ezer_sampling_types::CompactionAtTokens;
    let dm = crate::models::default_model();
...
"#;
    ...
    let leaked: Vec<&String> = unused
        .iter()
        .filter(|key| {
            ezer_workspace::permission::resolution::MANAGED_POLICY_CONFIG_KEYS
                .iter()
                .any(|policy_key| {
...
fn resolve_long_reasoning_reminder_precedence() {
    ...
    let _env = EnvGuard::unset("EZER_LONG_REASONING_REMINDER");
    assert_eq!(
...
fn ext_client() -> ezer_telemetry::external::config::ExternalClientInfo {
    ezer_telemetry::external::config::ExternalClientInfo::default()
}
#[test]
...
fn external_otel_pin_protocol_hides_unlisted_file_siblings() {
    ...
    assert_eq!(
        cfg.logs_transport,
        ezer_telemetry::external::config::OtlpTransport::HttpProtobuf,
...
fn external_otel_pin_exporter_beats_none() {
    ...
    assert_eq!(
        cfg.logs_exporter,
        ezer_telemetry::external::config::ExporterSelection::Otlp
    );
}
#[test]
...
fn remote_settings_disarm_managed_config_signatures() {
    ...
    assert!(ezer_config::signed_policy::verification_active());
