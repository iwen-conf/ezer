...
impl EnvKeys {
    ...
    pub(crate) fn resolve_value(&self) -> Option<String> {
        self.resolve_value_with(|name| ezer_login::auth_method::read_env_var(name).ok())
    }
...
pub struct RuntimeResolutionContext<'a> {
    ...
    /// Every entrypoint (TUI, `ezer agent stdio`, headless) passes `None` unless a flag was given, so they resolve identically.
...
pub use ezer_config::env_bool;
pub(crate) use ezer_env::env_string;
...
fn resolve_compat_cell(
    ...
    resolve_compat_cell_with_env(ezer_config::env_bool(env), cfg, remote, default)
}
...
pub(crate) fn compat_config_cell(
    ...
    cell: ezer_tools::types::compat::CompatCell,
...
pub(crate) use ezer_telemetry::config::env_telemetry_mode;
pub use ezer_telemetry::config::{TelemetryConfig, TelemetryMode};
...
impl PluginsConfig {
    ...
    pub(crate) fn merge_claude_enabled_plugins(&mut self, _cwd: Option<&std::path::Path>) {
        ...
        for path in &paths {
            let (claude_enabled, claude_disabled) =
                ezer_agent::plugins::marketplace::load_enabled_disabled_plugins(path);
    ...
    pub(crate) fn to_discovery_config(
        &self,
    ) -> ezer_agent::plugins::discovery::DiscoveryConfig {
        ezer_agent::plugins::discovery::DiscoveryConfig {
...
pub use ezer_agent::prompt::paths::PathsConfig;
...
pub struct Config {
    ...
    #[serde(skip)]
    pub auth_providers: IndexMap<String, ezer_config_types::AuthProviderConfig>,
    #[serde(skip)]
    ...
    #[serde(default, skip_serializing)]
    pub announcements: Vec<ezer_announcements::RemoteAnnouncement>,
    ...
    /// `[marketplace]`: also read by `ezer_plugin_marketplace::load_sources()`.
    #[serde(default, skip_serializing)]
    ...
    #[serde(skip)]
    pub cli_agents: Vec<ezer_agent::config::AgentDefinition>,
    #[serde(skip)]
...
pub struct CliAgentOverrides {
    ...
    pub permission_rules: Vec<ezer_workspace::permission::types::PermissionRule>,
    pub max_turns: Option<u32>,
    pub permission_mode: Option<ezer_agent::config::PermissionMode>,
}
impl CliAgentOverrides {
    ...
    pub(crate) fn apply_to_definition(&self, def: &mut ezer_agent::config::AgentDefinition) {
    ...
    pub(crate) fn apply_to_subagent_definition(
        &self,
        def: &mut ezer_agent::config::AgentDefinition,
    ) {
...
pub use ezer_agent::config::AgentDefinition;
pub use ezer_agent::config::Effort;
pub use ezer_agent::config::PermissionMode;
pub use ezer_shared::ui_config::{ContextualHints, UiConfig};
...
pub struct AgentSelectionConfig {
    ...
    /// Looked up via `ezer_agent::discovery::by_name_in_cwd()`.
...
impl Default for Config {
    fn default() -> Self {
        ...
        let mut cfg = Self {
            ...
            client_version: Some(ezer_version::VERSION.to_string()),
...
fn is_non_serde_config_path(path: &str) -> bool {
    NON_SERDE_CONFIG_PATHS.contains(&path)
        || ezer_workspace::permission::resolution::MANAGED_POLICY_CONFIG_KEYS.contains(&path)
...
fn parse_auth_providers(
    ...
    IndexMap<String, ezer_config_types::AuthProviderConfig>,
    ...
    for (name, value) in table {
        ...
        match serde_ignored::deserialize::<_, _, ezer_config_types::AuthProviderConfig>(
            value.clone(),
...
impl Config {
    ...
    pub fn resolve_runtime_fields(&mut self, ctx: &RuntimeResolutionContext<'_>) {
        ...
        self.media_gen_batch_limits = ezer_tools::media_gen_limits::MediaGenBatchLimits {
    ...
    pub(crate) fn resolve_doom_loop_recovery(
        &self,
    ) -> Option<ezer_sampling_types::DoomLoopRecoveryPolicy> {
        use ezer_sampling_types::DoomLoopRecoveryPolicy as Policy;
        let remote = self
            .remote_settings
            .as_ref()
...
pub(crate) fn external_otel_master_switch_resolved() -> bool {
    external_otel_master_switch_from(
        ezer_config::load_merged_requirements().as_ref(),
...
fn telemetry_otel_file_config(
    t: &toml::Value,
) -> ezer_telemetry::external::ExternalOtelFileConfig {
    ezer_telemetry::external::ExternalOtelFileConfig {
...
pub fn resolve_external_otel_config(
    client: ezer_telemetry::external::config::ExternalClientInfo,
) -> Option<ezer_telemetry::external::ExternalOtelConfig> {
    let requirements = ezer_config::load_merged_requirements();
...
pub(crate) fn resolve_external_otel_config_with(
    ...
    let file_cfg: Option<ezer_telemetry::external::ExternalOtelFileConfig> = effective_config
    ...
    let mut resolved = ezer_telemetry::external::ExternalOtelConfig::resolve_with(
        getenv_pinned,
        file_cfg.as_ref(),
    )?;
...
pub(crate) fn apply_external_otel_remote_policy(
    ...
    let policy = ezer_telemetry::external::ExternalOtelRemotePolicy {
    ...
    if policy.force_disable || policy.lock_content_gates {
        ezer_telemetry::external::apply_remote_policy(policy);
    }
}
...
fn managed_settings_env_flag(key: &str) -> Option<bool> {
    let path = ezer_config::claude_managed_settings_path()?;
    let content = std::fs::read_to_string(&path).ok()?;
    let json: serde_json::Value = serde_json::from_str(&content).ok()?;
    ezer_workspace::permission::resolution::json_env_flag(json.get("env"), key)
}
...
pub(crate) fn resolve_model_list(
    ...
    for (key, model_override) in &cfg.config_models {
        ...
        {
            entry.auth_provider = Some(ezer_login::AuthProviderRef::fail_closed(format!(
...
impl ConfigModelOverride {
    pub(crate) fn apply(
        ...
        if let Some(ref name) = self.auth_provider {
            entry.auth_provider = Some(ezer_login::AuthProviderRef::unresolved(name.clone()));
        }
...
pub struct ModelEntry {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub auth_provider: Option<ezer_login::AuthProviderRef>,
...
impl ModelEntry {
    ...
    pub(crate) fn effective_auth_provider(&self) -> Option<&ezer_login::AuthProviderRef> {
...
pub struct AutoModeConfig {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prompt_type: Option<ezer_workspace::permission::ClassifierPromptType>,
...
pub(crate) fn resolve_credentials(
    ...
    let (api_key, base_url, auth_type) = if let Some(key) = model.own_credential() {
        ...
        && ezer_login::backend::AuthBackend::may_receive_session(
            &ezer_login::backend::ActiveAuthBackend::default(),
            &info.base_url,
        )
    {
        (
...
pub use ezer_telemetry::config::deployment_id_from_key;
...
pub(crate) fn resolve_model_auth_facts_and_provider(
    model_id: &str,
) -> (ModelAuthFacts, Option<ezer_login::AuthProviderRef>) {
...
pub(crate) fn inject_url_derived_headers(
    ...
    if crate::util::is_cli_chat_proxy_url(base_url) {
        headers
            ...
            .or_insert_with(|| "ezer-cli".to_string());
        headers
