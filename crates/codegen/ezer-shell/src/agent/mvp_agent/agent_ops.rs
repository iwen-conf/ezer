...
struct SessionConfigInputs {
    ...
    current_effort: Option<ezer_sampling_types::ReasoningEffort>,
}
...
type PolledFields = (
    Option<Vec<ezer_announcements::RemoteAnnouncement>>,
    Vec<ezer_config_types::RemoteRequestEncoding>,
);
...
impl MvpAgent {
    ...
    ) -> (bool, ezer_agent::plugins::discovery::DiscoveryConfig) {
    ...
    pub(super) fn ensure_telemetry_client(&self) {
        ezer_login::credential_provider::sync_external_otel_identity();
        let cfg = self.cfg.borrow();
        ...
        if !mode.is_disabled() {
            let Some(auth) = self
                ...
                .current()
                .filter(|a| {
                    a.is_xai_auth() || a.auth_mode == ezer_login::AuthMode::ApiKey
                }) else {
                return;
            };
            ...
            ezer_telemetry::client::init_if_needed(
                cfg.telemetry.clone(),
                mode,
                user_id,
                team_id,
    ...
    pub(crate) fn plugin_registry_handle(
        &self,
    ) -> &ezer_agent::plugins::SharedPluginRegistryHandle {
    ...
    fn arm_local_workspace_watcher(
        ...
        let auth_path = ezer_workspace::hub_auth::default_auth_path().ok();
    ...
    fn ensure_local_workspace_ops(
        &self,
    ) -> Result<ezer_workspace::WorkspaceOps, acp::Error> {
        ...
        let workspace_identity = self
            ...
            .map(|a| match a.team_id.filter(|t| !t.is_empty()) {
                Some(team) => {
                    ezer_workspace::WorkspaceIdentity::team(a.user_id, team)
                }
                None => {
                    ezer_workspace::WorkspaceIdentity::new(
                        a.user_id,
                        a.principal_type,
                        a.principal_id,
                    )
            ...
            .unwrap_or_default();
        let ops = match ezer_workspace::handle::WorkspaceHandle::new_minimal(
            cwd.to_path_buf(),
            workspace_identity,
            project_lsp_trusted,
        ) {
            Ok(handle) => ezer_workspace::WorkspaceOps::local(handle),
            Err(e) => {
                tracing::error!(
                    error = %e,
    ...
    pub(super) fn prepare_image_gen_config(
        ...
        let version = cfg
            .client_version
            .clone()
            .unwrap_or_else(|| ezer_version::VERSION.to_string());
        let alpha_test_key = cfg.endpoints.alpha_test_key.clone();
        let mut headers = indexmap::IndexMap::new();
        headers.insert("user-agent".to_string(), format!("ezer-build/{version}"));
    ...
    pub fn with_models(
        ...
        let instance = Self {
            ...
            plugin_registry_handle: ezer_agent::plugins::SharedPluginRegistryHandle::new(
                None,
        ...
        if let Some(ref dk) = instance.cfg.borrow().endpoints.deployment_key {
            ezer_login::credential_provider::wire_otel_deployment_key(dk.clone());
        }
        instance
    }
    ...
    pub(crate) async fn kill_background_task(
        ...
        session_id: &str,
        task_id: &str,
        source: ezer_tools::types::KillSource,
    ) -> Result<ezer_tools::types::KillOutcome, String> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    pub async fn list_tasks(
        &self,
        session_id: &str,
    ) -> Option<Vec<ezer_tools::types::TaskSnapshot>> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    pub(crate) async fn execute_hooks_action(
            ...
            && let Some(cwd) = self.get_session_cwd(session_id)
        {
            self.interactive_trust_prompted
                .borrow_mut()
                .remove(&ezer_workspace::trust::workspace_key(&cwd));
        }
        let handle = self.get_session_handle(session_id)?;
    ...
    pub(super) fn diagnostic_upload_config(
        &self,
    ) -> Option<ezer_login::DiagnosticUploader> {
        self.sync_collection_config_gate();
        let cfg = self.cfg.borrow();
    ...
    pub(super) async fn upload_harness_trace_turns(
        ...
        model: &str,
        turns: Vec<Vec<ezer_sampling_types::conversation::ConversationItem>>,
    ) {
    ...
    pub(super) async fn build_harness_trace_uploads(
        ...
        for (offset, items) in turns.into_iter().enumerate() {
            ...
            let metadata = PromptMetadata::new(PromptMetadataParams {
                ...
                shell_version: Some(ezer_version::VERSION.to_string()),
    ...
    pub(crate) async fn get_trace_context(
        ...
        let upload_method = match upload_method {
            Some(method) => method,
            None => {
                ezer_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::TraceUploadSkipped {
    ...
    /// Strict-harness classification is structural; see [`ezer_agent::config::is_strict_harness_agent_type`]. Harness inheritance for a profile that pins its own model is applied by the caller via [`inherited_harness_template`], not here.
    pub fn resolve_agent_definition(
        cwd: &std::path::Path,
        agent_profile_path: Option<&std::path::Path>,
        agent_config: &config::AgentSelectionConfig,
        acp_agent_profile: Option<ezer_agent::AgentDefinition>,
        ...
        if let Some(ref name) = agent_config.name {
            ...
            if let Some(def) = ezer_agent::discovery::by_name_in_cwd(name, cwd) {
                return def;
            }
            tracing::warn!(
                agent_name = %name,
            ...
            && resolved.name != required
        {
            ...
            if let Some(def) = ezer_agent::discovery::by_name_in_cwd(required, cwd) {
                return def;
            }
            tracing::warn!(
                model_agent_type = %required,
    ...
    pub(super) fn resolve_status_line_capability(
        ...
        Self::resolve_bool_capability(
            meta,
            init,
            ezer_status_line::CLIENT_STATUS_LINE_META,
            ezer_status_line::STATUS_LINE_CAPABILITY,
            false,
        )
    }
    ...
    pub(super) async fn spawn_and_register_session(
        ...
        let envrc = envrc
            .unwrap_or_else(|| ezer_workspace::envrc::spawn_envrc_load(
        ...
        let fs: Arc<dyn ezer_workspace::file_system::AsyncFileSystem> = if use_acp_fs {
        ...
        let mut session_env = ezer_workspace::permission::claude_settings::load_claude_env_with_project(
            cwd.as_path(),
            project_env_trusted,
        );
        ...
        let pinned_model: Option<(acp::ModelId, ModelEntry)> = match &agent_definition
            .model
        {
            ezer_agent::config::ModelOverride::Override(id) => {
            ...
            ezer_agent::config::ModelOverride::Inherit => None,
        };
        ...
        if lsp_tools_enabled && tool_ctx.lsp.is_none() {
            ...
            let (
                ...
                .unzip();
            let sourced = ezer_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        ...
        let (init, session_thread) = {
            ...
            let attribution_callback: Option<
                ezer_sampler::SharedAttributionCallback,
            > = Some(
                ezer_login::attribution::ShellAttribution::new(
                    self.auth_manager.clone(),
            ...
            let agent_hook_registry_override = agent_definition
                ...
                .and_then(|hooks_config| {
                    ...
                    let (specs, errors) = ezer_hooks::config::parse_hooks_from_value_with_dir(
                        &hooks_val,
                        &format!(
                        "{}{}",
                        ezer_hooks::config::AGENT_HOOK_PREFIX,
                    ...
                    let git_root = {
                        ...
                        ezer_workspace::session::git::find_git_root_from_path(cwd)
                            .ok()
                    };
            ...
            spawn_session_on_thread(
                    ...
                    subagent_toggle,
                    Vec::new(),
                    ezer_agent::prompt::context::PromptAudience::Primary,
                    None,
                    None,
                    disable_web_search,
                    ...
                    Some(
                        Arc::new(
                            ezer_login::manager::SharedAuthKeyProvider(
                                self.auth_manager.clone(),
                            ),
                        ),
                    ),
