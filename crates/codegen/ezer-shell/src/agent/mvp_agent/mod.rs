...
use ezer_login::backend::AuthBackend as _;
use crate::config::StorageMode;
use crate::extensions::notification::{SessionNotification, SessionUpdate};
use ezer_telemetry::id::{agent_id, agent_instance_id};
use ezer_telemetry::session_ctx::log_event;
use ezer_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use ezer_workspace::permission::{ClientType, PermissionEvent};
...
pub(crate) fn resolve_subscription_tier_for_telemetry(
    ...
    let auth = auth?;
    if auth.auth_mode == ezer_login::AuthMode::ApiKey {
...
pub(crate) struct SessionSpawnOptions<'a> {
    ...
    pub envrc: Option<ezer_workspace::envrc::EnvrcLoad>,
    ...
    pub persisted_agent_profile: Option<ezer_agent::AgentDefinition>,
...
pub(crate) struct PromptResponseMeta {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub tool_overrides: Option<ezer_sampling_types::ToolOverrides>,
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a ezer_sampling_types::TokenUsage>,
...
fn announcements_push_payload(
    stored: Option<&[ezer_announcements::RemoteAnnouncement]>,
    last_emitted: &[ezer_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<ezer_announcements::RemoteAnnouncement>> {
    let current = ezer_announcements::filter_expired_at(
...
pub struct MvpAgent {
    ...
    pub(crate) plugin_registry_handle: ezer_agent::plugins::SharedPluginRegistryHandle,
    ...
    workspace_ops: RefCell<Option<ezer_workspace::WorkspaceOps>>,
...
pub(crate) fn inherited_harness_template(
    current: &ezer_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<ezer_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !ezer_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = ezer_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use ezer_agent::config::is_strict_harness_agent_type;
    match (
...
impl AuthRequestMeta {
    ...
    fn login_override(&self) -> ezer_login::LoginTransportOverride {
        if self.use_oauth {
            ezer_login::LoginTransportOverride::ForceLoopback
        } else {
            ezer_login::LoginTransportOverride::None
        }
    }
...
impl MvpAgent {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        for task in &orphaned {
            let snapshot = ezer_tools::types::TaskSnapshot {
                ...
                exit_code: None,
                signal: Some(
                    ezer_tools::computer::types::SESSION_RESTART_SIGNAL.to_string(),
                ),
                completed: true,
                kind: ezer_tools::computer::types::TaskKind::Bash,
                block_waited: false,
            ...
            let refresh_ok = if claim_already_current {
                ...
                match self
                        ...
                        .refresh_chain_bounded(
                            ezer_login::token_type::TokenType::OidcSession,
                            ezer_login::manager::RefreshReason::ServerRejected,
                            ezer_login::manager::BEST_EFFORT_REFRESH_TIMEOUT,
                        )
                        .await
                    {
                        Ok(_) => {
    ...
    pub(crate) fn auth_response_with_meta(&self) -> AuthenticateResponse {
        let (show_resolved_model, gate, subscription_tier) = {
            ...
            let gate = rs
                ...
                .filter(|m| !m.is_empty())
                .map(|message| ezer_login::GateInfo {
        ...
        let meta = self
            ...
            .map(|auth| {
                ...
                let auth_meta = ezer_login::AuthMeta {
                    ...
                    backend_billed: !ezer_login::backend::ActiveAuthBackend::default()
                        .is_xai_authority(),
                };
