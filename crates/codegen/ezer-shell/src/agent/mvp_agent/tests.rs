...
fn resolve_subscription_tier_prefers_display_then_api_key_then_jwt() {
    ...
    let api = auth_with_mode(ezer_login::AuthMode::ApiKey, "xai-not-a-jwt");
    assert_eq!(
    ...
    let oauth = auth_with_mode(ezer_login::AuthMode::Oidc, &jwt_with_tier(0));
    assert_eq!(
...
fn harness_pair(id: &str) -> Vec<ezer_sampling_types::conversation::ConversationItem> {
    use ezer_sampling_types::ToolCall;
...
fn inherited_harness_template_skips_nonstrict_model() {
    use ezer_agent::prompt::user_message::UserMessageTemplate;
    let tmp = tempfile::tempdir().unwrap();
    assert!(
...
fn resolve_agent_definition_agent_profile_with_model_override() {
    ...
    assert_eq!(
        def.model,
        ezer_agent::config::ModelOverride::Override("test-model-123".to_string()),
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    use ezer_tools::computer::local::{LocalFs, LocalTerminalBackend};
    ...
    let builder = ezer_tools::registry::types::ToolRegistryBuilder::new();
...
fn make_test_handle(
    ...
    crate::session::SessionHandle {
        ...
        tool_context: crate::tools::ToolContext::new_local_context(
            ezer_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap(),
            std::sync::Arc::new(ezer_workspace::file_system::LocalFs::new(
                std::path::PathBuf::from("/tmp"),
            )),
...
async fn model_state_prefers_session_reasoning_effort_over_global() {
    ...
    use ezer_sampling_types::{REASONING_EFFORT_META_KEY, ReasoningEffort};
    let agent = build_minimal_agent_for_tests();
...
async fn restore_effort_via_load(
    initial: Option<ezer_sampling_types::ReasoningEffort>,
    persisted: Option<ezer_sampling_types::ReasoningEffort>,
) -> Option<ezer_sampling_types::ReasoningEffort> {
    use crate::agent::config::{EndpointsConfig, ModelEntry};
    let agent = build_minimal_agent_for_tests();
    let mut entry = ModelEntry::fallback("effort-model", &EndpointsConfig::default());
    entry.info.supports_reasoning_effort = true;
...
struct ReloadHarness {
    _env: Vec<ezer_test_support::EnvGuard>,
...
fn chat_session_spawn_options_matches_thin_profile() {
    ...
    let cwd = ezer_paths::AbsPathBuf::new(std::env::temp_dir()).expect("temp cwd");
...
fn workspace_session_count(agent: &MvpAgent) -> usize {
    agent
        ...
        .borrow()
        .as_ref()
        .and_then(ezer_workspace::WorkspaceOps::workspace_handle)
        .map_or(
            0,
            ezer_workspace::handle::WorkspaceHandle::session_count,
        )
}
...
fn workspace_toolset(
    agent: &MvpAgent,
    sid: &acp::SessionId,
) -> std::sync::Arc<ezer_tools::registry::types::FinalizedToolset> {
    agent
        .workspace_ops
        .borrow()
        .as_ref()
...
fn new_session_records_setup_phases_for_bisection() {
    ...
    run_local_for_bridge_test(|| async {
        let agent = build_minimal_agent_for_tests();
        let cwd = tempfile::tempdir().unwrap();
        let mark = ezer_telemetry::unified_log::snapshot_log()
            .unwrap_or_default()
            .len();
        let sid = new_root_session(&agent, cwd.path()).await;
        agent.remove_session(&sid);
        let log = ezer_telemetry::unified_log::snapshot_log().unwrap_or_default();
    ...
    run_local_for_bridge_test(|| async {
        ...
        let rebind = async {
            ...
            let newer_toolset = std::sync::Arc::new(
                ezer_tools::registry::types::FinalizedToolset::empty_for_test(),
            );
            let rebound = agent
                .workspace_ops
                .borrow()
...
async fn spawn_seeds_root_conversation_group_in_turn_config() {
    ...
    local
        .run_until(async {
            ...
            let cwd = ezer_paths::AbsPathBuf::new(temp_dir.path().to_path_buf())
...
async fn remove_session_releases_workspace_binding_and_side_maps() {
    ...
    let ops = ezer_workspace::WorkspaceOps::for_test();
    let toolset =
        std::sync::Arc::new(ezer_tools::registry::types::FinalizedToolset::empty_for_test());
    ...
    *agent.workspace_ops.borrow_mut() = Some(ops);
    ...
    let (_permission_tx, permission_rx) =
        tokio::sync::mpsc::unbounded_channel::<ezer_workspace::permission::PermissionEvent>();
    agent
        .session_registry
...
async fn build_hydration_agent(
    ...
    let auth_manager = std::sync::Arc::new(
        AuthManager::new(temp_dir.path(), EzerComConfig::default()).with_proxy_base_url(proxy_url),
    );
...
fn team_auth() -> ezer_login::EzerAuth {
    ezer_login::EzerAuth {
        oidc_issuer: Some(ezer_login::XAI_OAUTH2_ISSUER.to_string()),
        email: Some(USER_A.into()),
        principal_type: Some(ezer_login::model::TEAM_PRINCIPAL_TYPE.into()),
...
async fn hydration_delayed_past_an_opt_out_keeps_the_opt_out() {
    let server = ezer_test_support::MockInferenceServer::start()
        .await
        .unwrap();
    server.set_user_can_administer_team(ezer_test_support::MockCanAdministerTeam::Denied);
    ...
    let disk = ezer_login::storage::read_auth_json(agent.auth_manager.auth_json_path())
        .unwrap()
        .into_values()
        .next()
        .unwrap();
...
async fn hydration_is_bound_to_the_account_that_asked() {
    ...
    let switch_then_release = async {
        server.user_info_arrived(1).await;
        agent.auth_manager.hot_swap(ezer_login::EzerAuth {
            key: "key-b".into(),
...
async fn hydration_merges_only_into_the_same_unresolved_principal() {
    let rotated = || ezer_login::EzerAuth {
...
async fn unresolved_or_failed_hydration_changes_nothing() {
    let opted_out = || ezer_login::EzerAuth {
        coding_data_retention_opt_out: true,
        ..team_auth()
    };
    ...
    let server = ezer_test_support::MockInferenceServer::start()
        .await
        .unwrap();
    server.set_user_can_administer_team(ezer_test_support::MockCanAdministerTeam::Unresolved);
    server.set_user_coding_data_retention_opt_out(Some(false));
    let (agent, _home) = build_hydration_agent(&server.url(), opted_out()).await;
    assert_eq!(hydrate(&agent, USER_A).await, None);
    assert_eq!(untouched(&agent), (None, true));
    assert_eq!(server.request_count_for("/v1/user"), 1);
    server.set_user_can_administer_team(ezer_test_support::MockCanAdministerTeam::Omitted);
    assert_eq!(hydrate(&agent, USER_A).await, None);
    assert_eq!(untouched(&agent), (None, true));
    assert_eq!(server.request_count_for("/v1/user"), 2);
...
impl Drop for RestoreOtelGate {
    fn drop(&mut self) {
        ezer_telemetry::external::mark_external_otel_settings_resolved();
    }
}
...
async fn post_auth_settings_xai_upgrades_writeback_emits_and_opens_gate() {
    ...
    assert!(!ezer_telemetry::external::is_settings_gate_open());
    agent.maybe_fetch_post_auth_settings().await;
    assert_eq!(
        agent.storage_mode(),
    ...
    assert!(
        ezer_telemetry::external::is_settings_gate_open(),
    ...
    run_local_for_bridge_test(|| async {
        ...
        agent.insert_resident(&sid, handle);
        {
            ...
            cfg.subagent_roles.insert(
                "refreshed".into(),
                ezer_subagent_resolution::config::SubagentRole {
...
fn project_roles_personas_gated_via_resolve_and_record_chain() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        ezer_workspace::folder_trust::grant_folder_trust(repo.path());
        let allowed = crate::agent::folder_trust::resolve_and_record(
            repo.path(),
            Some(&folder_trust_on()),
            false,
...
fn ann(id: &str) -> ezer_announcements::RemoteAnnouncement {
    ezer_announcements::RemoteAnnouncement {
...
fn settings_with(
    announcements: Option<Vec<ezer_announcements::RemoteAnnouncement>>,
) -> crate::util::config::RemoteSettings {
    crate::util::config::RemoteSettings {
        announcements,
        ..Default::default()
...
fn announcements_push_gate_ignores_expired_only_addition() {
    let now = test_now();
    let expired = ezer_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_emits_on_expiry_crossing() {
    let expiring = ezer_announcements::RemoteAnnouncement {
...
async fn polled_settings_apply_refreshes_accept_request_encodings() {
    use ezer_config_types::RemoteRequestEncoding;
    use ezer_sampler::RequestCompression;
...
fn status_line_meta(enabled: bool) -> acp::Meta {
    let mut meta = acp::Meta::new();
    meta.insert(
        ezer_status_line::CLIENT_STATUS_LINE_META.to_string(),
...
fn init_advertising_status_line(enabled: bool) -> acp::InitializeRequest {
    let mut meta = serde_json::Map::new();
    meta.insert(
        ezer_status_line::STATUS_LINE_CAPABILITY.to_string(),
        serde_json::json!(enabled),
    );
    acp::InitializeRequest::new(acp::ProtocolVersion::V1).client_capabilities(
        acp::ClientCapabilities::new()
