...
use ezer_telemetry::instrument_task;
use ezer_telemetry::region;
use ezer_telemetry::region::Parent;
use ezer_telemetry::startup;
use tracing::Instrument;
use ezer_login::{CachedTokenState, SilentRefresh};
...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        if let Some(meta) = arguments.meta.as_ref() {
            ezer_otel::link_current_span_to_meta(
                &serde_json::Value::Object(meta.clone()),
            );
        }
        tracing::debug!(target: "sampling_log", "Received initialize request");
        ezer_telemetry::unified_log::info("agent initialized", None, None);
        ...
        CLEANUP_PERMISSIONS_ONCE
            .call_once(|| {
                tokio::task::spawn(
                    ezer_workspace::permission::cleanup_stale_permission_state(
        ...
        ezer_workspace::trust::migrate_legacy_hook_trust();
        if let Some(auth) = self.auth_manager.current() {
            ...
            ezer_telemetry::unified_log::info(
                ...
                Some(
                    ...
                    "key_prefix": ezer_auth::bearer_suffix(&auth.key),
                    "rt_prefix": auth.refresh_token.as_deref().map(ezer_auth::bearer_suffix),
                }),
                ),
            );
            if needs_user_info {
        ...
        *self.buffering_settings.borrow_mut() = buffering_settings;
        ...
        let pre = self
            ...
            .current()
            .map(|a| (
                ezer_auth::bearer_suffix(&a.key).to_owned(),
                a
                    .refresh_token
                    .as_deref()
                    .map(|t| ezer_auth::bearer_suffix(t).to_owned()),
            ));
        {
            let _t = ezer_telemetry::instrumentation::timer(
        ...
        {
            ezer_login::auth_method::set_runtime_xai_api_key(&api_key);
        ...
        let first_party_env_ok = if ezer_login::should_probe_first_party_env_key(
            disable_api_key_auth,
            has_byok,
            auth_method::has_xai_api_key_env(),
            preferred_method_early.is_some(),
        ) {
            ezer_login::first_party_env_key_allows_advertise(
                    &xai_api_base_url,
                    ezer_login::DEFAULT_PROBE_TIMEOUT,
                )
                .await
        } else {
            true
        ...
        let has_external_api_key = match preferred_method {
            Some(ezer_login::PreferredAuthMethod::Oidc) => false,
            _ => has_external_api_key,
        };
        let has_cached_token = match preferred_method {
            Some(ezer_login::PreferredAuthMethod::ApiKey) => false,
            _ => has_cached_token,
        };
        let built = {
            let _t = ezer_telemetry::instrumentation::timer(
    ...
    async fn authenticate(
        ...
        if let Some(preferred) = self.cfg.borrow().ezer_com_config.preferred_method {
            ...
            let allowed = match preferred {
                ...
                ezer_login::PreferredAuthMethod::Oidc => kind.is_session_based(),
            };
            if !allowed {
                let msg = match preferred {
                    ezer_login::PreferredAuthMethod::ApiKey => {
                        auth_method::PREFERRED_API_KEY_UNAVAILABLE
                    }
                    ezer_login::PreferredAuthMethod::Oidc => {
        ...
        match arguments.method_id.0.as_ref() {
            auth_method::XAI_API_KEY_METHOD_ID => {
                ...
                log_event(ezer_telemetry::events::Login {
            ...
            auth_method::CACHED_TOKEN_AUTH_METHOD_ID => {
                ...
                let is_legacy = current_auth
                    .as_ref()
                    .is_some_and(|a| a.auth_mode == ezer_login::AuthMode::WebLogin);
                ...
                if auth.auth_mode == ezer_login::AuthMode::WebLogin {
                    ...
                    if let Err(e) = self
                        .auth_manager
                        .remove_scope(ezer_login::LEGACY_AUTH_SCOPE)
                    {
            ...
            auth_method::EZER_COM_METHOD_ID | auth_method::OIDC_METHOD_ID => {
                ...
                let auth_result = if !auth_meta.headless {
                    ...
                    let (cancel, _guard) = self
                        ...
                        .begin(
                            Some(
                                ezer_login::single_flight::AttemptChannels::new(
                                    code_tx,
                                    url_rx,
                                ),
                            ),
    ...
    async fn prompt(
            ...
            .and_then(|m| m.get("toolOverrides"))
        {
            None => None,
            Some(value) => {
                match ezer_sampling_types::ToolOverridesUpdate::parse(value) {
        ...
        let traceparent = ezer_otel::current_traceparent();
    ...
    async fn ext_method(
        ...
        if let Some(meta) = &request_meta {
            ezer_otel::link_current_span_to_meta(meta);
        }
        ...
        let result = match method.as_ref() {
            ...
            "ezer/cloud/terminate" => {
                crate::extensions::auth_gate::require_xai_auth(
                    ...
                    "Run `ezer login` to authenticate.",
                )?;
    ...
    async fn ext_notification(
        ...
        if args.method.as_ref() == "ezer/telemetry/non_git_decision" {
            ...
            >(args.params.get()) {
                ...
                ezer_telemetry::session_ctx::log_event(ezer_telemetry::events::NonGitDecisionEvent {
                    decision: params.decision,
                    session_id: params.session_id,
                    client_version: params.client_version,
                });
        ...
        if args.method.as_ref() == "ezer/telemetry/multi_agent_followup" {
            ...
            >(args.params.get()) {
                ...
                ezer_telemetry::session_ctx::log_event(ezer_telemetry::events::MultiAgentFollowup {
                    ...
                    other_agents: params
                        .other_agents
                        .into_iter()
                        .map(|(l, s, m)| ezer_telemetry::events::AgentInfo {
                            label: l.to_string(),
                            session_id: s,
                            model_id: m,
                        })
        ...
        if args.method.as_ref() == "ezer/telemetry/multi_agent_apply" {
            ...
            >(args.params.get()) {
                ...
                ezer_telemetry::session_ctx::log_event(ezer_telemetry::events::MultiAgentApply {
        ...
        if args.method.as_ref() == "ezer/telemetry/multi_agent_discard" {
            ...
            >(args.params.get()) {
                ...
                ezer_telemetry::session_ctx::log_event(ezer_telemetry::events::MultiAgentDiscard {
                    discarded_agents: params
                        .discarded_agents
                        .into_iter()
                        .map(|(l, s, m)| ezer_telemetry::events::AgentInfo {
        ...
        if args.method.as_ref() == ezer_telemetry::unified_log::LOG_METHOD
            && let Ok(params) = serde_json::from_str::<
                ezer_telemetry::unified_log::LogNotificationParams,
            >(args.params.get())
        {
