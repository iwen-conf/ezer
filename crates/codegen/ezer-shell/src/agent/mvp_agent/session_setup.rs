...
fn insert_applied_tool_overrides(
    ...
    echo: Option<&ezer_sampling_types::ToolOverrides>,
) {
...
struct RestoredSignals {
    ...
    turn_count: u64,
    tool_call_count: u64,
    plan_mode_state: ezer_telemetry::events::PlanModeState,
...
impl RestoredSignals {
    fn read(
        ...
        use ezer_telemetry::events::PlanModeState as Reported;
        Self {
...
impl AttachPolicy {
    pub(super) fn resolve(
        ...
        let explicit_restore_code = meta
            .and_then(|m| m.get("ezer/restore_code"))
...
/// Client-supplied routing an attach's replay must echo: the `ezer/persist` blob, the leader unicast target, and the reconnect cursor.
...
/// Wire method for the setup-progress notification, mirroring `ezer/mcp/init_progress`.
const SESSION_SETUP_METHOD: &str = "ezer/session/setup";
...
impl MvpAgent {
    ...
    async fn new_session_inner_impl(
        ...
        let spawn_res = {
            ...
            spawn_timer.with_subphase(ezer_telemetry::startup::Subphase::SessionSpawn);
            let spawn_trace = ezer_telemetry::startup::SpawnTraceContext::new(
        ...
        if product_analytics || ezer_telemetry::external::is_active() {
            ...
            tokio::spawn(async move {
                let git = ezer_telemetry::context::collect_git_context(&cwd_str);
                let ev = ezer_telemetry::events::SessionNew {
                    session_id: sid,
                ...
                ezer_telemetry::session_ctx::log_event_dual(product_analytics, ev);
            });
        }
        ...
        let (git_root, is_git_repo, discovery_failed) =
            match ezer_workspace::session::git::discover_git_root(cwd.as_path()) {
    ...
    async fn attach_session(
        ...
        let git_root =
            ezer_workspace::session::git::find_git_root_from_path(cwd.as_path()).ok();
        ...
        persistence_timer.with_subphase(ezer_telemetry::startup::Subphase::SessionLoad);
        ...
        let load_envrc = {
            let skip_envrc = request_meta
                .as_ref()
                .and_then(|m| m.get("ezer/skip_envrc"))
                .and_then(|v| v.as_bool())
                .unwrap_or(false);
            if skip_envrc {
                false
        ...
        let envrc = if !load_envrc {
            Some(ezer_workspace::envrc::spawn_envrc_load(
        ...
        let prompt_display_cwd = request_meta
            .as_ref()
            .and_then(|m| m.get("ezer/display_cwd"))
            .and_then(|v| v.as_str())
            .map(|s| s.to_string())
        ...
        let restored_from_disk = if !self.is_resident(&session_id) {
            ...
            let (restore_profile, restore_model_agent_type): (
                Option<ezer_agent::AgentDefinition>,
                Option<String>,
            ...
            let load_is_current = self
                .spawn_and_register_session(
                    ...
                    Some(ezer_telemetry::startup::SpawnTraceContext::new(
                        spawn_timer.subphase_span(),
                        tracing::Span::current(),
                    )),
                )
        ...
        let (model_state, response_meta) = self
            ...
            .await;
        ezer_telemetry::unified_log::info("session loaded", Some(session_id.0.as_ref()), None);
        ...
        if self.product_analytics_enabled() {
            log_event(ezer_telemetry::events::SessionLoad {
    ...
    async fn restore_session_code(
        ...
        if restore_code_requested && registry_client_for_restore.is_none() {
            ezer_workspace::session::git::warn_registry_disabled_restore(session_id.0.as_ref());
        }
        let restore_checkout_allowed =
            ezer_workspace::session::git::restore_code_checkout_allowed(
                cwd.as_path(),
        ...
        {
            tracing::warn!(
                target: ezer_workspace::session::git::RESTORE_CODE_LOG,
                session_id = %session_id.0,
        ...
        if restore_code_requested
            ...
            && let Some(ref target_sha) = summary.head_commit
        {
            use ezer_workspace::session::git::RestoreKind;
            let outcome = ezer_workspace::session::git::checkout_session_commit(
                cwd.as_path(),
                target_sha,
                true,
                session_id.0.as_ref(),
    ...
    async fn build_attach_response_meta(
        ...
        if let Some(persist) = persist_data {
            response_meta_map.insert("ezer/persist".to_string(), persist);
        }
        ...
        if summary.head_commit.is_some()
            ...
            && summary.git_root_dir.as_deref().is_none_or(|root| {
                ezer_workspace::session::git::find_git_root_from_path(std::path::Path::new(
                    cwd.as_str(),
                ))
                .ok()
            ...
            })
        {
            ...
            git_scan_timer.with_subphase(ezer_telemetry::startup::Subphase::SessionGitScan);
            git_scan_timer.with_field("git_op", "rev_parse_head");
            let cwd_path = std::path::Path::new(cwd.as_str());
            let current_head =
                ezer_workspace::session::git::git_cli(cwd_path, &["rev-parse", "HEAD"])
                    .await
                    .ok();
            if let Some(divergence) = ezer_workspace::session::git::detect_head_divergence(
        ...
        {
            response_meta_map.insert(
                "ezer/runningPromptId".to_string(),
