//! The interactive folder-trust prompt, an ACP round-trip (`ezer/folder_trust/request`) from the agent to a GUI client (ezer-desktop).
...
//!
//! Dormant in production: it fires only when the connected client advertised `ezer/folderTrust.interactive`.
//! The folder-trust feature flag must also be on, and the verdict must be [`ezer_workspace::folder_trust::TrustOutcome::Prompt`].
...
/// ACP `ezer/folder_trust/request` payload, sent by the agent to the GUI client.
...
/// ACP `ezer/folder_trust/request` response, sent by the GUI client to the agent.
...
impl MvpAgent {
    /// Parse the `ezer/folderTrust.interactive` capability from an initialize request.
    ...
    pub(crate) fn parse_interactive_trust_capability(init: &acp::InitializeRequest) -> bool {
        init.client_capabilities
            .meta
            .as_ref()
            .and_then(|m| m.get("ezer/folderTrust"))
    ...
    /// Dormant: it does nothing unless the client advertised `ezer/folderTrust.interactive` and [`folder_trust::prompt_warranted`] holds.
    ...
    pub(crate) fn maybe_spawn_interactive_trust_prompt(
        ...
        let key = ezer_workspace::trust::workspace_key(cwd);
        ...
        self.session_registry.for_each_resident(|_, h| {
            if ezer_workspace::trust::workspace_key(std::path::Path::new(&h.info.cwd)) == key {
        ...
        tokio::task::spawn_local(async move {
            ...
            let ext_request = acp::ExtRequest::new("ezer/folder_trust/request", raw_params.into());
            ...
            let outcome = ezer_workspace::folder_trust::grant_folder_trust_key(&key);
            if !outcome.dismisses_gate() {
                match &outcome {
                    ezer_workspace::folder_trust::GrantOutcome::Granted {
                        persist:
                            ezer_workspace::folder_trust::PersistStatus::ProcessLocalOnly { error },
                        ..
                    } => {
                    ...
                    ezer_workspace::folder_trust::GrantOutcome::Refused { reason } => {
            ...
            if ezer_workspace::trust::workspace_key(&cwd) != key {
                tracing::info!(
                    cwd = %cwd.display(),
...
struct ReloadAfterGrant<'a> {
    ...
    plugin_handle: &'a ezer_agent::plugins::SharedPluginRegistryHandle,
    compat: &'a ezer_tools::types::CompatConfig,
...
async fn reload_project_servers_after_grant(ctx: ReloadAfterGrant<'_>) {
    ...
    for target in ctx.targets {
        ...
        // This reloads the session's own project hooks (`.ezer/hooks`, `.cursor/hooks.json`), which `ReloadPlugins` does not touch
