...
async fn subagent_spawn_context_inherits_parent_permission_handle() {
    ...
    local
        .run_until(async {
            ...
            let cwd = ezer_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp"))
                .expect("absolute cwd");
            let (permission_handle, _events_rx) = ezer_workspace::permission::spawn_permission_manager(
                sid.clone(),
                gateway,
                cwd,
                ezer_workspace::permission::types::ClientType::Generic,
                Some(
            ...
            for access in [
                ezer_workspace::permission::AccessKind::Read(Some(".env".into())),
                ezer_workspace::permission::AccessKind::Bash("cat .env".into()),
            ] {
                let decision = inherited
                    .request(ezer_workspace::permission::PermissionRequest {
                        ...
                        ..ezer_workspace::permission::PermissionRequest::new(
                            access.clone(),
                ...
                assert!(
                    matches!(
                        decision,
                        ezer_workspace::permission::Decision::PolicyDeny(_)
                    ),
...
async fn subagent_spawn_context_inherits_parent_configured_cutoff() {
    ...
    let cutoff = ezer_sampling_types::ToolOverrides {
        x_search: Some(ezer_sampling_types::XSearchOptions {
            date_bound: Some(
                ezer_sampling_types::SearchDateBound::new(None, Some("2020-01-01".to_string()))
                    .unwrap(),
            ),
        }),
        web_search: None,
