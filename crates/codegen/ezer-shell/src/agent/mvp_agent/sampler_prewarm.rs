...
fn prewarm_and_record(base_url: String) -> impl std::future::Future<Output = ()> {
    ...
    async move {
        ...
        let report = ezer_sampler::prewarm_transport(&base_url).await;
...
fn record_prewarm_report(span: &tracing::Span, report: &ezer_sampler::PrewarmReport) {
...
mod tests {
    ...
    async fn prewarm_telemetry() {
        ...
        record_prewarm_report(
            &span,
            &ezer_sampler::PrewarmReport {
                outcome: ezer_sampler::PrewarmOutcome::Warmed,
                duration_ms: 7,
