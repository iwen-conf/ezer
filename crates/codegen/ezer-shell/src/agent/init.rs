...
pub fn bootstrap_with_cancel(
    ...
    ezer_telemetry::startup::enter(ezer_telemetry::startup::StartupPhase::Bootstrap);
    ...
    let pre_gate_prefetch = {
        ...
        timer.with_subphase(ezer_telemetry::startup::Subphase::RemoteSettings);
    ...
    let cfg = {
        ...
        timer.with_subphase(ezer_telemetry::startup::Subphase::ResolveConfig);
    ...
    ensure_bootstrap_not_cancelled(cancel)?;
    {
        ...
        timer.with_subphase(ezer_telemetry::startup::Subphase::InitProcess);
        init_process(&cfg, auth_manager);
    }
    ensure_bootstrap_not_cancelled(cancel)?;
    ezer_telemetry::startup::enter(ezer_telemetry::startup::StartupPhase::ModelCatalog);
    let models_manager = {
        let mut timer = crate::instrumentation_timer!("startup.model_catalog.models_manager");
        timer.with_subphase(ezer_telemetry::startup::Subphase::ModelsManager);
...
fn init_process(cfg: &AgentConfig, auth_manager: &AuthManager) {
    ...
    INIT.call_once(|| {
        ...
        ezer_telemetry::session_ctx::log_event(limits.into_event());
    });
}
...
/// Assemble the default OTel layer config both `ezer-pager` and `ezer-tui` need at tracing init time.
///
/// Owns the endpoint and exporter assembly here in shell; the bootstrap credential provider comes from auth.
pub fn build_default_otel_layer_config() -> ezer_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let (credentials, token_header_value) =
        crate::credential_factory::build_bootstrap_otel_credentials();
    let exporter = ezer_telemetry::otel_layer::OtelExporterConfig {
    ...
    ezer_telemetry::otel_layer::OtelLayerConfig {
        credentials,
        token_header_value,
        alpha_test_key: None,
        exporter,
...
pub fn run_cli_logout(ezer_com_config: &ezer_login::EzerComConfig) -> anyhow::Result<()> {
    ...
    let result =
        ezer_login::perform_logout(&auth_manager, None, crate::managed_config::clear_orphan)
