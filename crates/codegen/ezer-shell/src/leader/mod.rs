...
//!
//! ```ignore
//! use ezer_shell::leader::{connect_or_spawn, ClientCapabilities, ClientMode};
//!
...
const CLIENT_LEADER_VERSION: &str = ezer_version::VERSION;
                ...
                Ok(()) => {
                    ...
                    crate::unified_log::warn(
                        ...
                        Some(serde_json::json!({
                            ...
                            "reason": reason,
                            "killer_ver": ezer_version::VERSION,
                        })),
                    );
                }
                Err(e) => {
...
pub async fn connect_or_spawn(
    ...
    if let Some(profile) = ezer_sandbox::requested_confinement_profile() {
...
const LEADER_DEFAULT_LOG_DIRECTIVES: &str = "ezer_shell=info,ezer_gateway=info,ezer_login=info,xai_acp_lib=warn,ezer_mcp=warn";
...
mod tests {
    use super::*;
    use std::fs;
    /// Bridge diagnostics moved to `ezer_gateway`; the leader fallback filter must allowlist it.
    #[test]
    fn leader_default_log_directives_allowlist_gateway_target() {
        assert!(LEADER_DEFAULT_LOG_DIRECTIVES.contains("ezer_gateway=info"));
    }
