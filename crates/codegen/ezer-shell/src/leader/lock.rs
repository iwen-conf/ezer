...
use ezer_file_lock::{DEFAULT_SLOT_GRACE, LockOptions, LockedFile, SlotPolicy, lock_file};
pub use ezer_file_lock::{LockError, SLOT_DIR_ENV};
...
/// Every acquisition runs behind the machine-local acquire slot of `ezer_file_lock`, so a ezer home on a stalled
...
mod tests {
    ...
    fn hold_slot(slot_dir: &Path, lock_path: &Path) -> File {
        let mut slot = File::create(ezer_file_lock::slot_path_in(slot_dir, lock_path)).unwrap();
