...
use ezer_workspace::WorkspaceHandle;
...
async fn handle_workspace_start(
    ...
    let status_config = ezer_workspace::StatusConfig::from_env();
    let alpha_test_key = None;
    let auth = wait_for_leader_auth(ws, &cancel).await?;
    let server_id = workspace_server_id();
    let device_id = ezer_telemetry::id::agent_id_async().await;
    ...
    let handle = ezer_workspace::connect_local_workspace(
        cwd_path.clone(),
        url,
        auth,
