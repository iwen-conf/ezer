...
use ezer_workspace::permission::claude_settings::{
    find_claude_settings_paths, load_claude_settings,
};
use ezer_workspace::permission::rules::parse_permission_rule;
use ezer_workspace::permission::types::{PatternMode, PermissionRule, RuleAction, ToolFilter};
...
/// Merge `Hook` items into `<hooks_dir>/imported-from-claude.json`. The output JSON is the same shape that `ezer-hooks` natively understands (Claude-compatible).
...
mod tests {
    ...
    fn discover_hook_source_paths_skips_claude_when_marker_set() {
        ...
        let paths = ezer_hooks::discovery::discover_hook_source_paths(
            Some(dir.path()),
            &compat,
    ...
    fn gate_load_claude_env_returns_empty_when_marker_set() {
        ...
        refresh_marker_cache(true);
        let dir = tempfile::tempdir().unwrap();
        let env = ezer_workspace::permission::claude_settings::load_claude_env_with_project(
            dir.path(),
            true,
        );
        assert!(
    ...
    fn discover_hooks_honors_claude_compat_gate() {
        ...
        let has_probe = |reg: &ezer_hooks::discovery::HookRegistry| {
...
"#;
    ...
    async fn gate_resolve_permissions_with_provenance_skips_claude_when_marker_set() {
        ...
        if let Some(r) = resolved {
            ...
            use ezer_workspace::permission::types::RequirementSource;
