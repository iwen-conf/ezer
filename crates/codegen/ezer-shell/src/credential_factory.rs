...
//! cannot depend on. Everything else lives in `ezer_login::credential_provider`.
...
pub fn build_storage_client_for_proxy(
    ...
    let http_client = ezer_http::shared_upload_client();
    if let Some(am) = auth_manager {
        ...
        .with_client_identity(ezer_version::VERSION, client_identifier)
...
/// Call [`ezer_login::credential_provider::wire_otel_auth_manager`] after agent init to upgrade it.
pub fn build_bootstrap_otel_credentials() -> (Arc<dyn AuthCredentialProvider>, String) {
    ...
    ezer_login::credential_provider::install_bootstrap_otel_provider(
        proxy_base_url,
        std::sync::Arc::new(crate::managed_config::resolve_deployment_id),
    )
}
