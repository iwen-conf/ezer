...
use ezer_config::resolve_global_hook_sources;
use ezer_hooks::discovery::HookSource;
use ezer_hooks::error::HookError;
use ezer_workspace::HookSourceConfig;
...
fn include_claude_hooks(compat: &ezer_tools::types::compat::CompatConfig) -> bool {
    compat.claude.hooks
        && !crate::claude_import::is_claude_import_marked_with_log("discover_hook_source_paths")
}

fn include_cursor_hooks(compat: &ezer_tools::types::compat::CompatConfig) -> bool {
    compat.cursor.hooks
}
...
/// Vendor settings paths are pushed as settings files here and probed for presence by ezer_workspace::folder_trust::repo_configs_present; a new vendor path needs both.
...
/// The first-wins dedup in [`ezer_hooks::discovery::registry_from_specs_deduped`] then lets a config hook beat a byte-identical file hook.
...
pub(crate) fn assemble_hooks(
    ...
    let (mut specs, mut errors) =
        ezer_hooks::config::parse_hooks_from_config_layers(config_layers);
    ...
    let (file_specs, file_errors) =
        ezer_hooks::discovery::collect_specs_from_sources(&global_sources, &project_sources);
    specs.extend(file_specs);
    errors.extend(file_errors);

    (
        ezer_hooks::discovery::registry_from_specs_deduped(specs),
        errors,
    )
}
...
mod tests {
    use super::*;
    use ezer_hooks::config::HookProvenance;
    use ezer_hooks::event::HookEventName;
...
"#,
        ...
        let layers = ezer_config::hook_config_layers_at(Some(system_dir.path()), None);
...
"#,
        ...
        let (specs, errors) = ezer_hooks::config::parse_hooks_from_config_layers(&layers);
        assert!(errors.is_empty(), "errors: {errors:?}");
