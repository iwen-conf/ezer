...
pub(crate) fn auto_mode_classifier_defaults(
    ...
    ezer_workspace::permission::ClassifierPromptType,
    Option<ezer_sampling_types::ReasoningEffort>,
) {
    let prompt_type = cfg
        .prompt_type
        .unwrap_or(ezer_workspace::permission::ClassifierPromptType::Full);
    let reasoning_effort = cfg.reasoning_effort.or_else(|| {
        effective_supports_reasoning_effort.then_some(ezer_sampling_types::ReasoningEffort::Low)
    });
