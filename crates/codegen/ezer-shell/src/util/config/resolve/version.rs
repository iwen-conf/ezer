...
/// This is a lightweight duplicate of `ezer_update::channel_name()` for use in `ezer-shell` which cannot depend on `ezer-update`.
pub(crate) fn channel_name_from_cache() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        ...
        let current = semver::Version::parse(ezer_version::VERSION).ok()?;
