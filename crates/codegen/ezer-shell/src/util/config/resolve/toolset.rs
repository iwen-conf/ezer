...
/// Pass the **merged** requirements ([`crate::config::load_merged_requirements`]) so an org policy in any requirements layer — not only `~/.ezer/requirements.toml` — is honored. Returns `(find_bfs, grep_ugrep)`, which the caller bakes into a [`ezer_tools::computer::local::SearchShadowConfig`] on the local terminal backend.
pub(crate) fn resolve_search_tools_enabled(
    requirements: Option<&TomlValue>,
    user: Option<&TomlValue>,
    managed: Option<&TomlValue>,
) -> (bool, bool) {
    let disable = ezer_config::env_bool("DISABLE_EMBEDDED_SEARCH_TOOLS");
    ...
    let resolve = |primary: &str, alias: &str, key: &str| -> bool {
        let env = ezer_config::env_bool(primary).or_else(|| ezer_config::env_bool(alias));
...
) -> Option<ezer_tools::util::ShellEnvironmentPolicy> {
    let value = effective_cfg?.get("shell_environment_policy")?.clone();
    match value.try_into::<ezer_tools::util::ShellEnvironmentPolicy>() {
...
fn cap_web_search_domains(list: Option<Vec<String>>, field: &str) -> Option<Vec<String>> {
    const MAX: usize = ezer_sampling_types::MAX_WEB_SEARCH_DOMAINS;
...
-> Option<ezer_sampling_types::WebSearchOptions> {
...
fn web_search_options_from_section(
    ...
    let opts = ezer_sampling_types::WebSearchOptions {
    ...
    match opts.validate() {
        ...
        Err(e) => {
            ...
            Some(ezer_sampling_types::WebSearchOptions {
...
mod web_search_domains_tests {
    ...
    // Cross-layer precedence and allow/exclude atomicity live in ConfigLayers (see `ezer_config::loader` normalization tests)
...
mod shell_env_policy_tests {
    use super::*;
    use ezer_tools::util::{EnvironmentVariablePattern, ShellEnvironmentPolicyInherit};
