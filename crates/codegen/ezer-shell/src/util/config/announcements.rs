...
pub use ezer_announcements::RemoteAnnouncement;
...
/// Dev/test override for announcements via `EZER_ANNOUNCEMENTS_OVERRIDE` (a JSON array of announcements).
...
pub(crate) fn announcements_override() -> Option<Vec<RemoteAnnouncement>> {
    let raw = std::env::var("EZER_ANNOUNCEMENTS_OVERRIDE").ok()?;
    match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
        Ok(list) => Some(list),
        Err(_) => {
            tracing::warn!("invalid EZER_ANNOUNCEMENTS_OVERRIDE JSON; ignoring");
            None
        }
    }
}
...
/// The `EZER_ANNOUNCEMENTS_OVERRIDE` env var overrides everything (dev only).
