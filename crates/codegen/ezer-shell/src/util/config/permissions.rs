...
pub use ezer_telemetry::enums::PermissionMode;
...
use ezer_workspace::permission::resolution::yolo_disabled_by_policy;
...
mod tests {
    ...
    fn campaign_cannot_arm_permission_mode_gate() {
        ...
        layers.campaigns.managed = vec![ezer_config::CampaignEntry {
            id: "c1".into(),
    ...
    // Pure tests for the policy predicate itself live next to its canonical definition in `ezer_workspace::permission::resolution`

    #[test]
    fn resolve_launch_yolo_policy_pin_neutralizes_requested_bypass() {
        let warning =
            ezer_workspace::permission::resolution::YoloPinReason::DisableBypassPermissionsMode
                .message();
