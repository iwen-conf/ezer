...
use ezer_config::fs_atomic::BoundDest;
...
fn bind_user_config_dest_with(
    ...
    if follow_leaf && has_user_home {
        ezer_config::fs_atomic::bind_follow_destination(path)
    } else {
        ezer_config::fs_atomic::bind_slot_destination(path)
    }
}
...
pub fn read_follow_leaf(path: &std::path::Path) -> std::io::Result<(BoundDest, String)> {
    let dest = ezer_config::fs_atomic::bind_follow_destination(path)?;
    let content = read_to_string_or_empty(dest.as_path())?;
    Ok((dest, content))
}
...
pub fn atomic_write_follow_leaf(
    ...
    let dest = ezer_config::fs_atomic::require_same_bound_destination(slot, dest)?;
    atomic_write_resolved_string(&dest, content)
}
fn atomic_write_string_inner(
    ...
    let dest = if follow_leaf {
        let first = ezer_config::fs_atomic::bind_follow_destination(path)?;
        ezer_config::fs_atomic::require_same_bound_destination(path, &first)?
    } else {
        ezer_config::fs_atomic::bind_slot_destination(path)?
    };
    atomic_write_resolved_string(&dest, content)
...
pub(crate) fn atomic_write_resolved_string(dest: &BoundDest, content: &str) -> std::io::Result<()> {
    ...
    ezer_config::fs_atomic::write_atomically_bound(dest, content, None)
}
