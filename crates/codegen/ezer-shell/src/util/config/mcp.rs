...
use ezer_tools::types::compat::{CompatConfig, CompatConfigToml};

pub use ezer_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
// MCP server config value types moved to `ezer-config-types`; the re-export keeps `crate::util::config::*` paths working
...
pub use ezer_config_types::{McpConfig, RelaySyncConfig};
    ...
    if let Some(registry) = plugin_registry {
        ...
        for plugin in registry.active_plugins() {
            ...
            if let Some(ref inline_value) = plugin.inline_mcp_servers {
                let normalized =
                    ezer_agent::plugins::manifest::normalize_inline_mcp_servers(inline_value);
...
pub async fn delete_mcp_server_config_at(
    ...
    if wrote
        && let Ok(mut cred_store) = ezer_mcp::credentials::McpCredentialStore::load_default()
    {
...
// `.mcp.json` discovery moved to `ezer-workspace` (client-side, shared with the folder-trust gate)
// The re-export keeps `crate::util::config::*` paths working
pub use ezer_workspace::project_config::{
...
pub fn load_cli_plugin_registry(cwd: &std::path::Path) -> ezer_agent::plugins::PluginRegistry {
    ...
    let discovered = ezer_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    ezer_agent::plugins::PluginRegistry::from_discovered(
        discovered,
...
pub(crate) fn session_registry_from_env_opt() -> Option<bool> {
    ezer_config::env_bool(SESSION_REGISTRY_ENV_VAR)
}
...
mod tests {
    ...
    fn session_registry_local_override_precedence() {
        ...
        {
            let _g = ezer_test_support::EnvGuard::set(SESSION_REGISTRY_ENV_VAR, "false");
            assert_eq!(
        ...
        {
            let _g = ezer_test_support::EnvGuard::set(SESSION_REGISTRY_ENV_VAR, "bogus");
            assert_eq!(
                session_registry_local_override_sourced(Some(&toml_true)),
        ...
        {
            let _g = ezer_test_support::EnvGuard::unset(SESSION_REGISTRY_ENV_VAR);
    ...
    fn load_cli_plugin_registry_includes_project_config_path_plugins() {
        ...
        let agents = ezer_agent::discovery::plugin_agents(&registry);
        assert!(
            agents
                .iter()
