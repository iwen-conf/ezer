...
async fn fetch_bundle_inner(
    ...
    let client: reqwest_middleware::ClientWithMiddleware = if let Some(am) = auth_manager {
        let provider: std::sync::Arc<dyn ezer_auth::AuthCredentialProvider> = std::sync::Arc::new(
            ezer_login::credential_provider::ShellAuthCredentialProvider::with_deployment_id_resolver(
                am.clone(),
...
pub struct BackendClient {
    ...
    base_url: String,
    pub(crate) auth_manager: Option<std::sync::Arc<ezer_login::AuthManager>>,
}
...
impl BackendClient {
    fn build_default_client() -> reqwest::Client {
        ezer_extra_ca::build_reqwest_client(|builder| {
    ...
    pub(crate) fn with_auth_manager(
        ...
        let credentials: std::sync::Arc<dyn ezer_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
    ...
    /// See: crates/codegen/ezer-shell/src/agent/app.rs:run_headless
    async fn auth_header_map(&self) -> Result<reqwest::header::HeaderMap, BackendError> {
        ...
        headers.insert(
            ...
            HeaderValue::from_static(ezer_version::VERSION),
        );
        Ok(headers)
    }
    async fn send_with_auth(
        ...
        let builder = ezer_otel::inject_trace_context_into_request(
...
pub(crate) fn parse_remote_model_value(
    ...
    let (reasoning_efforts, reasoning_effort_server_default) = obj
        ...
        .map(|arr| ezer_sampling_types::parse_reasoning_effort_options(arr, "reasoningEfforts"))
        ...
        compactions_remaining: obj
            ...
            .or_else(|| {
                obj
                    ...
                    .and_then(|v| v.as_bool())
                    .map(ezer_sampling_types::CompactionsRemaining::Dynamic)
            }),
...
fn parse_capabilities_reasoning_efforts(
...
) -> Option<(Vec<ezer_sampling_types::ReasoningEffortOption>, bool)> {
    let arr = caps.get("reasoning_effort")?.as_array()?;
    let mut options = ezer_sampling_types::parse_reasoning_effort_options(
        arr,
        ...
        .filter(|v| !v.is_null())
    {
        let level = default
            .as_str()
            .and_then(|s| s.parse::<ezer_sampling_types::ReasoningEffort>().ok());
...
fn parse_compaction_at_tokens(
    v: &serde_json::Value,
) -> Option<ezer_sampling_types::CompactionAtTokens> {
...
fn parse_compactions_remaining(
    v: &serde_json::Value,
) -> Option<ezer_sampling_types::CompactionsRemaining> {
