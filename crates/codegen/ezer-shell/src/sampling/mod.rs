...
pub use self::types::*;
pub use ezer_sampler::ApiBackend;
pub use ezer_sampler::SamplingClient as Client;

// Re-export async-openai Responses API types under `rs` namespace
pub use async_openai::types::responses as rs;

// --------------------------------------------------------------------------- ezer-sampler re-exports --------------------------------------------------------------------------- The actual streaming / retry / HTTP-client logic lives in the `ezer-sampler` crate
// These re-exports keep `crate::sampling::{SamplerHandle, SamplerConfig, ...}` paths working for callers not yet ported to `ezer_sampler::*`
...
const CONVERSATION_GROUP_NAMESPACE: &str = "xai:ezer-build:conversation-group:";
