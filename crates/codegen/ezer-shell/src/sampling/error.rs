...
//!
//! The canonical error types live in `ezer_sampling_types::error`.
...
pub use ezer_sampling_types::error::*;
...
pub use ezer_sampler::SamplingErrorKind;
...
/// This stays in ezer-shell because it depends on `agent_client_protocol::Error`.
    ...
    match err {
        ...
        SamplingError::IdleTimeout { elapsed_secs } => {
            acp::Error::internal_error().data(terminal_error_data(
                ...
                None,
                ezer_sampler::SamplingErrorKind::IdleTimeout,
            ))
        }
...
mod tests {
    ...
    fn attach_prompt_usage_preserves_error_kind_and_round_trips() {
        ...
        ledger.record_main_loop_call(
            "m",
            &ezer_sampling_types::TokenUsage {
                prompt_tokens: 3,
                completion_tokens: 1,
                total_tokens: 4,
                reasoning_tokens: 0,
