...
use ezer_tools::util::truncate::estimate_tokens;
...
pub(crate) struct PermissionsReport {
    ...
    /// A Claude managed-settings `disableBypassPermissionsMode` request, which ezer deliberately does not enforce ([`claude_bypass_lock_request`](ezer_workspace::permission::resolution::claude_bypass_lock_request)).
...
async fn build_report(cwd: &Path) -> InspectReport {
    ...
    let trust_store = ezer_agent::plugins::TrustStore::load();
    ...
    let discovered_plugins = ezer_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    let plugin_registry = ezer_agent::plugins::PluginRegistry::from_discovered(
...
async fn list_instructions(
    ...
    let configs = ezer_agent::prompt::agents_md::read_agents_config_with_paths(
    ...
    configs
        ...
        .map(|c| {
            let (scope, file_type, vendor) =
                if c.source == ezer_agent::prompt::agents_md::InstructionSource::Configured {
...
fn policy_lockdown_sources(
    ...
    use ezer_workspace::permission::resolution::PolicySourceAuthority;
...
fn list_hooks(
    ...
    project_trusted: bool,
    discovered_plugins: &[ezer_agent::plugins::DiscoveredPlugin],
) -> Vec<HookEntry> {
    let all_on = ezer_tools::types::compat::CompatConfig::default();
    ...
    let config_layers = ezer_config::hook_config_layers();
    ...
    let mut entries: Vec<HookEntry> = registry
        ...
        .map(|h| {
            ...
            use ezer_hooks::config::HookOrigin as O;
            ...
            let source = match ezer_hooks::config::hook_origin(h) {
                O::SystemManaged | O::Managed => ConfigSource::Managed {
                    path: Some(config_file(ezer_config::MANAGED_CONFIG_FILENAME)),
                },
                O::Requirements => ConfigSource::Managed {
                    path: Some(config_file(ezer_config::REQUIREMENTS_FILENAME)),
                },
                O::UserConfig => ConfigSource::ConfigToml {
                    path: config_file(ezer_config::USER_CONFIG_FILENAME),
                },
...
async fn list_skills(
    cwd: &Path,
    plugin_registry: &ezer_agent::plugins::PluginRegistry,
    skills_config: &ezer_agent::prompt::skills::SkillsConfig,
    project_trusted: bool,
) -> Vec<SkillEntry> {
    let skills = ezer_agent::prompt::skills::list_skills_with_plugins(
...
fn slash_name_counts(
    skills: &[ezer_agent::prompt::skills::SkillInfo],
    ...
    for skill in skills.iter().filter(|s| s.user_invocable && s.enabled) {
        ...
        *counts
            .entry(
                ezer_tools::implementations::skills::skill::format_skill_name(skill)
                    .to_lowercase(),
            )
            .or_default() += 1;
    }
...
fn slash_collision(
    skill: &ezer_agent::prompt::skills::SkillInfo,
    ...
    let qualified =
        ezer_tools::implementations::skills::skill::format_skill_name(skill).to_lowercase();
...
fn skill_entry_source(s: &ezer_agent::prompt::skills::SkillInfo) -> ConfigSource {
    use ezer_tools::implementations::skills::types::SkillScope;
...
fn list_agents(
    ...
    let agents = ezer_agent::discovery::all_subagents_with_plugins(
        cwd,
        &HashMap::new(),
        Some(plugin_registry),
    );
...
fn list_plugins(discovered: &[ezer_agent::plugins::DiscoveredPlugin]) -> Vec<PluginEntry> {
    discovered
        .iter()
        .map(|p| {
            let scope = match p.scope {
                ezer_agent::plugins::PluginScope::CliOverride => Scope::Cli,
                ezer_agent::plugins::PluginScope::Project => Scope::Project,
                ezer_agent::plugins::PluginScope::User => Scope::User,
                ezer_agent::plugins::PluginScope::ConfigPath => Scope::Config,
            };
            PluginEntry {
                ...
                provides: PluginProvides {
                    ...
                    skills: ezer_agent::plugins::registry::skill_md_paths(&p.skill_dirs).len(),
...
fn list_marketplaces(git_root: Option<&Path>) -> Vec<MarketplaceEntry> {
    ...
    ezer_agent::plugins::marketplace::resolve(root)
        .into_iter()
...
fn list_lsp_servers(
    ...
    let servers = ezer_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        cwd,
        &plugin_lsp_paths,
        &inline_values,
        &plugin_names,
...
mod tests {
    ...
    use ezer_agent::prompt::skills::{SkillInfo, SkillsConfig};
    use ezer_tools::implementations::skills::types::SkillScope;
    ...
    fn managed_features(
        ...
        ms.features = ezer_workspace::permission::resolution::ManagedSettingsFeatures {
            disable_telemetry,
            disable_feedback,
            disable_yolo,
    ...
    fn requirements_lock_reports_always_approve_enforced() {
        let lock = ezer_workspace::permission::resolution::YoloPolicyLock {
            source_label: "/etc/ezer/requirements.toml".to_string(),
            reason: ezer_workspace::permission::resolution::YoloPinReason::DisableBypassPermissionsMode,
        };
        let PermissionPolicyReport {
            enforced,
            claude_bypass_lock_advisory: advisory,
    ...
    fn managed_pins_report_enforced_rows_with_sources() {
        use ezer_workspace::permission::resolution::{PolicyLayerOwnership, PolicyPin};
        let mut ms = ManagedSettings::default();
    ...
    async fn list_skills_honors_skills_config() {
        ...
        let registry = ezer_agent::plugins::PluginRegistry::from_discovered(vec![], &[], &[]);
