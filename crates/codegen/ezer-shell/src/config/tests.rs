...
fn media_gen_caps_resolve_env_over_toml_over_remote_over_default() {
    use ezer_tools::media_gen_limits::{
...
fn project_config_never_sources_feedback_user() {
    ...
    std::fs::write(
        ...
        .unwrap();
    let cwd = repo.path();
    ezer_workspace::folder_trust::grant_folder_trust(cwd);
    assert!(
...
fn managed_settings_disables_features_and_requirements_overrides() {
    use crate::agent::config::Feature;
    use ezer_workspace::permission::resolution::ManagedSettingsFeatures;
    let mut cfg = crate::agent::config::Config::default();
...
fn simulate_release_build() -> ezer_test_support::EnvGuard {
    ezer_test_support::EnvGuard::set(ezer_version::TEST_VERSION_ENV, "0.0.0-sim")
}
#[test]
