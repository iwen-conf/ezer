...
pub const MANAGED_ARTIFACT_FILES: [&str; 4] = [
    ...
    ezer_config::signed_policy::SIGNATURE_SIDECAR_FILE,
    ezer_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE,
];
...
pub fn clear_orphan() {
    ...
    if ezer_config::fail_closed_policy_armed_at(&home) {
        tracing::info!(
    ...
    for (name, content) in artifacts {
        ...
        match content.filter(|s| !s.is_empty()) {
            Some(content) => {
                ...
                match ezer_config::fs_atomic::write_atomically(&path, content, Some(0o600)) {
                    Ok(()) => changed = true,
                    Err(e) => {
...
pub(super) fn apply_fetched(
    ...
    let verified = if ezer_config::signed_policy::verification_active() {
    ...
    if let Some(parked_at) = parked_at
        && parked_at < ezer_config::managed_config_synced_at(&home).unwrap_or(0)
    {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(ezer_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        ezer_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
        if let Some(claim_sidecar) =
            verified_claim_sidecar(body, served_principal_of(&verified.payload))
        {
            clear_squatting_dir(
                &home.join(ezer_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE),
            );
            ezer_config::signed_policy::write_managed_identity_sidecar(&home, &claim_sidecar)?;
        }
    }
    // Marker last, still under the lock: post-release, a concurrent purge could delete
    // the files it describes.
    clear_squatting_dir(&home.join(ezer_config::MANAGED_CONFIG_CACHE_FILE));
...
fn stage_refresh(
    ...
    ezer_config::fs_atomic::write_atomically(&path, &json, Some(0o600))
}
...
pub(super) fn apply_staged_managed_config() {
    ...
    if !is_fetch_enabled() || !ezer_config::signed_policy::verification_active() {
    ...
    if staged.parked_at == 0
        || staged.parked_at < ezer_config::managed_config_synced_at(&home).unwrap_or(0)
    {
        let _ = std::fs::remove_file(&path);
        return;
    }
...
pub(super) fn verified_claim_sidecar(
...
) -> Option<ezer_config::signed_policy::SignatureEnvelope> {
    use ezer_config::signed_policy::now_unix;
    ...
    let claim = match ezer_config::signed_policy::verify_fetched_claim(&sidecar, now_unix()) {
        Ok(claim) => claim,
        Err(e) => {
...
pub fn classify_auth_mode() -> ezer_telemetry::startup::AuthMode {
    auth_mode(
        resolve_deployment_key().is_some(),
