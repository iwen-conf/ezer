...
//! Scanning and install logic live in the `ezer-plugin-marketplace` crate.
...
fn load_filtered_marketplace_sources() -> Vec<ezer_plugin_marketplace::MarketplaceSource> {
    ...
    let source_names: Vec<String> = sources
        ...
        .map(|s| {
            let url = match &s.kind {
                ezer_plugin_marketplace::SourceKind::Git { url, .. } => url.as_str(),
                ezer_plugin_marketplace::SourceKind::Local { path } => {
...
fn refresh_sources(
    ...
    for source in sources {
        ...
        if let ezer_plugin_marketplace::SourceKind::Git { url, branch } = &source.kind {
            let cache_root = ezer_plugin_marketplace::git::default_cache_root();
            if let Err(e) = ezer_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                &cache_root,
            ) {
...
async fn handle_update(
    ...
    let updated = acquire::run_marketplace_update(
        ezer_agent::plugins::install_registry::MarketplaceProvenance {
...
fn update_error_outcome(
    ...
    use ezer_agent::plugins::install_registry::InstallError;
...
fn uninstall_locked(
    ...
    use ezer_plugin_marketplace::installer;
    use xai_hooks_plugins_types::{ActionOutcome, OutcomeStatus};
    ...
    let mut registry = ezer_agent::plugins::install_registry::InstallRegistry::load();
...
fn scan_source(
    ...
    let (source_kind, source_url_or_path, root) = match &source.kind {
        ...
        ezer_plugin_marketplace::SourceKind::Git { url, branch } => {
            let cache_root = ezer_plugin_marketplace::git::default_cache_root();
            let t_git = std::time::Instant::now();
            match ezer_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache_root,
    ...
    let scan = ezer_plugin_marketplace::scan_marketplace(&root);
    let catalog_loaded = scan.catalog_loaded;
    let discovered = scan.entries;
    drop(lease);

    // Cross-reference with install registry.
    let registry = ezer_agent::plugins::install_registry::InstallRegistry::load();
    let plugins = discovered
        .into_iter()
        .map(|p| {
            let (install_status, installed_version) =
                match ezer_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    &registry,
                    &source_url_or_path,
                    &p.relative_path,
                ) {
...
fn to_plugin_entry(
    p: ezer_plugin_marketplace::MarketplaceEntry,
...
async fn handle_add_source(url: &str) -> xai_hooks_plugins_types::ActionOutcome {
    ...
    let already_configured = match &input {
        MarketplaceAddInput::GitUrl(git_url) => {
            ...
            existing.iter().any(|s| {
                matches!(&s.kind, ezer_plugin_marketplace::SourceKind::Git { url: u, .. }
                    if normalize_git_url(u) == normalized)
            })
        }
        MarketplaceAddInput::LocalPath(path) => existing.iter().any(|s| {
            matches!(&s.kind, ezer_plugin_marketplace::SourceKind::Local { path: p }
                if p == path)
        }),
    };
    ...
    if let MarketplaceAddInput::GitUrl(git_url) = &input {
        ...
        let probe = tokio::task::spawn_blocking(move || {
            ezer_plugin_marketplace::git::probe_git_remote(&probe_url)
        })
        .await;
        match probe {
            Ok(Ok(())) => {}
...
fn default_skills_repo_keys<'a>(
    repos: impl IntoIterator<
        Item = (
            &'a str,
            &'a ezer_agent::plugins::install_registry::InstalledRepo,
        ),
    >,
) -> Vec<&'a str> {
    repos
...
fn ensure_official_marketplace_source_with(
    ...
    if policy
        .add_block_reason(ezer_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL)
        .is_some()
    {
        ...
        tracing::info!(
            reason = %policy.block_reason(
                ezer_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
                ezer_workspace::permission::resolution::PolicySubjectOrigin::Foreign,
            ),
    ...
    let already_present = toml_sources.iter().chain(json_sources.iter()).any(|s| {
        matches!(&s.kind, ezer_plugin_marketplace::SourceKind::Git { url, .. }
            if ezer_plugin_marketplace::is_official_source_url(url))
    });
    ...
    let write_result = if already_present {
        ...
        add_marketplace_source(
            &config_path,
            ezer_plugin_marketplace::OFFICIAL_SOURCE_NAME,
            &crate::plugin::MarketplaceAddInput::GitUrl(
                ezer_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.to_string(),
            ),
            true,
        )
    };
    ...
    match write_result {
        Ok(()) if !already_present => {
            tracing::info!(
                url = ezer_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
...
mod official_source_tests {
    ...
    ) -> Vec<ezer_plugin_marketplace::MarketplaceSource> {
        let raw = std::fs::read_to_string(config_path).unwrap_or_default();
        let parsed: toml::Value =
            toml::from_str(&raw).unwrap_or_else(|_| toml::Value::Table(Default::default()));
        ezer_plugin_marketplace::load_sources(&parsed)
    }
    ...
    fn add_marketplace_source_local_path_writes_path_entry() {
        ...
        assert!(matches!(
            &source.kind,
            ezer_plugin_marketplace::SourceKind::Local { path } if path == &dir
        ));
    ...
    fn add_marketplace_source_dedupes_respelled_git_url() {
        ...
        assert!(matches!(
            &source.kind,
            ezer_plugin_marketplace::SourceKind::Git { url, .. }
    ...
    fn first_run_creates_source_and_sets_flag() {
        ...
        assert!(matches!(
            &source.kind,
            ezer_plugin_marketplace::SourceKind::Git { url, .. }
                if url == ezer_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL
        ));
    ...
    fn policy_blocked_first_run_skips_register_and_retries_after_lift() {
        ...
        ensure_official_marketplace_source_with(
            home,
            &ezer_workspace::permission::resolution::MarketplacePolicy::default(),
        );
        assert_eq!(
...
mod conversion_tests {
    ...
    fn to_plugin_entry_carries_homepage_and_keywords() {
        let entry = ezer_plugin_marketplace::MarketplaceEntry {
            name: "demo".into(),
