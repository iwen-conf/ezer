...
use ezer_telemetry::events::{FeedbackDraftOp, FeedbackDraftOpError, FeedbackDraftOpKind};
use ezer_telemetry::session_ctx::log_event_dual;
...
struct FeedbackDraftRequest {
    session_id: String,
    draft_id: ezer_feedback::FeedbackDraftId,
}

pub(super) fn feedback_store(
    agent: &MvpAgent,
    session_id: &str,
) -> Result<ezer_feedback::FeedbackDraftStore, acp::Error> {
    ...
    Ok(ezer_feedback::FeedbackDraftStore::new(
...
async fn draft_op<T: Send + 'static>(
    ...
    session_id: &str,
    op: FeedbackDraftOpKind,
    store_op: impl FnOnce() -> ezer_feedback::Result<T> + Send + 'static,
