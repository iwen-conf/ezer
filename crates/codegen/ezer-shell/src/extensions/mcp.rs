...
//!
//! - `ezer/mcp/list`: list available MCP servers (agent-scoped or session-annotated)
//! - `ezer/mcp/call`: invoke an MCP tool directly, outside the LLM loop
//! - `ezer/mcp/servers_updated`: the local and plugin catalog after launch-dir discovery or a folder-trust grant (not gateway connectors)
//! - `ezer/mcp/server_status`: per-server delta pushed by the `StatusDispatcher`.
...
// rmcp is quarantined in ezer-mcp; see that crate's docs.
use ezer_mcp::rmcp;
...
/// Agent-only `ezer/mcp/*` ACP method/notification names. Unlike [`wire::MCP_CALL`] (the cross-SDK contract, which stays in `ezer_mcp::wire`), these methods are NOT spoken by the SDK.
...
pub mod mcp_methods {
    ...
    pub const PREFIX: &str = "ezer/mcp/";

    pub const LIST: &str = "ezer/mcp/list";
    pub const READ_RESOURCE: &str = "ezer/mcp/read_resource";
    pub const AUTH_STATUS: &str = "ezer/mcp/auth_status";
    pub const AUTH_TRIGGER: &str = "ezer/mcp/auth_trigger";
    pub const SETUP: &str = "ezer/mcp/setup";
    pub const TOGGLE: &str = "ezer/mcp/toggle";
    pub const TOGGLE_TOOL: &str = "ezer/mcp/toggle_tool";
    pub const UPSERT: &str = "ezer/mcp/upsert";
    pub const DELETE: &str = "ezer/mcp/delete";

    pub const SERVERS_UPDATED: &str = "ezer/mcp/servers_updated";
    pub const TOOLS_CHANGED: &str = "ezer/mcp/tools_changed";
    pub const INIT_PROGRESS: &str = "ezer/mcp/init_progress";
}
...
pub struct McpServerEntry {
    ...
    pub icons: Vec<ezer_mcp::servers::McpIcon>,
...
// Re-export the `ezer/mcp/server_status` schema and method constant from the dispatcher module External callers then have a single import point alongside the other `ezer/mcp/*` types
...
pub async fn notify_servers_updated(
    ...
    if let Ok(params) = serde_json::value::to_raw_value(&payload) {
        ...
        tracing::info!("Sent ezer/mcp/servers_updated notification to client");
    }
}

// ── Dispatch ────────────────────────────────────────────────────────

/// Inbound `ezer/mcp/*` methods this agent services, resolved from the wire string. Single source of truth for forward-method routing: [`handle`] maps each variant to its handler.
...
pub(crate) async fn build_mcp_status(
    ...
    tool_bridge: &Arc<ezer_tools::bridge::ToolBridge>,
    event_writer: Option<&ezer_session_events::EventWriter>,
) -> McpStatusSnapshot {
    ...
    for client in &clients {
        ...
        if let Some(ew) = event_writer {
            ew.emit(ezer_session_events::Event::McpHealthCheck {
...
pub(crate) async fn init_agent_mcp_pool(
    ...
    let noop = ezer_session_events::EventWriter::noop();
    ...
    let clients: ezer_mcp::owned_clients::OwnedClients = results
        .into_iter()
...
async fn finish_pool_init(
    ...
    clients: ezer_mcp::owned_clients::OwnedClients,
) {
...
pub(crate) fn list_blocked_reasons<'a>(
    definitions: impl IntoIterator<
        Item = (
            &'a str,
            &'a acp::McpServer,
            ezer_workspace::permission::resolution::McpSubject,
        ),
    >,
    local_servers: &[acp::McpServer],
    project_names: impl FnOnce() -> std::collections::HashSet<String>,
    ms: &ezer_workspace::permission::resolution::ManagedSettings,
...
async fn spawn_discovery<T: Send + 'static>(
    ...
    plugin_registry: Option<std::sync::Arc<ezer_agent::plugins::PluginRegistry>>,
...
// Implements the `McpResourceProvider` trait from ezer-tools
// The `ListMcpResources` and `FetchMcpResource` tools can then access MCP servers without depending on `ezer-mcp` directly
...
impl ezer_tools::types::resources::McpResourceProvider for McpStateResourceProvider {
    async fn list_resources(
        &self,
        server: Option<String>,
    ) -> Result<Vec<ezer_tools::types::resources::McpResourceInfo>, String> {
            ...
            match mcp_service.list_all_resources().await {
                Ok(all_resources) => {
                    for r in all_resources {
                        resources.push(ezer_tools::types::resources::McpResourceInfo {
                            uri: r.uri.clone(),
    ...
    async fn read_resource(
        ...
        server: String,
        uri: String,
    ) -> Result<ezer_tools::types::resources::McpResourceReadResult, String> {
        let client = {
            let state = self.0.lock().await;
            Arc::clone(
                state
        ...
        match first {
            rmcp::model::ResourceContents::TextResourceContents {
                ...
                text,
                ..
            } => Ok(ezer_tools::types::resources::McpResourceReadResult {
                uri: content_uri,
                name: None,
                description: None,
                mime_type,
                content: Some(ezer_tools::types::resources::McpResourceContent::Text(
                    text,
                )),
            }),
            rmcp::model::ResourceContents::BlobResourceContents {
                ...
                description: None,
                mime_type,
                content: Some(ezer_tools::types::resources::McpResourceContent::Blob(
                    blob.into_bytes(),
                )),
            }),
...
fn org_policy_message(
    name: &str,
    reason: &ezer_workspace::permission::resolution::McpBlockReason,
) -> String {
...
pub(crate) fn policy_enable_error(
    ...
    subject: ezer_workspace::permission::resolution::McpSubject,
) -> Option<String> {
    match ms.mcp_verdict(server, subject) {
        ezer_workspace::permission::resolution::McpVerdict::Allowed => None,
        ezer_workspace::permission::resolution::McpVerdict::Blocked(reason) => {
...
mod tests {
    ...
    /// The emit-only reverse method (`ezer/mcp/sdk_call`) shares the `ezer/mcp/` prefix. `mvp_agent`'s dispatcher therefore routes an inbound copy of it to this module's `handle`.
    ...
    fn inbound_sdk_call_has_no_forward_route() {
        ...
        assert_eq!(
            ...
            None,
            "inbound ezer/mcp/sdk_call must not resolve to a forward handler"
        );
    ...
    async fn superseded_pool_pass_cannot_finish_the_successor() {
        ...
        let stale: ezer_mcp::owned_clients::OwnedClients =
    ...
    fn policy_enable_error_fails_closed_for_blocked_server() {
        use ezer_workspace::permission::resolution::{McpSubject, PolicySubjectOrigin};

        let ms = deny_evil_corp();
        let subject = McpSubject {
            origin: PolicySubjectOrigin::EzerNative,
    ...
    fn deny_evil_corp() -> ezer_workspace::permission::resolution::ManagedSettings {
    ...
    async fn toggle_merge_refusal_rolls_back_enable() {
        ...
        use ezer_workspace::permission::resolution::McpBlockReason;
    ...
    fn list_blocked_reasons_covers_project_pin_arm() {
        ...
        let mut ms = ManagedSettings::default();
        // The /etc/ezer grant is admin-owned like the pin, so it may carve the exception.
    ...
    fn org_policy_message_clamps_unbounded_server_names() {
        ...
        let reason = ezer_workspace::permission::resolution::McpBlockReason::Deny {
            source: std::path::PathBuf::from("/etc/ezer/managed_config.toml"),
        };
    ...
    fn upsert_subject_and_pin_refuse_project_claimed_name() {
        ...
        assert_eq!(fresh.origin, PolicySubjectOrigin::EzerNative);
    ...
    fn test_mcp_list_icons_serialization() {
        let entry = McpServerEntry {
            ...
            icons: vec![ezer_mcp::servers::McpIcon {
                src: "https://example.com/icon.png".to_string(),
                mime_type: Some("image/png".to_string()),
                sizes: Some(vec!["48x48".to_string()]),
                theme: Some(ezer_mcp::servers::McpIconTheme::Dark),
            }],
            source: McpServerSource::Local,
            source_label: None,
            setup: None,
    ...
    #[test]
    fn ezer_com_local_name_is_not_managed_in_catalog() {
        let local = acp::McpServer::Http(
            acp::McpServerHttp::new("remote_slack", "https://mcp.example.test/sse")
                .headers(vec![]),
        );
        ...
        assert_eq!(server.name, "remote_slack");
        assert_eq!(server.source, McpServerSource::Local);
        assert!(matches!(server.config, McpServerConfig::Http { .. }));

        let placeholder = disabled_server_placeholder_entry("remote_slack");
