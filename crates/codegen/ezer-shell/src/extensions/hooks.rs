...
use ezer_hooks::event::{HookEventEnvelope, HookEventName};
...
pub(crate) fn current_hook_infos(
    registry: Option<&ezer_hooks::discovery::HookRegistry>,
) -> Vec<HookInfo> {
    let Some(registry) = registry else {
        return Vec::new();
    };
    let disabled = ezer_hooks::trust::DisabledHooks::load();
...
fn hook_specs_to_infos(
    specs: &[&ezer_hooks::config::HookSpec],
    disabled: &ezer_hooks::trust::DisabledHooks,
...
fn hook_spec_to_info_with(
    spec: &ezer_hooks::config::HookSpec,
    disabled: &ezer_hooks::trust::DisabledHooks,
    registered_dirs: &HashSet<String>,
    ...
    HookInfo {
        ...
        source_dir,
        disabled: ezer_hooks::trust::hook_disabled_for_display_with(spec, disabled),
...
pub(crate) const ADVERTISED_BLOCKING_EVENTS: &[ezer_hooks::event::HookEventName] = &[
...
fn parse_hook_group(event: HookEventName, value: &serde_json::Value) -> Option<ClientHookGroup> {
    ...
    let matcher = match group.matcher.as_deref() {
        None | Some("") | Some("*") => None,
        Some(pattern)
            if event.traits().matcher == ezer_hooks::event::MatcherPolicy::Ignored =>
        {
...
mod tests {
    ...
    fn hook_spec_to_info_raw_display_wins_so_secrets_never_reach_dto() {
        let no_disabled = ezer_hooks::trust::DisabledHooks::from_names([]);
    ...
    fn hook_specs_to_infos_pins_removable_at_source_level() {
        ...
        policy.layer = ezer_hooks::config::HookProvenance::SystemManaged;
        ...
        req.layer = ezer_hooks::config::HookProvenance::Requirements;
    ...
    fn client_hook_dispatch_serializes_envelope() {
        use ezer_hooks::event::{HookEventEnvelope, HookPayload};
