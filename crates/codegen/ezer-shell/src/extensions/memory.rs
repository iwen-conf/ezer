...
pub const MEMORY_REWRITE_METHOD: &str = "ezer/memory/rewrite";
...
/// Request body for `ezer/memory/rewrite`: a raw `/remember` note and the conversation summary the
...
/// Response body for `ezer/memory/rewrite`: the note as Markdown
#[derive(Debug, Clone, Serialize, Deserialize)]
