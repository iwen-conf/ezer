//! Jujutsu extension handlers; delegates to [`ezer_workspace::session::jj`].

use agent_client_protocol as acp;

use super::{Empty, ExtResult, to_ext_response, to_ext_response_partial};
use ezer_workspace::session::git::{CommitData, StageData};
use ezer_workspace::session::jj;

/// Handle a `ezer/git/*` method for a jj-colocated repo.
///
...
pub(crate) async fn try_handle(
    ...
    match method {
        "ezer/git/status" => Some(to_ext_response(jj::status(git_root).await)),
        "ezer/git/info" => Some(to_ext_response(jj::info(git_root).await)),
        // git HEAD points at `@-` in a colocated repo; route to jj so we report the working-copy commit (`@`), consistent with `status`/`info`
        "ezer/git/current_commit" => Some(to_ext_response(jj::current_commit(git_root).await)),
        "ezer/git/branches" => Some(to_ext_response(jj::list_bookmarks(git_root).await)),

        // jj has no staging area: stage/unstage are no-ops
        "ezer/git/stage" => Some(to_ext_response(Ok(StageData { paths: Vec::new() }))),
        "ezer/git/stage/content" | "ezer/git/unstage" => Some(to_ext_response(Ok(Empty {}))),
        ...
        "ezer/git/checkout" => Some(Err(acp::Error::invalid_params()
            .data("checkout is not supported in jj repos; use `jj new` or `jj edit`"))),
        "ezer/git/stash" => Some(Err(acp::Error::invalid_params()
