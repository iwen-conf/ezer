...
use ezer_agent::prompt::skills::{
    CompatConfig, SkillInfo, SkillsConfig, collect_config_skills, list_skills_with_plugins,
};
use ezer_telemetry::events::{HarnessChangeOp, HarnessChanged, HarnessSurfaceKind};
...
async fn skills_at_config_path(resolved: &str, cwd: &str) -> Vec<SkillInfo> {
    ...
    let scan = tokio::task::spawn_blocking(move || {
        let git_root = ezer_agent::repo::RepoDirChain::resolve(&cwd).git_root;
...
fn log_harness_changed(skills: &[SkillInfo], op: HarnessChangeOp, success: bool) {
    for skill in skills.iter().take(HARNESS_CHANGED_MAX_ITEMS) {
        ezer_telemetry::session_ctx::log_event(HarnessChanged {
...
pub async fn handle(
    ...
    match args.method.as_ref() {
        "ezer/skills/add" => {
            ...
            .await
            {
                ezer_telemetry::session_ctx::log_event(
                    ezer_telemetry::events::SkillAdded {
                        added_count: 0,
                        total_skills: 0,
                        success: false,
                    },
            ...
            ezer_telemetry::session_ctx::log_event(ezer_telemetry::events::SkillAdded {
        ...
        "ezer/skills/remove" => {
            ...
            {
                ezer_telemetry::session_ctx::log_event(
                    ezer_telemetry::events::SkillRemoved { success: false },
                );
            ...
            ezer_telemetry::session_ctx::log_event(ezer_telemetry::events::SkillRemoved {
                success: true,
            });
...
mod tests {
    ...
    fn test_resolve_tilde_path() {
        use ezer_test_support::env::EnvGuard;
