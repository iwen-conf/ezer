...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "ezer/auth/hydrate_team_capability" => handle_hydrate_team_capability(agent, args).await,
    ...
    match key {
        Some(k) if !k.is_empty() => {
            ezer_login::store_api_key(&ezer_home, k)
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            ezer_login::auth_method::set_runtime_xai_api_key(k);
        }
        _ => {
            ezer_login::clear_api_key(&ezer_home)
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            ezer_login::auth_method::clear_runtime_xai_api_key();
        }
    }
...
fn handle_submit_code(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    match agent.interactive_auth.submit_code(params.code) {
        ...
        Err(ezer_login::single_flight::SubmitCodeError::SendFailed(e)) => {
            Err(acp::Error::internal_error().data(format!("failed to submit auth code: {e}")))
        }
        Err(ezer_login::single_flight::SubmitCodeError::NoPendingAttempt) => {
...
async fn handle_logout(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let result = ezer_login::perform_logout(
        &agent.auth_manager,
    ...
    ezer_telemetry::event_span!("auth.lifecycle", action = "logout", success = true);
...
fn handle_info(agent: &MvpAgent) -> ExtResult {
    ...
    to_raw_response(&AuthInfoResponse {
        ...
        coding_data_retention_opt_out: auth
            ...
            .unwrap_or_else(ezer_login::default_coding_data_retention_opt_out),
    })
}
