...
pub type DraftCleanup = (
    ezer_feedback::FeedbackDraftStore,
    ezer_feedback::FeedbackDraftId,
);
...
pub async fn draft_send_input(
    ...
    store: ezer_feedback::FeedbackDraftStore,
    ...
    let taxonomy = ezer_feedback::FeedbackTaxonomy {
        r#type: Some(body.input.r#type),
        task_category: body.input.task_category,
        failure_mode: body.input.failure_mode,
    };
    ezer_feedback::validate_feedback_draft_send(&body.input, !body.images.is_empty())
        .map_err(|error| acp::Error::invalid_params().data(error.to_string()))?;
    validate_images(&body.images)?;
    let metadata = Some(ezer_feedback::structured_feedback(
        ezer_feedback::FeedbackSource::Draft,
        taxonomy,
    ));
    Ok((
        ClientFeedbackInput {
            ...
            rating_type: None,
            rating_value: None,
            feedback_text: Some(ezer_feedback::post_text(
                &body.input.title,
                &body.input.details,
            )),
            images: body.images,
