...
use ezer_workspace::session::git::{self, DiscardScope, GitDiffsData, check_diff_size_limits};
...
async fn resolve_git_root(
    ...
    if let Some(sid) = session_id {
        if let Some(cwd) = agent.get_session_cwd(sid) {
            let result = ops
                .dispatch(
                    &ezer_workspace::workspace_ops::GitResolveRootReq { cwd },
                    None,
                )
                .await
                .map_err(|e| {
...
pub async fn handle(
    ...
    match args.method.as_ref() {
        ...
        "ezer/git/checkout_session_head" => {
            ...
            let result = ops
                .dispatch(
                    &ezer_workspace::workspace_ops::GitCheckoutCommitReq {
                        git_root: git_root.clone(),
                        head_commit,
