...
//! `ezer/git/worktree/create_from_worktree_sync`. Best effort: the worktree
...
async fn run_git(cwd: &Path, args: &[&str], timeout: Duration) -> Result<String, String> {
    ...
    let group = match ezer_tools::util::global_process_scope().enroll(&child) {
        Ok(group) => group,
        Err(err) => {
    ...
    let stderr = ezer_workspace::session::git::scrub_git_output(&stderr);
