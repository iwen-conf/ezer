...
fn cli_plugin_registry(cwd: &Path) -> ezer_agent::plugins::PluginRegistry {
...
async fn check_tools_list(server_name: &str, service: &mcp_servers::McpService) -> Check {
    use ezer_mcp::rmcp::model::PaginatedRequestParams;
    match service
...
) -> HashMap<String, ezer_workspace::permission::resolution::McpBlockReason> {
