//! Translates `ezer-tools` `ToolNotification` events into `ezer-shell`'s native systems (ACP gateway, hunk tracker, file state tracker).
...
use ezer_workspace::session::file_state::FileStateTracker;
...
pub(crate) struct NotificationBridgeConfig {
    ...
    pub task_completion_reservations:
        ezer_tools::reminders::task_completion::TaskCompletionReservations,
    pub task_wake_suppressed: ezer_tools::reminders::task_completion::TaskWakeSuppressed,
...
async fn handle_scheduled_task_removed(
    config: &NotificationBridgeConfig,
    removed: ezer_tools::notification::ScheduledTaskRemoved,
...
async fn emit_current_mode_update(
    config: &NotificationBridgeConfig,
    mode: ezer_tools::types::SessionMode,
) {
...
async fn handle_notification(
    ...
    match notification {
        ...
        ToolNotification::TaskCompleted(task_snapshot) => {
            let is_monitor =
                task_snapshot.kind == ezer_tools::computer::types::TaskKind::Monitor;
            ...
            if task_snapshot.is_auto_wake_suppressed() {
                ...
                let body = if is_monitor {
                    ezer_tools::reminders::task_completion::format_monitor_completion(
                        &task_snapshot,
                        tool_name,
                    )
                } else {
                    ezer_tools::reminders::task_completion::format_bash_completion(
                        &task_snapshot,
                        tool_name,
                        read_name,
                    )
                };
                let message = ezer_tools::reminders::wrap_reminder(&body);
                ...
                let enqueued = config
                    ...
                    .send(SessionCommand::Prompt {
                        ...
                        screen_mode: None,
                        verbatim: true,
                        traceparent: ezer_otel::current_traceparent(),
                        json_schema: None,
                        send_now: false,
        ...
        ToolNotification::PlanModeEntered(entered) => {
            ...
            if activated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
                ...
                emit_current_mode_update(config, ezer_tools::types::SessionMode::Plan).await;
            }
            tracing::info!(
            ...
            if deactivated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Agent;
                ...
                let _ = config
                    ...
                    .tx
                    .send(PersistenceMsg::PlanModeState(snapshot));
                emit_current_mode_update(config, ezer_tools::types::SessionMode::Default).await;
            }
            tracing::info!(
