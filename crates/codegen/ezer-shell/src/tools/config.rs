...
/// It is distinct from `ezer_tools::registry::types::ToolsetConfig` which holds tool-implementation-level config (bash limits, web search mode).
...
mod tests {
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            ezer_agent::config::AgentDefinition::plan(),
            ezer_agent::config::AgentDefinition::explore(),
        ] {
