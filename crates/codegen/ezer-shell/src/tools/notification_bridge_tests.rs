...
use ezer_tools::types::TaskSnapshot;
...
async fn monitor_event_skipped_after_task_completed_auto_wake() {
    ...
    handle_notification(
        &config,
        ToolNotification::MonitorEvent(ezer_tools::notification::types::MonitorEvent {
...
async fn scheduled_task_created_is_persisted() {
    ...
    let notification = ToolNotification::ScheduledTaskCreated(
        ezer_tools::notification::types::ScheduledTaskCreated {
            task_id: "loop-1".into(),
...
async fn bash_output_chunk_forwards_live_without_persisting() {
    ...
    let notification =
        ToolNotification::BashOutputChunk(ezer_tools::notification::types::BashOutputChunk {
            base: ezer_tools::notification::types::BashNotificationBase {
...
async fn scheduled_task_removed_is_persisted() {
    ...
    let (config, _gateway_rx, mut persistence_rx, _cmd_rx) = make_test_config_full();
    let removed = ezer_tools::notification::ScheduledTaskRemoved::new(
        "loop-1".into(),
        ezer_tools::notification::ScheduledTaskRemovedReason::Expired,
        "generation-a".into(),
        2,
    );
        ...
        PersistenceMsg::Update(crate::session::storage::SessionUpdate::Xai(notif)) => {
            assert!(matches!(
                ...
                crate::extensions::notification::SessionUpdate::ScheduledTaskDeleted {
                    reason: ezer_tools::notification::ScheduledTaskRemovedReason::Expired,
                    ..
                }
            ));
            assert!(
...
async fn acknowledged_scheduler_removal_appends_before_ack_and_broadcast() {
    ...
    let removed = ezer_tools::notification::ScheduledTaskRemoved::new(
        "loop-ack".into(),
        ezer_tools::notification::ScheduledTaskRemovedReason::Deleted,
        "generation-a".into(),
        17,
    );
...
async fn task_backgrounded_persisted_line_is_stamped() {
    ...
    let notification = ToolNotification::BashExecutionBackgrounded(
        ezer_tools::notification::types::BashExecutionBackgrounded {
            base: ezer_tools::notification::types::BashNotificationBase {
...
async fn scheduled_task_fired_is_not_persisted() {
    ...
    let notification = ToolNotification::ScheduledTaskFired(
        ezer_tools::notification::types::ScheduledTaskFired {
            task_id: "loop-1".into(),
            prompt: "check deploy".into(),
            human_schedule: "every 5 minutes".into(),
            next_fire_at: Some("2026-01-01T00:00:00Z".into()),
...
async fn plan_mode_entered_emits_current_mode_update_plan() {
    ...
    let notification =
        ToolNotification::PlanModeEntered(ezer_tools::notification::types::PlanModeEntered {
