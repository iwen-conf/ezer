...
use ezer_workspace::permission::PermissionEvent;
...
pub(super) fn record_upload_failure(ctx: &PromptTraceContext, f: UploadFailure<'_>) {
    ...
    if level == UploadFailureLogLevel::Warn {
        ezer_telemetry::unified_log::warn(&msg, sid, log_ctx);
    } else {
        ezer_telemetry::unified_log::error(&msg, sid, log_ctx);
    }
}
...
pub(crate) fn local_sandbox_telemetry() -> Option<LocalSandboxTelemetry> {
    let profile = ezer_sandbox::configured_profile_name()?;
    Some(LocalSandboxTelemetry {
        profile: profile.to_owned(),
        applied: ezer_sandbox::is_active(),
    })
}
...
pub(crate) async fn upload_plugin_state(
    ctx: &PromptTraceContext,
    registry: Option<&ezer_agent::plugins::PluginRegistry>,
) {
    use ezer_agent::plugins::discovery::PluginScope;
...
pub(crate) async fn upload_unified_log(ctx: &PromptTraceContext, wait: UploadWait) {
    ...
    let log_bytes = match tokio::task::spawn_blocking(move || {
        ezer_telemetry::unified_log::snapshot_session_log(&session_id)
    })
    .await
    {
        Ok(Some(bytes)) => bytes,
    ...
    .await;
    let full_log_bytes =
        tokio::task::spawn_blocking(ezer_telemetry::unified_log::snapshot_log).await;
    let user_id = ctx
        .auth_manager
        .current_or_expired()
        .map(|a| a.user_id)
...
fn serialize_chat_history_jsonl(
    messages: &[ezer_sampling_types::conversation::ConversationItem],
...
impl TraceExportSource for DynamicResolver {
    ...
    fn proxy_credentials(&self) -> Option<Arc<dyn ezer_auth::AuthCredentialProvider>> {
...
pub(crate) mod tests {
    ...
    async fn resolve_async_drives_refresh_chain_when_token_expired() {
        ...
        impl ezer_login::refresh::TokenRefresher for FreshRefresher {
            async fn refresh(
                &self,
    ...
    async fn proactive_refresh_makes_trace_resolve_a_cache_hit() {
        ...
        tokio::time::sleep(
            ezer_login::manager::PROACTIVE_MIN_SLEEP + std::time::Duration::from_millis(1000),
        )
        .await;
