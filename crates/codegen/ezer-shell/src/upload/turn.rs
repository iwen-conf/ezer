...
pub(crate) use ezer_telemetry::session_metrics::TraceUploadReason;
#[derive(Clone)]
pub(crate) struct PromptTraceContext {
    ...
    pub(crate) auth_manager: std::sync::Arc<ezer_login::AuthManager>,
}
    ...
    F: Future<Output = ()> + Send + 'static,
{
    ...
    let span = match attach {
        ...
        UploadSpanAttach::Link {
            ...
            ezer_otel::link_span_to_current(&root);
            root
        }
    };
...
pub(crate) fn parse_agent_profile_from_meta(
    ...
    if value.is_object() {
        return match ezer_agent::AgentDefinition::from_json(value) {
            Ok(def) => {
                tracing::info!(
                    agent_name = %def.name,
    ...
    if let Some(name) = value.as_str() {
        ...
        return ezer_agent::discovery::by_name(name);
    }
    tracing::warn!(
...
mod tests {
    ...
    fn parse_ask_user_question_returns_none_when_absent() {
        let meta = serde_json::json!({ "agentProfile": "ezer-build-plan" });
