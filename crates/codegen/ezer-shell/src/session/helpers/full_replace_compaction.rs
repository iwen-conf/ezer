//! ezer-build's L5 wiring onto the shared full-replace engine (`ezer_compaction::code_compaction`).
//!
//! The shared engine drives the loop: sample, retry, then classify the result as degenerate or failed.
//! The loop lives in [`sample_full_replace_summary`](ezer_compaction::sample_full_replace_summary).
//! This module adapts ezer-build's transport and telemetry to the engine's two traits: [`ShellCompactionSampler`] and [`ShellFullReplaceObserver`].
//!
//! The **input ladder** (verbatim, then fitted, then lossy) and auto-compaction suppression stay in L5 (`compaction.rs`).
//! Both are driven by the `context_overflow` / `deterministic` flags on [`FullReplaceError`](ezer_compaction::FullReplaceError).
...
use ezer_sampling_types::{ConversationItem, HostedTool, ToolSpec};
use ezer_telemetry::events::{CompactionRetryDegraded, CompactionTrigger};
...
/// Wraps `generate_session_compact` as the shared engine's [`CompactionSampler`] for ezer-build's full-replace pass.
...
/// Map ezer-build's [`CompactFailure`] onto the shared engine's [`CompactionSampleError`].
...
/// [`FullReplaceObserver`] that reproduces ezer-build's per-attempt telemetry without the shared engine depending on a telemetry backend.
    ...
    fn on_attempt(&self, _attempt: u32, outcome: &FullReplaceAttemptOutcome<'_>) {
        ...
        match outcome {
            ...
            FullReplaceAttemptOutcome::Degenerate {
                ...
                if *will_retry {
                    ezer_telemetry::session_ctx::log_event(CompactionRetryDegraded {
