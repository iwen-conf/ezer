...
pub(crate) fn normalize_compact_detail(raw: &str) -> String {
    ...
    ezer_tools::util::truncate_str_with_marker(&scrubbed, COMPACT_ERROR_DETAIL_MAX_BYTES)
        .into_owned()
}
...
pub(crate) use ezer_compaction::is_context_length_error;
...
fn classify_response_event_error(code: Option<&str>, message: &str) -> CompactFailure {
    ...
    if code.is_some_and(ezer_sampling_types::is_size_overflow_error_code)
...
pub(crate) fn build_compaction_prompt(
    ...
    if use_short_prompt {
        ...
        // Default (ezer-build, codex, ...): the concise summarize prompt the ezer-build models are RL-trained on
                    ...
                    Ok(chunk) => {
                        if let Some(choice) = chunk.choices.first() {
                            ...
                            if let Some(fr) = choice.finish_reason {
                                let sr = ezer_sampling_types::StopReason::from(fr);
                                truncated =
                                    matches!(sr, ezer_sampling_types::StopReason::Length);
            ...
            loop {
                ...
                match chunk_result {
                    Ok(event) => {
                        if !matches!(
                            &event,
                            ezer_sampling_types::messages::MessageStreamEvent::Ping
                        ) {
                            last_progress_at = std::time::Instant::now();
                        }
                        match event {
                        ezer_sampling_types::messages::MessageStreamEvent::ContentBlockDelta {
                            delta: ezer_sampling_types::messages::StreamDelta::TextDelta { text },
                            ..
                        } => {
                            timing.record_delta();
                            content.push_str(&text);
                        }
                        ezer_sampling_types::messages::MessageStreamEvent::MessageDelta { delta, .. } => {
                            if let Some(sr) = delta.stop_reason {
                                truncated = matches!(
                                    sr,
                                    ezer_sampling_types::messages::StopReason::MaxTokens
                                        | ezer_sampling_types::messages::StopReason::ModelContextWindowExceeded
                                );
