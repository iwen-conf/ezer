...
mod tests {
    ...
    fn main_turn_count_counts_real_users_only() {
        use std::sync::Arc;
        use ezer_sampling_types::{ContentPart, SyntheticReason, ToolCall, UserItem};
    ...
    fn mk_tool_call(id: &str, args: &str) -> ezer_sampling_types::ToolCall {
        use std::sync::Arc;
    ...
    fn budget_fast_path_keeps_reasoning_on_grok() {
        ...
        assert!(
            out.iter()
                .any(|i| matches!(i, ConversationItem::Reasoning(_))),
            "fits path on ezer must keep reasoning verbatim"
        );
    }
