...
use ezer_workspace::file_system::{CodebaseIndexManager, FileIndex, WalkOptions};
...
/// Dedup key for `ezer/git_head_changed`, shared by the watcher's `GitHead` consumer and the post-edit `maybe_notify_git_branch` path.
...
fn fs_event_to_delta(
    ...
    kind: FsEventKind,
    root: &Path,
) -> ezer_workspace::file_system::FileIndexDelta {
    use ezer_workspace::file_system::FileIndexDelta;
...
pub(crate) struct CapabilityInputs {
    ...
    pub hunk_tracking: bool,
    pub code_nav: bool,
    /// `ezer/gitHeadChanged`; opt-in (absent means off).
...
impl ClientNotify {
    async fn on_change(&self, paths: &[PathBuf], kind: FsEventKind) {
        ...
        match self.mode {
            ClientFsMode::Events => {
                // Present-tense strings are the `ezer/fs_notify` wire protocol; do not sync to internal variant names
...
impl GitHead {
    ...
    async fn emit(&self) {
        let branch = ezer_workspace::session::git::get_branch(&self.cwd).await;
        let commit = ezer_workspace::session::git::get_current_commit(&self.cwd).await;
        let worktree = ezer_workspace::session::git::get_worktree_info(&self.cwd).await;
        ...
        if changed {
            let params = ezer_workspace::session::git::GitHeadChanged {
...
impl FsWatchPlan {
    pub(crate) fn build(caps: FsWatchCapabilities, deps: FsWatchDeps) -> Self {
        ...
        let hunk = (caps.hunk_tracking && deps.hunk_tracking_enabled).then(|| {
            let git_root =
                ezer_workspace::session::git::find_git_root_from_path(&deps.cwd).ok();
            HunkTracking {
