//! Binds the `ezer-session-search` index to this crate's JSONL session store.
...
use ezer_session_search::{IndexableSession, SessionSource};

pub use ezer_session_search::{
...
pub(crate) async fn evict_session(root_dir: &Path, session_id: &str) {
    ezer_session_search::evict_session(root_dir, session_id).await;
}
...
pub async fn execute_search(
    ...
    ezer_session_search::execute_search(index, root_dir, req).await
}
