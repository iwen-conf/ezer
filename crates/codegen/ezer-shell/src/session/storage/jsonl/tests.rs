...
async fn workflow_run_manifest_round_trips_and_clear_tombstone_wins() {
    ...
    assert_eq!(
            ...
            Some(ezer_sampling_types::ReasoningEffort::High)
        );
...
async fn load_session_without_updates_defers_rewind_points() {
    use ezer_workspace::session::file_state::RewindPoint;
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
async fn merge_rewind_points_from_round_trips_file_snapshots() {
    use ezer_paths::RelPathBuf;
    use ezer_workspace::session::file_state::{FileSnapshot, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
async fn reads_never_modify_rewind_or_updates_files() {
    use ezer_workspace::session::file_state::{FileStateTracker, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
async fn init_session_stamps_configured_profile_on_new_session() {
    let tmp = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(tmp.path().to_path_buf());
    ezer_sandbox::set_configured_profile("workspace");
    let expected = ezer_sandbox::configured_profile_name().map(String::from);
    let info = Info {
...
/// Exercises the real on-read upgrade path end-to-end (loader, serde, and ezer_sampling_types::upgrade_legacy_reasoning).
...
/// The assistant carries `reasoning: { text, encrypted, id }` inline, the legacy ezer-build / Opus / chat-completions shape.
...
fn read_chat_history_upgrades_legacy_singular_reasoning_to_sibling() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"results...","reasoning":{"text":"the results are about cats","encrypted":"enc-blob","id":"rs_legacy"},"model_id":"ezer-build"}"#,
        ],
    );
    assert_eq!(
            items.len(),
    ...
    match item {
        ConversationItem::Reasoning(r) => {
            ...
            let Some(ezer_sampling_types::rs::SummaryPart::SummaryText(s)) = r
                .summary
                .first() else {
...
fn read_chat_history_handles_hybrid_legacy_and_post_pr_lines() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"a1","reasoning":{"text":"legacy thinking","encrypted":"enc","id":"rs_legacy"},"model_id":"ezer-build"}"#,
            ...
            r#"{"type":"assistant","content":"a2","model_id":"ezer-build"}"#,
        ],
    );
    let kinds: Vec<&'static str> = items
        .iter()
    ...
    let Some(ezer_sampling_types::rs::SummaryPart::SummaryText(s)) = reconstructed
        .summary
        .first() else {
...
fn read_chat_history_is_idempotent_on_post_pr_sessions() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"a","model_id":"ezer-build"}"#,
        ],
    );
    let kinds: Vec<&'static str> = items
        .iter()
...
fn read_chat_history_skips_merged_line_from_interrupted_append() {
    ...
    let good_2 = r#"{"type":"assistant","content":"after","model_id":"ezer-build"}"#;
    let raw = format!("{good_1}\n{partial}{merged_onto}\n{good_2}\n");
    let temp_dir = TempDir::new().unwrap();
...
/// Dirs loosened on disk (e.g. by an older ezer) re-tighten on next touch.
#[tokio::test]
#[cfg(unix)]
...
async fn usage_json_rewrites_session_and_appends_turns() {
    ...
    use ezer_sampling_types::TokenUsage;
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
