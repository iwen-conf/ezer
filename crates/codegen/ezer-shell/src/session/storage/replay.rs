...
pub fn load_updates_for_replay_at(
    ...
    let Some(updates_path) =
        resolve_replay_updates_path(session_id, ezer_home, ReplayPathHint::default())?
    else {
        return Ok(None);
    };
    Ok(Some(collect_replay_updates(&updates_path)?))
...
fn try_fast_replay_updates_path(
    ...
    for cwd in [hint.child_cwd, hint.parent_cwd].into_iter().flatten() {
        let encoded = ezer_config::encode_cwd_dirname(&cwd.to_string_lossy());
        let candidate = ezer_home.join("sessions").join(encoded).join(session_id);
...
/// Resolve `updates.jsonl` for `session_id` under `ezer_home`, or `None` when the session directory or the file is missing.
...
pub(crate) fn resolve_replay_updates_path(
    ...
    match try_fast_replay_updates_path(session_id, ezer_home, hint) {
...
/// Invoke `f` once per client-replay ACP update for a session under `ezer_home`, never building the full typed `Vec`.
...
pub fn stream_replay_updates_at<F: FnMut(acp::SessionUpdate)>(
    ...
    stream_replay_updates_at_hinted(session_id, ezer_home, ReplayPathHint::default(), |update| {
...
pub fn replay_would_emit(
    ...
    let Some(path) = resolve_replay_updates_path(session_id, ezer_home, hint)? else {
        return Ok(false);
    };
...
pub fn stream_replay_updates_at_hinted<F: FnMut(ReplayedUpdate)>(
    ...
    let Some(updates_path) = resolve_replay_updates_path(session_id, ezer_home, hint)? else {
