...
pub struct ContextInfo {
    ...
    /// Used by the TUI `/context` view so the displayed “Auto-compact at X%” matches the actual trigger (e.g. 65 for ezer-build in remote settings).
...
pub struct SessionInfoData {
    /// Agent definition name for this session (e.g. `ezer-build`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl StartupHints {
    ...
    pub(crate) fn resolve_mcp_strategy(&self) -> ezer_telemetry::enums::McpInitStrategy {
        use ezer_telemetry::enums::McpInitStrategy;
