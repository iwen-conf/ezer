...
#[cfg(test)]
pub(crate) async fn test_agent_default() -> ezer_agent::Agent {
    test_agent_with_tools(vec![]).await
}
#[cfg(test)]
pub(crate) async fn test_agent_backend_search(
    hosted_tools: Vec<ezer_sampling_types::HostedTool>,
) -> ezer_agent::Agent {
    let base = test_agent_default().await;
    ezer_agent::Agent::new(
        base.definition().clone(),
        ezer_agent::PromptContext::default(),
        String::new(),
        base.tool_bridge().clone(),
        ezer_agent::ReminderPolicy::default(),
        ezer_agent::CompactionPolicy::default(),
        hosted_tools,
        true,
    )
}
...
pub(crate) async fn test_agent_with_tools(
    tools: Vec<ezer_tools::registry::types::ToolConfig>,
) -> ezer_agent::Agent {
...
async fn test_agent_from_config(
    ...
    definition: ezer_agent::AgentDefinition,
    backend: std::sync::Arc<dyn ezer_tools::computer::types::TerminalBackend>,
) -> ezer_agent::Agent {
    use ezer_tools::computer::local::LocalFs;
    use ezer_tools::computer::types::AsyncFileSystem;
...
async fn create_test_actor_inner(
    ...
    let cwd = ezer_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap();
    let fs = Arc::new(ezer_workspace::file_system::MockFs::new(
        cwd.to_path_buf(),
    ));
    ...
    tool_context.task_completion_reservations =
        Some(ezer_tools::reminders::task_completion::TaskCompletionReservations::default());
    tool_context.task_wake_suppressed =
        Some(ezer_tools::reminders::task_completion::TaskWakeSuppressed::default());
    ...
    let chat_state_handle = xai_chat_state::ChatStateActor::spawn(
        vec![],
        ezer_sampling_types::SamplingConfig {
    ...
    let actor = SessionActor {
        ...
        permissions: ezer_workspace::permission::PermissionHandle::allow_all(),
        tool_context,
        ...
        vcs_kind: ezer_workspace::session::git::VcsKind::Git,
        ...
        sampler_handle: ezer_sampler::SamplerHandle::noop(),
        sampling_gate: None,
...
#[cfg(test)]
pub(crate) fn read_and_edit_toolset() -> Vec<ezer_tools::registry::types::ToolConfig> {
    use ezer_tools::registry::types::ToolConfig;
    vec![
...
pub(crate) fn pre_tool_use_spec(
    ...
    ezer_hooks::config::HookSpec {
        ...
        matcher: matcher.map(|m| ezer_hooks::matcher::HookMatcher::new(m).unwrap()),
        enabled: true,
...
pub(crate) fn post_tool_use_spec(
    ...
    ezer_hooks::config::HookSpec {
        event: ezer_hooks::event::HookEventName::PostToolUse,
...
pub(crate) fn post_tool_use_failure_spec(
    ...
    ezer_hooks::config::HookSpec {
        event: ezer_hooks::event::HookEventName::PostToolUseFailure,
        ..pre_tool_use_spec(name, matcher, script)
    }
}
#[cfg(test)]
pub(crate) fn install_pre_tool_use_hooks(
    actor: &mut SessionActor,
    specs: Vec<ezer_hooks::config::HookSpec>,
) {
    let (mut registry, _) = ezer_hooks::discovery::load_hooks(None, None);
...
pub(crate) async fn tool_result_text(actor: &SessionActor, call_id: &str) -> String {
    ...
    conversation
        ...
        .find_map(|item| match item {
            ezer_sampling_types::ConversationItem::ToolResult(result)
...
pub(crate) async fn actor_with_persistence_drain() -> std::sync::Arc<SessionActor> {
    actor_with_persistence_drain_and_sampler(ezer_sampler::SamplerHandle::noop()).await
}
#[cfg(test)]
pub(crate) async fn actor_with_persistence_drain_and_sampler(
    sampler: ezer_sampler::SamplerHandle,
) -> std::sync::Arc<SessionActor> {
...
impl ezer_tools::types::tool_metadata::ToolMetadata for StubMcpTool {
    fn kind(&self) -> ezer_tools::types::tool::ToolKind {
        ezer_tools::types::tool::ToolKind::Other
    }
    fn tool_namespace(&self) -> ezer_tools::types::tool::ToolNamespace {
        ezer_tools::types::tool::ToolNamespace::MCP
    }
...
impl xai_tool_runtime::Tool for StubMcpTool {
    ...
    type Output = ezer_tools::types::output::ToolOutput;
    ...
    async fn run(
        ...
        Ok(ezer_tools::types::output::ToolOutput::MCP(
            ezer_tools::types::output::MCPOutput::errored(
                self.0.into(),
                "stub".into(),
                "unused".into(),
            ),
