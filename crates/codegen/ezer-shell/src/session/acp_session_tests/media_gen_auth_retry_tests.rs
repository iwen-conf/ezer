...
fn succeeding_am() -> Arc<AuthManager> {
    ...
    impl ezer_login::refresh::TokenRefresher for Ok {
        async fn refresh(
            &self,
...
fn failing_am() -> Arc<AuthManager> {
    ...
    struct Fail;
    #[async_trait::async_trait]
    impl ezer_login::refresh::TokenRefresher for Fail {
        async fn refresh(
            &self,
            _: ezer_login::refresh::RefreshReason,
        ) -> ezer_login::refresh::RefreshOutcome {
            ezer_login::refresh::RefreshOutcome::permanent(
                ezer_login::RefreshTokenFailedReason::RefreshTokenRejected,
                None,
            )
        }
    }
...
fn is_auth_tool_error_classification() {
    ...
    let cases: Vec<(bool, xai_tool_runtime::ToolError)> = vec![
        ...
        // This mirrors the inference path's gate in ezer-sampling-types/src/error.rs 403 means "authenticated but not permitted" (content safety, ZDR, remote settings gates).
        (
            false,
            xai_tool_runtime::ToolError::new(
                xai_tool_runtime::ToolErrorKind::Custom,
