...
use ezer_workspace::file_system::MockFs;
use ezer_workspace::permission::PermissionHandle;
#[test]
            ...
            );
            {
                let head = disabled_actor
                    ...
                    .find_map(|item| match item {
                        ezer_sampling_types::ConversationItem::System(sys) => {
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.replace_conversation(vec![
                ...
                ezer_sampling_types::ConversationItem::assistant("looking at auth.rs"),
                ezer_sampling_types::ConversationItem::user(
                    "also add integration tests for the token refresh path",
                ),
                ezer_sampling_types::ConversationItem::assistant("found the issue"),
                ezer_sampling_types::ConversationItem::user(
                    "great, can you patch the logout handler as well",
                ),
                ezer_sampling_types::ConversationItem::assistant("done"),
            ]);
            let timer = ezer_telemetry::session_end::SessionEndTimer::new_shared();
...
async fn create_injection_ready_actor(
    initial_conversation: Vec<ezer_sampling_types::ConversationItem>,
) -> SessionActor {
    let (gateway_tx, _gateway_rx) = mpsc::unbounded_channel();
    let (persistence_tx, _persistence_rx) = mpsc::unbounded_channel();
    let mut config = crate::config::MemoryConfig::default();
    ...
    local
        .run_until(async {
            let actor = create_injection_ready_actor(vec![
                ezer_sampling_types::ConversationItem::system("You are a helpful assistant."),
                ezer_sampling_types::ConversationItem::user(
        ...
        .run_until(async {
            let persisted_block =
                crate::session::helpers::memory_context::format_memory_reminder(&[
                    ezer_tools::types::memory_backend::MemorySearchResult {
                ...
                .unwrap();
            let actor = create_injection_ready_actor(vec![
                ezer_sampling_types::ConversationItem::system(format!(
