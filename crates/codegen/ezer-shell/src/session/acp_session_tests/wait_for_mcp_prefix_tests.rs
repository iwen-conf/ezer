...
use ezer_mcp::servers::{McpInitStrategy, SharedMcpState};
...
async fn grace_body() {
    ...
    impl ezer_mcp::acp_transport::AcpReverseInvoker for NoopInvoker {
        async fn invoke(
            &self,
            _server_id: &str,
    ...
    acp_only.mcp_state.lock().await.set_acp_servers(
        vec![ezer_mcp::servers::AcpServerEntry {
            name: "sdk".to_string(),
...
async fn dispatch_body() {
    ...
    #[async_trait::async_trait]
    impl ezer_mcp::acp_transport::AcpReverseInvoker for ReadySdkServer {
        async fn invoke(
            &self,
            _server_id: &str,
