...
use ezer_telemetry::config::TelemetryMode;
use ezer_tools::implementations::ezer_build::grep::GrepTool;
use ezer_tools::implementations::ezer_build::read_file::ReadFileTool;
use ezer_tools::implementations::ezer_build::search_replace::SearchReplaceTool;
...
impl Drop for ResetTelemetry {
    fn drop(&mut self) {
        let config = ezer_telemetry::config::TelemetryConfig {
            mixpanel_enabled: false,
            mixpanel_token: None,
            ..ezer_telemetry::config::TelemetryConfig::default()
        };
        ezer_telemetry::init(
            config,
...
fn init_product(server: &ezer_test_support::MockInferenceServer, mode: TelemetryMode) {
    let config = ezer_telemetry::config::TelemetryConfig {
...
async fn execute_tool_calls_records_the_product_row_and_execution_span() {
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let product = ezer_test_support::MockInferenceServer::start()
                .await
                .expect("product");
            ...
            *actor.agent.borrow_mut() = test_agent_with_tools(vec![
            ...
            ezer_telemetry::session_ctx::drain_pending(std::time::Duration::from_secs(5)).await;
...
async fn read_profile_agrees_on_the_product_row_and_span() {
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let over = "Q".repeat(
                ezer_tools::implementations::ezer_build::read_file::READ_FILE_MAX_BYTES
            ...
            *actor.agent.borrow_mut() = test_agent_with_tools(vec![
                ...
                .expect("read execute");
            {
                ...
                resources.insert(ezer_tools::types::resources::Params(
                    ezer_tools::implementations::ezer_build::read_file::ReadFileParams {
