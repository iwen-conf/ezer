...
use ezer_test_support::sse::responses_api_script_exact;
use ezer_test_support::{MockInferenceServer, ScriptedResponse};
...
async fn actor_with_sampler_configured(
    ...
    let sampling_config = ezer_sampler::SamplerConfig {
    ...
    actor.client_hooks.borrow_mut().insert(
        ezer_hooks::event::HookEventName::UserPromptSubmit,
    ...
    config.api_backend = ezer_sampling_types::ApiBackend::Responses;
                        ...
                        matches!(
                            ...
                            ConversationItem::User(user)
                                if user.synthetic_reason
                                    == ezer_sampling_types::SyntheticReason::AgentMessage
...
async fn parent_skill_lookup_matches_advertised_gated_collision_and_skill_only_loader() {
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_tools(vec![
                ezer_tools::registry::types::ToolConfig::for_tool::<
                    ezer_tools::implementations::opencode::OpenCodeSkillTool,
                >(),
            ])
            .await;
            actor
