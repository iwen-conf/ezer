...
//! (the root-owned `/etc/ezer` tier and the signed synced `$EZER_HOME/requirements.toml`).
//! The per-hook `Disable` action must be refused and the bulk `ToggleSource` must skip it, both before writing any disable state.
//! The dispatcher-level exemption and the display predicate are covered with a sandboxed `EZER_HOME` in `ezer_hooks::dispatcher` tests.
...
/// A temp `EZER_HOME` alone cannot redirect it: `ezer_home()` is `OnceLock`-cached and another test in this binary may have resolved it first.
...
impl DisabledHooksGuard {
    fn capture() -> Self {
        let path = ezer_config::user_ezer_home().map(|home| home.join("disabled-hooks"));
...
fn managed_registry(name: &str, layer: HookProvenance) -> ezer_hooks::discovery::HookRegistry {
    ezer_hooks::discovery::registry_from_specs_deduped(vec![ezer_hooks::config::HookSpec {
        name: name.to_string(),
    ...
    local
        .run_until(async {
            ...
            *actor.hook_disabled.borrow_mut() =
                Arc::new(ezer_hooks::trust::DisabledHooks::new([], true));
...
async fn toggling_a_hook_refreshes_the_dispatch_snapshot() {
    ...
    local
        .run_until(async {
            ...
            let _ = ezer_hooks::trust::enable_hook(HOOK);
