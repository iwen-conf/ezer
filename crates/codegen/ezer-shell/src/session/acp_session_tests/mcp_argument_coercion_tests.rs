...
impl ezer_tools::types::tool_metadata::ToolMetadata for RegisteredSplunkTool {
...
impl xai_tool_runtime::Tool for RegisteredSplunkTool {
    ...
    async fn run(
        ...
        Ok(ToolOutput::Text(
            ezer_tools::types::output::TextOutput::from("unused".to_owned()),
        ))
    }
}
...
async fn renamed_tool_input_key_is_replaced_before_remap() {
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let mut config = ezer_tools::registry::types::ToolConfig::for_tool::<
                ezer_tools::implementations::use_tool::UseTool,
            >();
            ...
            *actor.agent.borrow_mut() = test_agent_with_tools(vec![config]).await;
            ...
            let accepted = ezer_tools::util::remap::remap_json_keys_checked(
