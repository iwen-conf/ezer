...
fn main_turn_input(items: Vec<ConversationItem>) -> Vec<serde_json::Value> {
    let request = ezer_sampling_types::ConversationRequest {
...
fn assert_messages_rides_parent_prefix(
    ...
    let request = ezer_sampling_types::ConversationRequest {
        ...
        reasoning_effort: Some(ezer_sampling_types::ReasoningEffort::High),
        ..Default::default()
    };
    let expected = serde_json::to_value(ezer_sampling_types::build_messages_request(&request))
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            cfg.api_backend = ezer_sampling_types::ApiBackend::Responses;
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            cfg.reasoning_effort = Some(ezer_sampling_types::ReasoningEffort::Low);
            actor.chat_state_handle.update_sampling_config(cfg);
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            let mut cfg = actor.chat_state_handle.get_sampling_config().await.unwrap();
            cfg.base_url = server.url();
            cfg.api_backend = ezer_sampling_types::ApiBackend::ChatCompletions;
            actor.chat_state_handle.update_sampling_config(cfg);
...
fn over_budget_recap_serializes_to_well_formed_messages_request() {
    ...
    use ezer_sampling_types::messages::{ContentBlock, MessageContent, MessageRole};
    use ezer_sampling_types::{ConversationRequest, ToolCall, rs};
...
async fn recap_request_sends_hosted_tools_under_backend_search() {
    use ezer_sampling_types::HostedTool;
    use ezer_test_support::MockInferenceServer;
        ...
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            // Backend-search fixture: agent carries hosted tools and both gates are on.
            {
                ...
                *agent_slot = ezer_agent::Agent::new(
...
async fn recap_hosted_tools_reflect_the_active_per_turn_override() {
    use ezer_sampling_types::{HostedTool, SearchDateBound, ToolOverrides, XSearchOptions};
    use ezer_test_support::MockInferenceServer;
...
async fn messages_side_calls_preserve_completed_reasoning() {
    use ezer_sampling_types::{ReasoningEffort, rs};
    use ezer_test_support::MockInferenceServer;
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            let mut cfg = actor.chat_state_handle.get_sampling_config().await.unwrap();
            cfg.base_url = server.url();
            cfg.api_backend = ezer_sampling_types::ApiBackend::Messages;
...
async fn messages_side_calls_strip_reasoning_without_supported_thinking_effort() {
    use ezer_sampling_types::{ReasoningEffort, synthesized_reasoning_item};
    use ezer_test_support::MockInferenceServer;
...
async fn side_question_trims_reasoning_orphaned_by_mid_turn_truncation() {
    use ezer_sampling_types::conversation::{AssistantItem, ToolCall};
    use ezer_test_support::MockInferenceServer;
