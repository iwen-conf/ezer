            ...
            *actor.agent.borrow_mut() = super::support::test_agent_with_active_message_tool().await;
            ...
            for (id, delivery, expected_operation) in [
                (
                    "queued",
                    "queue",
                    ezer_telemetry::events::ActiveAgentMessageOperation::Queue,
                ),
                (
                    "interjected",
                    "interject",
                    ezer_telemetry::events::ActiveAgentMessageOperation::Interject,
                ),
            ] {
                let events = execute_with_captured_active_message_events(
                    &actor,
                ...
                assert!(matches!(
                    ...
                    [
                        crate::session::telemetry::ActiveAgentMessageEvent::Completed(
                            ezer_telemetry::events::ActiveAgentMessageCompleted {
                                requested_operation,
                                ..
                            }
                        )
...
fn lock_path_for_args_buckets_ezer_build_and_compat_to_same_lock_for_same_file() {
    ...
    assert_eq!(
        lock_path_for_args(&ezer, Path::new("/cwd")),
...
fn test_skill_discovery_deferred_during_parallel_batch() {
    use ezer_sampling_types::{ConversationItem, SyntheticReason};
