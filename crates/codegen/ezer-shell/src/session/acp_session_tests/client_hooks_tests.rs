...
fn post_tool_use_envelope(actor: &SessionActor) -> ezer_hooks::event::HookEventEnvelope {
    actor.make_hook_envelope(
        ezer_hooks::event::HookEventName::PostToolUse,
        None,
        ezer_hooks::event::HookPayload::PostToolUse {
...
async fn client_hooks_fire_without_file_registry() {
    ...
    local
        .run_until(async {
            ...
            install_client_hook(
                &actor,
                ezer_hooks::event::HookEventName::Stop,
                &["cb_0"],
            );
            ...
            actor.fire_hook(
                ezer_hooks::event::HookEventName::Stop,
                None,
                ezer_hooks::event::HookPayload::Stop {
        ...
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_tools(vec![
                ezer_tools::registry::types::ToolConfig::for_tool::<
                    ezer_tools::implementations::use_tool::UseTool,
                >(),
            ])
            .await;
            ...
            client_hooks.insert(
                ...
                vec![crate::extensions::hooks::ClientHookGroup {
                    matcher: Some(
                        ezer_hooks::matcher::HookMatcher::new("linear__save_issue").unwrap(),
                    ),
...
impl ezer_tools::types::tool_metadata::ToolMetadata for McpErrorResultTool {
...
async fn post_tool_use_dispatch_merges_file_then_client_contributions() {
    use ezer_tools::types::output::{MCPOutput, ToolOutput, ToolRunResult};
...
pub(super) fn file_registry_with_spec(
    event: ezer_hooks::event::HookEventName,
    script: &str,
) -> ezer_hooks::discovery::HookRegistry {
...
fn file_registry(
    ...
    registry.append_specs(vec![ezer_hooks::config::HookSpec {
...
async fn subagent_session_gates_on_subagent_stop() {
    ...
    local
        .run_until(async {
            ...
            *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(file_registry_with_spec(
                ezer_hooks::event::HookEventName::SubagentStop,
...
async fn alias_envelope_serializes_canonical_event_name() {
    ...
    local
        .run_until(async {
            ...
            let envelope = actor.make_hook_envelope(
                ezer_hooks::event::HookEventName::SubagentEnd,
                None,
                ezer_hooks::event::HookPayload::SubagentStop {
                    phase: ezer_hooks::event::SubagentStopPhase::Observe,
...
async fn only_a_completed_stop_hook_is_the_turns_report() {
    ...
    local
        .run_until(async {
            ...
            install_client_hook(
                &actor,
                ezer_hooks::event::HookEventName::StopFailure,
                &["cb_fail"],
            );
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(file_registry(
                ...
                let reported = actor.claim_and_queue(
                    ...
                    super::turn_end_hooks::TurnEnd::Failed {
                        error: ezer_hooks::event::StopFailureKind::Unknown,
                        error_details: None,
                        last_assistant_message: None,
                    },
                );
...
async fn pre_tool_batch_start_and_outcome_share_one_identity() {
    ...
    local
        .run_until(async {
            ...
            let registry =
                ezer_hooks::discovery::registry_from_specs_deduped(vec![pre_tool_use_spec(
                    "test/pre",
                    Some("read_file"),
                    "true",
                )]);
