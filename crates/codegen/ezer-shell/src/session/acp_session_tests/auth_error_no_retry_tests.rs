...
impl ezer_login::refresh::TokenRefresher for AlwaysSucceedRefresher {
    async fn refresh(
        &self,
...
#[async_trait::async_trait]
impl ezer_login::refresh::TokenRefresher for AlwaysTransientFailRefresher {
    async fn refresh(
        &self,
        _reason: ezer_login::refresh::RefreshReason,
    ) -> ezer_login::refresh::RefreshOutcome {
        self.called.store(true, Ordering::SeqCst);
        ezer_login::refresh::RefreshOutcome::TransientFailure {
...
fn auth_error() -> ezer_sampler::SamplingErrorInfo {
    ezer_sampler::SamplingErrorInfo {
        kind: ezer_sampler::SamplingErrorKind::Auth,
...
async fn no_emit_when_auth_manager_is_none() {
    ...
    local
        .run_until(async {
            ...
            ezer_login::attribution::reset_test_emit_count();
            let _ = actor
                ...
                .await;
            assert_eq!(
                ezer_login::attribution::test_emit_count(),
                0,
...
async fn sampler_401_recovery_returns_refresh_and_retry() {
    ...
    local
        .run_until(async {
            ...
            let refresher: Arc<dyn ezer_login::refresh::TokenRefresher> =
...
async fn credential_less_401_with_deferred_refresh_parks_on_uncharged_resubmit() {
    ...
    local
        .run_until(async {
            ...
            let result = actor
                .handle_sampling_failure(
                    auth_error_with_credential(ezer_sampling_types::SentCredential::Missing),
                    0,
                    transient_state(0, true),
                    false,
                    TurnParkState::Fresh,
            ...
            assert!(
                matches!(
                    result,
                    Ok(SamplerFailureRecovery::RefreshAuthAndResubmit {
                        credential: ezer_sampling_types::SentCredential::Missing,
                        store: RecoveredStore::SessionToken,
                    })
                ),
    ...
    local
        .run_until(async {
            for park in [TurnParkState::Fresh, TurnParkState::Parked] {
                ...
                let error = ezer_sampler::SamplingErrorInfo {
...
async fn credentialed_401_with_deferred_refresh_stays_terminal() {
    ...
    local
        .run_until(async {
            for credential in [
                ezer_sampling_types::SentCredential::Sent,
                ezer_sampling_types::SentCredential::Unknown,
            ] {
                let refresher: Arc<dyn ezer_login::refresh::TokenRefresher> =
                    Arc::new(AlwaysTransientFailRefresher {
    ...
    local
        .run_until(async {
            ...
            am.record_permanent_failure(
                "initial-test-key".to_string(),
                ezer_login::error::RefreshTokenFailedReason::RefreshTokenRejected.into(),
            );
            let (actor, _rx) = make_actor_with_auth_manager(Some(am)).await;
            let result = actor
                .handle_sampling_failure(
...
async fn pre_flight_hard_expired_refresh_failure_skips_jwt_fallthrough() {
    ...
    local
        .run_until(async {
            ...
            let refresher: Arc<dyn ezer_login::refresh::TokenRefresher> = Arc::new({
                struct AlwaysFail(Arc<std::sync::atomic::AtomicU32>);
                #[async_trait::async_trait]
                impl ezer_login::refresh::TokenRefresher for AlwaysFail {
                    async fn refresh(
                        &self,
                        _: ezer_login::refresh::RefreshReason,
                    ) -> ezer_login::refresh::RefreshOutcome {
                        self.0.fetch_add(1, Ordering::SeqCst);
                        ezer_login::refresh::RefreshOutcome::transient("refresh failed")
                    }
                }
    ...
    local
        .run_until(async {
            ...
            let refresher: Arc<dyn ezer_login::refresh::TokenRefresher> = Arc::new({
                ...
                #[async_trait::async_trait]
                impl ezer_login::refresh::TokenRefresher for Counting {
                    async fn refresh(
                        &self,
                        _: ezer_login::refresh::RefreshReason,
                    ) -> ezer_login::refresh::RefreshOutcome {
            ...
            tokio::time::sleep(
                ezer_login::manager::PROACTIVE_MIN_SLEEP
...
fn model_not_found_error() -> ezer_sampler::SamplingErrorInfo {
    ezer_sampler::SamplingErrorInfo {
        ...
        .run_until(async {
            ...
            actor
                .handle_set_session_model(crate::session::SessionModelSwitch {
                    ...
                    system_prompt_label: ezer_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_owned(),
                })
                .await
...
async fn set_session_model_invalidates_byok_memo_for_same_model_id() {
    ...
    local
        .run_until(async {
            ...
            let cfg = ezer_sampler::SamplerConfig {
            ...
            let expected_max_retries = ezer_sampler::resolve_max_retries(Some(6));
            let stored = actor
                .chat_state_handle
                .get_sampling_config()
                .await
...
use ezer_login::test_counting_provider as counting_provider;

/// Seed the per-model memo so `model_auth_provider` resolves without a config load.
async fn seed_provider_memo(actor: &Arc<SessionActor>, provider: ezer_login::AuthProviderRef) {
    let model = actor
        .chat_state_handle
        .get_sampling_config()
        .await
        ...
        .run_until(async {
            ...
            let (actor, _rx) =
                make_actor_with_auth_and_credentials(None, xai_chat_state::AuthType::ApiKey, token)
                    .await;
            seed_provider_memo(&actor, provider).await;
            ezer_login::test_backdate_provider_mint(
                "test-4c-recover",
...
async fn sampler_non_auth_kind_401_on_provider_model_still_recovers() {
    ...
    local
        .run_until(async {
            ...
            error.kind = ezer_sampler::SamplingErrorKind::Api;
            let result = actor
                .handle_sampling_failure(
                    error,
                    0,
