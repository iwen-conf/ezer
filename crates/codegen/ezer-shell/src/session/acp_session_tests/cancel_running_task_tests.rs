...
async fn persist_ack_waits_for_disk_flush_before_success() {
    ...
    local
        .run_until(async {
            ...
            let sampling_client = crate::sampling::Client::new(ezer_sampler::SamplerConfig {
...
async fn cancel_running_task_teardown_clears_running_and_pending_work() {
    ...
    local
        .run_until(async {
            ...
            let cwd = AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap();
            let fs = Arc::new(
                ezer_workspace::file_system::MockFs::new(cwd.to_path_buf()),
            );
            let terminal = Arc::new(DummyTerminal {});
            let (hunk_tx, _hunk_rx) = tokio::sync::mpsc::unbounded_channel();
            let hunk_tracker_handle = xai_hunk_tracker::HunkTrackerActor::spawn(
    ...
    local
        .run_until(async {
            ...
            *actor
            ...
            let cancelled_strip = ezer_sampler::RequestId::from("cancelled-strip");
            let timed_out_strip = ezer_sampler::RequestId::from("older-timeout-strip");
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.push_assistant_response(
                ConversationItem::assistant_tool_calls(vec![ezer_sampling_types::ToolCall {
                    id: "call-1".into(),
    ...
    local
        .run_until(async {
            ...
            let cfg = ezer_sampler::SamplerConfig {
                ...
                model: "test-model".to_string(),
                api_backend: ezer_sampler::ApiBackend::Responses,
                context_window: 100_000,
                max_retries: Some(0),
            ...
            let (sampler_event_tx, _sampler_event_rx) = tokio::sync::mpsc::unbounded_channel::<
                ezer_sampler::SamplingEvent,
            >();
            let sampler_handle = ezer_sampler::SamplerActor::spawn(
                cfg,
                ezer_sampler::RetryPolicy::default(),
                sampler_event_tx,
            );
            let (gateway_tx, _gateway_rx) = tokio::sync::mpsc::unbounded_channel::<
                xai_acp_lib::AcpClientMessage,
...
async fn skill_reminder_deferred_while_turn_running_flushed_when_idle() {
    use ezer_tools::types::skill_discovery_tracker::{SkillUpdateEffects, SkillUpdateKind};
        ...
        actor
            ...
            .filter(|item| {
                matches!(item, ConversationItem::User(u) if u.content.iter().any(|p| matches!(
                    p,
                    ezer_sampling_types::ContentPart::Text { text } if text.contains("pdf-tools")
                )))
            })
            .count()
    }
