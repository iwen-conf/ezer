...
use ezer_test_support::{MockInferenceServer, MockModelEntry};
...
fn sampling_request_header_carries_the_turn_trace_id() {
    on_session_stack(|| {
        run_paused(|| async {
            ...
            let _trace = ezer_otel::set_local_trace_subscriber();
            ...
            let root_traceparent =
                ezer_otel::span_traceparent(&turn_root).expect("root span has a trace id");
