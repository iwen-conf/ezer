...
fn switch_target_config(model: &str, base_url: String) -> ezer_sampler::SamplerConfig {
    ezer_sampler::SamplerConfig {
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.replace_conversation(vec![
                ...
                ConversationItem::Reasoning(ezer_sampling_types::rs::ReasoningItem {
                ...
                ConversationItem::ToolResult(ezer_sampling_types::ToolResultItem {
...
async fn route_compaction_to_mock_server(
    actor: &SessionActor,
    summary: String,
) -> ezer_test_support::MockInferenceServer {
    let server = ezer_test_support::MockInferenceServer::start()
        .await
        .unwrap();
...
async fn compaction_paths_note_survives_second_compaction_and_filters_missing_files() {
    ...
    local
        .run_until(async {
            ...
            crate::util::ezer_home::create_dir_all_owner_only(&assets_dir).unwrap();
...
fn user_facing_compact_error_strips_prefix_single_lines_and_caps() {
    ...
    let nested = ezer_compaction::sampler::CompactionSampleError::Build(format!(
...
fn api_error_with_context_window(context_window: u64) -> ezer_sampler::SamplingErrorInfo {
