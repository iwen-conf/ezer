...
/// Env lane (desktop `EZER_SESSION_LIST_CONVERSATIONS`) OR process-wide `--chat` (`EZER_CHAT_MODE`); hard-off in release builds.
...
mod tests {
    ...
    async fn forced_kind_serves_conversations_only() {
        let addr = spawn_conversations_stub(
            ...
            .await;
        let _env = ezer_test_support::EnvGuard::set(
    ...
    fn conversations_lane_env_gating_matrix() {
        {
            let _off = ezer_test_support::EnvGuard::unset("EZER_SESSION_LIST_CONVERSATIONS");
            assert!(!conversations_lane_enabled());
        }
        {
            let _on = ezer_test_support::EnvGuard::set("EZER_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_enabled(), false);
        }
        {
            let _off = ezer_test_support::EnvGuard::set("EZER_SESSION_LIST_CONVERSATIONS", "0");
            assert!(!conversations_lane_enabled());
        }
    }
    ...
    fn conversations_lane_active_truth_table() {
        ...
        let _chat_off = ezer_test_support::EnvGuard::unset(EZER_CHAT_MODE_ENV);
        let _desktop_off =
            ezer_test_support::EnvGuard::unset("EZER_SESSION_LIST_CONVERSATIONS");
        assert!(
        ...
        {
            let _desktop =
                ezer_test_support::EnvGuard::set("EZER_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_active(), false);
        }
        {
            let _chat = ezer_test_support::EnvGuard::set(EZER_CHAT_MODE_ENV, "1");
            assert_eq!(
                conversations_lane_active(),
                false,
    ...
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        .to_string();
        {
            let _off = ezer_test_support::EnvGuard::unset(EZER_CHAT_MODE_ENV);
        ...
        {
            let _on = ezer_test_support::EnvGuard::set(EZER_CHAT_MODE_ENV, "1");
            let req = parse_list_req(&raw).expect("parse");
            let parsed = ParsedMeta::parse(req.meta.as_ref());
    ...
    fn relax_rows_scopes_to_repo_and_requires_messages() {
        ...
        let repo_url = ezer_workspace::session::git::normalize_repo_url(this_repo).unwrap();
