...
use ezer_tools::implementations::codex::CodexReadFileTool;
use ezer_tools::implementations::ezer_build::{ReadFileTool, SearchReplaceTool};
use ezer_tools::implementations::ezer_build_concise::{
    ReadFileConciseTool, SearchReplaceConciseTool,
};
use ezer_tools::implementations::opencode::OpenCodeWriteTool;
use ezer_tools::registry::types::{FinalizedToolset, RegisteredToolIdentity};
use ezer_tools::types::output::{GrepSearchOutput, ToolOutput};
use ezer_tools::types::resources::resolve_model_path;
use ezer_tools::types::source_summary::ToolSourceSummary;
...
pub(crate) fn record_tool_execution(
    ...
    // Approved ezer ids only, matching product serialization. Unknown stays absent.
...
fn path_scope_ids() -> &'static [String] {
    ...
    IDS.get_or_init(|| {
        ...
        let mut ids = vec![
            qualified_id(ToolNamespace::EzerBuild, &read_file.id()),
            qualified_id(ToolNamespace::EzerBuildConcise, &concise_read.id()),
            qualified_id(ToolNamespace::Codex, &codex_read.id()),
            qualified_id(ToolNamespace::EzerBuild, &search_replace.id()),
            qualified_id(ToolNamespace::EzerBuildConcise, &concise.id()),
            qualified_id(ToolNamespace::OpenCode, &write.id()),
        ];
        ids.extend(ezer_tools::implementations::extra_write_qualified_ids());
        ids
    })
}
...
mod tests {
    ...
    use ezer_tools::implementations::codex::CodexReadFileTool;
    use ezer_tools::implementations::ezer_build::{GrepTool, ReadFileTool, SearchReplaceTool};
    use ezer_tools::implementations::ezer_build_concise::{
        ReadFileConciseTool, SearchReplaceConciseTool,
    };
    use ezer_tools::implementations::opencode::OpenCodeWriteTool;
    use ezer_tools::types::output::{GrepSearchOutput, TextOutput};
    use ezer_tools::types::resources::resolve_model_path;
    use ezer_tools::types::tool::ToolNamespace;
    use xai_tool_runtime::Tool;
    ...
    fn path_scope_follows_the_registered_id_not_the_client_name() {
        let cwd = Path::new("/opt/repo");
        let read_file = registered(ToolNamespace::EzerBuild, &ReadFileTool);
        let concise_read = registered(ToolNamespace::EzerBuildConcise, &ReadFileConciseTool);
        let codex_read = registered(ToolNamespace::Codex, &CodexReadFileTool);
        let search_replace = registered(ToolNamespace::EzerBuild, &SearchReplaceTool);
        let concise = registered(ToolNamespace::EzerBuildConcise, &SearchReplaceConciseTool);
        let write = registered(ToolNamespace::OpenCode, &OpenCodeWriteTool);
        let grep = registered(ToolNamespace::EzerBuild, &GrepTool);
        ...
        for id in ezer_tools::implementations::extra_write_qualified_ids() {
