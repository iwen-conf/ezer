...
//!    In its `acp::McpServer::Stdio` arm, [`ezer_mcp::servers::start_mcp_server`] sets `kill_on_drop(true)` on the child it spawns.
...
//! 5. **Already-Empty**: see the [`ezer_mcp::servers::ClientStateKind::Empty`] doc: a previous handshake exhausted attempts.
...
use ezer_mcp::servers::{McpClientEventKind, McpServerName};
...
pub(crate) trait RestartActions {
    ...
    /// Push an already-built `ezer/mcp/server_status` payload to the pager.
...
/// Serialize a [`McpServerStatusPayload`] and send it to the gateway as an ACP `ezer/mcp/server_status` notification.
