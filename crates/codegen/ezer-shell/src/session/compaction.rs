...
use ezer_sampling_types::{ApiBackend, ConversationItem};
...
impl SessionActor {
    ...
    async fn maybe_pre_compaction_flush(
        ...
        if ezer_memory::flush::should_flush(
            total_tokens,
            context_window,
    ...
    pub(crate) async fn run_compact(
        ...
        if let Err(e) = self
            .run_compact_inner(
                user_context,
                None,
                ezer_telemetry::events::CompactionTrigger::Manual,
                false,
            )
            .await
        {
    ...
    async fn suppress_auto_compaction(
            ...
            .is_ok()
        {
            ...
            ezer_telemetry::session_ctx::log_event(
                ezer_telemetry::events::AutoCompactSuppressed {
                    reason: reason.into(),
                    estimated_tokens,
                    context_window,
                },
    ...
    fn user_facing_compact_error(raw: &str) -> String {
        ...
        const INTERNAL_PREFIXES: &[&str] = &[
            ...
            ezer_compaction::sampler::SAMPLER_BUILD_FAILED_PREFIX,
            ezer_compaction::sampler::SAMPLER_START_FAILED_PREFIX,
        ];
    ...
    pub(crate) async fn surface_compact_auth_failure(&self, err: acp::Error) -> acp::Error {
        ...
        acp::Error::auth_required().data(crate::sampling::error::terminal_error_data(
            message,
            Some(401),
            ezer_sampler::SamplingErrorKind::Auth,
        ))
    }
    ...
    async fn run_compact_inner(
        ...
        trigger: ezer_telemetry::events::CompactionTrigger,
        lossy_input: bool,
    ) -> Result<(), acp::Error> {
        let (cancel, _cancel_scope) = self.compaction.cancel.enter();
        ...
        let trigger_str = match trigger {
            ezer_telemetry::events::CompactionTrigger::Manual => "manual",
            ezer_telemetry::events::CompactionTrigger::Auto => "auto",
        };
        let sampling_config = self.chat_state_handle.get_sampling_config().await;
        let context_window = sampling_config
            .as_ref()
        ...
        let compaction = ezer_telemetry::events::CompactionScope::begin(
            ezer_telemetry::events::CompactionBeginParams {
                trigger,
                ...
                compaction_mode: match self.compaction.compaction_mode {
                    xai_chat_state::CompactionMode::Summary => {
                        ezer_telemetry::events::CompactionModeLabel::Summary
                    }
                    xai_chat_state::CompactionMode::Transcript => {
                        ezer_telemetry::events::CompactionModeLabel::Transcript
                    }
                    xai_chat_state::CompactionMode::Segments(_) => {
                        ezer_telemetry::events::CompactionModeLabel::Segments
                    }
                },
        ...
        self.dispatch_hook(
            ezer_hooks::event::HookEventName::PreCompact,
            ezer_hooks::event::HookPayload::PreCompact {
        ...
        let effective_tool_defs: Vec<ezer_sampling_types::ToolDefinition> = self
        ...
        let compaction_tools: Vec<ezer_sampling_types::ToolSpec> = effective_tool_defs
            .into_iter()
            .map(ezer_sampling_types::ToolSpec::from)
            .collect();
        let compaction_hosted_tools: Vec<ezer_sampling_types::HostedTool> =
        ...
        let auto_trigger = matches!(trigger, ezer_telemetry::events::CompactionTrigger::Auto);
        let wall_clock_budget_secs = self
            .agent
            .borrow()
            .compaction_policy()
        ...
        let fr_config = ezer_compaction::FullReplaceConfig {
        ...
        while compact_summary.is_none() {
            match ezer_compaction::sample_full_replace_summary(
                &sampler,
                &request_turns,
            ...
            .await
            {
                ...
                Err(ezer_compaction::FullReplaceError::NothingToCompact) => {
                    last_error = Some(
                ...
                Err(ezer_compaction::FullReplaceError::EmptyResponse) => {
                ...
                Err(ezer_compaction::FullReplaceError::Sampler {
                    message,
                    deterministic,
                    context_overflow,
                }) => {
                    ...
                    if context_overflow {
                        ...
                        if let Some(stage) = next_stage {
                            ...
                            ezer_telemetry::session_ctx::log_event(
                                ezer_telemetry::events::CompactionRetryDegraded {
                                    trigger,
                    ...
                    let running_tasks: Vec<_> = pending_tasks
                        ...
                        .map(|t| {
                            let tool_name = match t.kind {
                                ezer_tools::computer::types::TaskKind::Monitor => {
                                    monitor_tool_name.clone()
                                }
                                ezer_tools::computer::types::TaskKind::Bash => {
        ...
        let memory_ref: Option<&dyn ezer_tools::types::memory_backend::MemoryBackend> =
            if memory_opt_out {
                None
            } else {
                memory_backend_impl
                    .as_ref()
                    .map(|b| b as &dyn ezer_tools::types::memory_backend::MemoryBackend)
            };
            ...
            .and_then(|item| match item {
                ConversationItem::User(parts) => {
                    parts.content.into_iter().next().and_then(|p| match p {
                        ezer_sampling_types::ContentPart::Text { text } => {
        ...
        if self.memory.is_enabled() {
            tracing::info!(target: ezer_telemetry::memory_log::TARGET, "MEMORY_COMPACT: post-compaction reset, next turn re-checks injection (search only if no block persisted)");
        }
        let _ = self
            .notifications
            .persistence_tx
        ...
        if self
            .tool_bridge_handle()
            .toolset()
            .tool_name_for_kind(ezer_tools::types::tool::ToolKind::UseTool)
            .is_some()
        ...
        self.dispatch_hook(
            ezer_hooks::event::HookEventName::PostCompact,
            ezer_hooks::event::HookPayload::PostCompact {
                source: compact_source.into(),
            },
            None,
        )
        ...
        compaction.complete(
            ezer_telemetry::events::CompactionCompleteStats {
                tokens_after,
                two_pass_used,
                segments_queued,
            ...
            ezer_telemetry::events::CompactionTiming {
    ...
    pub(crate) async fn should_compact_on_error(
        &self,
        err: &ezer_sampler::SamplingErrorInfo,
    ) -> bool {
        if self.compaction.is_suppressed() {
            return false;
        }
    ...
    pub(crate) async fn run_compact_only(
        ...
        ezer_telemetry::session_ctx::log_event(ezer_telemetry::events::AutoCompactFired {
        ...
        let result = self
            .run_compact_inner(
                None,
                None,
                ezer_telemetry::events::CompactionTrigger::Auto,
                lossy_input,
            )
            .await;
    ...
    fn persist_compaction_request_artifact(
        ...
        tools: Vec<ezer_sampling_types::ToolSpec>,
        user_context: Option<&str>,
