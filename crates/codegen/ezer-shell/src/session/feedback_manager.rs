...
use ezer_feedback::{parse_structured_feedback, wire_value};
use ezer_telemetry::events::{FeedbackSendOutcome, UserFeedback};
...
pub(crate) fn new_submission(
    ...
    s.shell_version = Some(ezer_version::VERSION.to_string());
    s
}
...
mod tests {
    ...
    fn test_shutdown_budgets_fit_under_session_flush_grace() {
        ...
        let _unset = ezer_test_support::env::EnvGuard::unset("EZER_SESSION_EXIT_DRAIN_SECS");
        assert!(
    ...
    fn test_nonempty_drain_budget_env_raises_cap() {
        {
            let _guard =
                ezer_test_support::env::EnvGuard::set("EZER_SESSION_EXIT_DRAIN_SECS", "7");
            assert_eq!(
                nonempty_drain_budget(Duration::from_secs(30)),
        ...
        {
            let _guard =
                ezer_test_support::env::EnvGuard::set("EZER_SESSION_EXIT_DRAIN_SECS", "99");
            assert_eq!(
                nonempty_drain_budget(Duration::from_secs(30)),
    ...
    async fn test_has_token_refresher_requires_refresher_attached() {
        ...
        impl ezer_login::refresh::TokenRefresher for NoOpRefresher {
            async fn refresh(
                &self,
...
mod author_identity_tests {
    ...
    async fn env_var_identity_reaches_the_wire_end_to_end() {
        let _email =
            ezer_test_support::env::EnvGuard::set("EZER_TEST_WORK_EMAIL", "ada@corp.example");
        let _name =
            ezer_test_support::env::EnvGuard::set("EZER_TEST_WORK_NAME", "Ada Lovelace");
