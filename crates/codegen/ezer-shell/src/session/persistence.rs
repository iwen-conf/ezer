...
pub use ezer_shared::session::session_dir;
...
pub enum PersistedAgent {
    ...
    Inline(Box<ezer_agent::AgentDefinition>),
}
...
impl PersistedAgent {
    ...
    pub(crate) fn inline_definition(&self) -> Option<&ezer_agent::AgentDefinition> {
        match self {
...
impl From<&ezer_agent::AgentDefinition> for PersistedAgent {
    fn from(def: &ezer_agent::AgentDefinition) -> Self {
    ...
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        let selected = match raw.agent_name {
            ...
            Some(name) => {
                let inline = raw
                    .agent_profile
                    .and_then(|value| ezer_agent::AgentDefinition::from_json(&value).ok())
...
impl Summary {
    ...
    pub(crate) fn agent_profile(&self) -> Option<&ezer_agent::AgentDefinition> {
        self.agent
            .selected
            .as_ref()
    ...
    pub(crate) fn new(info: &Info, model_id: acp::ModelId) -> std::io::Result<Self> {
        let git_metadata =
            ezer_workspace::session::git::resolve_persisted_session_git_metadata_sync(
...
impl SessionPersistence {
    ...
    async fn upgrade_to_writeback(&mut self, auth_manager: Arc<ezer_login::AuthManager>) {
...
fn collect_session_files_recursive(base: &Path, dir: &Path, files: &mut Vec<CopiedSessionFile>) {
    let artifacts = ezer_feedback::FeedbackDraftArtifactSet::for_session(base);
...
fn collect_session_files_recursive_with_artifacts(
    ...
    artifacts: &ezer_feedback::FeedbackDraftArtifactSet,
) {
    ...
    for entry in entries.flatten() {
        if ezer_feedback::is_feedback_draft_artifact_name(&entry.file_name()) {
            continue;
        }
        let path = entry.path();
...
fn init_remote_sync(
    ...
    auth_manager: Option<Arc<ezer_login::AuthManager>>,
...
pub(crate) struct SessionDeps {
    ...
    pub(crate) auth_manager: Option<Arc<ezer_login::AuthManager>>,
...
pub async fn delete_session_history(
    ...
    needs_remote: bool,
    auth_manager: Arc<ezer_login::AuthManager>,
    search_index: Option<&ezer_session_search::SearchIndexManager>,
