//! ACP conversion functions for `ezer-tools`'s `ToolOutput`.
//!
//! These standalone functions convert `ezer_tools::types::output::ToolOutput`
...
pub(crate) fn acp_tool_update(
    ...
    match output {
        ToolOutput::ReadFile(read_file_output) => {
            let (content, status) = match read_file_output {
                ...
                ReadFileOutput::ImageContent(image_content) => {
                    ...
                    // through a `From` impl on the tools crate, so that `ezer-tools` stays free of an `agent-client-protocol` dependency
        ...
        ToolOutput::WebFetch(web_fetch_output) => {
            use ezer_tools::types::output::WebFetchOutput;
        ...
        ToolOutput::Todo(todo_output) => {
            use ezer_tools::types::output::TodoWriteOutput;
        ...
        ToolOutput::AskUserQuestion(ask) => {
            let message = match ask {
                ezer_tools::types::output::AskUserQuestionOutput::UserAnswered { message } => {
                    message.clone()
                }
                ezer_tools::types::output::AskUserQuestionOutput::QuestionsSent {
                    message,
                    ..
                } => message.clone(),
            };
        ...
        ToolOutput::EnterPlanMode(enter) => {
            let message = match enter {
                ezer_tools::types::output::EnterPlanModeOutput::Entered { message, .. } => {
                    message.clone()
                }
            };
            Some(acp::ToolCallUpdate::new(
        ...
        ToolOutput::ExitPlanMode(exit) => {
            let message = match exit {
                ezer_tools::types::output::ExitPlanModeOutput::PlanReady {
                    message, ..
                } => message.clone(),
                ezer_tools::types::output::ExitPlanModeOutput::EmptyPlan {
                    message, ..
                } => message.clone(),
            };
            Some(acp::ToolCallUpdate::new(
