//! Re-exports of the crate-internal event types that live in `ezer-session-events`.
//! The orphan-rule items (`From<&permission::Decision>` and the doom-loop categorizer) stay here since they need shell-local types.

pub(crate) use ezer_session_events::tracker::EventTracker;
pub(crate) use ezer_session_events::types::{
...
) -> Option<ezer_sampling_types::PriorTurnInterrupt> {
    use ezer_sampling_types::PriorTurnInterrupt;
    match category {
...
/// Committing it would silently run ezer-build flavor.
