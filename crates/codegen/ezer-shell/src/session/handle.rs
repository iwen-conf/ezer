...
pub const MEMORY_MODE_META_KEY: &str = "ezer/memoryMode";
...
pub struct SpawnSnapshot {
    pub applied_tool_overrides: Option<ezer_sampling_types::ToolOverrides>,
...
pub struct SessionHandle {
    ...
    /// Whether the client that created this session advertised `ezer/codeNavigation.enabled`.
    ...
    /// Exposed so the `ezer/toggle_plan_mode` handler can toggle plan mode without going through the session command channel.
    ...
    /// Set via `ezer/debug/arm_auto_compact`.
    pub force_compact: std::sync::Arc<std::sync::atomic::AtomicBool>,
    pub permission_handle: ezer_workspace::permission::PermissionHandle,
    ...
    pub terminal_backend:
        Option<std::sync::Arc<dyn ezer_tools::computer::types::TerminalBackend>>,
...
impl SessionHandle {
    ...
    /// Routes through the session actor to the ToolBridge's TerminalBackend.
    pub async fn list_tasks(&self) -> Option<Vec<ezer_tools::types::TaskSnapshot>> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
    ...
    pub(crate) async fn snapshot_tool_definitions(&self) -> Vec<ezer_sampling_types::ToolSpec> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
