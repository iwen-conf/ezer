...
fn test_actor_inner(
    ...
    let sampling_client = OaiCompatClient::new(ezer_sampler::SamplerConfig::default()).unwrap();
    let mut summary =
    ...
    actor
        ...
        .send(PersistenceMsg::WakeStart {
            start: WakeStart {
                ...
                reasoning_effort: Some(Some(ezer_sampling_types::ReasoningEffort::High)),
            },
...
fn save_session_titles(
    server: &ezer_test_support::MockInferenceServer,
    session_id: &str,
) -> Vec<SaveTitle> {
...
fn request_path_summary(server: &ezer_test_support::MockInferenceServer) -> Vec<String> {
    server
        .requests()
        .iter()
