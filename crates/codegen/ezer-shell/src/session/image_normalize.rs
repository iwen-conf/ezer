...
use bytes::Bytes;
use std::borrow::Cow;
use ezer_tools::util::format_bytes;
use ezer_tools::util::image_compress::{FilterType, ReEncodeParams, re_encode_under_limit};
...
/// Resolve the active reminder tag via the canonical constants in `ezer_tools::reminders`.
...
pub(crate) fn persisted_image_reject_reason(bytes: &[u8]) -> Option<String> {
    ...
    use ezer_tools::util::image_validate as iv;
...
pub(crate) fn inline_attach_verdict(data_b64: &str) -> InlineAttachVerdict {
    ...
    let Ok((w, h, _)) =
        ezer_tools::util::image_validate::validate_image_bytes_with(&raw, false)
    else {
        return InlineAttachVerdict::Unreadable;
    };
...
fn compute_normalized_blocking(
    ...
    let (orig_w, orig_h, orig_mime) =
        ezer_tools::util::image_validate::validate_image_bytes_with(&raw_bytes, false)
            .map_err(|e| NormalizeError(format!("validate: {e}")))?;
    if !ezer_tools::util::image_validate::image_structurally_complete(&raw_bytes) {
    ...
    if !exceeded_size && !exceeded_dimensions {
        if let Err(e) = ezer_tools::util::image_validate::validate_image_bytes(&raw_bytes) {
