...
impl SessionActor {
    ...
    ) -> Option<ezer_tools::types::skill_discovery_tracker::SkillUpdateEffects> {
        let bridge = self.agent.borrow().tool_bridge().clone();
    ...
    pub(crate) async fn slash_skills_for_resolve(
        &self,
    ) -> Vec<ezer_tools::implementations::skills::types::SkillInfo> {
        #[cfg(test)]
    ...
    pub(super) fn wrap_skill_reminder(
        &self,
        effects: &ezer_tools::types::skill_discovery_tracker::SkillUpdateEffects,
    ...
    pub(super) async fn maybe_refresh_model_metadata_on_resume(&self) {
        ...
        let _ = am.auth().await;
        let provider: Arc<dyn ezer_auth::AuthCredentialProvider> = Arc::new(
        ...
        let (response, stamp) =
            match ezer_auth::execute_with_stamp(&middleware_client, built).await {
                Ok(r) => r,
                Err(e) => {
        ...
        if response.status() == reqwest::StatusCode::UNAUTHORIZED {
            ezer_login::attribution::record_consumer_401(
                am,
                None,
                ezer_login::attribution::ConsumerKind::IdleResumeModelRefresh,
                "",
    ...
    pub(super) async fn handle_model_metadata_update(
        ...
        let updated_config = ezer_sampling_types::SamplingConfig {
    ...
    pub(super) async fn inject_deny_read_globs(&self) {
        ...
        self.agent
            .borrow()
            .tool_bridge()
            .update_resource(ezer_tools::types::resources::DenyReadGlobs(
