...
impl SessionActor {
    async fn evaluate_goal_round(
        ...
        let plan = match plan_file {
            Some(path) => tokio::fs::read_to_string(path)
                .await
                .ok()
                .map(|text| ezer_tools::util::truncate_str(&text, 16 * 1024).to_owned()),
            None => None,
        };
    ...
    fn record_goal_round_progress(&self, detail: &str, failed: bool) {
        let detail = ezer_tools::util::truncate_str(detail.trim(), 500).to_owned();
    ...
    pub(crate) async fn resolve_goal_role_override(
        ...
        if ezer_agent::config::is_strict_harness_agent_type(&pair.agent_type)
    ...
    pub(super) async fn prune_prior_goal_continuation_directives(&self) {
        use ezer_sampling_types::conversation::SyntheticReason;
