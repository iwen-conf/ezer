...
use ezer_telemetry::session_end::{self, Phase, SharedSessionEndTimer};
...
pub(super) async fn fire_session_end_hooks(
    ...
    let envelope = session.fire_hook(
        ezer_hooks::event::HookEventName::SessionEnd,
        None,
        ezer_hooks::event::HookPayload::SessionEnd {
    ...
    if let Some(registry) = session.hook_registry.borrow().clone() {
        ...
        let results = ezer_hooks::dispatcher::dispatch_non_blocking(
            &registry,
            ezer_hooks::event::HookEventName::SessionEnd,
            &envelope,
            &ctx,
...
async fn log_session_ended(session: &SessionActor) {
    ...
    if let Some(signals) = session.signals_handle().snapshot().await {
        ezer_telemetry::session_ctx::log_event(ezer_telemetry::events::SessionEnded {
...
async fn emit_session_end_timings(timer: &SharedSessionEndTimer, is_subagent: bool) {
    ...
    let mut event = ezer_telemetry::events::SessionEndTimings::default();
    timer.write_event_phases(&mut event);
    event.total_ms = Some(timer.elapsed_ms());
    let _ = tokio::time::timeout(
        SESSION_END_EMIT_BUDGET,
        ezer_telemetry::session_ctx::log_event_now(event),
    )
    .await;
}
...
pub(super) async fn run_session(
    ...
    let _elicitation_coordinator = if !session.startup_hints.is_subagent {
        let elicit_inbox = ezer_mcp::elicitation::ElicitationInbox::new();
        {
    ...
    if !session.startup_hints.is_subagent {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<ezer_mcp::servers::McpClientEvent>();
        {
            let mut mcp_state = session.mcp_state.lock().await;
        ...
        tokio::select! {
                ...
                _ = &mut idle_flush_sleep, if session.idle_flush_timeout.is_some()
                    ...
                    if current_len > last_len {
                        tracing::info!(target: ezer_telemetry::memory_log::TARGET,
                        ...
                        tracing::debug!(target: ezer_telemetry::memory_log::TARGET,
                    ...
                    match cmd {
                        ...
                        SessionCommand::InjectNotification { prompt_id, prompt_blocks, priority, source } => {
                            ...
                            if is_turn_active && priority == NotificationPriority::Next {
                                ...
                                if let Some(buffer) = &session.tool_context.monitor_event_buffer {
                                    ...
                                    buffer.push_capped(
                                        ezer_tools::implementations::ezer_build::monitor::types::MonitorEventNotification {
                        ...
                        SessionCommand::RefreshSkillBaseline => {
                            ...
                            tokio::task::spawn_local(async move {
                                ...
                                let new_skills = ezer_agent::prompt::skills::list_skills_with_plugins(
                                    Some(&cwd),
                                    &skills_config,
                                    pr.as_deref(),
                        ...
                        SessionCommand::ToggleMcpServer { server_name, enabled, server_config, respond_to } => {
                            session.events.emit(ezer_session_events::Event::McpServerToggled {
                        ...
                        SessionCommand::DispatchSessionStartHook { source } => {
                            ...
                            if !deferred_start.cancel.is_cancelled() {
                                ...
                                let handle = tokio::task::spawn_local(async move {
                                    ezer_hooks::runner::command::join_hook_group_reaps(async {
                                        let run = async {
                                            if cancel.is_cancelled() {
                                                return;
                                            }
                                            let envelope = s.fire_hook(
                                                ezer_hooks::event::HookEventName::SessionStart,
                                                None,
                                                ezer_hooks::event::HookPayload::SessionStart {
                                                    source,
                                                    model_id: None,
                                                    agent_type: None,
                                                },
                                            ...
                                            let results =
                                                ezer_hooks::dispatcher::dispatch_non_blocking(
                                                    &registry,
                                                    ezer_hooks::event::HookEventName::SessionStart,
                                                    &envelope,
                                                    &ctx,
