...
impl SessionActor {
    ...
    pub(super) async fn get_rewind_points(&self) -> RewindPointsResponse {
        ...
        let file_meta_map: std::collections::HashMap<
            usize,
            &ezer_workspace::session::file_state::RewindPointMeta,
    ...
    pub(super) async fn handle_rewind(
        ...
        let mut files_to_revert: std::collections::HashMap<
            ezer_workspace::session::file_state::FlexiblePath,
            Option<String>,
    ...
    /// Out-of-band history repair (`ezer/session/repair`) for a resident session.
