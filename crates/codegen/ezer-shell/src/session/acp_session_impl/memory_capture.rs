    ...
    fn drop(&mut self) {
        ...
        let Ok(runtime) = tokio::runtime::Handle::try_current() else {
            if let Err(error) = self
                ...
                && !matches!(error, ezer_memory::V2CaptureError::StaleLease)
            {
                tracing::warn!(
                    error = %error,
...
fn should_continue_capture_worker_after_failure(work: &ezer_memory::CaptureWorkState) -> bool {
    work.pending > 0
}
...
enum CaptureFlushPoll {
    Success,
    Waiting {
        cursors: ezer_memory::CaptureCursors,
        work: ezer_memory::CaptureWorkState,
...
fn classify_capture_error(
    error: &ezer_memory::V2CaptureError,
) -> ezer_telemetry::memory_telemetry::MemoryV2FailureClass {
    use ezer_memory::V2CaptureError;
    use ezer_telemetry::memory_telemetry::MemoryV2FailureClass;
    match error {
...
fn classify_extraction_output(
    ...
    model: &str,
    created_at: i64,
) -> Result<ezer_memory::CaptureOutcomeDraft, CaptureExtractionFailure> {
    use ezer_telemetry::memory_telemetry::MemoryV2FailureClass;
...
fn build_extraction_request(
    session_id: &str,
    range: ezer_memory::CaptureRange,
...
fn log_condensation(range: ezer_memory::CaptureRange, attempt: u32, stats: &CondensationStats) {
...
impl SessionActor {
    ...
    pub(super) async fn enqueue_v2_completed_turn(self: &Arc<Self>, source_prompt_index: usize) {
        ...
        let range = match tokio::task::spawn_blocking(move || {
            ...
            Ok::<_, ezer_memory::V2CaptureError>(Some(range))
        })
        .await
        {
    ...
    pub(super) async fn resume_v2_capture(self: &Arc<Self>) {
        if self.memory.can_expose_v2() {
            ...
            let clock = ezer_memory::system_v2_clock();
            let promotion =
                ezer_telemetry::session_ctx::spawn_local_in_session_ctx(async move {
    ...
    async fn run_v2_capture_worker(
        ...
        loop {
            ...
            Self::emit_v2_capture_activity_to(
                ...
                0,
                None,
                ezer_telemetry::memory_telemetry::MemoryV2ModelUsage::default(),
            )
            .await;
            ...
            match outcome {
                    ...
                    let persisted = tokio::task::spawn_blocking(move || {
                        match persist_store.commit(&persist_lease, &outcome, now) {
                            ...
                            Err(error) => {
                                ...
                                match release {
                                    Ok(()) | Err(ezer_memory::V2CaptureError::StaleLease) => {
                                        lease_guard.disarm();
                                        if terminal {
                ...
                Err(error) => {
                    ...
                    let released = matches!(
                        failure,
                        Ok(Ok(())) | Ok(Err(ezer_memory::V2CaptureError::StaleLease))
                    );
                    if !released {
                        tracing::warn!(
                            job_id = %lease.job.job_id,
    ...
    pub(super) async fn flush_v2_capture(self: &Arc<Self>) -> (FlushResult, Option<u32>) {
        ...
        let Some(storage) = self.memory.storage() else {
            return (
                FlushResult::TerminalFailure(
                    ezer_telemetry::memory_telemetry::MemoryV2FailureClass::Disabled,
                ),
                None,
            );
        };
        ...
        let (store, target) = match tokio::task::spawn_blocking(move || {
            ...
            Ok::<_, ezer_memory::V2CaptureError>((store, target))
        })
        .await
        {
            Ok(Ok(state)) => state,
        ...
        let wait = async {
            loop {
                ...
                let poll = tokio::task::spawn_blocking(move || {
                    ...
                    Ok::<_, ezer_memory::V2CaptureError>(CaptureFlushPoll::Waiting {
                        cursors,
                        work,
                        reconcile_error,
                    })
        ...
        };
        {
            ...
            ezer_telemetry::session_ctx::log_event(MemoryV2FlushResult {
                outcome,
    ...
    async fn emit_v2_capture_activity_with_memories(
        ...
        ezer_telemetry::session_ctx::log_event(MemoryV2CaptureLifecycle {
            stage,
            from_turn,
            through_turn,
            attempt,
...
mod tests {
    ...
    fn capture_debug_entries_pair_generated_content_with_committed_paths() {
        ...
        let entries = capture_debug_entries(
            CaptureOutcomeDraft::Observations(vec![ezer_memory::ObservationDraft {
                observation_type: ezer_memory::ObservationType::Project,
    ...
    fn empty_extraction_output_is_classified_separately_from_malformed_json() {
        ...
        assert_eq!(
            ...
            .unwrap(),
            ezer_memory::CaptureOutcomeDraft::Noop
        );
    }
    ...
    fn extractor_request_has_no_tool_or_hosted_resource_surface() {
        let request = build_extraction_request(
            "session",
            ezer_memory::CaptureRange::try_new(2, 4).unwrap(),
            CondensedTranscript {
                json: "[]".to_owned(),
    ...
    fn extractor_prompt_names_condensation() {
        let plain = build_extraction_request(
            "session",
            ezer_memory::CaptureRange::try_new(1, 1).unwrap(),
            CondensedTranscript {
                json: "[]".to_owned(),
                stats: CondensationStats::default(),
            },
    ...
    impl ezer_memory::V2Clock for FixedCaptureClock {
    ...
    fn capture_clock_is_injectable() {
        assert_eq!(
            ezer_memory::V2Clock::now_unix_seconds(&FixedCaptureClock(1_788_000_123)),
            1_788_000_123
        );
    }
    ...
    fn flush_processes_pending_work_before_returning_retryable_failure() {
        let failed_work = ezer_memory::CaptureWorkState {
            failed: 1,
    ...
    fn seed_range() -> ezer_memory::CaptureRange {
        ezer_memory::CaptureRange::try_new(1, 1).unwrap()
    }

    fn init_v2_scopes(root: &Path, global: &Path, workspace: &Path) {
        std::fs::create_dir_all(root.join("workspaces")).unwrap();
        ezer_memory::ensure_scope_initialized(root, global, V2MemoryScope::Global).unwrap();
        ezer_memory::ensure_scope_initialized(root, workspace, V2MemoryScope::Workspace)
            .unwrap();
    }
    ...
    fn hold_dream_lease(global: &Path, workspace: &Path) -> ezer_memory::ConsolidationLease {
        ...
        capture
            ...
            .unwrap();
        let store = ezer_memory::V2ConsolidationStore::open(
            workspace,
            V2MemoryScope::Workspace,
            global,
            workspace,
        ...
        .unwrap();
        store
            .claim(&ezer_memory::DreamClaimRequest {
