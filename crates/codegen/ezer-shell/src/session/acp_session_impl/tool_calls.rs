...
use ezer_hooks::result::HookDecision;
...
struct PreToolUseGate {
    ...
    additional_context: Vec<ezer_hooks::dispatcher::AdditionalContext>,
}
struct GateRewrite {
...
pub(super) fn is_file_backed_exit_plan_kind(
    kind: Option<ezer_tools::types::tool::ToolKind>,
) -> bool {
    matches!(kind, Some(ezer_tools::types::tool::ToolKind::ExitPlan))
}
...
fn split_exit_plan_tail(
    ...
    kind_of: impl Fn(&str) -> Option<ezer_tools::types::tool::ToolKind>,
) -> (
...
fn external_tool_bodies(
    ...
    match result {
        Ok(tool_result) if tool_result.output.is_error() => {
            ...
            (
                Some(ezer_telemetry::external::truncate::cap_bytes(
                    &body,
                    ezer_telemetry::external::truncate::MAX_CONTENT_BYTES,
                )),
                Some(ezer_telemetry::external::truncate::cap_bytes(
                    &body,
                    ezer_telemetry::external::truncate::MAX_TOOL_INPUT_JSON_BYTES,
                )),
            )
        }
        Ok(tool_result) => (
...
impl SessionActor {
    ...
    pub(super) fn stamp_tool_meta(
        ...
        ezer_tools::normalization::merge_tool_meta(
            &toolset,
    ...
    pub(super) fn media_gen_over_cap(
        &self,
        calls: &[ezer_sampling_types::ToolCall],
    ) -> Vec<ezer_tools::media_gen_limits::MediaGenOverCap> {
        let kind_of = |name: &str| self.agent.borrow().tool_bridge().tool_kind(name);
        ezer_tools::media_gen_limits::over_cap_by_name(
    ...
    async fn reject_excess_media_gen_calls(
        ...
        let kind_of = |name: &str| self.agent.borrow().tool_bridge().tool_kind(name);
        let (allowed, rejected) = ezer_tools::media_gen_limits::partition_media_gen_batch(
            tool_calls,
        ...
        for (name, rejected_n) in &rejected_by_name {
            let max = self
                ...
                .tool_bridge()
                .tool_kind(name)
                .and_then(|k| ezer_tools::media_gen_limits::max_calls_per_batch(k, limits))
                .unwrap_or(0);
    ...
    async fn execute_tool_calls_batch(
                ...
                .await?
            {
                ...
                Err(tool_loop) => {
                        ...
                        crate::session::mcp_servers::parse_mcp_tool_name(&call_name)
                    {
                        ...
                        self.emit_event(ezer_session_events::Event::McpToolCallCompleted {
        ...
        let (
            ...
            (
                param_names.clone(),
                toolset
                    .tool_name_for_kind(ezer_tools::types::tool::ToolKind::Workflow)
            ...
            .map(|(idx, prepared)| {
                ...
                async move {
                    ...
                    let latest_slot = std::sync::Arc::new(parking_lot::Mutex::new(
                        ezer_tools::types::source_summary::SourceSummarySlot::new(),
                    ));
                    let run_tool = || {
        ...
        while let Some((idx, result, duration_ms, projection)) = dispatch_rx.recv().await {
            ...
            let mut post_tool_use_failure_contexts: Vec<
                ezer_hooks::dispatcher::AdditionalContext,
            > = Vec::new();
            ...
            let (ext_tool_output, ext_error_message) = if ezer_telemetry::external::is_active()
            {
            ...
            for context in &prepared.additional_context {
                deferred_followups.push(self.wrap_hook_note(
                    ezer_hooks::event::HookEventName::PreToolUse,
            ...
            if let Some(delivery) = post_tool_use_delivery {
                for context in &delivery.additional_context {
                    deferred_followups.push(self.wrap_hook_note(
                        ezer_hooks::event::HookEventName::PostToolUse,
                        HookNoteKind::Context,
                        &context.hook_name,
                        &context.text,
                    ));
            ...
            for context in &post_tool_use_failure_contexts {
                deferred_followups.push(self.wrap_hook_note(
                    ezer_hooks::event::HookEventName::PostToolUseFailure,
                    HookNoteKind::Context,
                    &context.hook_name,
                    &context.text,
                ));
            ...
            self.observability_bridge
                ...
                .await;
            let (ext_file_path, ext_parameters) = if ezer_telemetry::external::is_active() {
            ...
            ezer_telemetry::session_ctx::log_event(crate::session::telemetry::completed_event(
                crate::session::telemetry::CompletedTool {
                    ...
                    tool_use_id: ezer_telemetry::external::is_active()
    ...
    async fn apply_pre_tool_use_gate(
        ...
        if let Some(registry) = hook_registry_snapshot {
            ...
            let pre_result =
                ezer_hooks::dispatcher::dispatch_pre_tool_use(&registry, &envelope, &ctx).await;
    ...
    async fn validate_hook_rewrite(
        ...
        resolved_tool_name: &str,
        dispatch_target_name: &Option<String>,
        rewrite: ezer_hooks::dispatcher::InputRewrite,
        mcp_preparation: &mcp_file_input::McpFilePreparation,
            ...
            .await
        {
            Ok(input) => input,
            Err(err) => {
                let schema_error = ezer_hooks::event::clip_reason(&err.to_string());
                return Ok(Err(self
    ...
    pub(crate) async fn prepare_tool_call(
        ...
        let wrapper_parse = (self.tool_bridge_handle().tool_kind(&call.function.name)
            == Some(ezer_tools::types::tool::ToolKind::UseTool))
        .then(|| {
        ...
        });
        {
            ...
            let subagent_background =
                ezer_tools::is_task_tool_id(&call.function.name).then(|| {
                    early_raw_input
                        .as_ref()
        ...
        let has_pre_tool_use_hooks =
            self.may_have_hooks_for(ezer_hooks::event::HookEventName::PreToolUse);
        ...
        if !plan_file_auto_approve {
            ...
            let (telemetry_access_kind, _access_detail) = match &access_kind {
                ezer_workspace::permission::AccessKind::Read(p) => (
                    ezer_telemetry::events::AccessKind::Read,
                    p.clone().unwrap_or_default(),
                ),
                ezer_workspace::permission::AccessKind::Edit(p) => {
                    (ezer_telemetry::events::AccessKind::Edit, p.clone())
                }
                ezer_workspace::permission::AccessKind::Bash(cmd) => {
                    (ezer_telemetry::events::AccessKind::Bash, cmd.clone())
                }
                ezer_workspace::permission::AccessKind::Grep { path, glob } => (
                    ezer_telemetry::events::AccessKind::Grep,
                    path.clone().or_else(|| glob.clone()).unwrap_or_default(),
                ),
                ezer_workspace::permission::AccessKind::MCPTool { name, .. } => {
                    (ezer_telemetry::events::AccessKind::Mcp, name.clone())
                }
                ezer_workspace::permission::AccessKind::WebFetch(u) => {
                    (ezer_telemetry::events::AccessKind::Web, u.clone())
                }
                ezer_workspace::permission::AccessKind::WebSearch(q) => {
                    (ezer_telemetry::events::AccessKind::Web, q.clone())
                }
                ezer_workspace::permission::AccessKind::AgentMessage { subagent_id } => (
                    ezer_telemetry::events::AccessKind::AgentMessage,
                    subagent_id.clone(),
                ),
                _ => (ezer_telemetry::events::AccessKind::Other, String::new()),
            };
            ...
            ezer_telemetry::session_ctx::log_event(
                ezer_telemetry::events::PermissionPrompted {
            ...
            let path_context = Some(ezer_workspace::permission::types::RequestPathContext {
            ...
            let perm_wait_span = ezer_telemetry::region::Region::from_span(
            ...
            self.events.permission_resolved(
                ...
                match &decision {
                    Decision::Allow | Decision::Ask => {
                        ezer_session_events::types::PermissionDecision::Allow
                    }
                    Decision::Reject(_) | Decision::PolicyDeny(_) => {
                        ezer_session_events::types::PermissionDecision::Deny
                    }
                    Decision::Cancelled => {
                        ezer_session_events::types::PermissionDecision::Cancelled
                    }
                    Decision::FollowupMessage(_) => {
                        ezer_session_events::types::PermissionDecision::Followup
                    }
                },
                perm_start,
            );
            ...
            ezer_telemetry::session_ctx::log_event({
                ...
                let tool_input = if ezer_telemetry::external::is_active() {
                    ezer_telemetry::events::ExternalToolInput {
                        parameters: Some(raw_input.clone()),
                        tool_use_id: Some(call.id.clone()),
                    }
                } else {
                    ezer_telemetry::events::ExternalToolInput::default()
                };
            ...
            match decision {
                Decision::PolicyDeny(ref reason) | Decision::Reject(ref reason) => {
                    ...
                    let (tool_input_value, tool_input_truncated) =
                        ezer_hooks::event::truncate_payload(raw_input.clone());
                    self.dispatch_hook(
                        ezer_hooks::event::HookEventName::PermissionDenied,
                        ezer_hooks::event::HookPayload::PermissionDenied {
        ...
        let invocation_id = ezer_telemetry::events::InvocationId::generate();
        ...
        if deactivated {
            ...
            *self.turn_prompt_mode.lock() = PromptMode::Agent;
            ...
            self.enqueue_current_mode_update(acp::SessionModeId::new(
                ezer_tools::types::SessionMode::Default.as_id(),
            ));
        }
    }
    ...
    pub(super) async fn send_tool_call_start(
        ...
        let (title, kind, locations, content) = match tool_call_input {
                ...
                (
                    ...
                    vec![
                        acp::ToolCallLocation::new(read_file.path)
                            ...
                            .line(
                                ezer_tools::normalization::norm_offset_i64(read_file.offset)
                                    .map(|l| l as u32),
                            ),
                    ],
                    Vec::new(),
            ...
            ToolInput::Skill(skill) => {
                ezer_telemetry::session_ctx::log_event(
                    ezer_telemetry::events::SkillDispatched {
                        skill_name: skill.skill.clone(),
                        plugin_source: None,
                        trigger: ezer_telemetry::events::SkillTrigger::SkillTool,
                        skill_source: None,
                        skill_origin: None,
                    },
                );
        ...
        if let Some(access) = self
            ...
            .borrow()
            .tool_bridge()
            .read_resource::<ezer_tools::types::memory_v2::MemoryV2AccessResource>()
            .await
        {
    ...
    async fn skill_for_read_path(
        &self,
        path: &str,
    ) -> Option<ezer_tools::implementations::skills::types::SkillInfo> {
        if self.is_chat_kind {
            return None;
        }
        let read_path = ezer_tools::types::resources::resolve_model_path(
            self.tool_context.cwd.as_path(),
    ...
    fn emit_skill_md_read(&self, skill: ezer_tools::implementations::skills::types::SkillInfo) {
        let skill_source =
        ...
        ezer_telemetry::session_ctx::log_event(ezer_telemetry::events::SkillDispatched {
            skill_name: skill.name,
            plugin_source: skill.plugin_name,
            trigger: ezer_telemetry::events::SkillTrigger::SkillMdRead,
    ...
    ) -> ezer_hooks::event::HookEventEnvelope {
        let (tool_input, tool_input_truncated) =
            ezer_hooks::event::truncate_payload(input.clone());
        self.make_hook_envelope(
            ezer_hooks::event::HookEventName::PreToolUse,
            None,
            ezer_hooks::event::HookPayload::PreToolUse {
    ...
    fn record_git_pr_signals(&self, effective_tool_name: &str, result: &ToolRunResult) {
        use ezer_telemetry::enums::PrCreationSource;
        use ezer_tools::util::git_detect;
        match &result.output {
            ezer_tools::types::output::ToolOutput::Bash(b) if b.exit_code == 0 => {
                ...
                if ops.pr_merged {
                    ...
                    ezer_telemetry::session_ctx::log_event(
                        ezer_telemetry::events::PrMerged {},
                    );
                }
            }
            ezer_tools::types::output::ToolOutput::MCP(m)
    ...
    fn record_pr_created(
        &self,
        pr: ezer_tools::util::git_detect::PrRef,
        source: ezer_telemetry::enums::PrCreationSource,
    ) {
        self.signals_handle()
    ...
    pub(super) async fn handle_bridge_tool_success(
        ...
        let consumed_ids =
            ezer_tools::reminders::task_completion::consumed_completion_ids(&result.output);
        if !consumed_ids.is_empty() {
        ...
        if matches!(
            ...
            ToolsToolOutput::SearchReplace(
                ezer_tools::types::output::SearchReplaceOutput::EditsApplied(_)
            ) | ToolsToolOutput::Bash(_)
        ) {
            self.maybe_notify_git_branch().await;
        }
        if let ezer_tools::types::output::ToolOutput::Bash(ref b) = result.output
            && b.was_bare_echo
        {
            ...
            acp_tool_update(&result.output, call_id, path_rewriter.as_ref(), tool_meta)
        {
            ...
            if matches!(
                &result.output,
                ezer_tools::types::output::ToolOutput::EnterPlanMode(_)
                    | ezer_tools::types::output::ToolOutput::ExitPlanMode(_)
            ) {
    ...
    async fn render_non_replaced_tool_body(
        ...
        tool_layer_images: Vec<ezer_tools::util::base64_images::ExtractedImage>,
    ) -> (
        String,
        Vec<ContentPart>,
        Vec<ezer_tools::util::base64_images::ExtractedImage>,
    ) {
        ...
        let extraction = if !self.is_cursor_harness()
            ...
            ezer_tools::util::base64_images::extract_base64_images(prompt_text)
        } else {
            ezer_tools::util::base64_images::ExtractionResult {
                text: prompt_text,
                images: Vec::new(),
            }
        };
...
fn execute_tool_call_parts(
    ...
    let display = ezer_tools::util::strip_redundant_session_cd(command, cwd);
    (
...
mod mcp_error_routing_tests {
    ...
    use ezer_tools::types::output::{MCPOutput, TodoWriteOutput, ToolOutput};
    #[test]
...
mod plan_mode_edit_gate_tests {
    ...
    use ezer_tools::types::ToolInput;
    use ezer_workspace::permission::AccessKind;
    ...
    fn write(path: &str) -> ToolInput {
        use ezer_tools::implementations::opencode::write::WriteInput;
    ...
    fn apply_patch_rejected_in_plan_mode() {
        use ezer_tools::implementations::codex::apply_patch::ApplyPatchInput;
        let t = active_tracker();
        assert_eq!(
            gate(
                &t,
