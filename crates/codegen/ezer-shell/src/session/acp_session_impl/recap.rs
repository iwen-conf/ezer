...
use ezer_sampling_types::SamplingError;
    ...
    fn attached_images_are_inlined_on_the_side_question_item() {
        ...
        assert!(
            user.content.iter().any(|part| matches!(
                part,
                ezer_sampling_types::ContentPart::Image { url }
...
impl SessionActor {
    ...
    pub(super) async fn handle_suggest_prompt(
        ...
        use ezer_telemetry::events::{PromptSuggestion, PromptSuggestionAction as PsAction};
        ...
        let Some(model) = prompt_suggest::effective_suggest_model(
            ...
            ezer_telemetry::session_ctx::log_event(PromptSuggestion {
        ...
        let suggest_reasoning = prompt_suggest::resolve_suggest_reasoning(
            ...
            self.models_manager.model_supports_reasoning_effort_value(
                &model,
                ezer_sampling_types::ReasoningEffort::None,
            ),
        );
        ...
        let sampling_client = match ezer_sampler::SamplingClient::new(sampling_config) {
            Ok(client) => client,
            Err(e) => {
        ...
        tracing::debug!(
            raw_preview = %ezer_tools::util::truncate_str(raw.trim(), 60),
