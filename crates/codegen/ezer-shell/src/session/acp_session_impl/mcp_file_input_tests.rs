...
pub(super) fn record_event<T: ezer_telemetry::TelemetryEvent>(event: &T) -> bool {
    FILE_EVENTS
        .try_with(|events| {
            events
                .borrow_mut()
...
async fn fixture(
    ...
    *actor.agent.borrow_mut() = test_agent_with_tools(vec![
        ezer_tools::registry::types::ToolConfig::for_tool::<UseTool>(),
    ])
    .await;
...
fn install_client_hooks(actor: &SessionActor, events: &[ezer_hooks::event::HookEventName]) {
    for event in events {
        actor.client_hooks.borrow_mut().insert(
            *event,
            vec![crate::extensions::hooks::ClientHookGroup {
                matcher: Some(
                    ezer_hooks::matcher::HookMatcher::new("fixture__update").unwrap(),
                ),
...
async fn inline_recovery_keeps_raw_duplicate_checks_and_file_forms_strict() {
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            install_client_hooks(&actor, &[ezer_hooks::event::HookEventName::PreToolUse]);
...
async fn file_advertisement_requires_host_preparation_and_bounded_filesystem() {
    use ezer_agent::{AgentBuilder, AgentDefinition};
    use ezer_tools::implementations::use_tool::UseToolParams;
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            for case in ["default", "forged", "delegated", "supported", "renamed"] {
                ...
                if case == "delegated" {
                    fields.fs_backend =
                        Arc::new(ezer_workspace::file_system::AcpFsAdapter::new(
                ...
                let agent = if matches!(case, "default" | "forged") {
                    ...
                    spec.build_agent(definition, ezer_agent::DEFAULT_SYSTEM_PROMPT_LABEL)
                        .await
                }
                .unwrap();
...
async fn finalized_randomized_wrapper_preserves_canonical_file_keys() {
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let mut config = ezer_tools::registry::types::ToolConfig::for_tool::<UseTool>();
...
async fn ignored_logical_symlink_source_is_denied_before_loading() {
    tokio::task::LocalSet::new()
        .run_until(async {
            use ezer_tools::types::resources::{GitignoreFilter, RespectGitignore};
            let dir = tempfile::tempdir().unwrap();
...
async fn transport_memory_policy_validates_and_denies_without_model_bookkeeping() {
    tokio::task::LocalSet::new()
        .run_until(async {
            use ezer_tools::types::memory_v2::{
...
async fn acp_filesystem_is_unsupported_without_read_text_file_fallback() {
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let backend = ezer_workspace::file_system::AcpFsAdapter::new(
        ...
        .run_until(async {
            ...
            for remaining in [FILE_OPERATION_TIMEOUT, Duration::ZERO] {
                ...
                if remaining.is_zero() {
                    ...
                    assert!(
                        text.len()
                            <= ezer_hooks::event::MAX_PAYLOAD_SIZE + " [truncated]".len()
                    );
                }
...
async fn snapshot_overflow_emits_once_on_the_session_task_with_measured_bytes() {
    use ezer_telemetry::session_ctx::{TelemetryCtx, with_session_ctx};
    ...
    let (error, events) = capture_events(with_session_ctx(context, async {
        let mut source = McpFileSource::start(PathBuf::from("source"), ezer_telemetry::events::McpFileInputKind::Arguments);
        source.bytes = 42;
        ...
        .run_until(async {
            for stage in [
                ...
                if matches!(stage, "source-deny" | "permission") {
                    use ezer_workspace::permission::types::{
                        PatternMode, PermissionConfig, PermissionRule, RuleAction, ToolFilter,
                    };
                    let (permissions, _events) =
                        ezer_workspace::permission::spawn_permission_manager(
                            actor.session_info.id.clone(),
                            actor.notifications.gateway.clone(),
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            *actor.hook_registry.borrow_mut() = Some(Arc::new(
            ...
            let auth = Arc::new(ezer_login::AuthManager::new(
                auth_dir.path(),
                ezer_login::EzerComConfig::default(),
            ));
            auth.hot_swap(ezer_login::EzerAuth {
                key: "fixture".into(),
            ...
            struct Refresh;
            #[async_trait::async_trait]
            impl ezer_login::refresh::TokenRefresher for Refresh {
                async fn refresh(
                    &self,
...
async fn source_and_resolved_approval_are_distinct_and_reject_prevents_send() {
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let (permission, _events) = ezer_workspace::permission::spawn_permission_manager(
                actor.session_info.id.clone(),
                xai_acp_lib::AcpAgentGatewaySender::new(gateway),
                ezer_paths::AbsPathBuf::new(PathBuf::from("/tmp")).unwrap(),
                ezer_workspace::permission::ClientType::Desktop,
                Some(PermissionConfig::new(vec![PermissionRule {
