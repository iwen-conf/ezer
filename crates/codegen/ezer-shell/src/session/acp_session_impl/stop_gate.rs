...
use ezer_hooks::{dispatcher, result};
...
fn stop_entry_from_task(task: &ezer_tools::types::TaskSnapshot) -> StopBackgroundTask {
    let command_text =
    ...
    async fn announce_keep_working(
        ...
        for block in blocks {
            ...
            ezer_telemetry::session_ctx::log_event(ezer_telemetry::events::HookBlocked {
                ...
                cause: ezer_telemetry::events::HookBlockCause::StopBlocked,
            });
        }
        if blocks.is_empty() {
            for context in additional_context {
...
mod stop_gate_snapshot_tests {
    ...
    fn task_snapshot(
        kind: ezer_tools::computer::types::TaskKind,
