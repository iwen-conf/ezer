use super::*;
use ezer_tools::reminders::task_completion::ReportedTaskCompletions;
...
fn reminder_text(content: &str, tag: &str) -> String {
    ezer_tools::reminders::wrap_reminder_with_tag(&escape_reminder_close_tag(content, tag), tag)
}
...
pub(super) fn wrap_untrusted_in_reminder_tag(content: &str, tag: &str) -> ConversationItem {
    ConversationItem::system_reminder(ezer_tools::reminders::wrap_reminder_with_tag(
...
pub(crate) fn resolve_reminder_policy(
    ...
    todo_gate: bool,
) -> ezer_agent::ReminderPolicy {
    let mut policy = ezer_agent::ReminderPolicy::default();
...
fn workflow_completion_detail(detail: &str) -> std::borrow::Cow<'_, str> {
    ...
    if normalized == detail {
        ezer_tools::util::truncate_str_with_marker(detail, WORKFLOW_RESULT_SUMMARY_REMINDER_CAP)
    } else {
        std::borrow::Cow::Owned(
            ezer_tools::util::truncate_str_with_marker(
                &normalized,
...
impl SessionActor {
    pub(super) fn push_workflow_launch_reminder(
        ...
        let mut body = format!(
            ...
            ezer_tools::util::truncate_str(&command_line, WORKFLOW_OBJECTIVE_REMINDER_CAP)
        );
        ...
        if !objective.is_empty() && !objective_redundant {
            body.push_str(&format!(
                "\nObjective: {}",
                ezer_tools::util::truncate_str(&objective, WORKFLOW_OBJECTIVE_REMINDER_CAP)
            ));
        }
        body.push_str(&format!(
        ...
        if run.status.is_paused() {
            if let Some(msg) = run.pause_message.as_deref() {
                let _ = write!(
                    buf,
                    "\n  Paused: {}",
                    ezer_tools::util::truncate_str(msg, WORKFLOW_RESULT_SUMMARY_REMINDER_CAP)
                );
            }
...
fn format_workflow_completion_reminder(
    ...
    for run in runs {
        ...
        if let Some(summary) = run.result_summary.as_deref() {
            let capped =
                ezer_tools::util::truncate_str(summary, WORKFLOW_RESULT_SUMMARY_REMINDER_CAP);
            buf.push_str("\n  Result:\n");
...
pub(super) fn todo_gate_active(
    policy: &ezer_agent::system_reminder::ReminderPolicy,
    audience: ezer_agent::prompt::context::PromptAudience,
...
impl SessionActor {
    ...
    pub(super) fn reminder_wrapper_tag(&self) -> &'static str {
        ezer_tools::reminders::DEFAULT_REMINDER_TAG
    }
        ...
        if !bash_completions.is_empty() {
            ...
            if goal_loop_active {
                ...
                let task_output_name =
                    ...
                    .await;
                let read_tool_name =
                    ezer_tools::reminders::task_completion::resolve_read_tool_name(&bridge)
                        .await;
                let reminder = ezer_tools::reminders::task_completion::format_between_turn_bash_completions(
                    &bash_completions,
    ...
    async fn drain_between_turn_subagent_completions(
        ...
        let bridge = self.agent.borrow().tool_bridge().clone();
        let reminder =
            ezer_tools::reminders::task_completion::format_between_turn_completion_reminder(
                &completions,
                &bridge,
            )
            .await;
    ...
    pub(super) async fn drain_between_turn_workflow_completions(&self, goal_loop_active: bool) {
        ...
        let read_tool_name =
            ezer_tools::reminders::task_completion::resolve_read_tool_name(&bridge).await;
    ...
    pub(super) fn todo_gate_policy(
        &self,
    ) -> Option<ezer_agent::system_reminder::TodoGateConfig> {
    ...
    pub(super) async fn collect_todo_gate_input(&self, prompt_id: &str) -> CollectedTodoGateInput {
        ...
        let incomplete_terminal_tasks = bridge
            ...
            .await
            .into_iter()
            .filter(ezer_tools::computer::types::TaskSnapshot::is_outstanding)
            .count();
