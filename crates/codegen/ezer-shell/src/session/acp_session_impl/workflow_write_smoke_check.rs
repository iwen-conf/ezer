//! When a write or edit tool touches a project `.ezer/workflows/*.rhai` script, run the workflow validator on it and warn the model on failure.
...
pub(super) fn is_project_workflow_rhai_path(path: &Path) -> bool {
    ...
    components.windows(2).any(|pair| {
        ...
        matches!(a, Component::Normal(name) if *name == ".ezer")
...
pub(super) async fn snapshot_authored_workflow(
    ...
    let path = ezer_tools::types::resources::resolve_model_path(cwd, display_cwd, input);
