...
static TURNS_ACTIVE: ezer_telemetry::activity::ActivityGauge =
    ezer_telemetry::activity::ActivityGauge::work(
        ezer_telemetry::activity::TURNS_ACTIVE_KEY,
    );
...
impl TurnTelemetryOutcome {
    fn from_result(result: &Result<TurnOutcome, acp::Error>) -> Self {
        use ezer_telemetry::events::Outcome;
    ...
    fn task_aborted() -> Self {
        Self {
            outcome: ezer_telemetry::events::Outcome::Cancelled,
    ...
    fn error(err: &acp::Error) -> Self {
        ...
        Self {
            outcome: ezer_telemetry::events::Outcome::Error,
...
impl TurnCompletionEmitter {
    ...
    fn emit(&mut self, outcome: TurnTelemetryOutcome, duration_ms: u64, tool_call_count: u32) {
        self.emitted = true;
        ezer_telemetry::session_ctx::log_event(ezer_telemetry::events::TurnCompleted {
...
pub(super) fn record_failed_sample_on_turn_span(
    ...
    let stop_reason = match kind {
        ezer_sampler::SamplingErrorKind::MaxTokensTruncation => {
            ezer_sampling_types::StopReason::Length.as_ref()
        }
...
impl SessionActor {
    ...
    pub(super) async fn handle_turn_input(
        ...
        if let Some(ref tp) = request.traceparent {
            ezer_otel::link_span_to_meta(&span, &serde_json::json!({ "traceparent": tp }));
        }
    ...
    async fn handle_turn_input_inner(
        ...
        *self.turn_prompt_mode.lock() = prompt_mode;
                ...
                match crate::session::slash_authority::resolve(
                    ...
                    crate::session::slash_authority::AuthorityResolution::ModelAuthoredSkillCandidate {
                        ...
                        let has_skill_loader = bridge
                            .tool_for_kind(ezer_tools::types::tool::ToolKind::Read)
                            .await
                            .is_some()
                            || bridge
                                .tool_for_kind(ezer_tools::types::tool::ToolKind::Skill)
                                .await
                                .is_some();
        ...
        let prompt_blocks = match resolved {
            ...
            Err(SlashCommandOutcome::Builtin(action)) => {
                ...
                let slash_used = ezer_telemetry::events::SlashCommandUsed {
                ...
                match action {
                    BuiltinAction::GoalSet {
                        ...
                        ezer_telemetry::session_ctx::log_event(slash_used);
            ...
            Err(SlashCommandOutcome::InvokeSkill {
                ...
                for sk in &parsed_skills {
                    ezer_telemetry::session_ctx::log_event(
                        ezer_telemetry::events::SlashCommandUsed {
                    ...
                    ezer_telemetry::session_ctx::log_event(
                        ezer_telemetry::events::SkillDispatched {
                            ...
                            trigger: ezer_telemetry::events::SkillTrigger::SlashCommand,
                    ...
                    if let Some(ref pname) = sk.plugin_name {
                        ezer_telemetry::session_ctx::log_event(
                            ezer_telemetry::events::PluginUsed {
        ...
        *self.doom_loop_turn_tally.lock() = Default::default();
        ...
        ezer_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::Turn {
        ...
        let prompt_block = if policy.authority != InputAuthority::ModelAuthoredUntrusted {
            let prompt_gate_verdict = self
                .dispatch_prompt_submit_hook(
                    ezer_hooks::event::HookPayload::UserPromptSubmit {
            ...
            match (
                ...
                (true, ezer_hooks::result::PromptDecision::Block { reason, hook_name }) => {
                    Some((hook_name, reason))
                }
                (false, ezer_hooks::result::PromptDecision::Block { reason, hook_name }) => {
                    ...
                    self.send_hook_annotation(
                        ...
                        ezer_hooks::config::hook_display_name(&hook_name)
                    ),
                        )
                        .await;
                    None
                }
                (_, ezer_hooks::result::PromptDecision::Allow) => None,
            }
        } else {
            None
        };
        if prompt_block.is_some() {
            ...
            *self.tool_context.prompt_index.lock().await = current_prompt_index;
            ...
            let (query, extra_images) = if !self.is_cursor_harness() {
                let extraction = ezer_tools::util::base64_images::extract_base64_images(query);
            ...
            if policy.analytics.is_human_prompt()
                && (self.telemetry_enabled || ezer_telemetry::external::is_active())
            {
                ...
                let ev = ezer_telemetry::events::PromptSubmitted {
                    ...
                    screen_mode: prompt_screen_mode,
                    prompt_text: ezer_telemetry::external::is_active()
                        .then(|| user_message.to_owned()),
                    command_name: otel_command_name,
                };
                ezer_telemetry::session_ctx::log_event_dual(self.telemetry_enabled, ev);
            }
            ...
            self.tool_bridge_handle()
                .update_resource(ezer_tools::types::resources::AttachedImages(
                    attached_image_refs,
                ))
                .await;
        ...
        let mut result = if let Some((hook_name, reason)) = prompt_block {
            ...
            ezer_telemetry::session_ctx::log_event(ezer_telemetry::events::HookBlocked {
                hook_name: hook_name.clone(),
                cause: ezer_telemetry::events::HookBlockCause::PromptBlocked,
            });
        ...
        self.observability_bridge
            ...
            .await;
        if ezer_telemetry::external::is_active() {
            let committed = self
                .chat_state_handle
        ...
        match &result {
            Ok(TurnOutcome::Completed { stop, .. }) => {
                ...
                if let CompletedStop::Refusal(explanation) = stop {
                    self.report_turn_end(
                        prompt_id,
                        TurnEnd::Failed {
                            error: ezer_hooks::event::StopFailureKind::InvalidRequest,
                            error_details: None,
            ...
            Err(err) => {
                ...
                .await;
                ezer_telemetry::session_ctx::log_session_event(
                    ezer_telemetry::events::ApiError {
    ...
    pub(crate) async fn inject_pending_monitor_events(&self) {
        ...
        let Some(body) = ezer_tools::reminders::task_completion::format_monitor_events(
            &mine,
            Some(&self.tool_context.task_output_tool_name),
        ) else {
            return;
        };
        let wrapped = ezer_tools::reminders::wrap_reminder(&body);
        self.chat_state_handle
    ...
    pub(crate) async fn first_turn_memory_reminder(&self) -> Option<String> {
        ...
        if is_v2 {
            ...
            if crate::session::helpers::memory_context::conversation_has_memory_context(
                ...
                let block = conversation.first().and_then(|item| match item {
                    ezer_sampling_types::ConversationItem::System(sys) => sys
                        .content
                ...
                crate::session::memory_observation::log_memory_injection(
                    ...
                    ezer_telemetry::memory_telemetry::MemoryInjectionOutcome::Skipped,
            ...
            return match context {
                Ok(context) => {
                    ...
                    crate::session::memory_observation::log_memory_injection(
                        self.session_info.id.to_string(),
                        ezer_telemetry::memory_telemetry::MemoryInjectionOutcome::Results,
                        crate::session::memory_observation::MemoryInjectionMetrics {
                ...
                Err(error) => {
                    ...
                    crate::session::memory_observation::log_memory_injection(
                        self.session_info.id.to_string(),
                        ezer_telemetry::memory_telemetry::MemoryInjectionOutcome::Error,
                        Default::default(),
                    );
                    None
                }
        ...
        use ezer_tools::types::memory_backend::MemoryBackend as _;
        ...
        let inject_search_span =
            ezer_telemetry::region::Region::from_span(tracing::info_span!(
        ...
        let (outcome, mut inject_results) = match search_result {
            Ok(results) if results.is_empty() => (
                ezer_telemetry::memory_telemetry::MemoryInjectionOutcome::Empty,
                results,
            ),
            Ok(results) => (
                ezer_telemetry::memory_telemetry::MemoryInjectionOutcome::Results,
    ...
    async fn handle_structured_output_tool_call(
        &self,
        tool_calls: &mut Vec<ezer_sampling_types::conversation::ToolCall>,
        validator: &Result<jsonschema::Validator, String>,
        retries: &mut u32,
    ...
    async fn is_run_true_step(
        &self,
        tool_calls: &[ezer_sampling_types::conversation::ToolCall],
    ) -> bool {
    ...
    pub(super) async fn report_turn_delta(
        ...
        if let Some(snap) = snapshot {
            for pr in &snap.delta.prs_created_this_turn {
                ezer_telemetry::session_ctx::log_event(ezer_telemetry::events::PrCreated {
                    source: pr.source,
        ...
        *self.current_turn_span_id.lock() = tracing::Span::current().id();
        ...
        loop {
            ...
            if identical_tool_calls.run_len >= identical_tool_calls.hard_stop_threshold() {
                ...
                ezer_telemetry::session_ctx::log_event(
                    ezer_telemetry::events::ActionStationarityStop {
                        true_noop,
            ...
            if identical_tool_calls.take_nudge() {
                ...
                ezer_telemetry::session_ctx::log_event(
                    ezer_telemetry::events::ActionStationarityNudge {
                        problematically_repeating,
                        run_len,
                        tool_name: tool_name.clone(),
                    },
            ...
            let messaging_grant = if bridge
                ...
                && bridge
                    .tool_for_kind(
                        ezer_tools::types::tool::ToolKind::ActiveAgentMessage,
                    )
                    .await
                    .is_some()
            {
            ...
            if salvage.enabled() {
                request.length_policy = ezer_sampling_types::LengthPolicy::CompletePartial;
            }
            ...
            let (response, latency) = match model_sampler_outcome {
                ...
                Ok(SamplerTurnOutcome::RetryTransient { kind, status_code }) => {
                    if matches!(kind, ezer_sampler::SamplingErrorKind::Api) {
                        auth_retry_schedule.reset_incident_keeping_park();
                    }
                    let delay = ezer_sampler::jitter_backoff(transient_backoff_delay(
                    ...
                    let cause = match kind {
                        ezer_sampler::SamplingErrorKind::IdleTimeout => "Response stalled",
                        ezer_sampler::SamplingErrorKind::Http => "Connection problem",
                        _ => "Server error",
                    };
            ...
            let model_duration_ms = model_timer.elapsed().as_millis() as u64;
            {
                let model_id = self.current_model_id().await;
                ezer_telemetry::session_ctx::log_event(
                    ezer_telemetry::events::ModelResponseReceived {
                        model_id,
            ...
            if ezer_tools::media_gen_limits::should_resample_egregious(
                &over_cap,
                media_gen_resamples,
                ...
                let reminder = ezer_tools::media_gen_limits::resample_reminder(&egregious);
                tracing::warn!(
                    session_id = %self.session_info.id,
            ...
            let turn_refused =
                stop_reason == Some(ezer_sampling_types::StopReason::ContentFilter);
            ...
            match length_salvage_streak.on_sample(
                stop_reason == Some(ezer_sampling_types::StopReason::Length)
                    && !tool_calls.is_empty(),
            ) {
            ...
            let record_response_span = ezer_telemetry::region::Region::from_span(
            ...
            let schema_complete_at_cap = if stop_reason
                == Some(ezer_sampling_types::StopReason::Length)
                && tool_calls.is_empty()
                && salvage.enabled()
            ...
            if stop_reason == Some(ezer_sampling_types::StopReason::Length)
                && tool_calls.is_empty()
            ...
            if is_true_noop {
                ezer_telemetry::session_ctx::log_event(
                    ezer_telemetry::events::ShellTrueNoop {
...
fn step_signature(tool_calls: &[ezer_sampling_types::conversation::ToolCall]) -> String {
...
mod identical_tool_call_run_tests {
    ...
    fn step_signature_ignores_key_order_and_call_order() {
        let call = |name: &str, args: &str| ezer_sampling_types::conversation::ToolCall {
            id: "id".into(),
            name: name.to_string(),
            arguments: args.into(),
        };
...
mod last_sample_span_tests {
    ...
    use ezer_sampling_types::conversation::{
    ...
    fn non_length_failure_and_unreported_stop_reason_overwrite_last_sample() {
        ...
        let mut totals = TurnSpanTotals::default();
        totals.record(&span, &sample(Some(StopReason::ToolCalls), true, 40));
        record_failed_sample_on_turn_span(&span, ezer_sampler::SamplingErrorKind::Api);
        assert_eq!(
            fields
                .lock()
                .unwrap()
