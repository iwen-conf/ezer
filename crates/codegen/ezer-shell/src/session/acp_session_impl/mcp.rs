...
impl SessionActor {
    ...
    /// Attempt to respawn MCP servers whose last spawn failed as unreachable ([`ezer_mcp::servers::McpError::Unreachable`]).
    ...
    pub(super) async fn maybe_inject_mcp_reminder(&self) {
        ...
        use ezer_tools::implementations::search_tool::fingerprint_servers;
    ...
    fn latch_and_push_mcp_reminder(
        &self,
        server_summaries: &[ezer_tools::types::tool_index::ServerSummary],
        new_fingerprints: std::collections::HashMap<
            String,
            ezer_tools::implementations::search_tool::ServerFingerprint,
        >,
    ...
    pub(super) fn apply_mcp_config_diff(
        ...
        dispatch_event_tx: Option<
            tokio::sync::mpsc::UnboundedSender<ezer_mcp::servers::McpClientEvent>,
        >,
    ) {
        if (!diff.added.is_empty() || !diff.removed.is_empty())
            && let Some(tx) = dispatch_event_tx
        {
            let _ = tx.send(ezer_mcp::servers::McpClientEvent::ConfigDiff {
    ...
    pub(crate) async fn respawn_stdio(
        ...
        if let Some(tx) = event_tx {
            let _ = tx.send(ezer_mcp::servers::McpClientEvent::ToolsChanged {
    ...
    pub(crate) fn connected_server_summaries(
        &self,
    ) -> Vec<ezer_tools::types::tool_index::ServerSummary> {
        use ezer_tools::types::tool_index::ToolSearchIndex;
    ...
    pub(super) async fn rendered_mcp_hint(&self) -> Option<String> {
        let file_supported = !self.mcp_file_forms_hidden().await
            && self
                ...
                .lock()
                .await
                .get::<ezer_tools::types::resources::Params<
                    ezer_tools::implementations::use_tool::UseToolParams,
                >>()
    ...
    pub(super) async fn mcp_announcement_snapshot(&self) -> Option<McpAnnouncementSnapshot> {
        let server_summaries = self.connected_server_summaries();
        let mut text =
            ezer_tools::implementations::search_tool::build_server_reminder(&server_summaries)?;
