...
use ezer_workspace::permission::{AccessKind, Decision, PermissionRequest};
...
fn log_file_event<T: ezer_telemetry::TelemetryEvent>(event: T) {
    #[cfg(test)]
    if tests::record_event(&event) {
        return;
    }
    ezer_telemetry::session_ctx::log_event(event);
}
...
pub(super) struct McpFileSource {
    ...
    pub(super) kind: ezer_telemetry::events::McpFileInputKind,
...
impl McpFileSource {
    fn start(path: PathBuf, kind: ezer_telemetry::events::McpFileInputKind) -> Self {
        log_file_event(ezer_telemetry::events::McpFileInputUsed { kind });
        McpFileSource {
            path,
            kind,
            bytes: 0,
...
impl PreparedMcpFile {
    pub(super) async fn freeze(
        ...
        measured.map_err(|observed| {
            log_limit(
                ezer_telemetry::events::McpFileLimitKind::Snapshots,
...
impl McpFileSource {
    fn complete(&self, success: bool) {
        ...
        log_file_event(ezer_telemetry::events::McpFileInputCompleted {
            kind: self.kind,
            outcome: if success {
                ezer_telemetry::events::McpFileInputOutcome::Success
            } else {
                ezer_telemetry::events::McpFileInputOutcome::Failed
            },
...
impl McpFileBatchBudget {
    pub(super) fn admit(&mut self, prepared: &PreparedToolCall) -> Result<(), String> {
        ...
        for (kind, observed, limit) in [
            (
                ezer_telemetry::events::McpFileLimitKind::Sources,
...
fn log_limit(kind: ezer_telemetry::events::McpFileLimitKind, limit: usize, observed: usize) {
    log_file_event(ezer_telemetry::events::McpFileInputLimitHit {
        kind,
            ...
            (
                ...
                let (arguments, preview) = tokio::time::timeout(
                    ...
                    tokio_util::task::AbortOnDropHandle::new(tokio::task::spawn_blocking(
                        move || {
                            let (preview, _) =
                                ezer_hooks::event::truncate_payload(arguments.clone());
                            (arguments, preview)
                        },
                    )),
                )
...
impl SessionActor {
    pub(super) async fn resolve_mcp_file(
        ...
        let path_context = ezer_workspace::permission::types::RequestPathContext {
        ...
        let resolved = tokio::time::timeout(FILE_OPERATION_TIMEOUT, async {
            let (physical, _) =
                ezer_tools::util::read_policy::resolve_read_path(&logical).await;
            ezer_tools::util::read_policy::validate_read_paths(
                &resources,
                &logical,
                &physical,
                Some(&logical),
            ...
            let bytes = fs
                ...
                .map_err(|error| {
                    if error.io_error_kind() == Some(io::ErrorKind::FileTooLarge) {
                        log_limit(
                            ezer_telemetry::events::McpFileLimitKind::Source,
                            MAX_SOURCE_BYTES,
                            MAX_SOURCE_BYTES + 1,
                        );
                    }
        ...
        let eligible = match toolset.is_mcp_target(target_name) {
            ...
            None => resources
                .lock()
                .await
                .get::<ezer_tools::types::resources::ManagedGatewayToolCatalog>()
    ...
    async fn authorize_mcp_source(
        ...
        id: &acp::ToolCallId,
        path: &std::path::Path,
        context: &ezer_workspace::permission::types::RequestPathContext,
