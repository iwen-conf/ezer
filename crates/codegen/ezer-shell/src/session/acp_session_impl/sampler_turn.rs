...
pub(super) fn transient_retry_eligible(error: &ezer_sampler::SamplingErrorInfo) -> bool {
    use ezer_sampler::SamplingErrorKind;
    if error.should_retry == Some(false)
        || ezer_sampling_types::is_context_length_error(&error.message)
        ...
        || matches!(
            error.error_code,
            Some(ezer_sampling_types::ApiErrorCode::InvalidImage)
        )
    {
        return false;
    }
        ...
        SamplingErrorKind::Api => error.status_code.is_some_and(|status| {
            status != 429
                && reqwest::StatusCode::from_u16(status)
                    .is_ok_and(ezer_sampling_types::is_retryable_api_status)
        }),
...
pub(super) async fn call_with_auth_retry<F, Fut>(
    auth_manager: Option<&std::sync::Arc<ezer_login::AuthManager>>,
    shared_recovery: Option<&tokio::sync::OnceCell<bool>>,
    tool_name: &str,
    mut call: F,
) -> Result<ezer_tools::types::output::ToolRunResult, xai_tool_runtime::ToolError>
where
    F: FnMut() -> Fut,
    Fut: std::future::Future<
            Output = Result<
                ezer_tools::types::output::ToolRunResult,
        ...
        >,
{
    ...
    let src = ezer_login::recovery::RecoverySource::Background;
...
fn error_after_stream_drain(
    ...
    original: ezer_sampler::SamplingErrorInfo,
...
fn revoked_sampling_info() -> ezer_sampler::SamplingErrorInfo {
    ezer_sampler::SamplingErrorInfo {
...
impl SessionActor {
    ...
    fn resolve_hosted(
        ...
        Vec<ezer_sampling_types::HostedTool>,
        ezer_sampling_types::ToolOverrides,
    ) {
        let mut tools = self.agent.borrow().hosted_tools().to_vec();
        let applied = ezer_sampling_types::apply_tool_overrides(
            &mut tools,
    ...
    pub(crate) fn effective_hosted_tools(&self) -> Vec<ezer_sampling_types::HostedTool> {
        self.resolve_hosted().0
    }

    pub(crate) fn hosted_tools_for_turn(&self) -> Vec<ezer_sampling_types::HostedTool> {
    ...
    pub(crate) fn effective_tool_overrides(
        &self,
    ) -> Option<ezer_sampling_types::ToolOverrides> {
        if !self.backend_search_active() {
            return None;
        }
    ...
    pub(crate) fn set_tool_overrides(&self, overrides: ezer_sampling_types::ToolOverrides) {
    ...
    pub(crate) fn apply_tool_overrides_update(
        &self,
        update: Option<ezer_sampling_types::ToolOverridesUpdate>,
    ) {
    ...
    pub(crate) async fn mcp_file_forms_hidden(&self) -> bool {
        self.chat_state_handle
            ...
            .await
            .is_some_and(|config| {
                config.api_backend == ezer_sampling_types::ApiBackend::Messages
            })
    }
    ...
    pub(super) fn model_auth_provider(
        &self,
        model_id: &str,
    ) -> Option<ezer_login::AuthProviderRef> {
    ...
    async fn refresh_provider_token_pre_turn(
        &self,
        provider: &ezer_login::AuthProviderRef,
        current_key: Option<&str>,
        model_id: &str,
    ) {
        match provider.ensure_fresh_token(current_key).await {
            ezer_login::ProviderRefreshOutcome::Rotated(new_key) => {
                tracing::info!(
                    model = %model_id,
            ...
            ezer_login::ProviderRefreshOutcome::Unchanged => {}
            // A genuine mint failure; the 401 arm handles the rejection.
            ezer_login::ProviderRefreshOutcome::MintFailed => {
                tracing::warn!(
            ...
            ezer_login::ProviderRefreshOutcome::Unusable => {}
        }
    }
    ...
    async fn try_provider_401_recovery(&self, provider: &ezer_login::AuthProviderRef) -> bool {
    ...
    pub(super) async fn reconstruct_full_config(&self) -> SamplingConfig {
        ...
        impl ezer_sampler::HeaderInjector for TraceContextInjector {
            fn inject(&self, headers: &mut reqwest::header::HeaderMap) {
                if let Some(tp) = ezer_otel::current_traceparent()
            ...
            fn set_span_parent(&self, span: &tracing::Span, traceparent: &str) {
                if !ezer_otel::set_parent_from_traceparent(span, traceparent) {
                    tracing::debug!(
        ...
        let cfg = self
            ...
            .get_sampling_config()
            .await
            .unwrap_or_else(|| ezer_sampling_types::SamplingConfig {
                base_url: String::new(),
                mtls_cert_dir: None,
                model: String::new(),
        ...
        SamplingConfig {
            ...
            bearer_resolver: if use_bearer_resolver {
                self.auth_manager.as_ref().map(|am| {
                    ezer_login::credential_provider::WireValidBearerResolver::shared(am.clone())
                })
            } else {
                None
            },
    ...
    pub(crate) async fn wire_permission_auto_llm_classifier(self: &Arc<Self>) {
        ...
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel::<(
            Vec<ezer_workspace::permission::ClassifierMessage>,
            tokio::sync::oneshot::Sender<
                Result<String, ezer_workspace::permission::ClassifierFailure>,
            >,
        )>();
                ...
                let result = async {
                    let (sampling_client, model, context_window) = match &aux_classifier_sampler {
                        ...
                        None => {
                            ...
                            let client = ezer_sampler::SamplingClient::new(config).map_err(|e| {
                                ezer_workspace::permission::ClassifierFailure::TransportError(
                                    e.to_string(),
                                )
                            })?;
                    ...
                    let items = messages
                        .into_iter()
                        .map(|m| match m.role {
                            ezer_workspace::permission::ClassifierMessageRole::System => {
                                ConversationItem::system(m.text)
                            }
                            ezer_workspace::permission::ClassifierMessageRole::User => {
                    ...
                    let request = ConversationRequest {
                        ...
                        json_schema: Some(
                            ezer_workspace::permission::classifier_output_json_schema(),
                        ),
                    ...
                    let response = tokio::time::timeout(classify_timeout, fut)
                        .await
                        .map_err(|_| ezer_workspace::permission::ClassifierFailure::Timeout)?
                        .map_err(|e| {
                            ezer_workspace::permission::ClassifierFailure::TransportError(
                                e.to_string(),
                            )
        ...
        let clf =
            ezer_workspace::permission::LlmPermissionClassifier::with_channel(tx, prompt_type);
        debug_assert!(
            clf.has_side_query(),
    ...
    pub(super) async fn resolve_aux_sampler_config(
        &self,
        slug: &str,
    ) -> Option<ezer_sampler::SamplerConfig> {
        let creds = self.chat_state_handle.get_credentials().await;
    ...
    async fn resolve_auto_classifier_sampler(
        &self,
        slug: &str,
    ) -> Option<(ezer_sampler::SamplingClient, String, u64)> {
        ...
        let client = ezer_sampler::SamplingClient::new(cfg)
            .map_err(
    ...
    pub(super) async fn prepare_chat_completion(
        &self,
        force_http1: bool,
    ) -> Result<ezer_sampler::SamplingClient, acp::Error> {
        ...
        let sampling_client =
            ezer_sampler::SamplingClient::new(full_config).map_err(|e| self.to_acp_error(e))?;
    ...
    pub(crate) async fn fail_turn_length_salvage_exhausted(&self) -> acp::Error {
        let kind = ezer_sampler::SamplingErrorKind::MaxTokensTruncation;
        let message = ezer_sampling_types::SamplingError::MaxTokensTruncation.to_string();
    ...
    fn log_terminal_failure(&self, error_type: &str, status_code: Option<u16>, message: &str) {
        ...
        ezer_telemetry::unified_log::warn(
            ...
            Some(serde_json::json!({
                ...
                "key_prefix": auth.as_ref().map(|a| ezer_auth::bearer_suffix(&a.key).to_owned()),
                "expires_at": auth
                    .as_ref()
    ...
    fn uncharged_401_park_eligible(
        &self,
        am: &ezer_login::AuthManager,
        credential: ezer_sampling_types::SentCredential,
    ) -> bool {
    ...
    pub(crate) async fn handle_sampling_failure(
        self: &Arc<Self>,
        error: ezer_sampler::SamplingErrorInfo,
        ...
        if auth_recovery_eligible && let Some(ref am) = self.auth_manager {
            ...
            if am
                .try_recover_unauthorized(ezer_login::recovery::RecoverySource::Turn)
                .await
            {
        ...
        let error_type = if error
            .error_code
            .as_ref()
            .is_some_and(ezer_sampling_types::ApiErrorCode::is_size_overflow)
            || ezer_sampling_types::is_context_length_error(&error.message)
        {
    ...
    async fn wait_for_stream_drain(
        &self,
        request_id: &ezer_sampler::RequestId,
    ...
    async fn submit_turn_request(
    ...
    ) -> Result<SamplerTurnOutcome, ezer_sampler::SamplingErrorInfo> {
        let request_id = ezer_sampler::RequestId::random();
        ...
        let collected = {
            ...
            request.traceparent = ezer_otel::span_traceparent(sampling_span.span());
            self.sampler_handle
        ...
        match collected.result {
            ...
            Err(rich_err) => {
                ...
                let original = ezer_sampler::SamplingErrorInfo::from(&rich_err);
    ...
    async fn recover_from_sampling_failure(
        self: &Arc<Self>,
        info: ezer_sampler::SamplingErrorInfo,
    ...
    pub(crate) async fn refresh_token_if_expired(&self) {
        ...
        use ezer_login::{is_jwt_expired_or_near, parse_jwt_expiration};
...
fn resolve_configured_cutoff(
    seed: Option<ezer_sampling_types::ToolOverrides>,
    base: Option<&ezer_sampling_types::ToolOverrides>,
) -> ezer_sampling_types::ToolOverrides {
    use ezer_sampling_types::{ToolOverrides, WebSearchOptions, XSearchOptions};
    let ToolOverrides {
        x_search: seed_x,
        web_search: seed_w,
...
mod stream_drain_tests {
    ...
    fn failed_drain_revocation_supersedes_original_error() {
        let original = ezer_sampler::SamplingErrorInfo {
            kind: ezer_sampler::SamplingErrorKind::RateLimited,
