...
use ezer_telemetry::region::Parent as SpanParent;
use ezer_telemetry::subagent_spawn::phase_region_under;
static SESSIONS_ACTIVE: ezer_telemetry::activity::ActivityGauge =
    ezer_telemetry::activity::ActivityGauge::residency(
        ezer_telemetry::activity::SESSIONS_ACTIVE_KEY,
    );
...
fn drop_cli_catchall_allows(
    rules: Vec<ezer_workspace::permission::types::PermissionRule>,
    policy_block: Option<&'static str>,
) -> (
    Vec<ezer_workspace::permission::types::PermissionRule>,
    Vec<ezer_workspace::permission::types::PermissionRule>,
) {
    ...
    for rule in rules {
        if ezer_workspace::permission::resolution::is_catchall_allow(&rule) {
            dropped.push(rule);
        } else {
            kept.push(rule);
        }
...
) -> ezer_telemetry::events::MemoryRetrievalMode {
    use ezer_telemetry::events::MemoryRetrievalMode::*;
...
fn subagent_sampler_rate_limit_threshold(is_subagent: bool, pacer_max_attempts: u32) -> u32 {
    if is_subagent && pacer_max_attempts > 0 {
        ezer_sampler::RATE_LIMIT_RETRY_DISABLED
    } else {
        ezer_sampler::RATE_LIMIT_RETRY_THRESHOLD
    }
}
...
mod cli_catchall_drop_tests {
    ...
    use ezer_workspace::permission::resolution::YoloPinReason;
    const PIN: &str = YoloPinReason::DisableBypassPermissionsMode.message();
    use ezer_workspace::permission::rules::parse_permission_rule;
    use ezer_workspace::permission::types::{PermissionRule, RuleAction, ToolFilter};
    ...
    fn disabled_memory_config_has_disabled_retrieval_mode() {
        assert_eq!(
            ...
            ezer_telemetry::events::MemoryRetrievalMode::Disabled
        );
    }
    #[test]
...
mod session_retry_policy_tests {
    ...
    use ezer_sampler::{RATE_LIMIT_RETRY_DISABLED, RATE_LIMIT_RETRY_THRESHOLD};
    #[test]
...
pub(crate) async fn spawn_session_actor(
    ...
    attribution_callback: Option<ezer_sampler::SharedAttributionCallback>,
    ...
    preloaded_skills: Option<Vec<ezer_tools::implementations::skills::types::SkillInfo>>,
    compat: CompatConfig,
    paths_config: ezer_agent::prompt::paths::PathsConfig,
    ...
    media_gen_batch_limits: ezer_tools::media_gen_limits::MediaGenBatchLimits,
    ...
    prompt_audience: ezer_agent::prompt::context::PromptAudience,
    ...
    plugin_registry_handle: Option<ezer_agent::plugins::SharedPluginRegistryHandle>,
    models_manager: crate::agent::remote_config::ModelsManager,
    inherited_permission_handle: Option<ezer_workspace::permission::PermissionHandle>,
    api_key_provider: Option<ezer_tools::types::SharedApiKeyProvider>,
    image_description_model: String,
    hook_registry_override: Option<std::sync::Arc<ezer_hooks::discovery::HookRegistry>>,
    workspace_ops: ezer_workspace::WorkspaceOps,
    cli_permission_rules: Vec<ezer_workspace::permission::types::PermissionRule>,
    todo_gate: bool,
    ...
    is_chat_kind: bool,
    spawn_ctx: Option<ezer_telemetry::subagent_spawn::SpawnPhaseContext>,
    sampling_gate: Option<Arc<tokio::sync::Semaphore>>,
) -> Result<(SessionInitResult, tokio::sync::oneshot::Receiver<()>), ezer_agent::AgentBuildError>
{
    if max_turns == Some(0) {
        return Err(ezer_agent::AgentBuildError::InvalidConfig(
    ...
    let (permissions, permission_events_rx, deny_read_globs) = async {
        use ezer_workspace::permission::resolution as permission_resolution;
        if let Some(handle) = inherited_permission_handle {
                ...
                match &mut permission_config {
                    ...
                    None => {
                        permission_config = Some(
                            ezer_workspace::permission::types::PermissionConfig::new(
                                cli_permission_rules,
                            ),
                        );
                    }
            ...
            let deny_read_globs = permission_config
                ...
                .unwrap_or_default();
            let hub_permission = if ezer_workspace::permission::hitl_permission_live_enabled() {
                let server = match workspace_ops.workspace_handle() {
                    Some(handle) => handle.hub_server_blocking().await,
                    None => None,
                };
                let transport = server
                    .and_then(|server| ezer_workspace::permission::ToolServerPermissionTransport::from_session_id(
                        server,
                    ...
                    .map(|t| {
                        std::sync::Arc::new(t)
                            as std::sync::Arc<
                                dyn ezer_workspace::permission::PermissionHookTransport,
                            >
                    });
            ...
            let (permissions, permission_events_rx) = ezer_workspace::permission::spawn_permission_manager_with_pin(
    ...
    let web_search_config = if disable_web_search {
        ezer_tools::implementations::WebSearchConfig::Disabled
    } else if let Some(cfg) = web_search_sampling_config {
        if let Some(api_key) = cfg.api_key {
            ezer_tools::implementations::WebSearchConfig::Enabled {
                api_key,
    ...
    let resolved_max_retries = ezer_sampler::resolve_max_retries(session_max_retries_source(
    ...
    let chat_state_sampling_config = ezer_sampling_types::SamplingConfig {
    ...
    let task_completion_reservations =
        ezer_tools::reminders::task_completion::TaskCompletionReservations::default();
    let task_wake_suppressed =
        ezer_tools::reminders::task_completion::TaskWakeSuppressed::default();
    ...
    *synthetic_trace_tx_shared.lock().unwrap() = tool_context.synthetic_trace_tx.clone();
    ...
    let index_root_for_session =
        ezer_workspace::session::git::find_git_root_from_path(tool_context.cwd.as_path())
    ...
    let resolve_search_shadows = || {
        ...
        ezer_tools::computer::local::SearchShadowConfig {
            find_bfs,
            grep_ugrep,
        }
    };
    let resolve_policy = || crate::util::config::resolve_shell_env_policy(effective_cfg.as_ref());
    let terminal_backend: std::sync::Arc<dyn ezer_tools::computer::types::TerminalBackend> =
        match terminal_backend_kind {
            ...
            TerminalBackendKind::AcpClient => {
                std::sync::Arc::new(crate::terminal::AcpTerminalAdapter::new(
                    ...
                    as std::sync::Arc<dyn ezer_tools::computer::types::TerminalBackend>
            }
    ...
    let fs_backend: std::sync::Arc<dyn ezer_tools::computer::types::AsyncFileSystem> =
        if client_fs_capable && tool_context.gateway.is_some() {
            std::sync::Arc::new(ezer_workspace::file_system::AcpFsAdapter::new(
                tool_context.gateway.clone().unwrap(),
                tool_context.session_id.clone().unwrap(),
            ))
        } else {
            std::sync::Arc::new(ezer_tools::computer::local::LocalFs)
        };
    ...
    let compaction_policy = ezer_agent::CompactionPolicy {
    ...
    match select_memory_storage(configured_memory_storage.as_ref(), memory_config.as_ref()) {
        ...
        MemoryStorageSelection::UnavailableForEphemeralWorkspace => {
            tracing::info!(
                target: ezer_telemetry::memory_log::TARGET,
    ...
    let memory_backend_for_spec: Option<
        std::sync::Arc<dyn ezer_tools::types::memory_backend::MemoryBackend>,
        ...
        .filter(|storage| storage.mode().is_legacy())
    {
        ...
        let embed_credentials = ezer_login::credential_provider::embedding_session_credentials(
            &embed_base_url,
            auth_manager.as_ref(),
        ...
        let backend: std::sync::Arc<dyn ezer_tools::types::memory_backend::MemoryBackend> =
        ...
        ezer_telemetry::session_ctx::log_event(
            ezer_telemetry::memory_telemetry::MemorySessionInit {
                session_id: session_info.id.to_string(),
                memory_enabled: true,
                memory_mode: ezer_telemetry::memory_telemetry::MemoryMode::Legacy,
                watcher_config_enabled: watcher_config.enabled,
                watcher_started,
        ...
        if let Some(storage) = memory_storage_for_session.as_ref() {
            ...
            ezer_telemetry::session_ctx::log_event(
                ezer_telemetry::memory_telemetry::MemorySessionInit {
                    session_id: session_info.id.to_string(),
                    memory_enabled: true,
                    memory_mode: ezer_telemetry::memory_telemetry::MemoryMode::V2,
            ...
            let rollout = match controls.rollout {
                crate::config::MemoryV2Rollout::Off => {
                    ezer_telemetry::memory_telemetry::MemoryV2Rollout::Off
                }
                crate::config::MemoryV2Rollout::RecordOnly => {
                    ezer_telemetry::memory_telemetry::MemoryV2Rollout::RecordOnly
                }
                crate::config::MemoryV2Rollout::Shadow => {
                    ezer_telemetry::memory_telemetry::MemoryV2Rollout::Shadow
                }
                crate::config::MemoryV2Rollout::Active => {
                    ezer_telemetry::memory_telemetry::MemoryV2Rollout::Active
                }
            };
            ezer_telemetry::session_ctx::log_event(
                ezer_telemetry::memory_telemetry::MemoryV2ControlsPinned {
                    rollout,
    ...
    let managed_gateway_tool_client = auth_manager.as_ref().map(|am| {
        ezer_tools::types::resources::ManagedGatewayToolClient(Arc::new(
    ...
    use ezer_telemetry::subagent_spawn::SubagentSpawnPhase;
    ...
    let (harness_metrics, scheduler_handle_for_handle, toolset) = async {
        ...
        let harness_metrics = if !startup_hints.is_subagent
            && (telemetry_enabled || ezer_telemetry::external::is_active())
        {
            ...
            Some(super::telemetry::SessionHarnessMetrics {
                ...
                permission_mode: if session_yolo_mode {
                    ezer_telemetry::enums::PermissionMode::AlwaysApprove
                } else if session_auto_mode
                    && crate::util::config::auto_permission_mode_enabled_from_disk()
                {
                    ezer_telemetry::enums::PermissionMode::Auto
                } else {
                    ezer_telemetry::enums::PermissionMode::Ask
                },
        ...
        let resolved_task_output =
            ezer_tools::reminders::task_completion::resolve_task_output_tool_name(
                agent.tool_bridge(),
            )
            .await;
        let resolved_read =
            ezer_tools::reminders::task_completion::resolve_read_tool_name(agent.tool_bridge())
                .await;
        let resolved_scheduler_delete =
            ezer_tools::reminders::task_completion::resolve_scheduler_delete_tool_name(
                agent.tool_bridge(),
            )
            .await;
        let resolved_scheduler_create =
            ezer_tools::reminders::task_completion::resolve_scheduler_create_tool_name(
                agent.tool_bridge(),
            )
            .await;
        ...
        tool_context.task_output_tool_name = resolved_task_output.unwrap_or_else(|| {
            ezer_tools::reminders::task_completion::DEFAULT_TASK_OUTPUT_TOOL.to_string()
        });
    ...
    let resolved_workspace_root = ezer_workspace::session::git::find_git_root_from_path(
    ...
    let sampler_retry_policy = ezer_sampler::RetryPolicy {
    ...
    let mut hook_discovery_errors: Vec<ezer_hooks::error::HookError> = Vec::new();
    let hooks_discovery_span = tracing::info_span!("spawn.hooks_discovery").entered();
    let built_hook_registry: Option<Arc<ezer_hooks::discovery::HookRegistry>> =
        if let Some(override_reg) = hook_registry_override {
            ...
            let git_root = ezer_workspace::session::git::find_git_root_from_path(cwd_path).ok();
    ...
    let resolved_tool_overrides: std::sync::Arc<
        arc_swap::ArcSwapOption<ezer_sampling_types::ToolOverrides>,
    ...
    let vcs_kind = {
        ...
        match ezer_workspace::session::git::discover_git_root(root) {
            ezer_workspace::session::git::GitDiscoveryResult::Found(git_root) => {
                ezer_workspace::session::git::detect_vcs_kind(&git_root)
            }
            _ => ezer_workspace::session::git::VcsKind::None,
        }
    };
    ...
    let repo_status_plan = if matches!(vcs_kind, ezer_workspace::session::git::VcsKind::None) {
    ...
    let session = Arc::new_cyclic(|weak: &std::sync::Weak<SessionActor>| SessionActor {
        ...
        hook_disabled: std::cell::RefCell::new(Arc::new(
            ezer_hooks::trust::DisabledHooks::load(),
        )),
    ...
    async {
        ...
        }
        {
            ...
            session
                ...
                .borrow()
                .tool_bridge()
                .update_resource(ezer_tools::types::tool_index::ToolIndex(
        ...
        }
        {
            ...
            session
                ...
                .borrow()
                .tool_bridge()
                .update_resource(ezer_tools::types::resources::PlanFilePath(plan_path))
                .await;
        }
        ...
        .filter(|storage| storage.mode().is_legacy())
    {
        ...
        let reindex_handle = tokio::task::spawn_local(async move {
            ...
            ) && let Ok(files) = storage.list_memory_files()
            {
                ...
                ezer_telemetry::session_ctx::log_event(
                    ezer_telemetry::memory_telemetry::MemoryReindex {
    ...
    let telemetry_ctx = ezer_telemetry::session_ctx::TelemetryCtx::new(
    ...
    if let Some(metrics) = harness_metrics {
        ...
        tokio::spawn(async move {
            ...
            ezer_telemetry::session_ctx::log_event_dual(telemetry_enabled, ev);
        });
    }
    let hosting = SESSIONS_ACTIVE.enter();
    tokio::task::spawn_local(async move {
        let _hosting = hosting;
        ezer_telemetry::session_ctx::with_session_ctx(
            telemetry_ctx,
            run_session(
                session,
                cmd_rx,
...
pub(crate) struct SessionInitResult {
    ...
    pub(crate) toolset: Arc<ezer_tools::registry::types::FinalizedToolset>,
}
...
pub(crate) fn bind_installed_toolset(
    ...
    toolset: &Arc<ezer_tools::registry::types::FinalizedToolset>,
) -> Option<Arc<ezer_tools::registry::types::FinalizedToolset>> {
...
pub(crate) async fn spawn_session_on_thread(
    ...
    spawn_ctx: Option<ezer_telemetry::subagent_spawn::SpawnPhaseContext>,
    sampling_gate: Option<Arc<tokio::sync::Semaphore>>,
    spawn_trace: Option<ezer_telemetry::startup::SpawnTraceContext>,
) -> Result<(SessionInitResult, SessionThread), acp::Error> {
    let (init_tx, init_rx) = tokio::sync::oneshot::channel::<
        Result<SessionInitResult, ezer_agent::AgentBuildError>,
    >();
        ...
        .spawn(move || {
            ...
            let rt = match build_session_runtime() {
                ...
                Err(e) => {
                    ...
                    let _ = init_tx
                        .send(Err(ezer_agent::AgentBuildError::RuntimeBuild(e)));
                    return;
                }
            };
                ...
                let _trace_span = parent_traceparent
                    ...
                    .map(|tp| {
                        let meta = serde_json::json!({ "traceparent": tp })
                            ...
                            .cloned()
                            .unwrap_or_default();
                        let span = ezer_otel::span_from_meta_traceparent(&meta);
                        span.entered()
                    });
            ...
            rt.block_on(ezer_telemetry::session_ctx::drain_at_session_exit());
        });
    ...
    conversation.iter().any(|item| {
        ...
        let contains = |needle: &str| {
            u.content.iter().any(|part| {
                matches!(
                    part,
                    ezer_sampling_types::conversation::ContentPart::Text { text }
