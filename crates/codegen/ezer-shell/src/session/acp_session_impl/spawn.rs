...
struct SpawnStep {
    ...
    _timer: ezer_telemetry::instrumentation::InstrumentationTimer,
}
impl SpawnStep {
...
macro_rules! spawn_await_step {
    ($step:literal $(, $field:ident = $value:expr)* $(,)?) => {
        ezer_telemetry::startup_step_grouped!("spawn_actor", $step $(, $field = $value)*)
    };
}
        ...
        .spawn(move || {
            let (initial_last_compaction, initial_prompt_texts) = {
                ...
                let initial_last_compaction = {
                    let _timer = ezer_telemetry::startup_step_timer_grouped!(
                        "spawn_actor",
