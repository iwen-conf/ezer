...
struct V2DreamModelFailure {
    class: ezer_telemetry::memory_telemetry::MemoryV2FailureClass,
    detail: String,
    usage: ezer_telemetry::memory_telemetry::MemoryV2ModelUsage,
}

fn classify_consolidation_error(
    error: &ezer_memory::V2ConsolidationError,
) -> ezer_telemetry::memory_telemetry::MemoryV2FailureClass {
    use ezer_memory::V2ConsolidationError;
    use ezer_telemetry::memory_telemetry::MemoryV2FailureClass;
    match error {
...
enum V2DreamClaimOutcome {
    NoWork,
    Resumed(ezer_memory::ConsolidationResult),
    Fresh {
        guard: V2DreamLeaseGuard,
        input: ezer_memory::ConsolidationInput,
    },
}

struct V2DreamLeaseGuard {
    resources: Option<(
        Box<ezer_memory::V2ConsolidationStore>,
        ezer_memory::ConsolidationLease,
    )>,
    clock: ezer_memory::SharedV2Clock,
}
...
impl V2DreamLeaseGuard {
    fn new(
        store: Box<ezer_memory::V2ConsolidationStore>,
        lease: ezer_memory::ConsolidationLease,
        clock: ezer_memory::SharedV2Clock,
    ) -> Self {
        Self {
    ...
    fn store(&self) -> &ezer_memory::V2ConsolidationStore {
    ...
    fn lease(&self) -> &ezer_memory::ConsolidationLease {
...
fn emit_v2_dream_lifecycle(
    disposition: ezer_telemetry::memory_telemetry::MemoryV2DreamDisposition,
    observation_count: usize,
    topic_change_count: usize,
    started_at: std::time::Instant,
    failure_class: Option<ezer_telemetry::memory_telemetry::MemoryV2FailureClass>,
) {
    ezer_telemetry::session_ctx::log_event(
        ezer_telemetry::memory_telemetry::MemoryV2DreamLifecycle {
            disposition,
            observation_count,
            topic_change_count,
            latency_ms: started_at.elapsed().as_millis() as u64,
            failure_class,
            usage: ezer_telemetry::memory_telemetry::MemoryV2ModelUsage::default(),
        },
    );
}
...
fn parse_v2_dream_plan(response: &str) -> Result<Vec<ezer_memory::TopicOperation>, String> {
    ...
    Ok(plan
        ...
        .map(|operation| match operation {
            V2DreamPlanOperation::Create {
                ...
                content,
                evidence,
            } => ezer_memory::TopicOperation::Create {
                path: path.into(),
                content,
            ...
            V2DreamPlanOperation::Update {
                ...
                content,
                evidence,
            } => ezer_memory::TopicOperation::Update {
                path: path.into(),
                content,
                evidence: paths(evidence),
            },
            V2DreamPlanOperation::Delete { path, evidence } => {
                ezer_memory::TopicOperation::Delete {
                    path: path.into(),
                    evidence: paths(evidence),
                }
            }
            V2DreamPlanOperation::Rename {
                ...
                content,
                evidence,
            } => ezer_memory::TopicOperation::Rename {
                from: from.into(),
                to: to.into(),
                content,
                evidence: paths(evidence),
            ...
            V2DreamPlanOperation::Merge {
                ...
                content,
                evidence,
            } => ezer_memory::TopicOperation::Merge {
            ...
            V2DreamPlanOperation::Split {
                ...
                destinations,
                evidence,
            } => ezer_memory::TopicOperation::Split {
                source: source.into(),
...
impl SessionActor {
    pub(super) async fn promote_v2_hidden_observations(
        ...
        for attempt in 0..=V2_PROMOTION_MAX_RETRIES {
            ...
            match tokio::task::spawn_blocking(move || {
                ezer_memory::V2CaptureStore::open_with_clock(
                    &attempt_workspace,
                    ezer_memory::V2MemoryScope::Workspace,
                    attempt_clock,
                )?
            ...
            .await
            {
                ...
                Ok(Err(error))
                    if retry_deferred
                        ...
                        && matches!(
                            error,
                            ezer_memory::V2CaptureError::Conflict(_)
                                | ezer_memory::V2CaptureError::Database(_)
                                | ezer_memory::V2CaptureError::Io { .. }
                        ) =>
                {
                    tokio::select! {
                        biased;
    ...
    pub(super) async fn on_v2_capture_completed(
        ...
        use ezer_telemetry::memory_telemetry::{
        ...
        if followups.run_maintenance {
            let retention = ezer_memory::RetentionPolicy {
            ...
            match tokio::task::spawn_blocking(move || {
                let maintenance = ezer_memory::V2MaintenanceStore::open_with_clock(
            ...
            .await
            {
                Ok(Ok(result)) => {
                    ezer_telemetry::session_ctx::log_event(
                        ezer_telemetry::memory_telemetry::MemoryV2GcCompleted {
                ...
                Ok(Err(error)) => {
                    let reason = match &error {
                        ezer_memory::V2MaintenanceError::ActiveLease => {
                            MemoryV2FailureClass::Lease
                        }
                        ezer_memory::V2MaintenanceError::Access(_) => {
                            MemoryV2FailureClass::AccessPolicy
                        }
                        ezer_memory::V2MaintenanceError::Index(_)
                        | ezer_memory::V2MaintenanceError::Manifest(_) => {
                            MemoryV2FailureClass::Convergence
                        }
                        _ => MemoryV2FailureClass::Storage,
                    };
                    tracing::warn!(error = %error, "memory-v2 maintenance failed");
                    ezer_telemetry::session_ctx::log_event(MemoryV2FailClosed {
        ...
        let eligibility = tokio::task::spawn_blocking(move || {
            let store = ezer_memory::V2ConsolidationStore::open_with_clock(
                &workspace,
                ezer_memory::V2MemoryScope::Workspace,
                &global,
                &workspace,
            ...
            let config = ezer_memory::DreamEligibilityConfig {
            ...
            Ok::<_, ezer_memory::V2ConsolidationError>(eligibility)
        })
        .await;
        if cancel.is_cancelled() {
            return;
        ...
        match eligibility {
            Ok(Ok(eligibility))
                if eligibility.disposition == ezer_memory::DreamTriggerDisposition::Ready =>
            {
                emit_v2_dream_lifecycle(
                    ezer_telemetry::memory_telemetry::MemoryV2DreamDisposition::Ready,
            ...
            Ok(Ok(eligibility)) => {
                let disposition = match eligibility.disposition {
                    ezer_memory::DreamTriggerDisposition::Ineligible => {
                        ezer_telemetry::memory_telemetry::MemoryV2DreamDisposition::Ineligible
                    }
                    ezer_memory::DreamTriggerDisposition::Coalesced => {
                        ezer_telemetry::memory_telemetry::MemoryV2DreamDisposition::Coalesced
                    }
                    ezer_memory::DreamTriggerDisposition::Ready => {
                        ezer_telemetry::memory_telemetry::MemoryV2DreamDisposition::Ready
                    }
                };
                emit_v2_dream_lifecycle(
    ...
    async fn run_v2_dream_with_cancel(
        ...
        let task = ezer_telemetry::session_ctx::spawn_local_in_session_ctx(async move {
    ...
    async fn execute_v2_dream(
        ...
        let claimed = tokio::task::spawn_blocking({
            ...
            move || {
                ...
                let request = ezer_memory::DreamClaimRequest {
                    owner,
                    now,
                ...
                let Some(lease) = lease else {
                    return Ok::<_, ezer_memory::V2ConsolidationError>(
        ...
        let outcome = match claimed {
            ...
            Ok(Err(ezer_memory::V2ConsolidationError::Busy)) => {
                emit_v2_dream_lifecycle(
    ...
    async fn run_v2_dream_model_call(
        ...
        (
            String,
            ezer_telemetry::memory_telemetry::MemoryV2ModelUsage,
        ),
        V2DreamModelFailure,
    > {
        use ezer_telemetry::memory_telemetry::MemoryV2FailureClass;
