...
impl SessionActor {
    pub(super) async fn handle_set_session_model(
        ...
        self.chat_state_handle
            .update_sampling_config(ezer_sampling_types::SamplingConfig {
                ...
                max_retries: Some(ezer_sampler::resolve_max_retries(
                ...
                if let ConversationItem::System(sys) = item {
                    if use_concise {
                        sys.content = std::sync::Arc::<str>::from(
                            ezer_agent::prompt::template::COMPACT_SYSTEM_PROMPT,
                        );
                    } else {
                        sys.content =
    ...
    /// Builds a fresh [`ezer_agent::Agent`] from the cached [`crate::session::agent_rebuild::AgentRebuildSpec`] and the supplied definition.
