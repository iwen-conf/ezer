...
fn partition_rules_by_scope(
    ...
    for file in files {
        ...
        let entry = ezer_agent::prompt::user_message::RuleEntry::from(file);
        if is_user_rule {
            user.push(entry);
        } else {
...
mod partition_rules_by_scope_tests {
    ...
    use std::path::Path;
    use ezer_agent::prompt::agents_md::AgentConfigFile;
    ...
    fn configured_rule_dir_is_a_user_rule_wherever_it_sits() {
        use ezer_agent::prompt::agents_md::InstructionSource;
    ...
    fn paths(entries: &[ezer_agent::prompt::user_message::RuleEntry]) -> Vec<&str> {
    ...
    fn partitioned_snapshot_bodies_match_rules_and_old_reminder() {
        ...
        let rules =
            ezer_agent::prompt::user_message::format_rules_section(&workspace, &user).unwrap();
        let reminder = ezer_agent::prompt::agents_md::format_agents_md_section(&files).unwrap();
        for body in [
            "repo-agents-body",
            "repo-claude-body",
            "home-ezer-body",
...
impl SessionActor {
                ...
                !matches!(
                    ...
                    ConversationItem::User(u)
                        if u.synthetic_reason
                            == ezer_sampling_types::SyntheticReason::SystemReminder
                )
            });
        }
    }
    ...
    pub(super) async fn build_user_message_prefix(&self) -> String {
        ...
        if uses_legacy_prefix {
            ...
            let mut user_rules = ezer_agent::prompt::user_message::built_in_user_rules();
            if include_verification {
                user_rules
                    .extend(ezer_agent::prompt::browser_verification::synthetic_user_rules());
                user_rules.extend(discovered_user_rules);
            }
            ezer_agent::prompt::user_message::append_rules_section(
                &mut out,
                &workspace_rules,
                &user_rules,
            );
    ...
    async fn build_templated_user_message(
        ...
        use ezer_agent::prompt::user_message::UserMessageContext;
        ...
        {
            user_rules.extend(ezer_agent::prompt::browser_verification::synthetic_user_rules());
        }
        user_rules.extend(discovered_user_rules);
    ...
    ) -> Vec<ezer_agent::prompt::user_message::McpServerEntry> {
        use ezer_agent::prompt::user_message::McpServerEntry;
    ...
    pub(super) async fn transcribe_user_images(
        ...
        let client = ezer_sampler::SamplingClient::new(sampler_config).map_err(|e| {
