...
pub(super) async fn dispatch_observed(
    ...
    prepared: &PreparedToolCall,
    session_id: &str,
    origin: ezer_tools::types::tool_call_origin::ToolCallOrigin,
    slot: ezer_tools::types::source_summary::SourceSummarySlot,
) -> Result<ToolRunResult, xai_tool_runtime::ToolError> {
...
impl SessionActor {
    ...
    pub(super) async fn handle_direct_bash_command(
        ...
        let title_command = ezer_tools::util::strip_redundant_session_cd(
            &command,
        ...
        // Bash mode has no model-issued wire name; resolve the toolset's execute tool by kind so the ezer/tool identity still stamps
        ...
        let exec_wire = {
            ...
            agent
                .tool_bridge()
                .toolset()
                .tool_name_for_kind(ezer_tools::types::tool::ToolKind::Execute)
        };
...
// `truncate_bytes` is the UTF-8-safe truncation helper from ezer-sampling-types
