...
impl SessionActor {
        ...
        *self.current_prompt_mode.lock() = prompt_mode;
        ...
        if mode.is_plan() {
            ...
            ezer_telemetry::session_ctx::log_event(
                ezer_telemetry::events::PlanModeToggled {
                    enabled: true,
                    trigger: ezer_telemetry::events::PlanModeTrigger::User,
                    turn_in_flight,
    ...
    /// The activation reminder template for the active template (no first-entry/reentry distinction), or ezer's reentry/full variant.
