...
use ezer_agent::prompt::agents_md::LEGACY_AGENTS_MD_REMINDER_PREFIX;
use ezer_agent::prompt::skills::SkillsConfig;
use ezer_sampler::SamplerConfig as SamplingConfig;
...
use ezer_workspace::file_system::CodebaseIndexManager;
use ezer_workspace::permission::{
    AccessKind, ClientType, Decision, HookAsk, PermissionEvent, PermissionHandle, PermissionRequest,
};
use ezer_workspace::session::file_state::{FileStateHandle, FileStateTracker};
...
pub(crate) struct InputItem {
    ...
    pub(crate) tool_overrides_update: Option<ezer_sampling_types::ToolOverridesUpdate>,
...
impl ezer_tools::types::resources::ManagedGatewayToolCaller for ShellManagedGatewayToolClient {
    async fn call_tool(
        &self,
        call_id: &str,
        ...
        ezer_tools::types::resources::ManagedGatewayToolCallResponse,
...
pub(crate) struct ModelAuthMemo {
    ...
    pub(crate) provider: Option<ezer_login::AuthProviderRef>,
}
...
pub(crate) struct StreamApplySpan {
    ...
    region: ezer_telemetry::region::Region,
    chunk_count: i64,
    bytes: i64,
}
...
pub(crate) struct SessionActor {
    ...
    /// The event fires at each of the six `OaiCompatClient` 401 arms in `ezer-sampler`.
    ...
    pub(crate) attribution_callback: Option<ezer_sampler::SharedAttributionCallback>,
    ...
    pub(crate) tool_overrides: std::cell::RefCell<Option<ezer_sampling_types::ToolOverrides>>,
    pub(crate) resolved_tool_overrides:
        std::sync::Arc<arc_swap::ArcSwapOption<ezer_sampling_types::ToolOverrides>>,
    pub(crate) compactions_remaining:
        std::cell::Cell<Option<ezer_sampling_types::CompactionsRemaining>>,
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<ezer_sampling_types::CompactionAtTokens>>,
    ...
    pub(crate) doom_loop_recovery: Option<ezer_sampling_types::DoomLoopRecoveryPolicy>,
    ...
    pub(crate) hook_registry:
        std::cell::RefCell<Option<Arc<ezer_hooks::discovery::HookRegistry>>>,
    ...
    pub(crate) hook_disabled: std::cell::RefCell<Arc<ezer_hooks::trust::DisabledHooks>>,
    ...
    pub(crate) vcs_kind: ezer_workspace::session::git::VcsKind,
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<ezer_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<ezer_agent::plugins::SharedPluginRegistryHandle>,
    ...
    pub(crate) turn_phases: std::sync::Arc<ezer_telemetry::turn_phases::TurnPhaseProfile>,
    ...
    pub(crate) turn_stream_drained:
        parking_lot::Mutex<std::collections::HashMap<ezer_sampler::RequestId, StreamOwnership>>,
    ...
    pub(crate) pending_image_strip: parking_lot::Mutex<
        std::collections::HashMap<ezer_sampler::RequestId, PendingImageStrip>,
    >,
    ...
    /// Handle to the per-session `ezer-sampler` actor.
    ...
    pub(crate) sampler_handle: ezer_sampler::SamplerHandle,
    ...
    /// Cached recipe for constructing this session's [`ezer_agent::Agent`].
    /// Reused by `handle_rebuild_agent_for_definition` to build a fresh `Agent`.
    /// A fresh `Agent` covers the system prompt, the [`ezer_tools::bridge::ToolBridge`], and the tool registry.
    ...
    pub(crate) workspace_ops: ezer_workspace::WorkspaceOps,
...
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<ezer_tools::bridge::ToolBridge> {
...
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &ezer_agent::PromptContext) {
...
) -> Option<ezer_agent::PromptContext> {
    let dir = crate::session::persistence::session_dir(session_info);
...
mod managed_gateway_descriptor_tests {
    use super::*;
    use ezer_tools::types::output::{MCPOutput, ToolOutput};
    use ezer_tools::types::tool::{ToolKind, ToolNamespace};
    #[derive(Debug, Default)]
    struct FixtureMcpTool;
    impl ezer_tools::types::tool_metadata::ToolMetadata for FixtureMcpTool {
...
mod tool_meta_stamp_tests {
    ...
    use ezer_tools::tool_taxonomy::TOOL_META_KEY;
    use ezer_workspace::permission::PermissionCommand;
                        ...
                        } = cmd
                        {
                            *captured_in_task.lock().await = Some(tool_call_update);
                            let _ = respond_to.send(
                                ezer_workspace::permission::PermissionResolution {
...
mod managed_gateway_tool_tests {
    ...
    use ezer_tools::types::output::{MCPOutput, ToolOutput};
    use ezer_tools::types::tool::{ToolKind, ToolNamespace};
    use ezer_tools::types::tool_metadata::ToolMetadata;
    #[derive(Debug)]
    struct FixtureMcpTool;
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        .await;
        let catalog = bridge
            .read_resource::<ezer_tools::types::resources::ManagedGatewayToolCatalog>()
            .await
