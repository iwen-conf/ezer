...
fn inline_definition(name: &str) -> ezer_agent::AgentDefinition {
    ezer_agent::AgentDefinition::from_json(&serde_json::json!({
        "name": name,
...
fn persisted_agent_from_definition_classifies_inline_vs_named() {
    ...
    let named = PersistedAgent::from(&ezer_agent::AgentDefinition::grok_build_plan());
    assert!(matches!(named, PersistedAgent::Named(_)));
}

#[test]
fn inline_profile_survives_json_round_trip() {
    let def = ezer_agent::AgentDefinition::from_json(&serde_json::json!({
        "name": "custom-profile",
        "description": "A custom profile",
...
fn named_agent_round_trips() {
    for name in [
        ...
        let mut summary = new_summary();
        summary.set_agent(PersistedAgent::Named(name.to_string()));
