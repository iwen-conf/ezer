//! `AgentRebuildSpec` is the canonical recipe for constructing an [`ezer_agent::Agent`] for a given session.
//!
//! INVARIANT: This is the **only** place in the shell crate that calls [`ezer_agent::AgentBuilder::new`].
...
//! ## Why this exists
//!
//! [`ezer_agent::Agent`] owns an [`ezer_tools::bridge::ToolBridge`] that carries session-scoped channels.
...
//!
//! ## WHEN ADDING A NEW [`ezer_agent::AgentBuilder`]`::with_*` KNOB
//!
...
use ezer_agent::config::AgentDefinition;
use ezer_agent::error::AgentBuildError;
use ezer_agent::prompt::context::PromptAudience;
...
use ezer_tools::types::SharedApiKeyProvider;
use ezer_tools::types::compat::CompatConfig;
use ezer_tools::types::memory_backend::MemoryBackend;
...
pub(crate) struct MemoryV2AccessSlot(
    parking_lot::Mutex<Option<ezer_tools::types::memory_v2::MemoryV2AccessResource>>,
);
impl MemoryV2AccessSlot {
    ...
    pub(crate) fn get(&self) -> Option<ezer_tools::types::memory_v2::MemoryV2AccessResource> {
        self.0.lock().clone()
    }
    pub(crate) fn set(
        &self,
...
pub(crate) struct AgentRebuildSpec {
    ...
    pub web_search_domains: Option<ezer_sampling_types::WebSearchOptions>,
    ...
    pub paths_config: ezer_agent::prompt::paths::PathsConfig,
    ...
    pub attribution_callback: Option<ezer_tools::SharedAttributionCallback>,
    ...
    pub managed_gateway_tool_client:
        Option<ezer_tools::types::resources::ManagedGatewayToolClient>,
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        ezer_telemetry::session_ctx::log_event(presentation_applied_event(
            &presentation,
            task_model_policy,
            *prompt_audience,
        ));
        ...
        agent
            ...
            .update_resources_with(|resources| {
                ...
                resources
                    .insert(
                        ezer_tools::types::resources::RespectGitignore(
                            *respect_gitignore,
                        ),
                    );
                resources
                    .insert(
                        ezer_tools::types::resources::PathNotFoundHints(
                            *path_not_found_hints,
                        ),
                    );
...
pub(crate) fn test_rebuild_spec_default() -> Arc<AgentRebuildSpec> {
    ...
    Arc::new(AgentRebuildSpec {
        ...
        terminal_backend: Arc::new(
            ezer_tools::computer::local::LocalTerminalBackend::new_local(
                ezer_tools::computer::local::SearchShadowConfig::default(),
            ),
        ),
...
mod legacy_tests {
    ...
    fn task_description(agent: &Agent) -> String {
        ...
        let task_name = toolset
            .tool_name_for_kind(ezer_tools::types::tool::ToolKind::Task)
    ...
    async fn config_web_search_domains_beat_agent_frontmatter() {
        use ezer_sampling_types::{HostedTool, ToolOverrides, WebSearchOptions};
        ...
        tokio::task::LocalSet::new()
            .run_until(async {
                let agent = spec_with(Some(configured.clone()))
                    .build_agent(definition(), ezer_agent::DEFAULT_SYSTEM_PROMPT_LABEL)
                    .await
