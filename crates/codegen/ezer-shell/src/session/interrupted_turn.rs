...
pub const INTERRUPTED_MESSAGE: &str = "ezer stopped before this turn finished (the agent process exited or was restarted). Committed tool results were kept.";
...
impl InterruptedTurn {
    ...
    pub(crate) fn close_events_turn(&self, session_dir: &Path) {
        ezer_session_events::EventWriter::open(session_dir).emit(
            ezer_session_events::Event::TurnEnded {
                outcome: ezer_session_events::TurnOutcomeLabel::Interrupted,
