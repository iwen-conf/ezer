...
async fn update_config_does_not_leak_requirements_into_user_config() {
    ...
    let effective = ezer_shell::config::load_effective_config().unwrap();
    let effective_cfg = ezer_shell::util::config::load_config_from_toml(&effective);
    assert_eq!(
    ...
    let user_cfg = ezer_shell::util::config::load_config_from_toml(&user_toml);
...
async fn update_config_refuses_unreadable_config_toml() {
    ...
    let err = ezer_shell::util::config::update_config(|cfg| {
        cfg.ui.simple_mode = Some(true);
    })
    .await
