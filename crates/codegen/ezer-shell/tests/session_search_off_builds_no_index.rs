...
async fn saved_session_is_neither_indexed_nor_found_with_search_off() {
    ...
    let root = dir.path();
    let _home = EnvGuard::set("EZER_HOME", root);
    let _off = EnvGuard::set("EZER_SESSION_SEARCH", "0");

    let config = ezer_shell::config::load_agent_config_disk_only().expect("config loads");
    let search = ezer_shell::session::storage::search::start_if_enabled(&config);
    assert!(
