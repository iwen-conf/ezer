use ezer_shell::sampling::{ApiBackend, Client, SamplerConfig};
...
pub mod leader {
    ...
    use ezer_test_support::leader::{LeaderFixture, LeaderStdioClient};
...
#[allow(dead_code)]
pub async fn run_bootstrap() -> Result<ezer_shell::agent::config::Config, String> {
    tokio::task::spawn_blocking(|| {
        let cfg = ezer_shell::agent::config::Config::default();
        let auth_manager = std::sync::Arc::new(cfg.create_auth_manager());
        ezer_shell::agent::init::bootstrap(&cfg, &auth_manager, None)
...
pub fn test_sampler_config(
    ...
    // Shell `Client` is `ezer_sampler::SamplingClient`, which takes a `SamplerConfig` directly
    SamplerConfig {
