...
use ezer_shell::agent::mvp_agent::MvpAgent;
...
fn spawn_agent_local(remote: Option<ezer_shell::util::config::RemoteSettings>) -> AgentPipes {
...
where
    F: FnOnce(std::path::PathBuf, std::rc::Rc<ezer_test_support::MockInferenceServer>) -> Fut,
    Fut: std::future::Future<Output = ()>,
{
    run_agent_test_with_models(
        vec![ezer_test_support::MockModelEntry::new("test-model")],
        body,
    )
}
...
pub fn run_agent_test_with_models<F, Fut>(
    models: Vec<ezer_test_support::MockModelEntry>,
    body: F,
) where
    F: FnOnce(std::path::PathBuf, std::rc::Rc<ezer_test_support::MockInferenceServer>) -> Fut,
    Fut: std::future::Future<Output = ()>,
{
    ...
    ezer_shell::agent::remote_config::settings_get::reset_startup_settings_for_tests();
...
pub fn assert_probes_emitted(name: &str, probes: &[&str]) {
    ...
    let _ = tracing_subscriber::registry()
        .with(ezer_shell::instrumentation::layer::<
    ...
    let _ = ezer_shell::instrumentation::finalize();
