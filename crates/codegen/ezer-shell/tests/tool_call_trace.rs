...
use ezer_telemetry::config::{TelemetryConfig, TelemetryMode};
use ezer_test_support::{MockInferenceServer, MockOtelServer, OtelSpan};
...
fn install_tracing(traces: &str) {
    set_env("EZER_INTERNAL_OTLP_TRACES_ENDPOINT", traces);
    set_env("EZER_INSTRUMENTATION", "server");
    set_env("EZER_OTEL_FILTER", "info");
    set_env("OTEL_BSP_SCHEDULE_DELAY", "50");
    set_env("OTEL_EXPORTER_OTLP_TIMEOUT", "2000");
    set_env("OTEL_TRACES_EXPORTER", "otlp");
    set_env("EZER_TELEMETRY_ENABLED", "true");
    unset_env("DISABLE_TELEMETRY");
    unset_env("EZER_EXTERNAL_OTEL");
    let config = ezer_shell::agent::init::build_default_otel_layer_config();
    ezer_shell::auth::credential_provider::wire_otel_deployment_key("test-key".into());
    let layer = ezer_telemetry::otel_layer::build_otel_layer(
...
fn init_product(server: &MockInferenceServer, mode: TelemetryMode) {
    ...
    ezer_telemetry::init(
        ...
        "test".into(),
        None,
        ezer_shell::http::shared_client(),
    );
}

fn projected(invocation: &str, exit_code: i32) -> ezer_telemetry::events::ToolCallCompleted {
    let parent = tracing::info_span!("tools.execute");
    let span = ezer_shell::session::tool_execution_span(
        &parent,
        "session",
        "grep",
        "grep",
    ...
    let output = ezer_shell::session::grep_output(exit_code);
    ezer_shell::session::complete_projected_call(span, invocation, &output, "success")
}
...
async fn span_and_product_row_agree_and_product_gate_is_independent() {
    ...
    set_env("EZER_HOME", home.to_str().unwrap());
    ...
    ezer_telemetry::session_ctx::log_event_now(silent).await;
    traces
        .recorder()
    ...
    ezer_telemetry::session_ctx::log_event_now(event).await;
