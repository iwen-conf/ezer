...
use ezer_shell::cpu_profile::ControlErrorCode;
...
async fn setup_control_test_server(
    temp: &TempDir,
) -> (std::path::PathBuf, ezer_shell::leader::ServerHandle) {
...
async fn test_session_ownership_cleanup_on_disconnect() {
    use ezer_shell::leader::run_leader_server;
    ...
    tokio::spawn(async move {
        ...
        let (shutdown_tx, _shutdown_rx) =
            tokio::sync::watch::channel(ezer_shell::leader::ShutdownReason::Manual);
        ...
        let _ = run_leader_server(
            ...
            client_count,
            agent_busy,
            ezer_shell::agent::activity::AgentActivity::default(),
            ready_rx,
...
async fn test_raw_registration_handshake_not_ready_then_ready() {
    ...
    tokio::spawn(async move {
        ...
        let _ = run_leader_server(
            ...
            watch::channel(ezer_shell::leader::ShutdownReason::Manual).0,
            None,
            control_state,
        )
        .await;
...
async fn test_version_mismatch_notification_sent_to_client() {
    ...
    use tokio::sync::{mpsc, watch};
    use tokio_util::sync::CancellationToken;
    use ezer_shell::leader::{ClientCapabilities, ClientMode, LeaderClient, run_leader_server};
...
async fn test_auto_update_shutdown_reason_reaches_client() {
    use ezer_shell::leader::{ClientCapabilities, ClientMode, LeaderClient, ShutdownReason};
    ...
    tokio::time::timeout(
        ...
        disconnect_rx.wait_for(|r| *r != ezer_shell::leader::DisconnectReason::Connected),
    )
    .await
    ...
    assert_eq!(
        *disconnect_rx.borrow(),
        ezer_shell::leader::DisconnectReason::LeaderShutdown
    );
}
...
async fn setup_persistent_test_server(
    ...
    tokio::spawn(async move {
        ...
        let _ = run_leader_server(
            ...
            tokio::sync::watch::channel(false).0,
            tokio::sync::watch::channel(ezer_shell::leader::ShutdownReason::Manual).0,
            None,
            control_state,
        )
        .await;
...
async fn wait_for_client_disconnected_log(client_id: u64) -> bool {
    let deadline = tokio::time::Instant::now() + Duration::from_secs(5);
    loop {
        let seen = ezer_telemetry::unified_log::snapshot_log().is_some_and(|bytes| {
...
async fn test_hung_agent_leaves_transport_healthy_and_forwards_cancel() {
    ...
    assert_eq!(
        *client.disconnect_reason().borrow(),
        ezer_shell::leader::DisconnectReason::Connected,
