...
fn start_index() -> SearchIndexManager {
    ...
    match start_if_enabled(&ezer_shell::agent::config::Config::default()) {
...
fn loading_config_applies_requirement_pins() {
    ...
    let loaded = ezer_shell::config::load_agent_config_disk_only();
    std::fs::remove_file(&pin).unwrap();
    let config = loaded.expect("config loads");

    let resolved = config.feature(ezer_shell::agent::config::Feature::SessionSearch);
    assert!(
        !resolved.value,
    ...
    assert_eq!(
        resolved.source,
        ezer_shell::agent::config::ConfigSource::Requirement
    );
}
