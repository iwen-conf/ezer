...
//!
//! ```bash
//! cargo test -p ezer-shell --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p ezer-shell --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```
...
use ezer_agent::plugins::SharedPluginRegistryHandle;
use ezer_agent::plugins::discovery::DiscoveryConfig;
use ezer_agent::plugins::git_install::{InstallSource, install_from_source};
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    assert_eq!(
        plugin.scope,
        ezer_agent::plugins::PluginScope::CliOverride
    );
