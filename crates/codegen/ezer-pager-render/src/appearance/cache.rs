...
//!
//! Disk writes live in `ezer_shell::util::config::set_<field>()`; this module only caches in memory.
//!
...
use ezer_shared::ui_config::UiConfig;
...
fn load_keep_text_selection_from_effective_config() -> TextSelection {
    let root = match ezer_config::load_effective_config_disk_only() {
        Ok(r) => r,
