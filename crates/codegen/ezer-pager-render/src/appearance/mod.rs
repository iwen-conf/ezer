...
//!
//! - **`config` + `watcher`**: dev-only `~/.ezer/pager.toml` RenderConfig
...
//!   Disk writes happen in `ezer_shell::util::config::set_<field>()` via `Effect::PersistSetting`, NOT here; this is a read-cache only.
