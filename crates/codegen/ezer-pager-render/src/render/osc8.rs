...
fn relative_link_target(
    ...
    let cwd = cwd?;
    let joined = ezer_paths::normalize_lexically(&cwd.join(path));
    joined
        .starts_with(ezer_paths::normalize_lexically(cwd))
        .then_some(joined)
}
...
mod tests {
    ...
    fn scan_relative_path_not_partially_linkified() {
        // A relative path like `crates/codegen/ezer-pager/src/render` should NOT produce a link for the `/ezer-pager/src/render` substring
        let line = make_line("find crates/codegen/ezer-pager/src/render -name '*.rs'");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
