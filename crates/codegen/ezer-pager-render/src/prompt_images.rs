...
pub fn decode_image_dimensions(bytes: &[u8]) -> Option<(u32, u32)> {
    ...
    ezer_tools::util::image_validate::validate_image_bytes_unrestricted(bytes, false)
        .ok()
        .map(|(w, h, _)| (w, h))
}
...
impl ImageViewerState {
    ...
    pub fn open_from_path(path: &std::path::Path) -> Option<Self> {
        ...
        let mime_type = ezer_shared::clipboard::mime_from_bytes(&bytes).to_owned();
...
use ezer_shared::clipboard::mime_to_extension;
...
fn read_image_at_path(path: &std::path::Path) -> Option<PastedImage> {
    ...
    let mime_type = ezer_shared::clipboard::mime_from_bytes(&data);
...
pub fn session_images_dir(
    ...
    let sid = session_id?;
    let info = ezer_shared::session::info::Info {
        id: sid.clone(),
        cwd: cwd.to_string_lossy().into_owned(),
    };
    Some(ezer_shared::session::session_dir(&info).join("images"))
}
...
pub fn session_mermaid_dir(
    ...
    Some(ezer_shared::session::session_dir(&info).join("mermaid"))
}
...
pub fn build_content_blocks_with_workspace_ref(
    ...
    let allowed: Option<Vec<std::path::PathBuf>> =
        workspace_cwd.map(ezer_shared::placeholder_images::default_allowed_prefixes);
    build_content_blocks_with_prefixes_ref(
        text,
        images,
        allowed.as_deref(),
        ezer_shared::placeholder_images::MAX_PLACEHOLDER_AGGREGATE_BYTES,
    )
}
...
fn build_content_blocks_with_prefixes_and_caps_ref(
    ...
    let rewritten_text =
        ezer_shared::placeholder_images::strip_paths_from_image_placeholders(rewritten_text);
    ...
    for img in images {
        ...
        blocks.push(ContentBlock::Image(
            ImageContent::new(data, mime_type)
                ...
                .meta(Some(
                    ezer_shared::placeholder_images::display_number_meta(img.display_number),
                )),
        ));
    }
...
fn resolve_orphan_placeholders(
    ...
    let placeholders = ezer_shared::placeholder_images::extract_placeholders(&text);
    ...
    for ph in &placeholders {
        ...
        match ezer_shared::placeholder_images::load_placeholder_image(&ph.path, allowed) {
            Ok(loaded) => {
                ...
                recovered.push(
                    ImageContent::new(data, loaded.mime_type)
                        ...
                        .meta(Some(
                            ezer_shared::placeholder_images::display_number_meta(
                                ph.display_number,
                            ),
                        )),
                );
...
mod tests {
    ...
    fn build_blocks_sets_display_number_meta() {
        ...
        assert_eq!(
            ezer_shared::placeholder_images::display_number_from_meta(ic.meta.as_ref()),
            Some(3),
    ...
    fn build_blocks_uri_prefers_durable_session_path() {
        ...
        img.session_image_path = Some(PathBuf::from("/Users/test/.ezer/session/image.png"));
        let blocks = build_blocks_no_workspace("text".into(), vec![img]);
        assert_eq!(blocks.len(), 2);
        if let agent_client_protocol::ContentBlock::Image(ic) = &nth(&blocks, 1) {
            assert_eq!(
                ic.uri.as_deref(),
                Some("file:///Users/test/.ezer/session/image.png"),
