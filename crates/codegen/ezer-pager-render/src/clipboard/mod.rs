...
/// Documented in `ezer-pager/docs/internal/22-environment-variables.md`.
...
fn is_remote() -> bool {
    ...
    *REMOTE.get_or_init(ezer_shared::clipboard::is_remote_session)
}
...
fn is_container_no_display() -> bool {
    ...
    *CONTAINER.get_or_init(ezer_shared::clipboard::is_containerized_without_display)
}
...
pub fn wayland_data_control_label() -> &'static str {
    match crate::host::DisplayServer::current() {
        crate::host::DisplayServer::Wayland => {
            if ezer_shared::clipboard::wayland_data_control_supported() {
                "yes"
            } else {
                "no"
            }
...
fn write_tmux_buffer(text: &str) -> bool {
    ...
    let result = (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        let stdin = ezer_shared::clipboard::spool_for_stdin(text.as_bytes())?;
        ...
        let status = ezer_shared::clipboard::wait_with_deadline(
            &mut child,
...
/// Delegates to [`ezer_shared::clipboard`], which uses `pbcopy`/`pbpaste` on macOS (avoiding AppKit GPU overhead) and `arboard` elsewhere.
///
...
impl ClipboardProvider for SystemClipboard {
    fn get(&mut self) -> Option<String> {
        ezer_shared::clipboard::get_text().ok().flatten()
    }
...
fn clipboard_write_with_route(text: &str, route: &ClipboardRoute) -> ClipboardWriteLegs {
    ...
    if route.native {
        let outcome = ezer_shared::clipboard::set_text_with_outcome(text);
    ...
    if route.osc52 {
        match ezer_shared::clipboard::set_text_osc52(text, route.osc52_tmux_passthrough) {
...
fn log_clipboard_copy_event(
    ...
    if !ezer_telemetry::client::is_enabled() {
        return;
    }
    ezer_telemetry::session_ctx::log_event(ezer_telemetry::events::ClipboardCopy {
...
pub fn system_clipboard_read_text() -> Result<Option<String>, ClipboardTextReadError> {
    ...
    ezer_shared::clipboard::get_text().map_err(|error| {
...
pub fn system_primary_selection_get() -> Option<String> {
    ...
    if !ezer_shared::clipboard::x11_display_env_present() {
        return None;
    }
    ezer_shared::clipboard::get_primary_text()
        .ok()
        .flatten()
        .filter(|text| !text.is_empty())
}
...
pub fn x11_primary_guidance_available() -> bool {
    #[cfg(target_os = "linux")]
    {
        is_native_x11(crate::host::DisplayServer::current())
            && ezer_shared::clipboard::x11_display_env_present()
    }
...
pub fn log_paste_key_empty_host_clipboard(surface: &str) {
    ...
    ezer_telemetry::session_ctx::log_event(
        ezer_telemetry::events::PasteKeyEmptyHostClipboard {
            terminal,
...
fn read_drop_reason(error: &anyhow::Error) -> ClipboardProbeDropReason {
    if error
        .downcast_ref::<ezer_shared::clipboard::WaitTimeout>()
        .is_some()
    {
...
fn read_path_telemetry(
    path: ezer_shared::clipboard::ClipboardReadPath,
) -> ezer_telemetry::events::ClipboardReadPath {
    use ezer_shared::clipboard::ClipboardReadPath as Shared;
    use ezer_telemetry::events::ClipboardReadPath as Path;
    match path {
...
fn log_clipboard_paste_event(
    ...
    read_path: Option<ezer_shared::clipboard::ClipboardReadPath>,
    started: std::time::Instant,
) {
    if !ezer_telemetry::client::is_enabled() {
        return;
    }
    ezer_telemetry::session_ctx::log_event(ezer_telemetry::events::ClipboardImagePaste {
        terminal: crate::terminal::terminal_context().telemetry_snapshot(),
...
pub fn log_clipboard_probe_dropped(
    ...
    ezer_telemetry::session_ctx::log_event(
        ezer_telemetry::events::ClipboardPasteProbeDropped {
            terminal: crate::terminal::terminal_context().telemetry_snapshot(),
            reason,
            image_hash: image.map(image_fingerprint).unwrap_or_default(),
            duration_ms: started.elapsed().as_millis() as u64,
...
fn system_clipboard_get_attachments() -> Result<AttachmentsProbeResult, ClipboardProbeDropReason> {
    let started = std::time::Instant::now();
    match ezer_shared::clipboard::get_attachments() {
        Ok(att) => {
...
pub use ezer_shared::clipboard::{ImageData, OSASCRIPT_WAIT};
...
pub fn clipboard_image_snapshot() -> (Option<u64>, bool) {
    ...
    ezer_shared::clipboard::clipboard_image_snapshot()
}
...
pub fn clipboard_change_count() -> Option<u64> {
    ...
    ezer_shared::clipboard::clipboard_change_count()
}
...
pub fn clipboard_image_probe_supported() -> bool {
    ...
    ezer_shared::clipboard::clipboard_image_probe_supported()
}
...
pub fn prewarm_image_probe() {
    ...
    WARMED.call_once(|| {
        std::thread::spawn(ezer_shared::clipboard::clipboard_prewarm);
    });
}
...
fn system_clipboard_get_image_result() -> Result<Option<ImageData>, ClipboardProbeDropReason> {
    let started = std::time::Instant::now();
    match ezer_shared::clipboard::get_image() {
        Ok(read) => {
...
mod tests {
    ...
    fn read_drop_reason_keeps_the_deadline_kill_apart() {
        use ezer_telemetry::events::ClipboardProbeDropReason as Reason;
        let timeout = anyhow::Error::from(ezer_shared::clipboard::WaitTimeout(OSASCRIPT_WAIT))
