...
/// Canonical definition lives in `ezer_workspace_types`; re-exported here for callers that historically imported it from this module.
pub use ezer_workspace_types::MCP_TOOL_NAME_DELIMITER;
...
/// Reqwest 0.13 twin of the 0.12 adapters in `ezer_extra_ca`.
fn with_extra_root_certificates(mut builder: reqwest::ClientBuilder) -> reqwest::ClientBuilder {
    ...
    for der in ezer_extra_ca::extra_root_ders() {
...
pub struct McpState {
    ...
    event_writer: ezer_session_events::EventWriter,
...
impl McpState {
    ...
    pub fn new_with_meta(configs: Vec<acp::McpServer>, meta_config_map: McpMetaConfigMap) -> Self {
        ...
        Self {
            ...
            event_writer: ezer_session_events::EventWriter::noop(),
    ...
    pub fn set_event_writer(&mut self, writer: ezer_session_events::EventWriter) {
        self.event_writer = writer;
    }

    pub fn event_writer(&self) -> &ezer_session_events::EventWriter {
        &self.event_writer
    }
...
/// MCP initialization strategy. Defined in `ezer-telemetry`; re-exported here so existing call sites continue to work.
pub use ezer_telemetry::enums::McpInitStrategy;
...
impl McpError {
    ...
    pub fn error_category(&self) -> ezer_session_events::McpErrorCategory {
        use ezer_session_events::McpErrorCategory;
        match self {
...
impl xai_tool_runtime::Tool for McpErasedTool {
    ...
    async fn run(
        ...
        let call_span = ezer_telemetry::region::Region::from_span(tracing::info_span!(
            "mcp.tool_call",
        ...
        event_writer.emit(ezer_session_events::Event::McpToolCallStarted {
            ...
            .await
        {
            ...
            Err(first_err) if client.has_auth() => {
                ...
                ew.emit(ezer_session_events::Event::McpAuthRetry {
                    server_name: server.clone(),
        ...
        let failure = match &dispatch_result {
            Ok(_) => None,
            Err(_) if is_timeout => Some(ezer_telemetry::events::McpCallFailure::Timeout),
            Err(e) if e.kind == xai_tool_runtime::ToolErrorKind::NetworkError => {
                Some(ezer_telemetry::events::McpCallFailure::Transport)
            }
            Err(_) => None,
        };
        ...
        event_writer.emit(ezer_session_events::Event::McpToolCallCompleted {
            server_name: server.clone(),
            tool_name: tool.clone(),
            call_id: qualified_name.clone(),
            duration_ms,
        ...
        ezer_telemetry::session_ctx::log_event(ezer_telemetry::events::McpToolCalled {
            server_name: server.clone(),
            tool_name: tool.clone(),
            qualified_name,
            success,
...
impl McpErasedTool {
    ...
    async fn try_call_tool(
        ...
        is_timeout: &mut bool,
        ew: &ezer_session_events::EventWriter,
        ...
        loop {
            attempt += 1;
            let attempt_span = ezer_telemetry::region::Region::from_span(
    ...
    async fn recover_and_retry(
        ...
        *reconnect_attempted = true;
        ...
        ew.emit(ezer_session_events::Event::McpTransportError {
        ...
        let mcp_service = match client.recover().await {
            Ok(service) => {
                ew.emit(ezer_session_events::Event::McpTransportReconnect {
                    server_name: self.tool.server_name.clone(),
                    success: true,
                    error: None,
                });
...
async fn decide_http_auth_over_network(
    ...
    let outcome = {
        ...
        .await
        {
            ...
            Err(_) => {
                ...
                ctx.event_writer
                    .emit(ezer_session_events::Event::McpOAuthDiscoveryTimeout {
    ...
    match outcome {
        ...
        OauthProbeOutcome::Inconclusive => {
            ...
            ctx.event_writer
                .emit(ezer_session_events::Event::McpOAuthProbeResolved {
                    server_name: server_name.to_string(),
    ...
    W: AsyncWrite + Send + Unpin + 'static,
{
    ...
    fn record_decode_error(&self, line: &[u8], err: &serde_json::Error) {
        ...
        self.event_writer
            .emit(ezer_session_events::Event::McpTransportDecodeError {
...
pub(crate) static MCP_SERVERS_CONNECTED: ezer_telemetry::activity::ActivityGauge =
    ezer_telemetry::activity::ActivityGauge::residency(
        ezer_telemetry::activity::MCP_SERVERS_CONNECTED_KEY,
    );
...
enum ClientState {
    ...
    Ready {
        service: McpService,
        _connected: ezer_telemetry::activity::ActivityGaugeGuard,
    },
}
...
impl McpClient {
    ...
    pub async fn ensure_initialized(&self) -> Result<McpService, McpError> {
        ...
        let handshake_span = ezer_telemetry::region::Region::from_span(tracing::info_span!(
            "mcp.handshake",
        ...
        tracing::info!(target: ezer_telemetry::instrumentation::TARGET, event = "timing", name = "mcp_try_handshake", server_name = %self.server_name, elapsed_us = handshake_elapsed);
    ...
    fn make_client_info(server_name: &str, advertise_elicitation: bool) -> ClientConfig {
        ...
        ClientConfig::new(
            ...
            Implementation::new(
                ...
                ezer_version::VERSION.to_string(),
            ),
        )
    ...
    async fn list_tool_registrations(
        ...
        let list_tools_span = ezer_telemetry::region::Region::from_span(tracing::info_span!(
            "mcp.list_tools",
        ...
        let registrations: Vec<_> = all_tools
            ...
            .filter_map(|tool| {
                ...
                match mcp_tool.into_registration() {
                    ...
                    Err(reason) => {
                        event_writer.emit(
                            ezer_session_events::Event::McpToolRegistrationFailed {
                                server_name: self.server_name.clone(),
                                tool_name: name,
...
static DEFAULT_USER_AGENT: LazyLock<reqwest::header::HeaderValue> = LazyLock::new(|| {
    format!("ezer-cli/{}", ezer_version::VERSION)
        .parse()
...
pub struct McpSpawnCtx<'a> {
    ...
    pub(crate) event_writer: &'a ezer_session_events::EventWriter,
...
impl<'a> McpSpawnCtx<'a> {
    ...
    pub fn standalone(event_writer: &'a ezer_session_events::EventWriter) -> Self {
        Self {
            session_id: None,
            event_writer,
        ...
        acp::McpServer::Stdio(acp::McpServerStdio {
            ...
            let cmd = {
                let mut stdio_spawn_timer =
                    ezer_telemetry::instrumentation::timer("mcp_stdio_spawn");
                ...
                ezer_sandbox::child_net::restrict_child_network(&mut cmd);
                cmd
            };
            ...
            let (transport, stderr_handle) = spawn_result.map_err(|e| {
                ...
                ezer_telemetry::session_ctx::log_event(
                    ezer_telemetry::events::McpServerFailed {
                        server_name: name.clone(),
                        error_type: ezer_telemetry::events::McpErrorType::SpawnFailed,
...
impl ClientHandler for EzerClientHandler {
    ...
    async fn on_custom_notification(
        ...
        if !ezer_tools::mcp_elicitation::chars_within(
            elicitation_id,
            ezer_tools::mcp_elicitation::MAX_ELICIT_ID_CHARS,
        ) {
            tracing::warn!(
                server = %self.server_name,
