...
pub fn elicit_result_from_wire(
    response: &ezer_tools::mcp_elicitation::McpElicitExtResponse,
) -> ElicitResult {
...
/// [`McpElicitExtRequest`]: ezer_tools::mcp_elicitation::McpElicitExtRequest
#[derive(Debug, Clone)]
pub struct WireElicitFields {
    pub message: String,
    pub mode: ezer_tools::mcp_elicitation::McpElicitModeFields,
}

pub fn wire_mode_and_fields(params: &ElicitRequestParams) -> Option<WireElicitFields> {
    use ezer_tools::mcp_elicitation::{
...
mod tests {
    ...
    fn wire_mapping_form_and_url() {
        use ezer_tools::mcp_elicitation::McpElicitModeFields;
        let schema = ElicitationSchema::builder()
    ...
    fn oversized_message_is_declined() {
        use ezer_tools::mcp_elicitation::MAX_ELICIT_MESSAGE_CHARS;
        let schema = ElicitationSchema::builder()
            .required_property("x", PrimitiveSchemaDefinition::String(StringSchema::new()))
            .build()
            .unwrap();
