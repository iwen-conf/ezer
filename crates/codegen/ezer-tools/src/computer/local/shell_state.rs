...
pub fn shell_env_overrides() -> HashMap<String, String> {
    HashMap::from([
        ...
        (
            ...
            crate::util::EZER_AGENT_ENV_VALUE.to_string(),
        ),
    ])
}
...
dump_bash_state() {
  ...
  local env_vars
  env_vars=$(builtin export -p 2>/dev/null | command grep -viE '_proxy=|EZER_SANDBOX|EZER_AGENT=|SUDO_ASKPASS|EZER_ASKPASS|ELECTRON_RUN_AS_NODE|SSH_AUTH_SOCK|DBUS_SESSION_BUS_ADDRESS|XDG_RUNTIME_DIR|WAYLAND_DISPLAY|GPG_TTY' || true)
...
impl ShellKind {
    ...
    pub fn detect() -> Self {
        match ezer_config::shell::detect_unix_shell_kind() {
            ezer_config::shell::UnixShellKind::Bash => Self::Bash,
            ezer_config::shell::UnixShellKind::Zsh => Self::Zsh,
        }
    }

    /// Resolved absolute path to the shell binary. Falls back from `$SHELL` → `which` → common dirs
    /// → `/bin/<name>`. Result is cached process-wide in `ezer_config::shell::unix_shell_path`.
    /// See that function for the full cascade. Returns `&'static str`.
    pub fn binary_path(&self) -> &'static str {
        let kind = match self {
            Self::Bash => ezer_config::shell::UnixShellKind::Bash,
            Self::Zsh => ezer_config::shell::UnixShellKind::Zsh,
        };
        ezer_config::shell::unix_shell_path(kind)
    }
    ...
    pub fn prepare_command(
        ...
        let wrapper = match self.shell {
            ShellKind::Bash => format!(
                ...
                // diagnostics (eval syntax errors, etc.) still land on the outer shell's stderr — those are unaffected. Re-export EZER_AGENT=1 after snapshot
...
mod tests {
    ...
    async fn test_prepare_command_and_roundtrip() {
        ...
        // Run "export EZER_TEST_VAR=hello" and capture the new state
        let prep = state
            .prepare_command(
                "export EZER_TEST_VAR=hello",
                None,
    ...
    async fn test_gpg_tty_excluded_from_snapshot_bash() {
        ...
        let (code, _) = run_command(&mut state, "export GPG_TTY=/ezer-sentinel-tty").await;
        assert_eq!(code, 0);

        let (code, stdout) = run_command(&mut state, "echo \"[$GPG_TTY]\"").await;
        assert_eq!(code, 0);
        assert!(
            !stdout.contains("/ezer-sentinel-tty"),
