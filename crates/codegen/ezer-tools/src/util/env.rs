...
//! heavyweight `ezer-tools` dependency. This module re-exports the public
...
/// human interactive shells. Note: the CLI also uses `EZER_AGENT` as an optional agent-definition selector for
/// launching `ezer` itself; child terminal processes only need the sentinel value `"1"`.
pub const EZER_AGENT_ENV: &str = "EZER_AGENT";

/// Sentinel value for [`EZER_AGENT_ENV`] on agent tool terminals.
pub const EZER_AGENT_ENV_VALUE: &str = "1";
...
/// Expand the four plugin-path tokens (`${CLAUDE_PLUGIN_ROOT}` / `${EZER_PLUGIN_ROOT}` and `${CLAUDE_PLUGIN_DATA}` /
/// `${EZER_PLUGIN_DATA}`) in `s`. Each pair is expanded only when its value is provided. Single source of truth for
...
#[cfg(test)]
mod tests {
    use super::{EZER_AGENT_ENV, EZER_AGENT_ENV_VALUE, substitute_plugin_tokens};

    const ALL_TOKENS: &str = "${CLAUDE_PLUGIN_ROOT}/a ${EZER_PLUGIN_ROOT}/b ${CLAUDE_PLUGIN_DATA}/c ${EZER_PLUGIN_DATA}/d";
    ...
    fn expands_only_root_when_data_none() {
        ...
        assert_eq!(
            out,
            "/root/a /root/b ${CLAUDE_PLUGIN_DATA}/c ${EZER_PLUGIN_DATA}/d"
        );
    }

    #[test]
    fn agent_marker_constants_match_cursor_parity() {
        assert_eq!(EZER_AGENT_ENV, "EZER_AGENT");
        assert_eq!(EZER_AGENT_ENV_VALUE, "1");
    }
}
