...
use regex::Regex;
use ezer_config::shell::AmpersandSemantics;
...
/// [`ezer_config::shell::ampersand_semantics`] reports POSIX `&` semantics (Unix + Git Bash).
...
// ───────────────────────────────────────────────────────────────────────────
// Bare `echo "<msg>"` detection (for statistics + hints in ezer_build bash)
// ───────────────────────────────────────────────────────────────────────────
...
${%- endif %}"#
    ...
    fn get_prefixed_command(cmd_prefix: &Option<String>, command: &str) -> String {
        match cmd_prefix {
            Some(prefix) => {
                let sep = ezer_config::shell::chain_separator();
...
impl xai_tool_runtime::Tool for BashTool {
    ...
    async fn run(
        ...
        let ampersand = ezer_config::shell::ampersand_semantics();
        ...
        if input.is_background {
            ...
            // We tag it for statistics (ezer_build backend) and can surface an educational
