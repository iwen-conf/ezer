...
impl SpawnRootSpan {
    pub fn new(span: tracing::Span) -> Self {
        let traceparent = ezer_otel::span_traceparent(&span);
        Self {
            span: Some(span),
            traceparent,
        }
...
/// Carries the current parent prompt/turn ID for TaskTool subagent scoping. Set by ezer-shell
...
/// True while a `/goal` loop is active. Set by ezer-shell at turn start. When true, `TaskCompletionReminder`
