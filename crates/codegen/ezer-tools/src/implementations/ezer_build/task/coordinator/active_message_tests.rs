...
use super::super::*;
use super::*;
use crate::implementations::ezer_build::task::active_message::SubagentActiveMessageRequest;
use crate::implementations::ezer_build::task::coordinator::graph::NestedSpawner;
use crate::implementations::ezer_build::task::coordinator_state::{
...
pub(in super::super) struct AdmissionCall {
    ...
    pub(in crate::implementations::ezer_build::task::coordinator) release:
        oneshot::Sender<ActiveMessageAdmission>,
}

pub(in crate::implementations::ezer_build::task::coordinator) struct TestControl {
...
pub(in crate::implementations::ezer_build::task::coordinator) struct TestRunner;
...
pub(in crate::implementations::ezer_build::task::coordinator) type TestCoordinator =
...
fn fixture_with_capacity(
    ...
    TestCoordinator,
    crate::implementations::ezer_build::task::backend::SubagentCoordinatorSender,
...
pub(in crate::implementations::ezer_build::task::coordinator) fn fixture() -> (
    TestCoordinator,
    crate::implementations::ezer_build::task::backend::SubagentCoordinatorSender,
    mpsc::UnboundedSender<AdmissionCall>,
    mpsc::UnboundedReceiver<AdmissionCall>,
...
pub(in crate::implementations::ezer_build::task::coordinator) fn insert_child(
...
pub(in crate::implementations::ezer_build::task::coordinator) fn insert_child_with(
    coordinator: &mut TestCoordinator,
    admissions: mpsc::UnboundedSender<AdmissionCall>,
    id: &str,
    parent: &str,
    ...
    let mut request =
        crate::implementations::ezer_build::task::coordinator::tests::request(id, true);
    request.parent_session_id = parent.to_owned();
    request.owner = owner;
    request.surface_completion = false;
    let address = crate::implementations::ezer_build::task::types::AgentAddress::mint(
...
pub(in crate::implementations::ezer_build::task::coordinator) fn begin_send(
    coordinator: &mut TestCoordinator,
    command_tx: &crate::implementations::ezer_build::task::backend::SubagentCoordinatorSender,
    id: &str,
    parent: &str,
...
pub(in crate::implementations::ezer_build::task::coordinator) async fn response_outcome(
...
pub(in crate::implementations::ezer_build::task::coordinator) fn finish_child(
    coordinator: &mut TestCoordinator,
    id: &str,
) {
...
async fn runner_panic_parks_until_uncertain_admission_terminalizes_failed() {
    ...
    let backend =
        crate::implementations::ezer_build::task::backend::ChannelBackend::for_coordinator_session(
            command_tx, "parent",
        );
    let mut spawn = tokio::spawn({
        let backend = backend.clone();
        async move {
            let mut request = crate::implementations::ezer_build::task::coordinator::tests::request(
                "panic-child",
                false,
            );
            request.parent_session_id = "parent".to_owned();
            crate::implementations::ezer_build::task::backend::SubagentBackend::spawn(
    ...
    let response = tokio::spawn({
        let backend = backend.clone();
        async move {
            crate::implementations::ezer_build::task::backend::SubagentBackend::send_active_message(
                &backend,
...
async fn parent_session_cancel_unblocks_parked_send() {
    ...
    assert!(matches!(
        ...
        crate::implementations::ezer_build::task::types::SubagentCancelOutcome::Cancelled
    ));
    assert_eq!(
        ActiveAgentMessageOutcome::NotActiveOrFinalizing,
        response_outcome(response).await
...
async fn send_to_cancelled_or_workflow_spawning_child_fails_fast() {
    ...
    coordinator
        ...
        .expect("pending")
        .request
        .owner = crate::implementations::ezer_build::task::types::SubagentOwner::workflow("run-1");
    assert_eq!(
        ActiveAgentMessageOutcome::NotActiveOrFinalizing,
...
pub(in crate::implementations::ezer_build::task::coordinator) fn begin_human_send(
    coordinator: &mut TestCoordinator,
    command_tx: &crate::implementations::ezer_build::task::backend::SubagentCoordinatorSender,
    address: &str,
    parent: &str,
...
async fn human_pending_address_is_not_active() {
    ...
    let address_text = address.to_string();
    let mut request =
        crate::implementations::ezer_build::task::coordinator::tests::request("child", true);
    request.parent_session_id = "parent".to_owned();
    coordinator.graph.insert_root_child("child", "parent");
    coordinator.pending.insert(
        "child".to_owned(),
        crate::implementations::ezer_build::task::coordinator_state::PendingChild {
            request,
            started_at: std::time::Instant::now(),
            cancellation: CancellationToken::new(),
            spawn_reply: None,
