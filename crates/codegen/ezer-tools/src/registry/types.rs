...
/// [`ToolRegistryBuilder::new`]. This inverts the dependency for harness code that must live outside this crate: instead of `ezer-tools`
...
impl ToolRegistryBuilder {
    ...
    pub fn new() -> Self {
        ...
        b.register_with_params::<
                ezer_build::GetTerminalCommandOutputTool,
                ezer_build::task_output::terminal_command::TerminalCommandOutputParams,
            >();
        b.register::<ezer_build::WaitTasksTool>();
        b.register_with_params::<ezer_build::TaskTool, ezer_build::task::TaskParams>();
    ...
    pub fn finalize_with_trunc_config(
        ...
        let concise_ns = crate::types::tool::ToolNamespace::EzerBuildConcise.to_string();
...
impl FinalizedToolset {
    ...
    /// Drives the first-party `ezer/*` tool `_meta` contract (tool normalization). Returns `None` for unknown tools (e.g.
...
mod tests {
    ...
    async fn tool_name_for_kind_resolves_execute() {
        ...
        let tmp = TempDir::new().unwrap();
        let ezer = ToolRegistryBuilder::new()
            .finalize(
                ToolServerConfig {
                    tools: vec![
        ...
        assert_eq!(
            ezer.tool_name_for_kind(ToolKind::Execute).as_deref(),
            Some("run_terminal_cmd")
        );
    }
    /// `merge_tool_meta` (the harness emission path) must stamp `ezer/tool` for a
    ...
    async fn registered_identity_uses_the_qualified_id_not_the_alias() {
        ...
        let toolset = ToolRegistryBuilder::new()
            .finalize(
                ToolServerConfig {
                    tools: vec![
                        ToolConfig::for_tool::<crate::implementations::ezer_build::grep::GrepTool>(
                        )
