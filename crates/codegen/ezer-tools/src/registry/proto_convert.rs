//! Conversion from the gRPC wire config types (`ezer-tools-api`) to the
//! runtime registry types ([`ToolConfig`] / [`ToolServerConfig`]).
//!
//! The `params_json` parse/validation contract lives in
//! [`ezer_tools_api::config_validation`] so every consumer (this
...
pub use ezer_tools_api::config_validation::{ToolConfigEntryError, ToolConfigEntryErrorKind};

/// Convert one wire [`ezer_tools_api::ToolConfigEntry`] to a runtime [`ToolConfig`]. `index` is
...
pub fn tool_config_from_entry(
    index: usize,
    entry: ezer_tools_api::ToolConfigEntry,
) -> Result<ToolConfig, ToolConfigEntryError> {
    let ezer_tools_api::ToolConfigEntry {
        id,
        params_json,
        name_override,
    ...
    let params = ezer_tools_api::config_validation::parse_params_json(
        index,
        &id,
        params_json.as_deref(),
    )?;
    ezer_tools_api::config_validation::validate_name_override(
        index,
        &id,
...
pub fn tool_server_config_from_entries(
    entries: Vec<ezer_tools_api::ToolConfigEntry>,
...
mod tests {
    ...
    fn entry(id: &str) -> ezer_tools_api::ToolConfigEntry {
        ezer_tools_api::ToolConfigEntry {
            id: id.to_owned(),
            params_json: None,
            name_override: None,
