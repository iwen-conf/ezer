...
//!
//! `MemoryBackend` is defined in `ezer-tools` to keep the tool crate
...
///
/// Use `tracing::info!(target: MEMORY_LOG_TARGET, ...)` in `ezer-tools`.
/// Mirrors `ezer_shell::session::memory_log::TARGET`.
