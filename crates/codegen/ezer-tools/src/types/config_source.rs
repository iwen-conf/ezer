...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.ezer/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.ezer/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.ezer/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.ezer/ or ~/.claude/.
    User { path: PathBuf },
