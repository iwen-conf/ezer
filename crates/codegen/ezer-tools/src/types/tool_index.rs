...
//!
//! `ToolSearchIndex` is defined in `ezer-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `ezer-shell`
...
/// concrete implementation in `ezer-shell` maps `mcp_initialized` to `is_ready`.
...
/// `MemoryBackend` — stored as an ephemeral resource (not serialized), injected by `ezer-shell`
