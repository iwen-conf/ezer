...
//! ezer-home (`$EZER_HOME` or `<home>/.ezer`). Shared by `xai-grok-config`
...
//! - [`default_grok_home`]: the `<home>/.ezer` default, ignoring `$EZER_HOME`, so callers can detect an override.
...
//!
//! Function names keep the historical `grok_home` identifiers so the crate
//! graph does not need a rename. The on-disk directory is `~/.ezer`.
...
/// Env override for the user state root (`config.toml`, sessions, logs, …).
pub const HOME_ENV: &str = "EZER_HOME";

/// Deprecated alias accepted so existing test harnesses and grok-era scripts
/// that already export `$GROK_HOME` keep isolating state. Never used to pick
/// `~/.grok` when unset.
pub const LEGACY_HOME_ENV: &str = "GROK_HOME";

/// Default directory name under the OS home (`~/.ezer`).
pub const USER_STATE_DIR_NAME: &str = ".ezer";

/// Project-local config / skills / hooks directory (`<repo>/.ezer`).
pub const PROJECT_DIR_NAME: &str = ".ezer";

/// Legacy project-local directory still read as a migration fallback.
...
pub enum GrokHomeSource {
    ...
    EnvOverride,
    /// A non-empty deprecated `$GROK_HOME` override.
    LegacyEnvOverride,
...
fn grok_home_in(home: &Path) -> PathBuf {
    dunce::canonicalize(home)
        ...
        .join(USER_STATE_DIR_NAME)
}

/// `$EZER_HOME` (then deprecated `$GROK_HOME`) verbatim when non-empty, else `<home>/.ezer`.
...
fn resolve_grok_home_from(
    ...
    if let Some(env) = ezer_home_env.filter(|env| !env.is_empty()) {
...
/// [`default_grok_home`] when neither `$EZER_HOME` nor a home resolves.
...
mod tests {
    ...
    #[test]
    fn ezer_env_wins_over_legacy_and_os_home() {
    ...
    #[test]
    fn legacy_grok_home_env_is_alias_only() {
        let resolved =
            resolve_grok_home_from(None, Some(OsStr::new("/custom/home")), Some(Path::new("/home/u")));
        assert_eq!(
            resolved,
            Some((
                PathBuf::from("/custom/home"),
                GrokHomeSource::LegacyEnvOverride
            ))
        );
    }
    ...
    #[test]
    fn default_ezer_home_has_no_verbatim_prefix() {
        ...
        assert!(home.ends_with(USER_STATE_DIR_NAME));
