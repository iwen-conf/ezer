...
fn tool_id_accepts_bare_and_namespaced_names() {
    ...
    let namespaced = ToolId::new("Ezer:read_file").unwrap();
    assert_eq!(namespaced.as_str(), "Ezer:read_file");
...
fn server_id_synthesis_starts_with_auto_prefix() {
    ...
    assert_eq!(synth_ns.as_str(), "auto:tool:Ezer:read_file");
}

#[test]
fn server_id_synthesis_is_deterministic() {
    let conn = ConnectionId::new("conn-abc").unwrap();
    let tool = ToolId::new("Ezer:read_file").unwrap();
