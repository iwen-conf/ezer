//! Observability seam for the full-replace (ezer-build) pass.
//!
//! The shared orchestrator reports per-attempt and terminal outcomes through
//! this trait so each harness can emit its own telemetry (ezer-build:
...
//!
//! Emission points are part of the behavior contract: the ezer-build observer
